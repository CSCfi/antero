  ...
  for i in j:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"{strftime('%Y-%m-%d %H:%M:%S', localtime())} ERROR {cnt} -- {row['koodi']} (try {attempt}/{max_retries}): {e}")

            if attempt == max_retries:
                # all retries failed
                print(f"{strftime('%Y-%m-%d %H:%M:%S', localtime())} FAILED {cnt} -- {row['koodi']}, skipping")
                break
