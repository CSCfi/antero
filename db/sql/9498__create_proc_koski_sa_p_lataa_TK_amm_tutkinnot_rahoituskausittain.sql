...
DROP PROCEDURE IF EXISTS [dbo].[p_lataa_TK_amm_tutkinnot_rahoituskausittain]
GO
...
CREATE PROCEDURE [dbo].[p_lataa_TK_amm_tutkinnot_rahoituskausittain] @rahoituskaudenPaattymisvuosi int = null AS
...
FROM (
	...
	SELECT
		...
		,suorv = q.vuosi
		,q.vahvistus_paiva --1.13 rajausta varten
	...
	cross apply (SELECT rahoituskausi_alku = datefromparts(@vuosi2-1,7,1), rahoituskausi_loppu = datefromparts(@vuosi2,6,30)) ca
	...
	WHERE 1=1
	and ca1.tilv_pvm between ca.rahoituskausi_alku and ca.rahoituskausi_loppu
	and ca3.luvallinen = 1
	and ca3.lupa_suorituskieli != -2
	and opr.sisaltyy_okm_suoritusrahoitus = 1
	and q.duplikaatti_rahoituskausittain = 0 --uusi sarake testissä
	and q.paallekkainen_tuva = 0 --sis. testissä myös valma ja telma
