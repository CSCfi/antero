...
DECLARE @maxRahoituskausiLahtotaso varchar(24) = (select max(rahoituskausi) from dw.v_TK_K1_16_lahtotaso)
DECLARE @maxSuorituskausiLahtotaso varchar(24) = (select max(tarkastelukausi) from dw.v_TK_K1_16_lahtotaso)
...
SELECT 
	...
	,[opiskelijavuodet_painottamaton] = cast([opiskelijavuodet] as decimal(30,20))
...
left join (
	...
	left join dw.d_amos_spl_opiskelijavuodet_hylkaamisperuste d5 on d5.id = f.d_amos_spl_opiskelijavuodet_hylkaamisperuste_id
	where 1=1
...
--2a: ei maksuttomat TK-aineistosta; järjestäjälle löytyy pohjakoulutusosuus aineistosta
left join dw.v_TK_K1_16_lahtotaso lt_pk 
	on lt_pk.rahoituskausi = case when right(f.rahoituskausi, 4) > right(@maxRahoituskausiLahtotaso, 4) then @maxRahoituskausiLahtotaso else f.rahoituskausi end
	and lt_pk.tarkastelukausi =  case when right(f.suorituskausi, 4) > right(@maxSuorituskausiLahtotaso, 4) then @maxSuorituskausiLahtotaso else f.suorituskausi end
	and lt_pk.ytunnus = coalesce(d0.ytunnus_amos_spl, d0.ytunnus_avain) 
--2b: ei maksuttomat TK-aineistosta; jos järjestäjälle ei edellä löydy pohjakoulutusosuutta
cross apply (
	select 
		opv_kerroin_kaikki = cast(avg(opv_kerroin_kaikki) as decimal(30, 20))
	from dw.v_TK_K1_16_lahtotaso 
	where 1=1
	and rahoituskausi = case when right(f.rahoituskausi, 4) > right(@maxRahoituskausiLahtotaso, 4) then @maxRahoituskausiLahtotaso else f.rahoituskausi end
	and tarkastelukausi =  case when right(f.suorituskausi, 4) > right(@maxSuorituskausiLahtotaso, 4) then @maxSuorituskausiLahtotaso else f.suorituskausi end
) oa

--varainhoitovuoden alun järjestämislupien enimmäismäärät
outer apply (
	select 
		opv_vet_max = nullif(sum(v.vaativa_erityinen_tuki), 0) --2026 alkaen arvo 0 tarkoittaa rajoittamatonta
		,opv_ei_maksuton_max = sum(v.ei_maksuttomat)
		...
		opv_ei_maksuton_kerroin = 
			cast(
				case 
					...
					when opv_ka.opv_ei_maksuton_ka > jl.opv_ei_maksuton_max then jl.opv_ei_maksuton_max / opv_ka.opv_ei_maksuton_ka
					else 1.0
				end
				as decimal(30,29)
			)
