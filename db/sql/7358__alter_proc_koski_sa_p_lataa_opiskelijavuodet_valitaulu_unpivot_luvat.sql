USE [Koski_SA]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_opiskelijavuodet_valitaulu_unpivot_luvat]    Script Date: 10.2.2023 17:40:20 ******/
SET ANSI_NULLS ON
GO
	...
	,lupa_koulutus = 
		case	
			when d21.kytkin_koodi = 0 or q.koulutusmuoto_pihvi = 'lukiokoulutus' or (year(q.alkamispaiva) < 2018 and d11.koulutusastetaso2_koodi in ('33','41')) then 1
				...
				case
					when q.kustannusryhma = 'muu' and v0.jarjestajaYtunnus is null then 11
...
(
	case 
		when (d11.tutkintotyyppi_koodi = '02' and q.alkamispaiva >= '2018-08-01') 
			...
			or q.kustannusryhma = 'muu' --muulle katsotaan onko amm. järjestämislupaa ylipäätään. JS 10.2.2023
			then 1
		else 0 
	end
)
...
--muu amm. koulutus, tarkistus onko ylipäätään järjestämislupa
OUTER APPLY (
