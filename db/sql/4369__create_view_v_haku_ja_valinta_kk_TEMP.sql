...
/****** Object:  View [dw].[v_haku_ja_valinta_kk_TEMP]    Script Date: 27.1.2021 0:41:18 ******/
DROP VIEW IF EXISTS [dw].[v_haku_ja_valinta_kk_TEMP]
GO
...
CREATE VIEW [dw].[v_haku_ja_valinta_kk_TEMP] AS

--26.1.2021 / JS
--Alla olevaa käytettiin perusnäkymän paikalla vietäessä alkamisvuosi kerrallaan kuution partitioksi käsin. Vastaavasti toimittiin yo-näkymän kanssa.
--Huom. loadtime on kommentoitu tilapäisesti 1. selectin päättelyistä pois ja 1. joiniksi on tuotu inner join kiinteän alkamisvuoden mukaan.
--Kunkin latauksen jälkeen partitioon lisättiin ehto 1=0 vahinkojen välttämiseksi.
--Poikkeuksena alkuperäinen partitio, johon ladattiin kaikki aloituspaikat kerralla muokkaamaalla alkuun tilapäisesti top 0 (loadtime pidettiin mukana 2. selectissä).
--Mietittävä pitääkö pilkkoa vai voiko optimoida vuosikelloratkaisua siirtäen logiikkaa aiempiin latauksiin.
...
INNER JOIN (select 1 as x) o on f.koulutuksen_alkamisvuosi = 2015
