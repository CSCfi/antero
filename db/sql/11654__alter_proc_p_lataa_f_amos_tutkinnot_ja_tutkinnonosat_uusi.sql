...
ALTER PROCEDURE [dw].[p_lataa_f_amos_tutkinnot_ja_tutkinnonosat_uusi] AS
...
TRUNCATE TABLE dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi

--1

--rahoituskausi, jolloin suorituskausi ensi kertaa mukana
INSERT INTO dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi
SELECT 
	generoitu = 0
	,d.paivays
	,rahoituskausi_amm_1 
...
WHERE d.paivays between '2024-07-01' and eomonth(dateadd(month, -1, getdate()))
...
--siirretään vuosiraportointiin myös keskeneräinen suorituskausi
DECLARE @maxRahoituskausi varchar(24) = (select max(rahoituskausi) from dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi)
DECLARE @maxPvm date = datefromparts(right(@maxRahoituskausi, 4), 6, 1)

UPDATE dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi
SET vuosiraportti_1_0 = 1
WHERE @maxPvm = datefromparts(right(suorituskausi, 4), 6, 1)


--2

DECLARE @maxRahoituskausiEdellinen varchar(24) = (select max(rahoituskausi) from dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi where rahoituskausi != @maxRahoituskausi)
DECLARE @maxToteumaPvm date = (select max(paivays) from dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi)
...
BEGIN
	...
	FROM dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi
	...
	INSERT dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi
	SELECT *
	FROM #generoidutKuukaudet
...
--siirretään estimaattikausi pois vuosiraportoinnista
UPDATE dw.f_amos_tutkinnot_ja_tutkinnonosat_uusi
SET vuosiraportti_1_0 = 0
WHERE generoitu = 1
