...
ALTER PROCEDURE [dw].[p_lataa_api_amm_rahoitus_yhteenvetotaulukko] AS
...
BEGIN
	...
	SELECT 
		[defaultorder] = ROW_NUMBER() OVER (ORDER BY [varainhoitovuosi] DESC, [generoitu] DESC, [nimi_amos_spl])
		,[varainhoitovuosi]
		,[toteuma_estimaatti] = case when generoitu = 1 then 'Estimaatti' else 'Toteuma' end
		,[ytunnus_amos_spl]
		,[nimi_amos_spl]
		...
		,[laskennallinen_rahoitus_rajoitekerroin] = 
			case 
				when generoitu = 1 then [laskennallinen_rahoitus_rajoitekerroin_estimaatti] 
				else [laskennallinen_rahoitus_rajoitekerroin] 
			end
		...
		,[laskennallinen_rahoitus_osuus] = 
			case 
				when generoitu = 1 then [laskennallinen_rahoitus_osuus_estimaatti] 
				else [laskennallinen_rahoitus_osuus] 
			end
