...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_opettajakelpoisuudet]    Script Date: 12.1.2023 8:58:20 ******/
SET ANSI_NULLS ON
GO
...
-- TEMP-tauluun kombinaatiot kelpoisuuskoodeista sekä kelpoisuuskoulujen korkeakoulu ja pvm
	...
	FROM (
		SELECT DISTINCT
			henkilo,
			CASE 
				WHEN kelpoisuuskoodi1 = 'OP' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','IX','EL') THEN 'OP'
				WHEN kelpoisuuskoodi1 = 'ER' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','IX','EL') THEN 'ER'
				WHEN kelpoisuuskoodi1 in ('AI','JA','JB','JC','IX','EL') AND ainepatevyys is not null AND ainepatevyys not in ('gf','ms') THEN 'AI'
				WHEN kelpoisuuskoodi1 = 'PE' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','IX','EL') THEN 'PE'
				WHEN kelpoisuuskoodi1 = 'EL' AND kelpoisuuskoodi2 = 'VA' THEN 'EL'
				WHEN kelpoisuuskoodi1 = 'VA' THEN 'VA'
				WHEN kelpoisuuskoodi1 = 'VS' THEN 'VS'
			END as kelpoisuus,
			CASE 
				WHEN kelpoisuuskoodi1 = 'OP' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','IX','EL') THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'ER' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','IX','EL') THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 in ('AI','JA','JB','JC','IX','EL') AND ainepatevyys is not null AND ainepatevyys not in ('gf','ms') THEN 
					CASE WHEN pvm1 < aine_pvm THEN aine_pvm ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'PE' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','IX','EL') THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
		...
		WHERE ainepatevyys in ('gf','ms') and kelpoisuuskoodi1 in ('AI', 'JA', 'JB', 'JC', 'IX', 'EL')) f
...
FROM (
	...
	FROM (
		...
		OUTER APPLY (
			Select
				CASE 
					WHEN p.kelpoisuus = 'VA_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('VA','PE','AI','JA','JB','JC','IX','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
					WHEN p.kelpoisuus = 'AI_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','JA','JB','JC','IX','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
					...
					WHEN p.kelpoisuus = 'OP' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('OP','AI','JA','JB','JC','IX','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
					WHEN p.kelpoisuus = 'ER' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('ER','AI','JA','JB','JC','IX','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
					WHEN p.kelpoisuus = 'PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','JA','JB','JC','IX','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
					WHEN p.kelpoisuus in('AI', 'PD', 'LA') THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('AI','JA','JB','JC','IX', 'EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
