IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'api.kvliikkuvuus'))
EXEC dbo.sp_executesql @statement = N'
CREATE VIEW api.kvliikkuvuus AS
SELECT 1 AS a
'
GO

-- copy from with some columns dropped: dw.v_virta_otp_kvliikkuvuus
-- alter view to same just so column types are refreshed
ALTER VIEW api.kvliikkuvuus AS 
