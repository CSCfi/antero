...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_esi_ja_perus_oppilaat_ja_paattaneet]    Script Date: 27.10.2023 18:27:33 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,CASE
		WHEN ((d14.maakunta_koodi NOT IN ('21', '-1', '98') AND d14.maakunta_koodi IS NOT NULL) AND --Kotikunta Manner-Suomessa
			 ...
			 ((k.vuosi-YEAR(henk.syntymaaika) > 7 AND FLOOR(DATEDIFF(DAY, henk.syntymaaika, DATEFROMPARTS(k.vuosi, k.kuukausi, 1)) / 365.25) < 18) OR --Henkilö täyttää kyseisenä vuonna 8-18 vuotta
			 (k.vuosi-YEAR(henk.syntymaaika) = 7 AND DATEFROMPARTS(k.vuosi, k.kuukausi, 1) >= DATEFROMPARTS(k.vuosi, 8, 1)) OR --Henkilö täyttää 7 vuotta ja opintovelvollisuus alkaa kyseisen vuoden elokuun alusta
			 (ooa.pidennetty_oppivelvollisuus = 1 AND DATEFROMPARTS(k.vuosi, k.kuukausi, 1) BETWEEN ooa.alku AND ooa.loppu)) AND --Tarkastelupäivänä Koski-opiskeluoikeusdatan mukaan pidennetyn oppivelvollisuuden piirissä
...
LEFT JOIN Koski_SA.sa.sa_koski_esiopetus_opiskeluoikeus_aikajakso ooa ON k.opiskeluoikeus_oid = ooa.opiskeluoikeus_oid AND DATEFROMPARTS(k.vuosi, k.kuukausi, 1) BETWEEN ooa.alku AND ooa.loppu
LEFT JOIN (
	SELECT
	oo.oppija_oid,
...
SELECT 	
	...
	,oppija_oid
	,oppilaitos_oid
	,k.paatason_suoritus_id --pari jääLuokalle-tapausta, voidaan tarvittaessa huomioida välitaulun rangissa alkamisaika
	,k.suoritustapa
	...
	,d_kieli_muu_suorituskieli_id = COALESCE(d6b.id, -1)
	,d_kieli_kielikylpykieli_id = COALESCE(d6c.id, -1)
	,d_erityinen_tuki_johdettu_id = COALESCE(d7.id,-1) --huom. 20.9. tilanteen osalta tämä, vaikka jäljempänä max(erityisen_tuen_paatos)
	,d_vuosiluokka_id = COALESCE(d9.id,-1)
	,d_organisaatioluokitus_toimipiste_id = COALESCE(d11.id,-1)
	,d_organisaatioluokitus_oppilaitos_id = COALESCE(d12.id,-1)
	,d_organisaatioluokitus_jarj_id = COALESCE(d13.id,-1)
	...
	,max(opiskelee_toiminta_alueittain) as 'opiskelee_toiminta_alueittain'
