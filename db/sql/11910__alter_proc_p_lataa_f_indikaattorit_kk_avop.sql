...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_avop]    Script Date: 16.3.2026 10.49.24 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dw].[p_lataa_f_indikaattorit_kk_avop] as
...
USING (
	...
	FROM (
		...
		FROM (		
			...
			WHERE d1.kysymysryhma_fi = 'Opiskelutyytyväisyys' and d2.organisaatio_koodi <> '02557' and d3.koulutusastetaso2_fi = 'Ammattikorkeakoulututkinto' and 
			f.kyselyvuosi <= YEAR(DATEADD(MONTH, -1, getdate())) - 1
			GROUP BY f.kyselyvuosi, d2.organisaatio_koodi, f.vastaajaid	
		) f		
	...
	OUTER APPLY (
		...
		FROM (		
			...
			WHERE d1b.kysymysryhma_fi = 'Opiskelutyytyväisyys' and d3b.koulutusastetaso2_fi = 'Ammattikorkeakoulututkinto' and fb.kyselyvuosi < year(getdate())
			and fb.kyselyvuosi = f.tilastovuosi and d2b.organisaatio_koodi <> '02557'
