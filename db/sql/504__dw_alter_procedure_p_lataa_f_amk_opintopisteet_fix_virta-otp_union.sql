...
WHERE s.koulutustyyppi_koodi<>'-1'
...
LEFT JOIN sa.sa_suorat_hist_amk2a_opintopisteet s ON sb.korkeakoulu_tunnus=s.amk_tunnus and sb.vuosi=s.vuosi and sb.koulutusala_koodi=s.koulutusala_koodi
WHERE s.koulutustyyppi_koodi='-1'



UNION -- switch to VIRTA-OTP
...
LEFT JOIN dw.d_kalenteri as d4 ON d4.kalenteri_avain=cast(dateadd(second,cast(s.luontipaivamaara/1000 as int),'1970-1-1') as date)
...
,coalesce((select d.id
  ...
  where(d.kalenteri_avain=cast(dateadd(second,cast(s.luontipaivamaara/1000 as int),'1970-1-1') as date))
...
,coalesce((select d.id
  from dw.d_kalenteri as d
  where(d.kalenteri_avain=cast(dateadd(second,cast(a.luontipaivamaara/1000 as int),'1970-1-1') as date))
),-1) as d_tilannepvm_id
,a.vuosi
