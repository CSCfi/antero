...
/****** Object:  StoredProcedure [sa].[p_lataa_sa_hakeneet_toinen_aste_koonti]    Script Date: 25.2.2026 12.43.17 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @haku7_oid varchar(100)
DECLARE @haku8_oid varchar(100)
...
AND f.koulutuksen_alkamisvuosi BETWEEN 2015 and 2021
AND d2.koodi = 'S'
...
BEGIN
	...
	SET @haku7_oid = (SELECT haku_oid FROM #taulu WHERE koulutuksen_alkamisvuosi = @vuosi AND hakuryhma_koodi = '6' AND hakutyyppi_koodi = '01')
	SET @haku8_oid = (SELECT haku_oid FROM #taulu WHERE koulutuksen_alkamisvuosi = @vuosi AND hakuryhma_koodi = '6' AND hakutyyppi_koodi = '03')

	EXEC [sa].[p_lataa_sa_hakeneet_toisen_asteen_hakijat] 
	@vuosi, @haku1_oid, @haku2_oid, @haku3_oid, @haku4_oid, @haku5_oid, @haku6_oid,  @haku7_oid,  @haku8_oid;
