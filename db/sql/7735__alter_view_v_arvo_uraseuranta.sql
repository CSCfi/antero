...
SELECT  
      ...
      ,CASE 
			...
			WHEN Äidinkieli = 'ruotsi' THEN 'ruotsi'
			WHEN Äidinkieli = 'Tieto puuttuu' THEN 'Tieto puuttuu'
			ELSE 'muut kielet' 
		END AS [Äidinkieli]
      ,[Kansalaisuus]
     ...
     ,[Kysymysryhmä]
      ,[Kysymys]
	 , CASE 
		WHEN Rahoitusmallikysymys = 'Kyllä' THEN
		CASE
			WHEN Kysymys like '%Koulutus antoi riittävät valmiudet työelämään%' THEN '1. Koulutus antoi riittävät valmiudet työelämään.'
			WHEN Kysymys like '%Miten tyytyväinen olet kokonaisuudessaan vuonna%' THEN '2. Miten tyytyväinen olet kokonaisuudessaan vuonna x suorittamaasi tutkintoon työurasi kannalta?'
			WHEN Kysymys like '%Pystyn hyödyntämään yliopistossa oppimiani tietoja ja taitoja nykyisessä työssäni hyvin%' THEN '3. Pystyn hyödyntämään yliopistossa oppimiani tietoja ja taitoja nykyisessä työssäni hyvin.'
			WHEN Kysymys like '%Työni vastaa vaativuustasoltaan hyvin yliopistollista koulutustani%' THEN '4. Työni vastaa tällä hetkellä vaativuustasoltaan hyvin yliopistollista koulutustani.'
			...
			WHEN Kysymys like '%tieteiden%' THEN '4) tieteidenvälisyys/moniammatillisissa ryhmissä toimiminen'
			WHEN Kysymys like '%itseohjautuvuus%' THEN '5) itseohjautuvuus/oma-aloitteisuus'
			WHEN Kysymys like '%yrittäjyys%' THEN '6) yrittäjyystaidot'
			ELSE NULL
		END
		ELSE NULL
	END as 'Kysymys (rahoitusmalli)'
	,CASE 
		WHEN Rahoitusmallikysymys = 'Kyllä' THEN
		CASE 
			WHEN Kysymys like '%Koulutus antoi riittävät valmiudet työelämään%' THEN '1. Utbildningen gav tillräckliga färdigheter för arbetslivet.'
			WHEN Kysymys like '%Miten tyytyväinen olet kokonaisuudessaan vuonna%' THEN '2. Med tanke på din karriär, hur nöjd är du med den examen du avlade år x i sin helhet?'
			WHEN Kysymys like '%Pystyn hyödyntämään yliopistossa oppimiani tietoja ja taitoja nykyisessä työssäni hyvin%' THEN '3. Jag kan i mitt nuvarande arbete utnyttja väl de kunskaper och färdigheter som jag lärt mig vid universitetet.'
			WHEN Kysymys like '%Työni vastaa vaativuustasoltaan hyvin yliopistollista koulutustani%' THEN '4. Kravnivån på mitt arbete motsvarar väl min universitetsutbildning.'
			WHEN Kysymys like '%analyyttiset%' THEN '1) färdigheter för analytiskt och systematiskt tänkande'
			WHEN Kysymys like '%tiedonhankinta%' THEN '2) färdigheter i informationssökning'
			WHEN Kysymys like '%kyky oppia%' THEN '3) förmåga att lära sig och tillägna sig nytt'
			WHEN Kysymys like '%tieteiden%' THEN '4) tvärvetenskaplighet/fungera i mångprofessionella grupper'
			WHEN Kysymys like '%itseohjautuvuus%' THEN '5) självstyrande/eget initiativtagande'
			WHEN Kysymys like '%yrittäjyys%' THEN '6) entreprenörs-/företagarförmåga'
			ELSE NULL
		END
		ELSE NULL
	END as 'Fråga (finansieringsmodell)'
	,CASE 
		WHEN Rahoitusmallikysymys = 'Kyllä' THEN
		CASE 
			WHEN Kysymys like '%Koulutus antoi riittävät valmiudet työelämään%' THEN '1. The studies equipped me sufficiently for the working life.'
			WHEN Kysymys like '%Miten tyytyväinen olet kokonaisuudessaan vuonna%' THEN '2. How satisfied are you overall with the degree you completed in year x in terms of your career?'
			WHEN Kysymys like '%Pystyn hyödyntämään yliopistossa oppimiani tietoja ja taitoja nykyisessä työssäni hyvin%' THEN '3. The skills and knowledge I learned at the university can be applied well in my current job.'
			WHEN Kysymys like '%Työni vastaa vaativuustasoltaan hyvin yliopistollista koulutustani%' THEN '4. The requirements of my current job correspond well with my academic qualifications.'
			...
			WHEN Kysymys like '%yrittäjyys%' THEN '6) entrepreneurship skills'
			ELSE NULL
		END
		ELSE NULL
	END as 'Question (funding model)'
    , [Kyselypohja]
	 ...
	 ,[Kyselyvuosi] as 'Tilastovuosi'
	  ...
	  -- CSCVIPUNEN-3471
      ,CASE 
		WHEN Kysymys = '5. Mikä seuraavista vaihtoehdoista kuvaa parhaiten tähänastisen työurasi kokonaisuutta?'  and Kyselyvuosi IN (2021, 2022) THEN 'Vastauksia ei kerätty vuosina 2021 ja 2022'
		ELSE [Monivalintavaihtoehto]
	  END as Monivalintavaihtoehto
      ,CASE 
		WHEN Kysymys = '5. Mikä seuraavista vaihtoehdoista kuvaa parhaiten tähänastisen työurasi kokonaisuutta?'  and Kyselyvuosi IN (2021, 2022) THEN NULL
		ELSE numerovalinta
	...
	,CASE 
			...
			WHEN Äidinkieli = 'ruotsi' THEN 'svenska'
			WHEN Äidinkieli = 'Tieto puuttuu' THEN 'Information saknas'
			ELSE 'övriga språk' 
		END AS [Modersmål]
	  ,[Högskola]
      ,[Utbildningens språk]
	  ...
	  -- CSCVIPUNEN-3471
      ,CASE 
		WHEN Kysymys = '5. Mikä seuraavista vaihtoehdoista kuvaa parhaiten tähänastisen työurasi kokonaisuutta?'  and Kyselyvuosi IN (2021, 2022) THEN 'Svar har inte samlats in åren 2021 och 2022'
     ...
     --ENGLANTI
	  ...
	  ,CASE 
			...
			WHEN Äidinkieli = 'ruotsi' THEN 'Swedish'
			WHEN Äidinkieli = 'Tieto puuttuu' THEN 'Missing data'
	  ...
	  -- CSCVIPUNEN-3471
      ,CASE 
		WHEN Kysymys = '5. Mikä seuraavista vaihtoehdoista kuvaa parhaiten tähänastisen työurasi kokonaisuutta?'  and Kyselyvuosi IN (2021, 2022) THEN 'Answers were not collected in years 2021 and 2022'
     ...
     ,[Koodit Korkeakoulu]
      ...
      ,[jarjestys_sukupuoli]
	  ,CASE 
			WHEN Äidinkieli = 'suomi' OR Äidinkieli = 'pohjoissaame' THEN 1
			WHEN Äidinkieli = 'ruotsi' THEN 2
			WHEN Äidinkieli = 'Tieto puuttuu' THEN 99999
			ELSE 3 
      ...
      , CASE
		WHEN Kysymys = '5. Mikä seuraavista vaihtoehdoista kuvaa parhaiten tähänastisen työurasi kokonaisuutta?'  and Kyselyvuosi IN (2021, 2022) THEN 99
		...
		WHEN [ANTERO].dw.f_arvo_yo_uraseuranta_2018.kysymys='13. Mitkä seuraavista kuvaavat parhaiten tämänhetkisten työtehtäviesi luonnetta?' and [ANTERO].dw.f_arvo_yo_uraseuranta_2018.kyselyvuosi = 2018 THEN
		...
		WHEN Rahoitusmallikysymys = 'Kyllä' THEN
		CASE
			WHEN Kysymys like '%Koulutus antoi riittävät valmiudet työelämään%' THEN 1
			WHEN Kysymys like '%Miten tyytyväinen olet kokonaisuudessaan vuonna%' THEN 2
			WHEN Kysymys like '%Pystyn hyödyntämään yliopistossa oppimiani tietoja ja taitoja nykyisessä työssäni hyvin%' THEN 3
			WHEN Kysymys like '%Työni vastaa vaativuustasoltaan hyvin yliopistollista koulutustani%' THEN 4
			...
			WHEN Kysymys like '%yrittäjyys%' THEN 10
			ELSE NULL
		END
		ELSE NULL
	END as 'jarjestys_kysymys_rahoitusmalli'
