...
FROM (
	...
	and (
		kk.tyyppi not in ('ammatillisen-opiskelijapalaute', 'tyopaikkaohjaajakysely', 'amk-uraseuranta', 'kandipalaute', 'avop') 
		or (kk.tyyppi = 'ammatillisen-opiskelijapalaute' and concat([kuukausi],'-01') < '2021-07-01')
	)
	...
	JOIN Arvo_SA.sa.sa_arvo_kohteet_taustatiedot tt on tt.vastaajatunnusid=k.vastaajatunnusid
	...
	JOIN Arvo_SA.sa.sa_arvo_kohteet_taustatiedot tt on tt.vastaajatunnusid=k.vastaajatunnusid
	JOIN Arvo_SA.sa.sa_arvo_kyselykerrat kk on kk.[kyselyid]=k.[kyselyid]
	WHERE (kk.tyyppi = 'ammatillisen-opiskelijapalaute' and k.[taustatiedot.heratepvm] > '2024-06-30' and k.vastaajatunnusid != 1224658)
	GROUP BY  
		k.[kyselyid]
		,k.kyselykertaid
		,tt.toimipiste_oid
...
INCLUDE ([kyselykertaid],toimipiste_oid,[oppilaitoskoodi],[tutkintokoodi],[kuukausi],[vastaajatunnukset_lkm])
