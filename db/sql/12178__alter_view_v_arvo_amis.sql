...
/****** Object:  View [dw].[v_arvo_amis]    Script Date: 10.9.2026 17.21.17 ******/
SET ANSI_NULLS ON
GO
...
SELECT --top 1000
	...
	,CASE
		WHEN d4.kysymysid in ('5782','5793','5828') and f.numerovalinta is null THEN 'Ei kokemusta' -- CSCVIPUNEN-5011
		ELSE COALESCE(NULLIF(d6.vastaus_fi,'Tieto puuttuu'),numerovalinta)	--ei-monivalintakysymyksissä näytetään numerovalinta
	...
	, d10.suorituskieli_ammatillinen_fi			AS 'Suorituskieli'

															--TOIMIPISTE
	,o1.organisaatio_fi							AS 'Toimipiste'
	,a1.kunta_fi								AS 'Toimipisteen kunta'
	,a1.maakunta_fi								AS 'Toimipisteen maakunta'
	...
	, av6.vastaus_fi						    AS 'Korkein aiempi koulutus'
	...
	,CASE
		WHEN d4.kysymysid in ('5782','5793','5828') and f.numerovalinta is null THEN 'Ingen erfarenhet'
		ELSE COALESCE(NULLIF(d6.vastaus_sv,'Information saknas'),numerovalinta)
	...
	,o1.organisaatio_sv							AS 'Verksamhetsställe'
	,a1.kunta_sv								AS 'Verksamhetsställets kommun'
	,a1.maakunta_sv								AS 'Verksamhetsställets landskap'
	...
	, d10.suorituskieli_ammatillinen_sv			AS 'Studiespråk'
	...
	, av6.vastaus_sv						    AS 'Den högsta tidigare utbildningen'
	...
	,CASE
		WHEN d4.kysymysid in ('5782','5793','5828') and f.numerovalinta is null THEN 'No experience'
		ELSE COALESCE(NULLIF(d6.vastaus_en,'Missing data'),numerovalinta)
	...
	,o1.organisaatio_en							AS 'Site'
	,a1.kunta_en								AS 'Municipality of site'
	,a1.maakunta_en								AS 'Region of site'
	...
	, d10.suorituskieli_ammatillinen_en			AS 'Language of education'
	...
	, av6.vastaus_en						    AS 'The highest prior education'
	...
	, NULL /*p.priorisointi	*/				    AS 'Järjestys korkein aiempi koulutus'
	, d9.jarjestys								AS 'Järjestys äidinkieli'
	, av3.jarjestys								AS 'Järjestys ikäryhmä'
	,CASE
		WHEN d8.koodi in (0, 1) THEN d8.jarjestys
		ELSE 3
	END											AS 'Järjestys sukupuoli'
	, av.jarjestys								AS 'Järjestys hakeutumisväylä'
	, av5.jarjestys								AS 'Järjestys arvio tulevaisuudesta'
	...
	, a1.jarjestys_maakunta_koodi				AS 'Järjestys toimp maakunta'
...
LEFT JOIN dw.d_organisaatioluokitus o1 ON f.d_organisaatio_toimipiste_id = o1.id
LEFT JOIN dw.d_organisaatioluokitus o2 ON f.d_organisaatio_oppilaitos_id = o2.id
LEFT JOIN dw.d_alueluokitus a1 ON a1.kunta_koodi = coalesce(o1.kunta_koodi, '-1')
...
LEFT JOIN dw.d_arvotiedonkeruu d5 ON f.d_arvokysely_id = d5.id
...
LEFT JOIN dw.d_arvovastaus_monivalinta d8 ON d8.id = f.d_arvovastaus_taustakysymys_sukupuoli_id
LEFT JOIN dw.d_arvovastaus_monivalinta d9 ON d9.id = f.d_arvovastaus_taustakysymys_aidinkieli_id
...
--
LEFT JOIN dw.d_arvovastaus_monivalinta av ON av.id = f.d_arvovastaus_taustakysymys_hakeutumisvayla_id
--LEFT JOIN dw.d_arvovastaus_koulutustausta av2 ON av2.vastaajaid = f.vastaajaid
LEFT JOIN dw.d_arvovastaus_monivalinta av3 ON av3.id = f.d_arvovastaus_taustakysymys_ika_id
LEFT JOIN dw.d_oppisopimuskoulutus av4 ON av4.id = f.d_oppisopimuskoulutus_id
LEFT JOIN dw.d_arvovastaus_monivalinta av5 ON av5.id = f.d_arvovastaus_taustakysymys_arvio_tilanteesta_id
LEFT JOIN dw.d_arvovastaus_monivalinta av6 ON av6.id = f.d_arvovastaus_koulutustausta_id
LEFT JOIN dw.f_arvo_luodut_vastaajatunnukset f2 ON f2.kyselyid = d5.kyselyid
	and concat(f2.toimipiste_oid, '|', f2.oppilaitoskoodi, '|', f2.ytunnus) = concat(nullif(o1.organisaatio_oid,'-1'), '|', nullif(o2.organisaatio_koodi,'-1'), '|', d12.ytunnus_avain)
	and coalesce(f2.tutkintokoodi,'-1') = d7.koulutusluokitus_koodi
	and YEAR(f2.kuukausi) = d3.vuosi and MONTH(f2.kuukausi) = d3.kuukausi
LEFT JOIN [dw].[f_amos_opiskelijapalaute_vastaajamäärä_vahvistettu] f3 ON (f3.rahoituskausi = d3.rahoituskausi_amm and f3.[Koulutuksen järjestäjän Y-tunnus] = d1.organisaatio_koodi)
