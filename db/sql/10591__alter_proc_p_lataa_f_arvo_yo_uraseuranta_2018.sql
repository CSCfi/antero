...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_yo_uraseuranta_2018_testi]    Script Date: 20.2.2025 12.28.20 ******/
SET ANSI_NULLS ON
GO
...
SELECT  
	coalesce(d_ika.id,-1) as d_ika_id,		
	coalesce(d_sukupuoli.id,-1) as d_sukupuoli_id,
    coalesce(d_kieli.id,-1) as d_kieli_aidinkieli_id,
    coalesce(d_maatjavaltiot2.id,-1) as d_maatjavaltiot2_kansalaisuus_id,
	coalesce(d_koulutusluokitus.id,-1) as d_koulutusluokitus_id,
    ...
    dw.d_organisaatioluokitus.id as d_organisaatioluokitus_id,
...
LEFT JOIN dw.d_ika on d_ika.ika_avain=coalesce(round(cast(f2.ika_valmistuessa as float),0),-1)	
LEFT JOIN dw.d_sukupuoli on d_sukupuoli.sukupuoli_koodi=round(cast(f2.sukupuoli as float),0)
LEFT JOIN dw.d_kieli on d_kieli.kieli_koodi=f2.aidinkieli
LEFT JOIN dw.d_maatjavaltiot2 on d_maatjavaltiot2.maatjavaltiot2_koodi=round(cast(f2.kansalaisuus as float),0)
LEFT JOIN dw.d_koulutusluokitus on d_koulutusluokitus.koulutusluokitus_koodi= round(cast(f2.koulutusalakoodi as float),0)
LEFT JOIN dw.d_organisaatioluokitus on d_organisaatioluokitus.organisaatio_koodi=RIGHT('00000'+isnull(f1.organisaatio_koodi,''),5) 
OUTER APPLY (
	SELECT * FROM (
		SELECT 
			k.*,
