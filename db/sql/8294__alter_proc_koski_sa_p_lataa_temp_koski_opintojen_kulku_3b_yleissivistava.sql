...
/****** Object:  StoredProcedure [sa].[p_lataa_temp_koski_opintojen_kulku_3b_yleissivistava]    Script Date: 2.11.2023 9:50:22 ******/
SET ANSI_NULLS ON
GO
		...
		SELECT
			...
			,min_priorisoitu_tilanne_yleissivistava_koodi --vaihdosten kytkemiseksi priorisointiin ylemmässä kyselyssä
				= min(priorisoitu_tilanne_yleissivistava_koodi) 
					over(partition by kohortti_vp,[oppija_oid],opiskeluoikeus_oid,koulutus_kytkin,tutkinto_koodi,oppimaara,ika,ika_aloittaessa,ika_paattyessa,alkamispaiva,paattymispaiva,lopputulema,sarja)
			,vaihtanut_jarjestajaa --= MIN(vaihtanut_jarjestajaa)
			,vaihtanut_oppilaitosta --= MIN(vaihtanut_oppilaitosta)
		FROM (
			SELECT 
