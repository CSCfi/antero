...
/****** Object:  StoredProcedure [dbo].[tasaarvoindikaattori_r]    Script Date: 29.4.2020 18:22:42 ******/
DROP PROCEDURE [dbo].[tasaarvoindikaattori_r]
GO
...
# muokattu SES-pisteytys ja rajaukset vain äidin mukaan, muuten vastaamaan Otuksen indikaattoria
...
# vaihdettu ala rajausehdosta vasteeksi
# lisätty yo-vasteen laskenta koko joukolle
# huomioitu monistamalla rivit joilla "uusi > 1"
...
# otetaan talteen tarkasteltava sektori ja vasteena toimiva OKM ohjauksenala
  ...
  vaste_sektori <- df$vaste_sektori[1]
...
#---------alue-, sukupuoli- ja kielimuuttuja----------
  ...
  df$alue <- abs(df$alue - 3)						# jälkeen: maaseutumainen = 0, taajaan asuttu = 1, kaupunkimainen = 2
...
#-----------------SES-luku---------------------
  ...
  df$SES <- as.factor(df$koul_aiti + df$sose_aiti + df$tulot_aiti)	
  
  ## + df$koul_isa + df$sose_isa + df$tulot_isa
  ...
  ## df$SES <- cut(df$SES, breaks = c(0, 3, 6, 9, 12), include.lowest = TRUE, right = TRUE)

#------------vastemuuttuja------------

  # kaikki alat -> yliopisto = 1, amk = 0; muuten vastesektorin vasteala = 1, muut = 0
  ...
  # amk-sektori:
  if (vaste_sektori == "AMK") {
    df$yo <- 1 - df$yo
  }

  # vasteala:
  if (vaste_ala_koodi != "Kaikki") {
    df$yo[as.numeric(df$OKM_ohjauksen_ala_koodi) != vaste_ala_koodi] <- 0
  }
...
##############################################################################
  ...
  df <- df[, c("uusi", "tilastovuosi", "yo", "SES", "mies", "alue")]	
  ...
  {	  
	...
	## kielimuuttuja poistettu 
	## + kielifisv + SES:kielifisv
	## , "kielifisv"
	  
	fit <- glm(yo ~ SES + mies + alue + SES:mies + SES:alue + SES:mies:alue,				
					family = "binomial", 
					data = df3[, c("yo", "SES", "mies", "alue")]
				)								

	iLSES <- getRIndicator(fit$formula, sampleData = df3, family = "binomial")

	  # valitut muuttujat, ei-ehdolliset

	  ses_koko_eiehd <- iLSES$partialCV$byVariables$CVu[[1]]				# SES koko
	  ses0_eiehd <- iLSES$partialCV$byCategories$SES$CVuUnadj[[1]]			
	  ses1_eiehd <- iLSES$partialCV$byCategories$SES$CVuUnadj[[2]]	  
	  ses2_eiehd <- iLSES$partialCV$byCategories$SES$CVuUnadj[[3]]
	  ses3_eiehd <- iLSES$partialCV$byCategories$SES$CVuUnadj[[4]]

	  sp_koko_eiehd <- iLSES$partialCV$byVariables$CVu[[2]]					# sukupuoli koko
	  sp_nainen_eiehd <- iLSES$partialCV$byCategories$mies$CVuUnadj[[1]]
	  sp_mies_eiehd <- iLSES$partialCV$byCategories$mies$CVuUnadj[[2]]

	  alue_koko_eiehd <- iLSES$partialCV$byVariables$CVu[[3]]				# alue koko
	  alue_1_eiehd <- iLSES$partialCV$byCategories$alue$CVuUnadj[[1]]			# maaseutumainen
	  alue_2_eiehd <- iLSES$partialCV$byCategories$alue$CVuUnadj[[2]]			# taajaan asuttu
	  alue_3_eiehd <- iLSES$partialCV$byCategories$alue$CVuUnadj[[3]]			# kaupunkimainen

	  ## kielimuuttuja poistettu
	  ## kieli_koko_eiehd <- iLSES$partialCV$byVariables$CVu[[4]] 					# kieli koko
	  ## kieli_muu_eiehd <- iLSES$partialCV$byCategories$kielifisv$CVuUnadj[[1]]		# muu/tuntematon
	  ## kieli_fisv_eiehd <- iLSES$partialCV$byCategories$kielifisv$CVuUnadj[[2]]		# suomi/saame/ruotsi


	  # valitut muuttujat, ehdolliset

	  ses_koko_ehd <- iLSES$partialCV$byVariables$CVc[[1]]					# SES koko
	  ses0_ehd <- iLSES$partialCV$byCategories$SES$CVcUnadj[[1]]		
	  ses1_ehd <- iLSES$partialCV$byCategories$SES$CVcUnadj[[2]]
	  ses2_ehd<- iLSES$partialCV$byCategories$SES$CVcUnadj[[3]]
	  ses3_ehd<- iLSES$partialCV$byCategories$SES$CVcUnadj[[4]]

	  sp_koko_ehd <- iLSES$partialCV$byVariables$CVc[[2]]					# sukupuoli koko
	  sp_nainen_ehd <- iLSES$partialCV$byCategories$mies$CVcUnadj[[1]] 			
	  sp_mies_ehd <- iLSES$partialCV$byCategories$mies$CVcUnadj[[2]] 			

	  alue_koko_ehd <- iLSES$partialCV$byVariables$CVc[[3]]					# alue koko
	  alue_1_ehd <- iLSES$partialCV$byCategories$alue$CVcUnadj[[1]]				# maaseutumainen
	  alue_2_ehd <- iLSES$partialCV$byCategories$alue$CVcUnadj[[2]]				# taajaan asuttu
	  alue_3_ehd <- iLSES$partialCV$byCategories$alue$CVcUnadj[[3]]				# kaupunkimainen

	  ## kielimuuttuja poistettu
	  ## kieli_koko_ehd <- iLSES$partialCV$byVariables$CVc[[4]] 					# kielifisv
	  ## kieli_muu_ehd <- iLSES$partialCV$byCategories$kielifisv$CVcUnadj[[1]]			# muu/tuntematon
	  ## kieli_fisv_ehd <- iLSES$partialCV$byCategories$kielifisv$CVcUnadj[[2]]			# suomi/saame/ruotsi
	...
	if (i == min(df$tilastovuosi)) {
		...
		df_output <- data.frame("Tilastovuosi"					=	i+n,
								...
								"SES: 0 (vakioitu)"						=	ses0_ehd, 
								"SES: 1 (vakioitu)"						=	ses1_ehd, 
								"SES: 2 (vakioitu)"						=	ses2_ehd, 
								"SES: 3 (vakioitu)"						=	ses3_ehd,
...
################################################################################################# 
  ...
  WHERE v.klaji IN (''41'',''42'',''51'')
  AND v.uusi > 0
  AND 0+v.tilv - v.syntv BETWEEN 18 AND 29
  AND d1.kuntaryhma_koodi <> ''-1''
  AND (v.tulo_aiti <> ''00'' OR v.sose_aiti2 <> ''99'')
  '
