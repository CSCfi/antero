...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_perusopetus_ainevalinnat]    Script Date: 20.12.2021 9:35:45 ******/
DROP PROCEDURE IF EXISTS [dw].[p_lataa_f_koski_perusopetus_ainevalinnat]
GO
...
CREATE PROCEDURE [dw].[p_lataa_f_koski_perusopetus_ainevalinnat] AS
  ...
  --id
	...
	FROM (SELECT * FROM Koski_SA.dbo.perusopetus_ainevalinnat_valitaulu) k
	...
	LEFT JOIN dw.d_kieli d6 ON d6.kieli_koodi = k.vuosiluokan_suorituskieli
	LEFT JOIN dw.d_kieli d6b ON d6b.kieli_koodi = k.aineen_suorituskieli
	LEFT JOIN dw.d_vuosiluokka_tai_koulutus d9 ON d9.koodi = k.vuosiluokka
