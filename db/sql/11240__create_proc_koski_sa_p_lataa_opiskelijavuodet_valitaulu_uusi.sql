...
CREATE OR ALTER PROCEDURE [dbo].[p_lataa_opiskelijavuodet_valitaulu_uusi] @minVuosi int = 2023, @maxVuosi int = NULL AS

--huom. vanha malli käyttää toistaiseksi samaa kohdetaulua

DECLARE @startDate DATE, @endDate DATE
SET @startDate = CONCAT(@minVuosi,'-07-01')
...
BEGIN
	...
	EXEC sa.p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_lasna_loma_valmistunut 
	...
	INSERT INTO [dbo].[opiskelijavuodet_valitaulu] (
		...
		,[hylatty_opiskelijavuosi_paallekkainen_perusop_valmistava]
	...
	SELECT 
		...
		,opiskelijavuosi_lasna = 0.0 															
		,opiskelijavuosi_loma =	0.0 
		,opiskelijavuosi_opva =	0.0  

		,hylatty_opiskelijavuosi_loma =	0.0 
		...
		,hylatty_opiskelijavuosi_paallekkainen_perusop_valmistava =	case when ca.hylatty_opiskelijavuosi_sisaltyy_toiseen>0 then 0 else ca.hylatty_opiskelijavuosi_paallekkainen_perusop_valmistava end
		,hylatty_opiskelijavuosi_sisaltyy_toiseen = ca.hylatty_opiskelijavuosi_sisaltyy_toiseen

		--uudet
		,opiskelijavuosi_uusi = 
			case 
				when month(cast(@startDate AS date)) = 7 
					and ps.suorituksen_tyyppi in ('ammatillinentutkinto','tuvakoulutuksensuoritus','valma','telma')  
					then 0.0 
				else ooa.lasna_loma_valmistunut_pv_hyv 
			end
		,hylatty_opiskelijavuosi_heinakuu = 
			case 
				when month(cast(@startDate AS date)) = 7 
					and ps.suorituksen_tyyppi in ('ammatillinentutkinto','tuvakoulutuksensuoritus','valma','telma')  
					then ooa.lasna_loma_valmistunut_pv_hyv 
				else 0.0
			end
	...
	JOIN sa.temp_opiskelijavuodet_opiskeluoikeus_aikajakso_lasna_loma_valmistunut ooa ON ooa.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
	
	LEFT JOIN ANTERO.dw.d_opintojenrahoitus d ON d.koodi = ooa.opintojen_rahoitus
	...
	CROSS APPLY (
		...
		SELECT 
			hylatty_opiskelijavuosi_paallekkainen_amm =	
				case
					...
					when (coalesce(nullif(ooa.aiempi_koulutusmuoto,''),'ammatillinenkoulutus')='ammatillinenkoulutus' or ooa.aiempi_tuva_jarjestamislupa='ammatillinen')
						...
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu 
					when (ooa.aiempi_koulutusmuoto='ammatillinenkoulutus' or ooa.aiempi_tuva_jarjestamislupa='ammatillinen') 
						and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu - ooa.lasna_loma_valmistunut_pv_hyv
					else 0.0 
				end
			...
			,hylatty_opiskelijavuosi_paallekkainen_tuva_lukio =	
				case
					when ooa.valiaikainen_keskeytys = 1 then 0.0
					when ooa.aiempi_tuva_jarjestamislupa = 'lukio' and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu - ooa.lasna_loma_valmistunut_pv_hyv
					else 0.0 
				end
			...
			,hylatty_opiskelijavuosi_paallekkainen_tuva_perusopetus =	
				case
					when ooa.valiaikainen_keskeytys = 1 then 0.0
					when ooa.aiempi_tuva_jarjestamislupa = 'perusopetus' and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu - ooa.lasna_loma_valmistunut_pv_hyv
					else 0.0 
				end
			...
			,hylatty_opiskelijavuosi_paallekkainen_lukio =	
				case
					when ooa.valiaikainen_keskeytys = 1 then 0.0
					when ooa.aiempi_koulutusmuoto = 'lukiokoulutus' and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu - ooa.lasna_loma_valmistunut_pv_hyv
					else 0.0 
				end
			...
			,hylatty_opiskelijavuosi_paallekkainen_perusop_lisaopetus =	
				case
					when ooa.valiaikainen_keskeytys = 1 then 0.0
					when ooa.aiempi_koulutusmuoto = 'perusopetuksenlisaopetus' and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu - ooa.lasna_loma_valmistunut_pv_hyv
					else 0.0 
				end
			...
			,hylatty_opiskelijavuosi_paallekkainen_perusop_valmistava =	
				case
					when ooa.valiaikainen_keskeytys = 1 then 0.0
					when ooa.aiempi_koulutusmuoto = 'perusopetukseenvalmistavaopetus' and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 
						and eomonth(@startDate) < dateadd(year, 17, h.syntymaaika) 
						then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu - ooa.lasna_loma_valmistunut_pv_hyv
					else 0.0 
				end

			,hylatty_opiskelijavuosi_sisaltyy_toiseen = case when ooa.sisaltyy_toiseen_opiskeluoikeuteen = 1 then ooa.lasna_loma_valmistunut_pv_hyv_ei_muokattu else 0.0 end
	) ca
