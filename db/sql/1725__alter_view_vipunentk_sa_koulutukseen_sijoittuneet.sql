USE [VipunenTK_SA]
GO
/****** Object:  View [dbo].[v_K3_13_Perusk_paattaneet_amm_tutk_suorittaneet]    Script Date: 13.7.2018 11:25:27 ******/
DROP VIEW IF EXISTS [dbo].[v_K3_13_Perusk_paattaneet_amm_tutk_suorittaneet]
GO
/****** Object:  View [dbo].[v_K3_12_Perusk_paattaneet_amm_tutk_suorittaneet]    Script Date: 13.7.2018 11:25:27 ******/
...
where 
suorv = (CASE 
			WHEN NOT EXISTS (SELECT TOP 1 1 FROM TK_H9098_CSC.sys.tables t INNER JOIN TK_H9098_CSC.sys.partitions p on t.object_id = p.object_id WHERE t.name = ''TK_K3_12_sopv_18'' and p.rows > 0) THEN ''2015'' 
		END)
and (suorlk = ''1'')
...
where 
suorv = (CASE 
			WHEN NOT EXISTS (SELECT TOP 1 1 FROM TK_H9098_CSC.sys.tables t INNER JOIN TK_H9098_CSC.sys.partitions p on t.object_id = p.object_id WHERE t.name = ''TK_K3_12_sopv_19'' and p.rows > 0) THEN ''2016''
		 END)
and (suorlk = ''1'')
