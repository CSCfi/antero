USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_yo_rekrytointi_valitut]    Script Date: 24.9.2019 15:28:02 ******/
SET ANSI_NULLS ON
GO
...
LEFT JOIN [dw].[d_arvokysymys] s19 ON  --eliminate double rows (dublicate kysymysryhma for kysymys) when  kysymysryhmaid might vary
--  WHEN Finnish question
	([s19].[kysymys_fi] = [s1].[Kysymys] AND (select top 1 kysymysryhmaid from [dw].[d_arvokysymys] d1 where s1.kysymys= d1.kysymys_fi) = [s19].[kysymysryhmaid])
	 OR
--  WHEN Swedish question
	([s19].[kysymys_sv] = [s1].[Kysymys] AND (select top 1 kysymysryhmaid from [dw].[d_arvokysymys] d1 where s1.kysymys= d1.kysymys_sv) = [s19].[kysymysryhmaid])

LEFT JOIN [dw].[d_arvovastaus] s20 ON
--     WHEN Finnish question            OR    WHEN Swedish question
	[s20].[vastaus_fi] = [s1].[vastaus] OR [s20].[vastaus_sv] = [s1].[vastaus]
