...
/****** Object:  StoredProcedure [dw].[p_lataa_d_alueluokitus]    Script Date: 15.5.2023 15:50:40 ******/
SET ANSI_NULLS ON
GO
...
USING (
  SELECT DISTINCT
    ...
    tk.hyvinvointialue_koodi,
    COALESCE(tk.hyvinvointialue, tk.hyvinvointialue_sv, tk.hyvinvointialue_en) AS hyvinvointialue,
    COALESCE(tk.hyvinvointialue_sv, tk.hyvinvointialue, tk.hyvinvointialue_en) AS hyvinvointialue_sv,
    COALESCE(tk.hyvinvointialue_en, tk.hyvinvointialue, tk.hyvinvointialue_sv) AS hyvinvointialue_en,
    avikoodi,
    ...
    COALESCE(kuntaryhmakoodi, case when tk.kuntaryhma_koodi > 0 then tk.kuntaryhma_koodi end) as kuntaryhmakoodi,
    COALESCE(kuntaryhmanimi, kuntaryhmanimi_sv, kuntaryhmanimi_en, case when tk.kuntaryhma_koodi > 0 then COALESCE(tk.kuntaryhma, tk.kuntaryhma_SV, replace(kuntaryhma_EN, '-', ' ')) end)  AS kuntaryhmanimi,
    COALESCE(kuntaryhmanimi_sv, kuntaryhmanimi, kuntaryhmanimi_en, case when tk.kuntaryhma_koodi > 0 then COALESCE(tk.kuntaryhma_SV, tk.kuntaryhma, replace(kuntaryhma_EN, '-', ' ')) end) AS kuntaryhmanimi_sv,
    COALESCE(kuntaryhmanimi_en, kuntaryhmanimi, kuntaryhmanimi_sv, case when tk.kuntaryhma_koodi > 0 then COALESCE(replace(kuntaryhma_EN, '-', ' '), tk.kuntaryhma, tk.kuntaryhma_SV) end) AS kuntaryhmanimi_en,
  ...
  WHERE koodi != '200' --ulkomaat käsitellään lopussa
...
--ulkomaat erikseen, koska koodistopalvelussa tiedot kuntaa lukuun ottamatta merkitty Uudellemaalle
