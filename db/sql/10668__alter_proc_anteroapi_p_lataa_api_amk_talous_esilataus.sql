...
/****** Object:  StoredProcedure [dw].[p_lataa_api_amk_talous_esilataus]    Script Date: 10.3.2025 10.28.05 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dw].[p_lataa_api_amk_talous_esilataus] AS

TRUNCATE TABLE [dw].[api_amk_talous]

INSERT INTO [dw].[api_amk_talous]

SELECT
  ROW_NUMBER() OVER(ORDER BY f.id ASC, d1.id ASC, d2.id ASC, d3.id ASC, d5.id ASC, d6.id ASC) as defaultorder
  ,d1.organisaatio_koodi AS 'Ammattikorkeakoulun tunnus'
  ...
  FROM antero.dw.f_amk_talous_vanha_koodisto f
