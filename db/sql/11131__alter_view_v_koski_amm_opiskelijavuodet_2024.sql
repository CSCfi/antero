...
/****** Object:  View [dw].[v_koski_amm_opiskelijavuodet_2024]    Script Date: 3.9.2025 11.42.45 ******/
SET ANSI_NULLS ON
GO
...
ALTER   VIEW [dw].[v_koski_amm_opiskelijavuodet_2024] AS

/* 
Tätä näkymää käytetään ammatillisen koulutuksen suoritetietojen toimituksessa kustannuskyselyä varten. 
Jokaiselle vuodelle oma näkymä ja vuosi tieto nimessä sekä päivitettynä WHERE ehtoon.
*/
...
--Lisätty Luvaton koulutus JK 2.9.2025
--Vaihdettu faktataulu taulusta dw.f_amos_opiskelijavuodet tauluun dw.f_amos_opiskelijavuodet_painotetut

SELECT
	[Opiskelijavuosi] = sum(opiskelijavuodet_painottamaton) + SUM(case when d_amos_spl_opiskelijavuodet_hylkaamisperuste.koodi in ('2','3','4','10','11','12') then opiskelijavuodet_hylatyt else 0 end)
	...
	,[Luvaton Koulutus] = d_amos_luvaton_koulutus.selite_fi
...
LEFT JOIN dw.d_kalenteri AS d_kalenteri ON f.d_kalenteri_tilastokuukausi_id = d_kalenteri.id 
...
LEFT JOIN dw.d_amos_luvaton_koulutus AS d_amos_luvaton_koulutus ON d_amos_luvaton_koulutus.id = f.d_amos_luvaton_koulutus_id 

WHERE d_kalenteri.vuosi=2024  -- Tähän sama vuosi kuin näkymän nimessä
...
GROUP BY d_kytkin_osa_aikaisuus.kytkin_fi
	...
	,d_amos_luvaton_koulutus.selite_fi

HAVING sum(opiskelijavuodet_painottamaton) + SUM(case when d_amos_spl_opiskelijavuodet_hylkaamisperuste.koodi in ('2','3','4','10','11','12') then opiskelijavuodet_hylatyt else 0 end) > 0.0