...
/****** Object:  StoredProcedure [dw].[p_lataa_api_tavoiteajassa_tutkinnon_suorittaneet_esilataus]    Script Date: 22.3.2024 7:45:45 ******/
SET ANSI_NULLS ON
GO
...
TRUNCATE TABLE [AnteroAPI].[dw].[api_tavoiteajassa_tutkinnon_suorittaneet]
 ...
 BEGIN
	...
	SET @sql = 
		...
		' ,[Sektori] = case d1.oppilaitostyyppi_koodi' +
		...
		' , [Koodi Sektori] = case d1.oppilaitostyyppi_koodi' +
		' when ' + '''' + '41' + '''' + ' then ' + '''' + '4' + '''' +
		' when ' + '''' + '42' + '''' + ' then ' + '''' + '5' + '''' +
		' else ' + '''' + '-1' + '''' +
		' end' +
		...
		' , MONTH(dateadd(S, f.[tutkinnonsuorituspvm]/1000, ' + '''' + '1970-01-01 03:00:00' + '''' + ')) as suorituskuukausi' + 
		' , oppilaitosnro' +
		' , tutkintokoodi' +
		' , [Tutkinnot]	= count(*)' +
		...
		' , onaiempitutkinto' +
		' , tutkintokerroin' +
		' , MONTH(dateadd(S, f.[tutkinnonsuorituspvm]/1000, ' + '''' + '1970-01-01 03:00:00' + '''' + '))' +
		' ) t' +
		...
		--' WHERE d1.oppilaitostyyppi_koodi in (' + '''' + '41' + '''' + ',' + '''' + '42' + '''' +') and (cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) = 2020 or cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) = t.tutkintovuosi) and cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) >= t.tutkintovuosi'
		' WHERE d1.oppilaitostyyppi_koodi in (' + '''' + '41' + '''' + ',' + '''' + '42' + '''' +') and ((cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) = 2020 and t.tutkintovuosi < 2019) or cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) = 2024 and t.tutkintovuosi >= 2019)'
