...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_vakajarjestajat_toimipaikat2021]    Script Date: 3.5.2023 12:32:57 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	FROM varda_sa.sa.sa_varda_vakajarjestaja2021 s
	INNER JOIN varda_sa.sa.sa_varda_toimipaikka2021 s2 on s2.vakajarjestaja_id = s.vakajarjestaja_id
	...
	OUTER APPLY (
		SELECT d0.id FROM (
			SELECT 
				...
				COUNT(DISTINCT s02.organisaatio_oid) as lkm FROM varda_sa.sa.sa_varda_vakajarjestaja2021 s0
			INNER JOIN varda_sa.sa.sa_varda_toimipaikka2021 s02 on s02.vakajarjestaja_id = s0.vakajarjestaja_id
