...
/****** Object:  View [dw].[v_st_rekrytointi_valitut]    Script Date: 28.3.2019 13:01:11 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_st_rekrytointi_valitut] AS
 ...
 --pois korkeakoulut, joilla vastausprosentti alle 25
 where cast(f.vuosi as varchar) + d1.organisaatio_koodi in (
 
	select 
		cast(f.vuosi as varchar) + d.yo_nykyinen_tunnus
	from dw.f_yo_rekrytointi_valitut f
	join dw.d_yo d on d.id = f.d_yliopisto_id
	group by vuosi,d.yo_nykyinen_tunnus
	having cast(count(distinct case when d_arvovastaus_id  != '-1' then henkilonumero else null end) as float)/ cast(count(distinct henkilonumero) as float) >= 0.25

	union

	select 
		cast(f.vuosi as varchar) + d.amk_nykyinen_tunnus
