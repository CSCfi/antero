USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_yo_rekrytointi_valitut]    Script Date: 12.9.2018 15:42:39 ******/
SET ANSI_NULLS ON
GO
...
MERGE dw.f_yo_rekrytointi_valitut AS target
USING (
		SELECT
			s2_1.[tilastovuosi] AS [tilastovuosi]
			,s2_2.[id] AS [yliopisto_id]
			,s2_1.[henkilonumero] AS [henkilonumero]
			,s2_3.[id] AS [tohtoritutkintokorkeakoulu_id]
			,s2_4.[id] AS [tohtoritutkintomaa]
			,[tohtoritutkintosuoritusvuosi]
		FROM [ANTERO].[sa].[sa_virta_otp_rekrytohtorit] s2_1
		LEFT JOIN [dw].[d_yo] s2_2 ON [s2_2].[yo_tunnus] = [s2_1].[rekrykorkeakoulu]
		LEFT JOIN [dw].[d_yo] s2_3 ON [s2_3].[yo_tunnus] = [s2_1].[rekrykorkeakoulu]
...
ON target.[vuosi] = src2.[tilastovuosi]
	AND target.[d_yliopisto_id] = src2.[yliopisto_id]
	AND target.[henkilonumero] = src2.[henkilonumero]
WHEN MATCHED
	THEN UPDATE
		SET  target.[d_tohtorintutkinnon_suoritusmaa_id] = src2.[tohtoritutkintomaa]
			,target.[d_tohtorintutkinnon_yliopisto_id] = src2.[tohtoritutkintokorkeakoulu_id]
