...
/****** Object:  View [dw].[v_amos_opiskelijavuodet_uusi]    Script Date: 25.9.2026 15.45.14 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_amos_opiskelijavuodet_uusi] AS 
...
SELECT
	...
	,vuosiraportti_1_0
	,[Järjestäjärakenne] = case when d34.sisaltyy_amos_spl = 1 then 'Viimeisin suoritepäätös' else 'Poistuneet järjestäjät' end
	...
	,[jarjestys Vaativa erityinen tuki] =
		case 
			when d13.erityisopetus_koodi in ('4','5','6') then 1 --'Tutkintokoulutus'
			when d13.erityisopetus_koodi in ('7','8','9') and d4.koulutusluokitus_koodi in ('999901','999908') then 2 --'TUVA/VALMA'
			when d13.erityisopetus_koodi in ('7','8','9') and d4.koulutusluokitus_koodi = '999903' then 3 --'TELMA'
			else 0 --'Ei vaativaa erityistä tukea'
		end

	,[Toisen asteen pohjakoulutus (2018-)] = d0.selite_fi
	,[jarj_toisen_asteen_pohjakoulutus_2018] = d0.jarjestys	
	,[Suoritettu tutkinto, maksuttomuus (2018-)] = d0b.selite2_fi
	,[jarj_suoritettu_tutkinto_maksuttomuus_2018] = d0b.jarjestys	
	,[Maksuttomuus (rahoitus)] = case when maksuttomuus = 1 then 'Maksuton (rahoitus)' else 'Ei maksuton (rahoitus)' end
	...
	,[Perusop. suoritusvuosi] = perusopetuksen_suoritusvuosi
	,[Perusop. keskiarvoluokka] = d36.selite_fi
	,[Perusop. keskiarvoluokka 2] = d36.selite2_fi
	,[Perusop. yksilöllistäminen] = case yksilollistaminen when 1 then 'Alueittain' when 2 then 'Kokonaan' when 3 then 'Osittain' when 4 then 'Ei' else 'Tieto puuttuu' end
	,[jarj_perusop_keskiarvoluokka] = d36.jarj_keskiarvo
	,[jarj_perusop_keskiarvoluokka_2] = d36.jarj_keskiarvo2
	,[jarj_perusop_yksilollistaminen] = coalesce(yksilollistaminen, 9)
	...
	,d34.[Toiminnanohjauksen kokeilu]
	,d34.[jarjestys Toiminnanohjauksen kokeilu]
	,d34.[Koul. järjestäjän PILKE-alue (kokeilu)]
	,d34.[jarjestys Koul. järjestäjän PILKE-alue (kokeilu)]
	
	,[Vaativan erityisen tuen opiskelijavuosien enimmäismäärä] = coalesce(cast(opv_vet_max as varchar), 'Tieto puuttuu')
	,[jarjestys Vaativan erityisen tuen opiskelijavuosien enimmäismäärä] = coalesce(opv_vet_max, 999999)
	,[Ei maksuttomien opiskelijavuosien enimmäismäärä] = coalesce(cast(opv_ei_maksuton_max as varchar), 'Tieto puuttuu')
	,[jarjestys Ei maksuttomien opiskelijavuosien enimmäismäärä] = coalesce(opv_ei_maksuton_max, 999999)

	,d19.sisaltyy_okm_perusrahoitus --koontinäkymää varten
	
	,opv_ei_maksuton_kerroin
	,opv_kerroin_keskiarvoluokka = case when maksuttomuus = 1 then d36.opv_kerroin end --aineiston K1.16 sisältää erillisraporttia varten

    ,painottamaton = [opiskelijavuodet_painottamaton]
    ,painotettu_lahtotaso = [opiskelijavuodet_painotettu_lahtotaso]
	,painotettu_kustannusryhma = [opiskelijavuodet_painotettu_kr_ja_tt_yhteensa]
	,painotettu_erityistuki = [opiskelijavuodet_painotettu_erityistuki_hyv]
    ,painotettu_majoitus = [opiskelijavuodet_painotettu_majoitus_hyv]
  ...
  --
	...
	,[Koodit Oppilaitos] = coalesce(nullif(d5.organisaatio_koodi, ''), '-1')
	,[Koodit Toimipiste (OID)] = d21.organisaatio_oid
	...
	,left(d4.jarjestys_koulutusalataso1_koodi, 2) AS jarj_koulutusala_taso1
	,left(d4.jarjestys_koulutusalataso2_koodi, 3) AS jarj_koulutusala_taso2
	,left(d4.jarjestys_koulutusalataso3_koodi, 4) AS jarj_koulutusala_taso3
...
LEFT JOIN dw.d_amos_spl_jarjestaja_linkki d34 ON d34.id = f.d_amos_spl_jarjestaja_linkki_id  -- CSCVIPUNEN-3358: Taustatiedot uusimman varainhoitovuoden järjestäjärakenteen mukaan. JK
	...
	SELECT
		lv_suorituskieli =
			case	
				...
				when d33.kieli_koodi = '-2' then 'Ei luvassa mainittu suorituskieli' 
				else 'Luvassa mainittu suorituskieli' 
			end
		,lv_suorituskieli_koodi = 
			case	
				when d18.koodi = 'muu' then 3
				when d33.kieli_koodi = '-2' then 2
				else 1
			end
) ca
