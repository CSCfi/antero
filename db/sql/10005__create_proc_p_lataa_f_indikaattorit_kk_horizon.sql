...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_horizon]    Script Date: 23.8.2024 8:06:52 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_indikaattorit_kk_horizon] as
...
USING (
	...
	FROM (
		...
		LEFT JOIN (
			SELECT
					CAST(CASE WHEN startDate like '%/%' THEN right(startDate, 4) ELSE left(startDate,4) END as int) as tilastovuosi,
					CASE
					...
					COUNT(DISTINCT CASE WHEN d6.[Ohjelman lyhenne] = 'European Research Council (ERC)' THEN project_id ELSE NULL END) as arvo
				FROM [dw].[f_horizon] f
				...
				AND d2.maa = 'Finland' and CAST(CASE WHEN startDate like '%/%' THEN right(startDate, 4) ELSE left(startDate,4) END as int) >= 2018
				GROUP BY CAST(CASE WHEN startDate like '%/%' THEN right(startDate, 4) ELSE left(startDate,4) END as int),
				CASE
					WHEN [name] = 'AALTO KORKEAKOULUSAATIO SR' THEN '10076'
					WHEN [name] = 'HELSINGIN YLIOPISTO' THEN '01901'
					WHEN [name] = 'ITA-SUOMEN YLIOPISTO' THEN '10088'
					...
					WHEN [name] = 'ABO AKADEMI' THEN '01903'
				END
		) h on h.tilastovuosi = f.vuosi and h.organisaatio_koodi = o.organisaatio_koodi
	...
	OUTER APPLY (
		...
		CAST(CASE WHEN d3b.startDate like '%/%' THEN right(d3b.startDate, 4) ELSE left(d3b.startDate,4) END as int) = f.tilastovuosi
	) s
