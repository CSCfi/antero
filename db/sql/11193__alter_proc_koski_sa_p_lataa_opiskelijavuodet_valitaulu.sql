...
--@pv annetaan nollana kutsuttaessa uuden mallin latauksessa
		...
		,opiskelijavuosi_uusi = 
			case 
				when month(cast(@startDate AS date)) = 7 
					...
					then 0.0 
				else ooa.lasna_pv_hyv + ooa.loma_pv_nyk /*huom _nyk*/ + ooa.opva_pv_hyv
			end
		,hylatty_opiskelijavuosi_heinakuu = 
			case 
				when month(cast(@startDate AS date)) = 7 
					and ps.suorituksen_tyyppi in ('ammatillinentutkinto','tuvakoulutuksensuoritus','valma','telma')  
					then ooa.lasna_pv_hyv + ooa.loma_pv_nyk /*huom _nyk*/ + ooa.opva_pv_hyv
				else 0.0
			end
