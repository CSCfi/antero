...
/****** Object:  StoredProcedure [dbo].[p_lataa_OTV_f_2_8]    Script Date: 22.8.2018 10:06:14 ******/
SET ANSI_NULLS ON
GO
...
(case
	 ...
	 when f.tilastointivuosi = (select max(tilastointivuosi) from VipunenTK_DW.dbo.f_2_8_Korkeakouluopiskelijat) and not exists (select 1 from VipunenTK_DW.dbo.f_2_8_Korkeakouluopiskelijat where tilastointivuosi = (select max(tilastointivuosi) from VipunenTK_DW.dbo.f_2_8_Korkeakouluopiskelijat) and aineisto = 'L') then 'E' 
	 else 'L' 
	 end	
)
