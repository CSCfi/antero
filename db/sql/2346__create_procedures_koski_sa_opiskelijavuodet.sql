USE [Koski_SA]
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_lasna_valmistunut]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_loma]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_opva]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
		...
		FROM (
			...
			FROM (
				SELECT distinct 
					...
					,oppisopimus_jossain_paatason_suorituksessa 
					,opiskeluvalmiuksia_tukevat_opinnot
					,alkanut_samana_paivana_hylataan = case when hyl.opiskeluoikeus_oid_hylataan is not null then 1 else 0 end
					,toisen_opiskeluoikeuden_sisalla = case when oo_l.toisen_opiskeluoikeuden_sisalla = 1 then 1 else 0 end
					,sisaltyy_toiseen_opiskeluoikeuteen
					,aiempi_koulutusmuoto = coalesce(oo_l.aiempi_koulutusmuoto,'')
					,ed_jatkuu
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_opiskeluoikeus_oidit]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
from (
	...
	select distinct
		...
		,hyl.opiskeluoikeus_oid_hylataan
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_osatutkinnot_kustannusryhma]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_paallekkaiset_opiskeluoikeudet]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
		...
		inner join (
			...
			and oo.sisaltyy_opiskeluoikeuteen_oid is null
			   ...
			   and (													--ammatillinen alkanut ennen nykyistä
				(oo_la.alkamispaiva < oo_a.alkamispaiva and oo_la.koulutusmuoto = 'ammatillinenkoulutus') 
				or														--lukio alkanut ennen nykyistä tai samana päivänä
				(oo_la.alkamispaiva <= oo_a.alkamispaiva and oo_la.koulutusmuoto = 'lukiokoulutus') 
			   )
			   and oo_la.paattymispaiva > oo_a.alkamispaiva				--ei ole päättynyt ennen nykyistä
			   and ((oo_la.loppu between ooa.alku and ooa.loppu) or (oo_la.alku between ooa.alku and ooa.loppu) or (ooa.alku between oo_la.alku and oo_la.loppu) or (ooa.loppu between oo_la.alku and oo_la.loppu)) --aikajaksot menevät päällekkäin
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_paatason_suoritus]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_samana_paivana_alkaneet_hylattavat]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
INSERT INTO Koski_SA.sa.temp_opiskelijavuodet_samana_paivana_alkaneet_hylattavat

SELECT DISTINCT
	oppija_oid
	,opiskeluoikeus_oid_hylataan = case when oid1 = oid_valittu then null else oid1 end 
	--,oid_ei_hylata = case when oid1 = oid_valittu then oid1 else null end 
...
GO
/****** Object:  StoredProcedure [sa].[p_lataa_osasuoritus_kustannusryhma]    Script Date: 2.8.2019 15:21:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
