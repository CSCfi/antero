...
/****** Object:  View [dbo].[v_sa_2_4efg_Lukio_ja_amm_koulutuksen_tutkinnot]    Script Date: 5.6.2026 15.28.30 ******/
SET ANSI_NULLS ON
GO
...
SELECT
	  ...
	  ,[jarj_opisk] 
	  ,[kkun_opisk] 
	  ,[tietolahde]='K2_4h_sopv_26'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K2_4h_sopv_26] --tämäkin mukaan
WHERE toteuma_tutk is not null 
...
SELECT
	  ...
	  ,[jarj_opisk] = NULL
	  ,[kkun_opisk] = NULL
	  ,[tietolahde]='2_4g_Lukio_ja_amm_koulutuksen_opiskelijat_26'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K2_4g_sopv_26]
