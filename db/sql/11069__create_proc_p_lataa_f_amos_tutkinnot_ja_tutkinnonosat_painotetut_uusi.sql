...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_amos_tutkinnot_ja_tutkinnonosat_painotetut_uusi] @generoitu2 int = null
AS

DECLARE @maxSuorituskausi as varchar(24) = (select max(suorituskausi) from sa.sa_amos_tutkinnot_pohjakoulutus_kaikki_unpivot_uusi)
...
FROM (
	...
	SELECT
		...
		,[painotettu_tutkinnot] = 
			...
			* coalesce(d4.ei_pk_osuus, ca.ei_pk_osuus_kaikki)
	...
	--tutkintojen pohjakoulutuksellisten osuudet; HUOM. aineistoon lopulta myös rahoituskausi, mikä huomioitava alla sa-taulun latauksessa sekä liitoksessa/rajausehdossa
	left join sa.sa_amos_tutkinnot_pohjakoulutus_kaikki_unpivot_uusi d4 
		on d4.suorituskausi = (case when right(f.suorituskausi,4) > right(@maxSuorituskausi,4) then @maxSuorituskausi else f.suorituskausi end) 
		and d4.ytunnus = coalesce(d0.ytunnus_amos_spl, d0.ytunnus_avain) 
		and d4.tutktyyppi = (case when d1.tutkintotyyppi_koodi in ('19','20') then '19_20' else d1.tutkintotyyppi_koodi end)
	...
	cross apply (
		...
		where 1=1
		and suorituskausi = (case when right(f.suorituskausi,4) > right(@maxSuorituskausi,4) then @maxSuorituskausi else f.suorituskausi end) 
		and tutktyyppi = (case when d1.tutkintotyyppi_koodi in ('19','20') then '19_20' else d1.tutkintotyyppi_koodi end) 
	) ca
	...
	*/
	...
	/*
	--osaamispisteiden enimmäismäärä leikkuria varten
	outer apply (
		select
			osp_ka_max = 120.0 * (
				coalesce(opv_ka.opv_maksuton_ka, 0.0)
