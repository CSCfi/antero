...
/****** Object:  View [dw].[v_haku_ja_valinta_kk_yo_keskiarvo]    Script Date: 5.2.2026 8.46.15 ******/
SET ANSI_NULLS ON
GO
...
WHERE d44.selite_fi = 'I sykli' AND d46.selite_fi <> 'Tieto puuttuu' and master_oid is not null and d1.visualisoinnit <= cast(getdate() as date)) f
INNER JOIN (SELECT master_oid, AVG(CAST(CASE arvosana_kirjain WHEN 'L' THEN 7 WHEN 'E' THEN 6 WHEN 'M' THEN 5 WHEN 'C' THEN 4 WHEN 'B' THEN 3 WHEN 'A' THEN 2 WHEN 'I' THEN 0 END as float)) as keskiarvo
FROM [ANTERO].[dw].[f_haku_ja_valinta_yoarvosanat]
GROUP BY master_oid) f2 on f.[Henkilö OID] = f2.master_oid