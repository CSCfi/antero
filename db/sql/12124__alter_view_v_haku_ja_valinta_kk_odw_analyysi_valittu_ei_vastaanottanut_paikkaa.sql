...
/****** Object:  View [dw].[v_haku_ja_valinta_kk_odw_analyysi_valittu_ei_vastaanottanut_paikkaa]    Script Date: 27.7.2026 11.58.25 ******/
SET ANSI_NULLS ON
GO
...
ALTER   VIEW [dw].[v_haku_ja_valinta_kk_odw_analyysi_valittu_ei_vastaanottanut_paikkaa] AS

SELECT 
	f.master_oid as HenkiloOID,
	f.koulutuksen_alkamisvuosi as 'Koulutuksen alkamisvuosi',
	CASE
		WHEN d2.ika_avain BETWEEN 0 and 21 THEN '20–vuotiaat ja sitä nuoremmat'
		WHEN d2.ika_avain BETWEEN 21 and 24 THEN '21–24-vuotiaat'
		WHEN d2.ika_avain BETWEEN 25 and 29 THEN '25–29-vuotiaat'
		WHEN d2.ika_avain > 24 THEN '30–vuotiaat ja sitä vanhemmat'
		ELSE 'Tieto puuttuu'
	END as 'Ikäryhmä',
	CASE 
		WHEN d3.maakunta_fi = 'Uusimaa' THEN 'Kyllä'
		WHEN d3.maakunta_fi = 'Tieto puuttuu' THEN 'Tieto puuttuu'
		ELSE 'Ei'
	END as [Kotimaakunta Uudellamaalla],
	d4.kytkin_fi as [Kansainvälinen opiskelija],
...
FROM [dw].[f_haku_ja_valinta_ovara_analyysi_toiminta] f
...
LEFT JOIN ANTERO.dw.d_kytkin d4 on d4.kytkin_koodi = f.kansainvalinen_opiskelija
...
LEFT JOIN ANTERO.dw.d_kytkin d7 on d7.kytkin_koodi = f.aidosti_ensikertainen
