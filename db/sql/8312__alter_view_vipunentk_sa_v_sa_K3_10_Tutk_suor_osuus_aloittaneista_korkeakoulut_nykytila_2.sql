...
/****** Object:  View [dbo].[v_sa_K3_10_Tutk_suor_osuus_aloittaneista_korkeakoulut_nykytila_2]    Script Date: 8.11.2023 12:44:41 ******/
SET ANSI_NULLS ON
GO
...
/*    [tilv]
   ...
   ,[alvv]
   ,[allk] = 
   -- Tampereen teknisen yliopiston kev‰‰n ja syksyn opiskelijam‰‰r‰t ovat v‰‰rin p‰in aloitusvuosilla 2004 ja 2006.
   -- Korjataan virhe k‰‰nt‰m‰ll‰ aloituskaudet toisin p‰in.
   -- CSC Jarmo 21.3.2014
    CASE 
      ...
      ELSE allk
    END
   ,[alvv_kk] = coalesce(nullif([alvv_kk],''), [alvv]) --t‰yttˆ siirretty myˆhemm‰st‰ latauksesta / CSC Juha 10.9.2021
	,[allk_kk] =
    -- Tampereen teknisen yliopiston kev‰‰n ja syksyn opiskelijam‰‰r‰t ovat v‰‰rin p‰in aloitusvuosilla 2004 ja 2006.
    -- Korjataan virhe k‰‰nt‰m‰ll‰ aloituskaudet toisin p‰in.
    -- CSC Jarmo 21.3.2014
		coalesce(nullif( --t‰yttˆ siirretty myˆhemm‰st‰ latauksesta / CSC Juha 10.9.2021
			CASE 
...
-- UUSI VUOSI
-- lis‰‰ sarakkeet ylle ja p‰ivit‰ tietol‰hde sek‰ l‰hdetaulut alle

    ,[lkm]
    ,[tietolahde]='K3_10_sopv_23'
    ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv, alvv, allk)
    ,[aineistorivinumero] = ROW_NUMBER() OVER(ORDER BY tilv, alvv, allk)
	,f1.[rivinro]
  FROM [TK_H9098_CSC].[dbo].[TK_K3_10_sopv_23_1] f1
  inner join [TK_H9098_CSC].[dbo].[TK_K3_10_sopv_23_2] as f2 on f1.rivinro=f2.rivinro 
