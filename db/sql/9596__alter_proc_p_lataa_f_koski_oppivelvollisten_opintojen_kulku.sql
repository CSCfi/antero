...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvollisten_opintojen_kulku]    Script Date: 14.2.2024 11:10:30 ******/
SET ANSI_NULLS ON
GO
...
BEGIN	
	...
	FROM (
		...
		FROM (
			...
			WHERE aika.alku < ov.oppivelvollisuus_suoritettu 
			...
			and ooa.tila <> 'katsotaaneronneeksi' 
			and aika.alku BETWEEN @alkuPvm and @loppuPvm 
			and ((ov.perusopetus_alkanut = aika.alku and oo.koulutusmuoto in ('aikuistenperusopetus', 'perusopetus', 'internationalschool', 'europeanschoolofhelsinki')))
			AND (
				ooa.opiskeluoikeus_oid is null
				OR NOT EXISTS (
					SELECT * FROM Koski_SA.sa.tuva_lasna_loma_jaksot tuvat
	...
	BEGIN
		...
		FROM (
			...
			FROM (
				...
				OR NOT EXISTS (
					...
					WHERE oo.koulutusmuoto != 'tuva'
					AND tuvat.oppija_oid = oo.oppija_oid
					AND DATEADD(month,@m,ov.alkamispaiva) between tuvat.alku and tuvat.loppu
				)
			) f
		...
		FROM (
			...
			FROM (
				...
				LEFT JOIN ANTERO.dw.d_kytkin d22 on d22.kytkin_koodi = s3.keskeyttanyt_valpas
			) s 
		) s 
...
DROP TABLE Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku_tilanne
DROP TABLE Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku_B
DROP TABLE Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku_rajaukset
DROP TABLE Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku
DROP TABLE Koski_SA.sa.temp_oppivelvolliset_esirajaukset_opintojen_kulku
