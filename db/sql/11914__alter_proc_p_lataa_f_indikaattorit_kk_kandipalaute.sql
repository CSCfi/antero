...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_kandipalaute]    Script Date: 16.3.2026 10.51.16 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dw].[p_lataa_f_indikaattorit_kk_kandipalaute] as
...
USING (
	...
	FROM (
		...
		FROM (		
			...
			WHERE d1.rahoitusmallikysymys = 1 and f.kyselyvuosi BETWEEN 2021 AND YEAR(DATEADD(MONTH, -1, getdate())) - 1
	...
	OUTER APPLY (
		...
		FROM (		
			...
			WHERE d1b.rahoitusmallikysymys = 1 and fb.kyselyvuosi = f.tilastovuosi
