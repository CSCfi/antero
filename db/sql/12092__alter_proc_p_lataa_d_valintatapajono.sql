...
/****** Object:  StoredProcedure [dw].[p_lataa_d_valintatapajono]    Script Date: 2.6.2026 14.32.51 ******/
SET ANSI_NULLS ON
GO
...
USING (
	SELECT 
		valintatapajono_id as valintatapajono_oid, 
	...
	FROM ANTERO.sa.sa_ovara_gen_valintarekisteri_valintatapajono
	WHERE valintatapajono_nimi is not null

	UNION ALL

	SELECT 
		concat('odw_', cast(vtj.id as varchar)) as valintatapajono_oid,
