...
/****** Object:  View [dw].[v_haku_ja_valinta_kk_odw_analyysi_yo_arvosanat]    Script Date: 28.4.2026 8.09.24 ******/
SET ANSI_NULLS ON
GO
...
ALTER   VIEW [dw].[v_haku_ja_valinta_kk_odw_analyysi_yo_arvosanat] AS
...
SELECT DISTINCT
	...
	CASE
		WHEN d7.laajuus_fi = 'äidinkieli' THEN yo_aineen_arvosana
		WHEN d7.aine_selite_fi = 'Matematiikka' THEN yo_aineen_arvosana
		ELSE NULL
	END as yo_aineen_arvosana,
	f.yo_arvosanojen_keskiarvo,
	CASE
		WHEN d7.laajuus_fi = 'äidinkieli' THEN 'Äidinkieli' 
		WHEN d7.aine_selite_fi = 'Matematiikka' THEN concat(d7.aine_selite_fi,', ', d7.laajuus_fi)
		ELSE NULL
	END as Aine
...
LEFT JOIN ANTERO.dw.d_koskiyokokeet d7 on d7.id = f.d_koskiyokokeet_id