...
/****** Object:  View [dw].[v_amos_spl_koski_tutkinnon_osien_suoritukset]    Script Date: 16.9.2019 16:14:10 ******/
SET ANSI_NULLS ON
GO
...
FROM   (
	...
	SELECT DISTINCT
		...
		CASE 
			WHEN Year(arviointi_paiva) = 2018 /* Year(Getdate()) - 1 */ THEN 1 
			ELSE 0 
	...
	LEFT JOIN [Koski_SA].[sa].[f_eperusteet_osaamisalat_tutkinnonosat_laajuudet] as ep ON os.koulutusmoduuli_koodiarvo_numeerinen = ep.tutkinnonosa_koodi and rnk2=1
	--jhe 30.4 tuodaan ep:stä usean tutkinnonosan liputus
	LEFT JOIN [Koski_SA].[sa].[v_eperusteet_liputa_usean_tutkinnon_tutkinnonosat] as epu ON os.koulutusmoduuli_koodiarvo_numeerinen = epu.tutkinnonosa_koodi
