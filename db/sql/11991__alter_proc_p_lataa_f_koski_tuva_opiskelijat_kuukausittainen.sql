...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_tuva_opiskelijat_kuukausittainen]    Script Date: 21.4.2026 8.32.00 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	-- Temp-tauluun läsnä/loma päivät kuukauden aikana opiskeluoikeuksittain
	SELECT
		oo0.opiskeluoikeus_oid,
		COUNT(DISTINCT d0.kalenteri_avain) as pv_kk
	INTO #temp_pv_kk
	FROM Koski_SA.sa.sa_temp_tuva_opiskeluoikeus oo0
	LEFT JOIN ANTERO.dw.d_kalenteri d0 on d0.kalenteri_avain BETWEEN GREATEST(@alkuPvm, oo0.alku) AND LEAST(@loppuPvm, oo0.loppu)
	WHERE oo0.alku <= @loppuPvm and oo0.loppu >= @alkuPvm
	GROUP BY oo0.opiskeluoikeus_oid
	...
	FROM (	
		SELECT DISTINCT
			...
			,case when YEAR(oo.alkamispaiva) >= 2026 and oo.erityisopetus in (1,2,3) then -1 else oo.erityisopetus end as erityisopetus
		...
		-- Läsnä/loma päivät kuukauden aikana opiskeluoikeuksittain
		LEFT JOIN #temp_pv_kk pv_kk on pv_kk.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
		WHERE oo.alku <= @loppuPvm and oo.loppu >= @alkuPvm
	) f
	WHERE f.muut_opiskeluoikeudet = f.muut_opiskeluoikeudet_rnk

	DROP TABLE IF EXISTS #temp_pv_kk
...
-- Aputaulu visualisointia varten
TRUNCATE TABLE ANTERO.sa.sa_tuva_koulutuksen_jarjestajat_ilman_opiskelijoita
INSERT INTO ANTERO.sa.sa_tuva_koulutuksen_jarjestajat_ilman_opiskelijoita
SELECT 
	f.*, 
	getdate() as loadtime 
FROM (
	...
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d1 on d1.id = f.d_koulutuksen_jarjestaja_id
) f