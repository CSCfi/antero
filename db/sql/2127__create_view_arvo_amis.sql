USE [ANTERO]
GO
/****** Object:  View [dw].[v_arvo_amis]    Script Date: 26.2.2019 14:35:22 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dw].[v_arvo_amis]'))
...
, av2.koulutustausta_fi						AS ''Aiempi koulutus''
, case av4.kytkin_koodi
	when 1 then ''Oppisopimuskoulutus''
	when 0 then ''Ei oppisopimuskoulutus''
	else ''Tieto puuttuu''								
  end										AS ''Oppisopimuskoulutus''
...
, av2.priorisointi_vastaajaid				AS ''pohjakoulutus_priorisointi_vastaajaid''
...
, av2.koulutustausta_sv						AS ''Tidigare utbildning''
, case av4.kytkin_koodi
	when 1 then ''Läroavtalsutbildning''
	when 0 then ''Ej läroavtalsutbildning''
	else ''Information saknas''								
  end										AS ''Läroavtalsutbildning''
...
, av2.koulutustausta_en						AS ''Prior education''
, case av4.kytkin_koodi
	when 1 then ''Apprenticeship training''
	when 0 then ''Not apprenticeship training''
	else ''Missing data''								
  end										AS ''Apprenticeship training''
...
, d3.kuukausi								AS ''Koodit Tilastokuukausi''     
														...
														--JÄRJESTYSMUUTTUJAT
, av2.priorisointi							AS ''Järjestys aiempi koulutus''
...
LEFT JOIN dw.d_kytkin av4 ON av4.id = f.d_arvovastaus_taustakysymys_oppisopimus_id
