...
/****** Object:  StoredProcedure [sa].[p_lataa_temp_koski_opintojen_kulku_3b_yleissivistava]    Script Date: 7.10.2021 9:17:01 ******/
SET ANSI_NULLS ON
GO
...
SELECT
 ...
 ,min_priorisoitu_tilanne_yleissivistava_koodi --vaihdosten kytkemiseksi priorisointiin ylemmässä kyselyssä
  = min(priorisoitu_tilanne_yleissivistava_koodi) over(partition by kohortti_vp
																 ,[oppija_oid]
																 ,opiskeluoikeus_oid
																 ,koulutus_kytkin
																 ,tutkinto_koodi
...
SELECT 
 ...
 ,priorisoitu_tilanne_yleissivistava_koodi = 
		...
		CASE
			WHEN taso1 = 'Valmistunut' OR (valm.oppimaara = q1.oppimaara /*JUHAN TESTI, SIIRTO LOPUN RAJAUKSEEN: and valm.vahvistus_paiva BETWEEN q1.alkamispaiva AND tarkastuspaiva*/) then 1
			...
			WHEN /*JUHAN TESTI, SIIRTO LOPUN RAJAUKSEEN: valm.vahvistus_paiva BETWEEN q1.alkamispaiva AND tarkastuspaiva AND */
				valm.tutkinto_koodi NOT IN ('301101', '301102', '301103', '301104') THEN 5
			WHEN uusi.suorituksen_tyyppi IN ('ammatillinentutkinto', 'ammatillinentutkintoosittainen' 
				/*JUHAN TESTI*/
				, 'muuammatillinenkoulutus','tutkinnonosaapienemmistäkokonaisuuksistakoostuvasuoritus','valma','telma') AND uusi.tila IN ('lasna', 'loma', 'valiaikaisestikeskeytynyt', 'valmistunut'
			) THEN 6
			ELSE 8 /*JUHAN TESTI, AINEOPISKELUUN SIIRTYMINEN SIIS TULKITAAN KESKEYTYKSEKSI*/
  ...
  LEFT JOIN Koski_SA.sa.temp_koski_opintojen_kulku_2c_muut_opiskeluoikeudet uusi 
