...
/****** Object:  StoredProcedure [dw].[p_lataa_f_oiva_luvat_tuva]    Script Date: 29.8.2024 14:35:11 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dw].[p_lataa_f_oiva_luvat_tuva] @vuosi as int = 2018 AS
...
BEGIN 
	...
	BEGIN
		...
		,opetuskieli as (	--Sisältää "suomi ja ruotsi"
			select
				pvt.lupaId
		...
		FROM (
			SELECT DISTINCT 
				...
				--,oppilaitoksenopetuskieli = ok.opetuskieli_koodi		--Sisältää "suomi ja ruotsi"
			...
			--LEFT JOIN opetuskieli ok ON ok.lupaId=l.paalupa_id	--Sisältää "suomi ja ruotsi"
