...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amos_tase_ja_tunnusluvut]    Script Date: 5.3.2026 15.11.49 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dw].[p_lataa_f_amos_tase_ja_tunnusluvut] AS
...
SELECT 
		...
		,rahoitusomaisuus
		,edel_ylialijaama
		,poistoero_vapaaehtvar = poistoero
		,vieras_paaoma
		,lyhyt_vieras_paaoma
		,rahoitustuotot
		,rahoituskulut
...
FROM(
	...
	PIVOT ( 
			SUM(maara_euroa)
			FOR sarake IN (toimintakulut, toimintatuotot, poistot, vastattavaa, rahoitusomaisuus, poistoero, vieras_paaoma, rahoitustuotot, rahoituskulut, lyhyt_vieras_paaoma, oma_paaoma, ylialijaama, investoinnit, vos_rahoitus, edel_ylialijaama)
			) AS p ) MQ
