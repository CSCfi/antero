USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amis_generoi_tyhjat_kuukaudet]    Script Date: 7.12.2022 14:04:00 ******/
SET ANSI_NULLS ON
GO
...
--
right join dw.f_arvo_luodut_vastaajatunnukset f2 on f2.kyselykertaid = d2.kyselykertaid and coalesce(f2.oppilaitoskoodi,'-1') = d4.organisaatio_koodi and coalesce(f2.tutkintokoodi,'-1') = d3.koulutusluokitus_koodi and year(f2.kuukausi) = d1.vuosi and month(f2.kuukausi) = d1.kuukausi and coalesce(f2.rahoitusryhma,'-1') = d4b.koodi and f2.vastaajatunnukset_lkm > 0 
--
...
inner join dw.d_organisaatioluokitus d8 on d8.organisaatio_koodi = coalesce(f2.oppilaitoskoodi,'-1')
...
left join dw.f_arvo_amis_generoi_tyhjat_kuukaudet_temp k1 on k1.kyselykertaid = d6.kyselykertaid
inner join dw.d_arvokysymys d12 on d12.kysymysid = k1.kysymysid
inner join dw.d_rahoitusryhma d13 on d13.koodi = coalesce(f2.rahoitusryhma,'-1')

WHERE (d2.kyselykertaid is null or d4.organisaatio_koodi is null or d3.koulutusluokitus_koodi is null or d1.vuosi is null or d1.kuukausi is null or d4b.koodi is null)
