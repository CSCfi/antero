...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_tuva_opiskelijat_kuukausittainen]    Script Date: 21.10.2024 7:41:42 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	FROM (	
		SELECT DISTINCT
			...
			,muut_opiskeluoikeudet_rnk = max(case when ooa2.opiskeluoikeus_oid is null THEN 0 else 1 end) over (partition by oo.oppija_oid)
		...
		LEFT JOIN Koski_SA.sa.sa_oppivelvolliset ov on ov.oppija_oid = oo.oppija_oid and @alkuPvm BETWEEN ov.alkuPvm and ov.loppuPvm
		LEFT JOIN [Koski_SA].[sa].[sa_koski_opiskeluoikeus] AS oo2 on oo2.oppija_oid = oo.oppija_oid and oo2.koulutusmuoto not in ('tuva', 'perusopetuksenlisaopetus') and oo2.sisaltyy_opiskeluoikeuteen_oid is null 
