...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_ainekelpoisuudet]    Script Date: 30.11.2022 8:31:58 ******/
SET ANSI_NULLS ON
GO
...
INTO #temp_ope_pat
FROM (
	SELECT p.*, rnk = row_number() over (partition by p.henkilo, p.aine order by paivamaara_aine)
	FROM (
		SELECT DISTINCT
			pat.henkilo, 
...
FROM (
	SELECT DISTINCT
		...
		case
			when pat.paivamaara_aine >= pat2.paivamaara then pat.organisaatiokoodi_aine
