...
/****** Object:  View [dbo].[v_K3_12_Perusk_paattaneet_amm_tutk_suorittaneet]    Script Date: 4.9.2020 20:13:34 ******/
SET ANSI_NULLS ON
GO
...
/* LUE TÄMÄ!!!
1. KOPIOI KOLME ALINTA SELECTIÄ JA MUUTA NIISSÄ KAIKKIIN KOHTIIN VUOSI+1
...
--tuoreimmat välittömän sijoittumisen tiedot haetaan uusimmasta saapuneesta aineistosta
SELECT
tilv, 
CAST(tilv + '0101' AS date) AS tilv_date,
tutkryh,
...
rivinumero = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K3_12_sopv_20] 
where suorv = '2018' and suorlk = '1'

UNION ALL

--ellei uusi aineisto ole saapunut, kaksi selectiä yllä ovat tyhjät ja tuorein välittömän sijoittumisen tieto haetaan aiemmasta aineistosta 
SELECT
tilv, 
CAST(tilv + '0101' AS date) AS tilv_date,
tutkryh,
...
suorv = (CASE 
			WHEN NOT EXISTS (SELECT TOP 1 1 
								...
								WHERE t.name = 'TK_K3_12_sopv_20' and p.rows > 0) 
			THEN '2017'
		 END)
and suorlk = '1'
