...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_korkea_aste_valintatapajonot]    Script Date: 6.9.2021 9:41:29 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	SELECT
		CASE 
			WHEN f.JonoNimi like 'Avoim%' OR f.JonoNimi like '%Avoin%' OR d1.haun_nimi_fi like '%Avoi%' OR d1.haun_nimi_fi like '%Öppna%' OR d1.haun_nimi_fi like '%Öppen%' then 'av'
			ELSE f.JonoTyyppi
		END AS JonoTyyppi,
		f.JonoNimi,
		CASE 
