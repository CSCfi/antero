...
DROP PROCEDURE IF EXISTS [dw].[p_lataa_f_rekrytointi_valitut_yhteinen]
GO
...
CREATE PROCEDURE [dw].[p_lataa_f_rekrytointi_valitut_yhteinen]
AS
...
SELECT s.[Tilastovuosi]
      ...
      ,COALESCE([Tohtorintutkinnon suoritusvuosi], '-1') as 'Tohtorintutkinnon suoritusvuosi'
	  ...
	  ,COALESCE(d22.id, '-1') as d_kansainvalinen_haku_id
	  ,'-1' as d_kutsuhaku_id
...
FROM [ANTERO].[sa].[sa_suorat_amk7d_rekrytointi_valitun_kaikki_tiedot] s
...
GROUP BY Tilastovuosi, Korkeakoulu, Hakunumero) d22
on d22.Hakunumero = s.Hakunumero AND d22.Korkeakoulu = s.Korkeakoulu AND d22.Tilastovuosi = s.Tilastovuosi
...
SELECT s.[Tilastovuosi]
      ...
      ,COALESCE([Sopimuksen loppupäivämäärä], '-1') as sop_lop_pvm
      ,COALESCE(d21.id, '-1') as d_ika_id
	  ,COALESCE(d22.id1, '-1') as d_kansainvalinen_haku_id
	  ,COALESCE([Valinta kutsumenettelyllä], '-1') as d_kutsuhaku_id
	  ,COALESCE(d22.id2, '-1') as d_yhteisprofessuuri_id
	  ...
	  ,suser_sname() as username
FROM [ANTERO].[sa].[sa_suorat_yo8d_rekrytointi_valitun_kaikki_tiedot] s
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d on d.organisaatio_koodi = s.Korkeakoulu
LEFT JOIN ANTERO.dw.d_sukupuoli d2 on d2.sukupuoli_koodi = s.Sukupuoli
...
--     WHEN Finnish question            OR    WHEN Swedish question
	d20.vastaus_fi = s.vastaus OR d20.vastaus_sv = s.Vastaus
LEFT JOIN ANTERO.dw.d_ika d21 on d21.ika_avain = s.Ikä
LEFT JOIN (SELECT Tilastovuosi, Korkeakoulu, Hakunumero, MAX([Haun kansainvälisyys]) as id1, MAX(Yhteisprofessuuri) as id2 FROM ANTERO.sa.sa_suorat_yo8a_rekrytointihaut
GROUP BY Tilastovuosi, Korkeakoulu, Hakunumero) d22
on d22.Hakunumero = s.Hakunumero AND d22.Korkeakoulu = s.Korkeakoulu AND d22.Tilastovuosi = s.Tilastovuosi
