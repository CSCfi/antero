...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_taloustiedot]    Script Date: 12.3.2025 11.14.39 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	FROM (
		...
		HAVING CAST(SUM(CASE WHEN d2.tili_taso1_fi in ('Kotimainen ulkopuolinen rahoitus', 'Ulkomainen ulkopuolinen rahoitus') THEN f.arvo ELSE 0 END) as float) > 0
	) f
	OUTER APPLY (
		SELECT
