USE [ANTERO]
GO
/****** Object:  StoredProcedure [sa].[p_lataa_sa_hakeneet_toisen_asteen_hakijat]    Script Date: 13.11.2019 17:21:34 ******/
SET ANSI_NULLS ON
GO
...
--aputaulut
declare @valintaLista table (tila varchar(50))
...
declare @vastaanottoLista table (tila varchar(50))
...
where d.haku_oid in (@haku1_oid,@haku2_oid,@haku3_oid,@haku4_oid,@haku5_oid,@haku6_oid)
...
update sa.sa_haku_ja_valinta_hakijat_toinen_aste 
set valinnat= (case 
			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu amm/lukioon'

			when ( not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu erityisopetukseen'

			when ( not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu valmaan'

			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu amm/lukioon ja erityisopetukseen'

			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu amm/lukioon ja valmaan'

			when ( not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu erityisopetukseen ja valmaan'

			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Valittu kaikkiin kolmeen'
...
set vastaanotto= (case 
			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut amm/lukioon'

			when ( not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut erityisopetukseen'

			when ( not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut valmaan'

			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut amm/lukioon ja erityisopetukseen'

			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut amm/lukioon ja valmaan'

			when ( not exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut erityisopetukseen ja valmaan'

			when ( exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
					and exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) ) then 'Vastaanottanut kaikkiin kolmeen'
