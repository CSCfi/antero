...
/****** Object:  View [dw].[v_arvo_kandi]    Script Date: 28.1.2021 9:47:05 ******/
SET ANSI_NULLS ON
GO
...
, COALESCE(d12.selite_fi, 'Tieto puuttuu') 	AS 'Teema'
...
, case 
	when d4.vastaustyyppi = 'arvosana6_ja_eos' and f.numerovalinta = 6 then 0
	when d4.vastaustyyppi = 'arvosana4_ja_eos' and f.numerovalinta = 1 then 0
	when d4.vastaustyyppi like 'arvosana%' then 1
	else 0 
...
, COALESCE(d12.selite_sv, 'Information saknas') 	AS 'Tema'
...
, COALESCE(d12.selite_en, 'Missing data') 	AS 'Theme'
...
, COALESCE(d12.koodi, 'Tieto puuttuu')		AS 'Koodit Teema'
...
LEFT JOIN sa.sa_arvo_hul d12 ON cast(d12.kysymysid as int) = d4.kysymysid
