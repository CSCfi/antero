...
/****** Object:  View [dbo].[v_sa_2_8_Korkeakoulut_opiskelijat]    Script Date: 12.5.2022 14:29:16 ******/
SET ANSI_NULLS ON
GO
...
SELECT --SELECT distinct aikielir1x from Antero.sa.[sa_virta_otp_tk_opisk_ennakko]
	tilv
	, aineisto
	, lahde
	, tunn
	...
	, avo
	, ltop = case when ltop = '1' then '1' end 
	--opiskelijaennakossa ltop päätelty lastusta k/e; TK-aineistoissa ja ilm. tutkintoennakossa ltop ~ lastu / CSC Juha 24.9.2021
	, laak = CASE koulk WHEN '775101' THEN 1 WHEN '775201' THEN 2 ELSE NULL END
	,[tietolahde] = 'antero sa_virta_otp_tk_opisk_ennakko'
    ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM Antero.sa.[sa_virta_otp_tk_opisk_ennakko]
