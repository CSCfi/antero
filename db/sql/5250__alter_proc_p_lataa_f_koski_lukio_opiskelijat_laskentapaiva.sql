USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opiskelijat_laskentapaiva]    Script Date: 22.12.2021 12:41:53 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT [opiskeluoikeus_oid]
		  ...
		  ,suorituskieli_koodiarvo = --ylläpidettäisiin (12/2021) taulussa koski_sa.sa.sa_lukio_paatason_suoritus, mutta tässä tuota laajempi muuttujisto
			case 
				when suorituskieli_koodiarvo in ('EN', 'FR', 'RI', 'SV', 'SE', 'DE', 'FI', 'RU', 'VK') then suorituskieli_koodiarvo
				else 'XX'
			end	
		  ,[alkamispaiva]
		  ,[paattymispaiva]
