...
/****** Object:  View [dbo].[v_sa_5_2_Yliopistojen_tutkimustyovuodet_yliopistoittain]    Script Date: 23.1.2026 9.52.06 ******/
SET ANSI_NULLS ON
GO
...
SELECT [tilv] = CAST(kaikkivuodet.tilv as nvarchar(4))
      ...
      ,[tutktaso] = null --ei käyttökelpoinen, jatkossa erikseen aineistosta 5.2b tai TK:n rajapinnasta
	  ,[kaste_t2] = null --ei käyttökelpoinen, jatkossa erikseen aineistosta 5.2b tai TK:n rajapinnasta
...
SELECT [tilv]
      ...
      ,[ttv_lkm]
	  ,[apuraha]
      ,[tietolahde]='TK_K5_2_sopv_21'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K5_2_sopv_21]
...
SELECT [tilv]
      ...
      ,[ttv_lkm]
	  ,[apuraha]
      ,[tietolahde]='TK_K5_2_sopv_22'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K5_2_sopv_22]
...
SELECT [tilv]
      ...
      ,[ttv_lkm]
	  ,[apuraha]
      ,[tietolahde]='TK_K5_2_sopv_23'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K5_2_sopv_23]
...
SELECT [tilv]
      ...
      ,[ttv_lkm]
	  ,[apuraha]
      ,[tietolahde]='TK_K5_2_sopv_24'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K5_2_sopv_24]
...
SELECT [tilv]
      ...
      ,[ttv_lkm]
	  ,[apuraha]
      ,[tietolahde]='TK_K5_2_sopv_25'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K5_2_sopv_25]
