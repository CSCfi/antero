USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_avop]    Script Date: 15.10.2019 15:33:14 ******/
SET ANSI_NULLS ON
GO
...
;WITH CTE AS (
	...
	where c.tyyppi = 'avop'
	and b.taustakysymyksen_tyyppi in ('sukupuoli','ika','pohjakoulutus')
)
...
LEFT JOIN dw.d_kalenteri d5a ON d5a.kalenteri_avain = CAST(MAIN_QUERY.vastaajatunnus_alkupvm AS varchar)
LEFT JOIN dw.d_kalenteri d5b ON d5b.kalenteri_avain = CAST(MAIN_QUERY.vastausaika AS VARCHAR)
...
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_kandi]    Script Date: 15.10.2019 15:33:14 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dw].[p_lataa_f_arvo_kandi]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dw].[p_lataa_f_arvo_kandi] AS' 
END
GO
