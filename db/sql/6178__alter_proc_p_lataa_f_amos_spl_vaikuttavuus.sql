USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_amos_spl_vaikuttavuus]    Script Date: 2.5.2022 23:55:19 ******/
SET ANSI_NULLS ON
GO
...
--Huom. Lopun okm8-joinissa poiskommentoitavaa uuden siirtotiedoston saavuttua. JS 3.5.2022

--Tässä latauksessa hyödynnetään TK_K1_13-aineistoa. Aineisto sisältää tietoja
...
SELECT DISTINCT 
    ...
    ,[tyov]
	,jarj_tutk = COALESCE(
		CASE WHEN jarj_tutk = '2082223-9' THEN '0872020-5' END, 
    ...
    ,[jarjnimi_tutk] = CASE WHEN jarjnimi_tutk = 'NULL' THEN NULL ELSE jarjnimi_tutk END
    ,[jarjmaak]
    ,[jarjmaaknimi]
    ,[tutkmaak]
    ,[tutkmaaknimi]
...
FROM (
	...
	FROM ANTERO.sa.z_tmp_TK_K1_13_sopv
	WHERE toteuma_tutk = 1 OR (toteuma_tutk IN (1,2) AND tilv >= 2018)
	GROUP BY tilv, jarj_tutk
) t
LEFT JOIN ANTERO.dw.d_amos_spl_jarjestaja_linkki d ON t.jarj_tutk = d.ytunnus_avain AND (t.tilv + 4 >= YEAR(d.voimaan_alkaen) OR d.voimaan_alkaen IS NULL)
...
SELECT 
	...
	,COALESCE(spl.nimi_amos_spl, spl2.nimi_amos_spl, k13.jarjnimi_tutk, k13.jarjnimi_opisk) AS [Koulutuksen järjestäjä (tutkinnon)]
...
--	AND k13.tilv + 4 >= YEAR(spl.voimaan_alkaen)
LEFT JOIN ANTERO.sa.z_tmp_TK_K1_K13_sopv_jarjkerroin_fuusiot jk ON jk.jarj_tutk = (CASE WHEN k13.tilv + 4 < YEAR(spl.voimaan_alkaen) THEN k13.jarj_tutk ELSE spl.ytunnus_amos_spl END) AND jk.tilv = k13.tilv
LEFT JOIN ANTERO.sa.sa_amos_siirtotiedosto_okm8 okm8 ON okm8.koul_jarj = spl.ytunnus_amos_spl AND (k13.tilv + 4 = YEAR(okm8.voimaan_alkaen) /*poiskommentoi okm8 saavuttua ->*/ or k13.tilv + 3 = YEAR(okm8.voimaan_alkaen) and YEAR(okm8.voimaan_alkaen) = 2022)
LEFT JOIN ANTERO.dw.f_amos_spl_vaikuttavuus_maakunnat maak ON okm8.paa_asiallinen_toiminta_maakunta_koodi = maak.tilvasmaak AND maak.tilv = k13.tilv
