...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_henkilostotiedot_yliopisto]    Script Date: 26.8.2026 7.47.09 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	FROM (
		SELECT 
			f.vuosi as tilastovuosi,
			d1.organisaatio_nykyinen_koodi as organisaatio_koodi,
			CAST(SUM(CASE WHEN d3.maatjavaltiot2_fi NOT IN ('Suomi', 'Tieto puutuu', 'Tuntematon') THEN f.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(f.henkilotyovuosi) as float) as arvo,
...
USING (
	...
	FROM (
		...
		HAVING SUM(CAST(f.ulkopuolinen_rahoitus as float)) > 0
	) f
	OUTER APPLY (
		SELECT 
...
-- I uraportaan osuus yliopistojen opetus- ja tutkimushenkilöstöstä 
...
USING (
	...
	FROM (
		SELECT 
			f.vuosi as tilastovuosi,
			d1.organisaatio_nykyinen_koodi as organisaatio_koodi,
			CAST(SUM(CASE WHEN d5.koodi = 1 THEN f.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(f.henkilotyovuosi) as float) as arvo,
			'i_up_osuus' as indikaattori
		FROM [ANTERO].[dw].[f_yo_henkilon_tyo] f
		LEFT JOIN ANTERO.dw.d_organisaatioluokitus d1 on d1.id = f.d_yliopisto_id
		LEFT JOIN ANTERO.dw.d_yo_tehtavanjaottelu d2 on d2.id = f.d_tehtavanjaottelu_id
		LEFT JOIN ANTERO.dw.d_yo_tutkijanuravaihe d5 on d5.id=f.d_tutkijanuravaihe_id
	...
	OUTER APPLY (
		SELECT 
			CAST(SUM(CASE WHEN d5b.koodi = 1 THEN fb.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(fb.henkilotyovuosi) as float) as arvo_sektori
		FROM [ANTERO].[dw].[f_yo_henkilon_tyo] fb
		LEFT JOIN ANTERO.dw.d_yo_tehtavanjaottelu d2b on d2b.id = fb.d_tehtavanjaottelu_id
		LEFT JOIN ANTERO.dw.d_yo_tutkijanuravaihe d5b on d5b.id=fb.d_tutkijanuravaihe_id
