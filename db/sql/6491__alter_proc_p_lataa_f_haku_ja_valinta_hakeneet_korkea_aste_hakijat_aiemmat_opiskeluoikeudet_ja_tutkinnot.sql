...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste_hakijat_aiemmat_opiskeluoikeudet_ja_tutkinnot]    Script Date: 28.9.2022 10:09:47 ******/
SET ANSI_NULLS ON
GO
...
) AS OP ON OP.henkilo_oid = f.henkilo_oid AND (OP.opisk_alkamisPvm <= d4.loppuPvm OR OP.opisk_alkamisPvm <= d4b.loppuPvm) AND NOT (OP.tavoitetutkinto = d5.koulutusluokitus_koodi AND (OP.opisk_alkamisPvm >= d4.alkuPvm OR OP.opisk_alkamisPvm >= d4b.alkuPvm))
--tutkinnot
LEFT JOIN (
...
) AS TU ON TU.henkilo_oid = f.henkilo_oid AND (TU.tutk_suoritusPvm <= d4.loppuPvm OR TU.tutk_suoritusPvm <= d4b.loppuPvm)
...
LEFT JOIN (
	...
	GROUP BY HenkiloOID
) AS EKH ON EKH.HenkiloOID = f.henkilo_oid and (EKH.HakuaikaAlku >= d4.alkuPvm OR EKH.HakuaikaAlku >= d4b.alkuPvm)
