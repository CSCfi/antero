...
/****** Object:  View [dw].[v_koski_lukio_opiskelijat_uudet_cultures]    Script Date: 13.4.2026 9.08.55 ******/
SET ANSI_NULLS ON
GO
...
-- JÄRJESTYSMUUTTUJAT
  ...
  LEFT JOIN (
	...
	LEFT JOIN ANTERO.dw.d_organisaatioluokitus d4 on d4.id = f.d_organisaatioluokitus_oppilaitos_id 
	WHERE d.hakuryhma_koodi != 2 and d3.koulutusastetaso2_koodi in (31, -1)
	and (f.vastaanottanut_paikan = 1 or f.ilmoittautunut = 1)
	) d14 on d14.master_oid = f.master_oid and d14.organisaatio_nykyinen_koodi = d8.organisaatio_nykyinen_koodi and d14.koulutuksen_alkamisvuosi = f.vuosi
	LEFT JOIN (
	SELECT DISTINCT
		 f.master_oid
		,f.koulutuksen_alkamisvuosi
	...
	WHERE d.hakuryhma_koodi != 2 and d3.koulutusastetaso2_koodi in (31, -1)
	and (f.vastaanottanut_paikan = 1 or f.ilmoittautunut = 1)
	) d15 on d15.master_oid = f.master_oid and d15.organisaatio_nykyinen_koodi = d8.organisaatio_nykyinen_koodi and d15.koulutuksen_alkamisvuosi + 1 = f.vuosi
	LEFT JOIN (
	SELECT DISTINCT
		 f.master_oid
		,f.koulutuksen_alkamisvuosi
	...
	WHERE d.hakuryhma_koodi != 2 and d3.koulutusastetaso2_koodi in (31, -1)
	and (f.vastaanottanut_paikan = 1 or f.ilmoittautunut = 1)
	) d16 on d16.master_oid = f.master_oid and d16.organisaatio_nykyinen_koodi = d8.organisaatio_nykyinen_koodi and d16.koulutuksen_alkamisvuosi + 2 = f.vuosi
