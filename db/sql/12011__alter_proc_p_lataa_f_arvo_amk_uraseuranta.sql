...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amk_uraseuranta]    Script Date: 22.4.2026 8.27.04 ******/
SET ANSI_NULLS ON
GO
...
-- Koodit numerovalinnaksi
UPDATE Arvo_SA.sa.sa_arvo_v2_vastaukset_amk_uraseuranta
SET numerovalinta = CAST(koodi as int)
WHERE numerovalinta is null and koodi is not null
...
FROM Arvo_SA.sa.sa_arvo_v2_vastaukset_amk_uraseuranta a 
inner join Arvo_SA.sa.sa_arvo_v2_kysymykset b on b.kysymysid = a.kysymysid and b.kysymysversio = a.kysymysversio and b.taustakysymyksen_tyyppi in ('pohjakoulutus','tyokokemus') and a.kyselyid = b.kyselyid
left join dw.d_arvovastaus_monivalinta pk on pk.koodi = a.numerovalinta and pk.kysymysid = a.kysymysid and pk.kysymysversio = a.kysymysversio and b.taustakysymyksen_tyyppi = 'pohjakoulutus'
left join dw.d_arvovastaus_monivalinta tk on tk.koodi = a.numerovalinta and tk.kysymysid = a.kysymysid and tk.kysymysversio = a.kysymysversio and b.taustakysymyksen_tyyppi = 'tyokokemus'
WHERE YEAR(a.vastausaika) >= 2022
GROUP BY vastaajaid
...
SELECT DISTINCT
	...
	,numerovalinta = 
		case 
			...
			else coalesce(f2.numerovalinta, luku)
		end						
	,rahoituskysymys_paino = 
		case 
	...
	,d_kylla_ei_valinta_id = 
		case
			when f3.kysymystyyppi = 'kylla_ei_valinta' and f2.vaihtoehto = 'kylla' then 1
			when f3.kysymystyyppi = 'kylla_ei_valinta' and f2.vaihtoehto = 'ei' then 2
			else -1
		end
	,f.valmistumisvuosi AS valmistumisvuosi
	,'ETL: p_lataa_f_arvo_amk_uraseuranta' as source
	,CAST(RIGHT(f4.tiedonkeruu,4) as int)			AS tilastovuosi
	
FROM Arvo_SA.sa.sa_arvo_v2_vastaajatunnukset f 
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_vastaukset_amk_uraseuranta f2 ON f2.vastaajatunnusid = f.vastaajatunnusid
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_kysymykset f3 on f3.kysymysid = f2.kysymysid and f3.kysymysversio = f2.kysymysversio and f3.kyselyid = f.kyselyid
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_kyselyt f4 on f4.kyselyid = f.kyselyid
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_kysymysryhmat f5 on f5.kysymysryhmaid = f3.kysymysryhmaid and f5.kyselyid = f.kyselyid
...
LEFT JOIN dw.d_kalenteri d5 ON d5.kalenteri_avain = f.voimassa_alkupvm
LEFT JOIN dw.d_kalenteri d5b ON d5b.kalenteri_avain = f2.vastausaika
LEFT JOIN dw.d_sukupuoli d6 ON d6.sukupuoli_koodi=f.sukupuoli
LEFT JOIN dw.d_kieli d8 on d8.kieli_koodi=f.aidinkieli
LEFT JOIN dw.d_ika d9 on d9.ika_avain=f.ika_valmistuessa
LEFT JOIN dw.d_kieli d10 on d10.kieli_koodi=f.suorituskieli
LEFT JOIN dw.d_arvokysymys d11 on d11.kysymysid = f3.kysymysid and d11.kyselyid = f.kyselyid and d11.rnk = 1 and d11.kysymysversio = f3.kysymysversio
...
where f4.kyselytyyppi = 'amk-uraseuranta'
and f5.kategoria in ('valinnainen','valtakunnallinen') 
and coalesce(f3.taustakysymyksen_tyyppi,'') not in ('pohjakoulutus','tyokokemus')
and f3.kysymystyyppi != 'vapaateksti'
and CAST(RIGHT(f4.tiedonkeruu,4) as int) >= 2022
...
left join dw.d_arvovastaus_monivalinta pk on pk.vastaus_fi = a.monivalintavaihtoehto_fi and pk.kysymysid = a.kysymysid and pk.kysymysversio = a.kysymysversio and b.taustakysymyksen_tyyppi = 'pohjakoulutus'
left join dw.d_arvovastaus_monivalinta tk on tk.vastaus_fi = a.monivalintavaihtoehto_fi and tk.kysymysid = a.kysymysid and tk.kysymysversio = a.kysymysversio and b.taustakysymyksen_tyyppi = 'tyokokemus'
WHERE YEAR(a.vastausaika) < 2022
GROUP BY vastaajaid
...
LEFT JOIN dw.d_arvokysymys d11 on d11.kysymysid = f3.kysymysid and f2.kyselyid = d11.kyselyid and d11.rnk = 1 and d11.kysymysversio = f3.kysymysversio
LEFT JOIN dw.d_arvovastaus_monivalinta d15 ON d15.koodi = f2.numerovalinta and d15.kysymysid = f2.kysymysid and d15.kysymysversio = f2.kysymysversio
LEFT JOIN dw.d_virta_rahoituslahde d13 on d13.rahoituslahde_koodi = f.rahoituslahde
--pohjakoulutus ja työkokemus temp-taulusta
LEFT JOIN sa.temp_amk_uraseuranta_taustatiedot tt ON tt.vastaajaid = f2.vastaajaid  
...
and f4.kyselykerta_vuosi BETWEEN 2018 and 2021
...
SELECT DISTINCT
	...
	,YEAR(vt.voimassa_alkupvm) - 5 as valmistumisvuosi
	,'ETL: p_lataa_f_arvo_amk_uraseuranta' as source
	,YEAR(vt.voimassa_alkupvm) as tilastovuosi
...
and kk.kyselytyyppi = 'amk-uraseuranta' 
and YEAR(vt.voimassa_alkupvm) >= 2022
and concat(vt.kyselyid, '_', d3.id) not in (SELECT avain FROM ANTERO.dbo.arvo_amk_uraseuranta_temp)
...
SELECT DISTINCT
	...
	,'-1' as d_arvovastaus_taustakysymys_tyokokemus_id
	,'-1' as d_kylla_ei_valinta_id
	,kk.kyselykerta_vuosi - 5 as valmistumisvuosi
	,'ETL: p_lataa_f_arvo_amk_uraseuranta' as source
...
and concat(kk.kyselyid, '_', d3.id) not in (SELECT avain FROM ANTERO.dbo.arvo_amk_uraseuranta_temp)

DROP TABLE IF EXISTS ANTERO.dbo.arvo_amk_uraseuranta_temp
DROP TABLE IF EXISTS ANTERO.sa.temp_amk_uraseuranta_taustatiedot

-- Visualisointien rajaukset
SELECT 
	f.tilastovuosi,
	f.d_koulutusluokitus_id,
	f.d_organisaatio_oppilaitos_id,
	COUNT(DISTINCT f.vastaajaid) as vastaajat
INTO #uraseuranta_temp
FROM ANTERO.dw.f_arvo_amk_uraseuranta f
GROUP BY f.tilastovuosi,f.d_koulutusluokitus_id,f.d_organisaatio_oppilaitos_id

UPDATE f
SET visualisointiin = case when t.vastaajat > 1 then 1 else 0 end
FROM ANTERO.dw.f_arvo_amk_uraseuranta f
LEFT JOIN #uraseuranta_temp t on t.tilastovuosi = f.tilastovuosi and t.d_koulutusluokitus_id = f.d_koulutusluokitus_id and t.d_organisaatio_oppilaitos_id = f.d_organisaatio_oppilaitos_id
WHERE f.vastaajaid is not null

DROP TABLE IF EXISTS #uraseuranta_temp