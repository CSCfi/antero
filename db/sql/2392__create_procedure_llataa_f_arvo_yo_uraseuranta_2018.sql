...
SELECT  
      ...
      , d_ika.ikaryhma1_fi									as [Ikäryhmä (valmistuessa)]
      ,d_sukupuoli.sukupuoli_fi								as [Sukupuoli]
      ,d_kieli.kieli_fi										as [Äidinkieli]
      ...
      , dw.d_koulutusluokitus.okmohjauksenala_fi			as [OKM ohjauksen ala]
      ...
      ,kysymykset.kysymysryhma_fi							as [Kysymysryhmä]
      ,kysymykset.kysymys_fi								as [Kysymys]
      ,f1.kyselypohja									as [Kyselypohja]
      ,2018													as [Kyselyvuosi]
      , case when f1.kysymysnumero in ( 6805,6836,6837
		,6889,6890,6908,6910,6913,6914) 
      ...
      ,kysymykset.vastaustyyppi								as [vastaustyyppi]
      ,null 												as [vaihtoehto]
      ...
      ,CASE WHEN kysymykset.kysymysryhma_fi in ('I  TYÖMARKKINATILANTEESI','I  TAUSTAKYSYMYKSET')						then 1
			...
			WHEN kysymykset.kysymysryhma_fi in ('III TOHTORIKOULUTUS JA SEN MERKITYS(1/4)','IV TYÖTILANNE')				then 4
			WHEN kysymykset.kysymysryhma_fi in ('III TOHTORIKOULUTUS JA SEN MERKITYS(1/4)','V TYÖLLISTYMISEN LAATU')	then 5
...
/*and f1.kysymysnumero not in ('#N/A')  -- Kristin, miksi tämä merkki on datassa? */
