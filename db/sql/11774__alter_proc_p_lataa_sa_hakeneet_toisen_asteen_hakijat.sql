...
/****** Object:  StoredProcedure [sa].[p_lataa_sa_hakeneet_toisen_asteen_hakijat]    Script Date: 25.2.2026 12.44.29 ******/
SET ANSI_NULLS ON
GO
...
,@haku6_oid varchar(255) = NULL
,@haku7_oid varchar(255) = NULL
,@haku8_oid varchar(255) = NULL  AS
...
where d.haku_oid in (@haku1_oid,@haku2_oid,@haku3_oid,@haku4_oid,@haku5_oid,@haku6_oid, @haku7_oid, @haku8_oid)
...
(
	case
		when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku7_oid or h.haku_oid = @haku8_oid))
		then 'Hakenut perusopetuksen jälkeiseen koulutukseen'
		else
		case 
			when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku1_oid or h.haku_oid = @haku2_oid))
				then 
					case 
						when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku3_oid or h.haku_oid = @haku4_oid)) 
							then 
								case
									when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
										then 'Hakenut kaikissa kolmessa haussa'
									else 'Hakenut amm/lukioon ja erityisopetukseen'
								end			
						when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid))
							then 'Hakenut amm/lukioon ja valmaan'
						else 'Hakenut vain amm/lukioon'
					end
			...
			when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku3_oid or h.haku_oid = @haku4_oid))
				then 
					case 
						when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
							then 'Hakenut erityisopetukseen ja valmaan'
						else 'Hakenut vain erityisopetukseen'
					end

			when exists (select haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
				then 'Hakenut vain valmaan' 
...
(
	case 
		when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku8_oid or h.haku_oid = @haku7_oid))
		then 'Valittu perusopetuksen jälkeiseen koulutukseen'
		else
		case
			when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku2_oid or h.haku_oid = @haku1_oid))
				then 
					case
						when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku3_oid or h.haku_oid = @haku4_oid))
							then
								case	
									when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
										then 'Valittu kaikkiin kolmeen'
									else 'Valittu amm/lukioon ja valmaan'
								end
						when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
							then 'Valittu amm/lukioon ja erityisopetukseen'
						else 'Valittu amm/lukioon'
					end
			...
			when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku3_oid or h.haku_oid = @haku4_oid))
				then
					case
						when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
							then 'Valittu erityisopetukseen ja valmaan'
						else 'Valittu erityisopetukseen'
					end

			when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.valittu = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
						then 'Valittu valmaan'
...
(
	case 
		when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku8_oid or h.haku_oid = @haku7_oid))
		then 'Vastaanottanut perusopetuksen jälkeiseen koulutukseen'
		else
		case
			when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku2_oid or h.haku_oid = @haku1_oid))
				then
					case
						when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku3_oid or h.haku_oid = @haku4_oid))
							then
								case 
									when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
										then 'Vastaanottanut kaikkiin kolmeen' 
									else 'Vastaanottanut amm/lukioon ja erityisopetukseen'
								end
						when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid))
							then 'Vastaanottanut amm/lukioon ja valmaan'
						else 'Vastaanottanut amm/lukioon'
					end
			...
			when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku3_oid or h.haku_oid = @haku4_oid))
				then
					case
						when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
							then 'Vastaanottanut erityisopetukseen ja valmaan'	
						else 'Vastaanottanut erityisopetukseen'
					end

			when exists (select distinct haku_oid from sa.sa_ovara_hakeneet h where h.master_oid=oid and h.vastaanottanut_paikan = 1 and (h.haku_oid = @haku5_oid or h.haku_oid = @haku6_oid)) 
				then 'Vastaanottanut valmaan'
...
set haut_jarjestys=(case haut
						...
						when 'Hakenut perusopetuksen jälkeiseen koulutukseen' then 8
						else '99'
					end)
where vuosi=@vuosi
...
set valinnat_jarjestys=(case valinnat
						...
						when 'Valittu perusopetuksen jälkeiseen koulutukseen' then 8
...
set vastaanotto_jarjestys=(case vastaanotto
						...
						when 'Vastaanottanut perusopetuksen jälkeiseen koulutukseen' then 8
						when 'Valittiin mutta ei vastaanottanut' then 9
						when 'Ei valittu' then 10
						else '99'
					end)
where vuosi=@vuosi
...
set haut=(case haut_jarjestys
			...
			when 8 then 'Haki perusopetuksen jälkeisen koulutuksen yhteishaussa'
			else 'Tuntematon'
		end)
where vuosi=@vuosi
...
set haut_SV=(case haut_jarjestys
			...
			when 8 then 'Ansökte till utbildning efter den grundläggande utbildningen'
...
set haut_EN=(case haut_jarjestys
			...
			when 8 then 'Applied for upper secondary education and preparatory education (joint application)'
			else 'Missing data'
		end)
where vuosi=@vuosi
...
set valinnat=(case valinnat_jarjestys
			...
			when 8 then 'Valittiin perusopetuksen jälkeisen koulutuksen yhteishaussa'
...
set valinnat_SV=(case valinnat_jarjestys
			...
			when 8 then 'Blev antagen till utbildning efter den grundläggande utbildningen'
...
set valinnat_EN=(case valinnat_jarjestys
			...
			when 7 then 'Was selected to upper secondary education and preparatory education (joint application)'
			when 8 then 'Was selected to all three'
...
set vastaanotto=(case vastaanotto_jarjestys
			...
			when 8 then 'Vastaanotti paikan perusopetuksen jälkeisen koulutuksen yhteishaussa'
			when 9 then 'Valittiin mutta ei vastaanottanut'
			when 10 then 'Ei valittu'
			else 'Tuntematon'
		end)
where vuosi=@vuosi
...
set vastaanotto_SV=(case vastaanotto_jarjestys
			...
			when 8 then 'Tog emot plats i utbildning efter den grundläggande utbildningen antagningen'
			when 9 then 'Blev antagen men tog inte emot platsen'
			when 10 then 'Blev inte antagen'
		end)
where vuosi=@vuosi
...
set vastaanotto_EN=(case vastaanotto_jarjestys
			...
			when 8 then 'Accepted a place for upper secondary education and preparatory education (joint application)'
			when 9 then 'Was selected but did not accept a place'
			when 10 then 'Was not selected'
			else 'Missing data'
		end)
where vuosi=@vuosi