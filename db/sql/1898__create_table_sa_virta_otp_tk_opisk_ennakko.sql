--  Tilastokeskus Opiskelijatiedonkeruu OKM Ennakko sa taulu
USE ANTERO
GO

IF NOT EXISTS (
  SELECT * FROM INFORMATION_SCHEMA.TABLES
  WHERE TABLE_SCHEMA='sa' AND TABLE_NAME='sa_virta_otp_tk_opisk_ennakko'
) BEGIN

CREATE TABLE sa.sa_virta_otp_tk_opisk_ennakko  (
  ...
  CONSTRAINT PK__sa_virta_otp_tk_opisk_ennakko PRIMARY KEY CLUSTERED (id ASC)
)
;

ALTER TABLE sa.sa_virta_otp_tk_opisk_ennakko ADD  CONSTRAINT DF__sa_virta_otp_tk_opisk_ennakko__loadtime  DEFAULT (getdate()) FOR [loadtime]
;
ALTER TABLE sa.sa_virta_otp_tk_opisk_ennakko ADD  CONSTRAINT DF__sa_virta_otp_tk_opisk_ennakko__username  DEFAULT (suser_name()) FOR username
;
