...
DROP PROCEDURE IF EXISTS [dw].[p_lataa_api_yo_opintopisteet_esilataus]
GO
...
CREATE PROCEDURE [dw].[p_lataa_api_yo_opintopisteet_esilataus] AS

TRUNCATE TABLE Antero.dw.[api_yo_opintopisteet];

INSERT INTO Antero.dw.[api_yo_opintopisteet]
...
FROM (
	...
	LEFT JOIN ANTERO.dw.d_yo d3 on d3.id=f.d_yo_id
	...
	SELECT 
		  ...
		  [koodit_yliopisto] = d3.yo_tunnus,
		  [aineistotyyppi] = 'Live',
		  [tietojen_ajankohta] = right('0'+cast(day(dateadd(day, -1, sa.loadtime))-1 as nvarchar(2)), 2) + '.' + right('0'+cast(month(dateadd(day, -1, sa.loadtime)) as nvarchar(2)), 2) + '.' + cast(year(dateadd(day, -1, sa.loadtime)) as nvarchar(4))
