...
/****** Object:  StoredProcedure [dbo].[p_lataa_sa_oppivelvolliset]    Script Date: 21.10.2024 7:56:44 ******/
SET ANSI_NULLS ON
GO
...
DROP PROCEDURE IF EXISTS [dbo].[p_lataa_sa_oppivelvolliset_esi_ja_perusopetus]
...
LEFT JOIN [Koski_SA].[sa].[sa_koski_henkilo] h2 on h.master_oid = h2.master_oid -- Generoidaan kaikille saman masteroid:n oppijaoid:lle samat tiedot
LEFT JOIN (
	SELECT DISTINCT 
		oo.oppija_oid
...
LEFT JOIN (
	...
	GROUP BY oo.oppija_oid
) pov on pov.oppija_oid = h.oppija_oid
WHERE YEAR(h.syntymaaika) >= 2004 AND oo.oppija_oid is null AND DATEADD(YEAR,18,h.syntymaaika) > kh.muutto_pvm AND kh.poismuutto_pvm >= cast('2021-08-01' as date)
GROUP BY h2.[oppija_oid], COALESCE(pov.pidennetty_oppivelvollisuus,0), kh.muutto_pvm
...
GROUP BY h2.[oppija_oid], kh.muutto_pvm
