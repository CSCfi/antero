USE [VipunenTK_DW]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_OTV_f_3_15]    Script Date: 9.8.2021 22:15:52 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[p_lataa_OTV_f_3_15] AS

TRUNCATE TABLE vipunentk.dbo.f_OTV_3_15_Lukio_ja_ammatillisen_koulutuksen_tutkintojen_mediaani_iat;

INSERT INTO vipunentk.dbo.f_OTV_3_15_Lukio_ja_ammatillisen_koulutuksen_tutkintojen_mediaani_iat
...
SELECT
	...
	,tutkinnon_suoritusvuosi_id = coalesce(d12.kausi_id,'-1')
	,tutkinnon_suorituskuukausi = f.tutkinnon_suorituskuukausi
	,tutkinnon_suorituskuukausi_id = coalesce(d13.kausi_id,'-1')
	,lukumaara = f.lukumaara
	,tutkinnon_toteuma = f.tutkinnon_toteuma
	,tutkinnon_toteuma_id = coalesce(d15.id,'-1')
	,ammatillisen_koulutuksen_koulutuslaji = f.ammatillisen_koulutuksen_koulutuslaji
	,ammatillisen_koulutuksen_koulutuslaji_id = coalesce(d14.id,-1)
	...
	,opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus_id = coalesce(d16.id,-1)
	,opetushallinnon_koulutus= f.opetushallinnon_koulutus
	,opetushallinnon_koulutus_id = coalesce(d17.id,-1)
	,nuorten_aikuisten_koulutus= f.nuorten_aikuisten_koulutus
	...
	,koulutussektori_id = coalesce(d19.id,-1)
	,ika_1v = f.ika_1v
	,ika_1v_id = coalesce(d20.id,-1)
	,koulutuksen_jarjestamismuoto = f.koulutuksen_jarjestamismuoto
	,koulutuksen_jarjestamismuoto_id = coalesce(d22.id,-1)
	,f.amm_sopimusjaksot
	,amm_sopimusjaksot_id = coalesce(d22b.id,-1)
	,ammatillinen_peruskoulutus_lisakoulutus = f.ammatillinen_peruskoulutus_lisakoulutus
	,ammatillinen_peruskoulutus_lisakoulutus_id = coalesce(d21.id,-1)
...
FROM VipunenTK_DW.dbo.f_3_15_Lukio_ja_ammatillisen_koulutuksen_tutkintojen_mediaani_iat AS f
...
LEFT JOIN VipunenTK..d_kausi AS d12 ON d12.kausi_id	=(case when isnull(f.tutkinnon_suoritusvuosi,'') in ('9999','XXXX','') then '-1' else f.tutkinnon_suoritusvuosi end)
...
LEFT JOIN VipunenTK..d_ammatillisen_koulutuksen_koulutuslaji AS d14 ON d14.ammatillisen_koulutuksen_koulutuslaji_koodi=f.ammatillisen_koulutuksen_koulutuslaji AND f.tilv_date between d14.alkaa and d14.paattyy
LEFT JOIN VipunenTK..d_tutkinnon_toteuma AS d15 ON d15.tutkinnon_toteuma_koodi=f.tutkinnon_toteuma and f.tilv_date between d15.alkaa and d15.paattyy
LEFT JOIN VipunenTK..d_opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus AS d16 ON d16.opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus_koodi=f.opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus AND f.tilv_date between d16.alkaa and d16.paattyy
LEFT JOIN VipunenTK..d_opetushallinnon_koulutus AS d17 ON d17.opetushallinnon_koulutus_koodi=f.opetushallinnon_koulutus AND f.tilv_date between d17.alkaa and d17.paattyy
...
LEFT JOIN VipunenTK..d_koulutussektori AS d19 ON d19.koulutussektori_koodi=f.koulutussektori and f.tilv_date between d19.alkaa and d19.paattyy
LEFT JOIN VipunenTK..d_ika AS d20 ON d20.ika_avain =f.ika_1v
LEFT JOIN VipunenTK..d_ammatillinen_peruskoulutus_lisakoulutus AS d21 ON d21.ammatillinen_peruskoulutus_lisakoulutus_koodi=f.ammatillinen_peruskoulutus_lisakoulutus and f.tilv_date between d21.alkaa and d21.paattyy
LEFT JOIN VipunenTK..d_koulutuksen_jarjestamismuoto AS d22 ON d22.koulutuksen_jarjestamismuoto_koodi=f.koulutuksen_jarjestamismuoto and f.tilv_date between d22.alkaa and d22.paattyy
LEFT JOIN VipunenTK..d_amm_sopimusjaksot AS d22b ON d22b.koodi=f.amm_sopimusjaksot AND f.tilv_date between d22b.alkaa and d22b.paattyy
