...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_huoltajatiedot]    Script Date: 19.8.2024 12:37:25 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER   PROCEDURE  [dw].[p_lataa_f_varda_huoltajatiedot]
AS
...
BEGIN
	...
	OUTER APPLY (
		...
		FROM (
			...
			WHERE alkamis_pvm <= cast(concat(@alkuVuosi, '-12-31') as date) and (paattymis_pvm >= cast(concat(@alkuVuosi, '-12-31') as date) or paattymis_pvm < '1900-12-30') and s0.action <> 'DELETED'
			GROUP BY s0.lapsi_id
		) m
		WHERE m.lapsi_id = s1.lapsi_id
	) cum_sum

	WHERE s1.alkamis_pvm <= cast(concat(@alkuVuosi, '-12-31') as date) and (coalesce(paattymis_pvm, '9999-12-31') >= cast(concat(@alkuVuosi, '-12-31') as date) or paattymis_pvm < '1900-12-30') and s1.action <> 'DELETED'
	and s1.action <> 'DELETED'
	and s2.action <> 'DELETED'
