...
/****** Object:  StoredProcedure [dbo].[p_lataa_TK_amm_tutkinnot_rahoituskausittain]    Script Date: 21.10.2024 14:07:29 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[p_lataa_TK_amm_tutkinnot_rahoituskausittain] @rahoituskaudenPaattymisvuosi int = null AS

--ks. huom-kohdat ennen tulevia poimintoja. JS 21.10.2024

DECLARE @vuosi2 int = case when @rahoituskaudenPaattymisvuosi is null then year(getdate()) else @rahoituskaudenPaattymisvuosi end
DECLARE @rahoituskausiAlku date = datefromparts(@vuosi2-1,7,1)
DECLARE @rahoituskausiLoppu date = datefromparts(@vuosi2,6,30)
...
SELECT 
	rahoituskausi = concat('01.07.',@vuosi2-1,' - ','30.06.',@vuosi2) --huom. aineiston 1.13b tapauksessa jatkossa p.o. "suorituskausi" tms.
	,suorv
...
FROM (
	...
	SELECT
		suorv = q.vuosi --huom. mahd. tulevissa poiminnoissa  year(tilv_pvm) osan/osien koulutuksesta valmistuneille
		...
		,rnk = 
			row_number() over (
				...
				order by ca3.luvallinen desc, opr.sisaltyy_okm_suoritusrahoitus desc, q.paataso desc, q.suorittaa_koko_tutkintoa desc, coalesce(q.vahvistus_paiva,'9999-12-31'), q.arviointi_paiva
			)
	...
	--pelkän tutkinnon osan/osia suorittaneet huomioidaan vain opiskeluoikeudesta valmistuttaessa
	left join (
		select opiskeluoikeus_oid, sisaltyy_opiskeluoikeuteen_oid 
		from sa.sa_koski_opiskeluoikeus 
		where sisaltyy_opiskeluoikeuteen_oid is not null
	) hankittu on hankittu.opiskeluoikeus_oid = q.opiskeluoikeus_oid
	outer apply (
		select paattymispaiva
		from sa.sa_koski_opiskeluoikeus 
		where opiskeluoikeus_oid = coalesce(hankittu.sisaltyy_opiskeluoikeuteen_oid, q.opiskeluoikeus_oid)
		and viimeisin_tila = 'valmistunut'
	) oo 
	cross apply (
		select tilv_pvm = 
			case 
				when paataso=1 and suorittaa_koko_tutkintoa=1 then datefromparts(vuosi,kuukausi,1)
				else coalesce(oo.paattymispaiva, '9999-12-31')
			end
	) ca1
	...
	WHERE 1=1
	and ca1.tilv_pvm between @rahoituskausiAlku and @rahoituskausiLoppu
	and ca3.luvallinen = 1
	and ca3.lupa_suorituskieli != -2
	and opr.sisaltyy_okm_suoritusrahoitus = 1
	and q.duplikaatti_rahoituskausittain = 0
	and q.paallekkainen_tuva = 0 --huomioitu myös valma ja telma
