...
/****** Object:  StoredProcedure [dbo].[p_lataa_f_ammattirakenne_ennuste]    Script Date: 19.12.2023 14:55:33 ******/
SET ANSI_NULLS ON
GO
...
SET @min_ika = (SELECT MIN(d3.ika_int) FROM [VipunenTK].[dbo].[f_alueella_tyossakayvat_1v] f LEFT JOIN dbo.d_ika d3 on d3.id=f.ika_id WHERE f.oef_ammatti_id <> -1)
SET @max_ika = (SELECT MAX(d3.ika_int) FROM [VipunenTK].[dbo].[f_alueella_tyossakayvat_1v] f LEFT JOIN dbo.d_ika d3 on d3.id=f.ika_id WHERE f.oef_ammatti_id <> -1)
SET @min_vuosi = (SELECT MIN(tilv) FROM [VipunenTK].[dbo].[f_alueella_tyossakayvat_1v] f WHERE f.oef_ammatti_id <> -1)
SET @max_vuosi = (SELECT MAX(tilv) FROM [VipunenTK].[dbo].[f_alueella_tyossakayvat_1v] f WHERE f.oef_ammatti_id <> -1)
...
FROM (SELECT DISTINCT oef_ammatti_id, oef_toimiala_id FROM [VipunenTK].[dbo].[f_alueella_tyossakayvat_1v] WHERE tilv >= @min_vuosi) f
