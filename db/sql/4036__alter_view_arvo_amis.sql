...
/****** Object:  View [dw].[v_arvo_amis]    Script Date: 23.10.2020 12:17:53 ******/
SET ANSI_NULLS ON
GO
...
/*,case	when month(Getdate()) between 7 and 12																		--AJOHETKELLÄ LOPPUVUOSI
		and  CONCAT(YEAR(Getdate()),'-06-30')>d3.paivays															--JA PÄIVÄYS ON PIENEMPI KUIN VIIME KESÄKUU
		then (case when d3.kuukausi between 1 and 6 then  CONCAT('01.07.',d3.vuosi-1,' - ','30.06.',d3.vuosi)		--ALKUVUODEN PÄIVÄYS
				   when d3.kuukausi between 7 and 12 then CONCAT('01.07.',d3.vuosi,' - ','30.06.',d3.vuosi+1) END)	--LOPPUVUODEN PÄIVÄYS
	
		when month(Getdate()) between 1 and 6																		--AJOHETKELLÄ ALKUVUOSI
		and  CONCAT(YEAR(Getdate())-1,'-06-30')>d3.paivays															--JA PÄIVÄYS ON PIENEMPI KUIN VIIME KESÄKUU
		then (case when d3.kuukausi between 1 and 6 then  CONCAT('01.07.',d3.vuosi-1,' - ','30.06.',d3.vuosi)		--ALKUVUODEN PÄIVÄYS
				   when d3.kuukausi between 7 and 12 then CONCAT('01.07.',d3.vuosi,' - ','30.06.',d3.vuosi+1) END)	--LOPPUVUODEN PÄIVÄYS
	
		ELSE	--Jos päivämäärä ei ole pienempi kuin viime kesäkuun loppu niin viimeinen kuukausi muuttuu
		
		CONCAT('01.07.'
		,(CASE	WHEN d3.kuukausi < 7 
				THEN d3.vuosi - 1 
...
, d4.kiusaamiskysymys						AS 'Kiusaamiskysymys'
														...
														---KOULUTUSTOIMIJA
, coalesce(d1.organisaatio_fi,d1_2.organisaatio_fi)	AS 'Koulutuksen järjestäjä'
...
LEFT JOIN dw.d_organisaatioluokitus d1 ON d1.organisaatio_koodi = d0.ytunnus_amos_spl								
LEFT JOIN dw.d_organisaatioluokitus d1_2 ON d1_2.id = f.d_organisaatio_koulutustoimija_id
LEFT JOIN dw.d_alueluokitus d2 ON d2.kunta_koodi = d0.kunta_koodi																
...
LEFT JOIN [ANTERO].[dw].[f_amos_opiskelijapalaute_vastaajamäärä_vahvistettu] f3 ON (f3.rahoituskausi=d3.rahoituskausi_amm	
																				AND  f3.[Koulutuksen järjestäjän Y-tunnus]= d0.ytunnus_amos_spl)
WHERE 1=1
