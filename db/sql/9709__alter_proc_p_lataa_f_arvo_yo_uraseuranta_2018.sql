...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_yo_uraseuranta_2018]    Script Date: 21.3.2024 9:40:47 ******/
SET ANSI_NULLS ON
GO
...
SELECT  
      ...
      ,CASE		-- LISÄTÄÄN MONIVALINTAKYSYMYSTEN KYSYMYSRYHMÄÄN KYSYMYKSEN ALIOTSIKKO
			...
			WHEN kysymykset.kysymysryhma_fi='III  TOHTORIKOULUTUS JA SEN MERKITYS (2/5)'	THEN 'III  TOHTORIKOULUTUS JA SEN MERKITYS (2/5). Kuinka tärkeitä seuraavat tiedot ja taidot ovat työssäsi'	
			...
			WHEN kysymykset.kysymysryhma_fi='III  TOHTORIKOULUTUS JA SEN MERKITYS (3/5)'	THEN 'III  TOHTORIKOULUTUS JA SEN MERKITYS (3/5). Miten tohtorikoulutus kehitti kyseisiä valmiuksia?'
			...
			WHEN kysymykset.kysymysryhma_fi='III  TOHTORIKOULUTUS JA SEN MERKITYS (4/5)'	THEN 'III  TOHTORIKOULUTUS JA SEN MERKITYS (4/5). Arvioi seuraavien taitokokonaisuuksien merkityksen kehittymistä seuraavan viiden vuoden aikana?'
			WHEN kysymykset.kysymysryhma_fi='III  TOHTORIKOULUTUS JA SEN MERKITYS (5/5)'	THEN 'III  TOHTORIKOULUTUS JA SEN MERKITYS (5/5). Arvioi, miten seuraavat tekijät ovat vaikuttaneet työllistymiseesi valmistumisen jälkeen?'
      ...
      ,CASE 
		WHEN f1.kyselypohja='URASEURANTA-TOHTORI' and f1.Kyselyvuosi = 2021 THEN REPLACE(kysymykset.kysymys_fi, '2019', '2018')
		WHEN f1.kyselypohja='URASEURANTA-TOHTORI' and f1.Kyselyvuosi = 2023 THEN REPLACE(kysymykset.kysymys_fi, '2019', '2020')
		WHEN f1.kyselypohja='URASEURANTA-MAISTERI' and f1.Kyselyvuosi = 2021 THEN REPLACE(kysymykset.kysymys_fi, '2017', '2016')
		WHEN f1.kyselypohja='URASEURANTA-MAISTERI' and f1.Kyselyvuosi = 2023 THEN REPLACE(kysymykset.kysymys_fi, '2017', '2018')
		ELSE kysymykset.kysymys_fi
	  END																	as [Kysymys]
	  ...
	  ,13354, 13356, 13359, 13360, 13335, 13336, 13434, 13435, 13443, 13445)	-- Vuoden 2021 kysymykset  	
      ...
      , CASE 
		WHEN f1.kyselypohja='URASEURANTA-TOHTORI' and f1.Kyselyvuosi = 2021 THEN REPLACE(kysymykset.kysymys_sv, '2019', '2018')
		WHEN f1.kyselypohja='URASEURANTA-TOHTORI' and f1.Kyselyvuosi = 2023 THEN REPLACE(kysymykset.kysymys_sv, '2019', '2020')
		WHEN f1.kyselypohja='URASEURANTA-MAISTERI' and f1.Kyselyvuosi = 2021 THEN REPLACE(kysymykset.kysymys_sv, '2017', '2016')
		WHEN f1.kyselypohja='URASEURANTA-MAISTERI' and f1.Kyselyvuosi = 2023 THEN REPLACE(kysymykset.kysymys_sv, '2017', '2018')
		ELSE kysymykset.kysymys_sv
	   END																	as [Fråga]
      , f1.vastaus_sv														as [Flervalsalternativ]
      , coalesce(dw.d_koulutusluokitus.koulutusastetaso1_sv,'Information saknas')	as [Utbildningsnivå, nivå 1]
      , coalesce(dw.d_koulutusluokitus.koulutusastetaso2_sv,'Information saknas')	as [Utbildningsnivå, nivå 2]
      , coalesce(dw.d_koulutusluokitus.koulutusalataso1_sv,'Information saknas')	as [Utbildningsområde, nivå 1]
      , coalesce(dw.d_koulutusluokitus.koulutusalataso2_sv,'Information saknas')	as [Utbildningsområde, nivå 2]
      , coalesce(dw.d_koulutusluokitus.koulutusalataso3_sv,'Information saknas')	as [Utbildningsområde, nivå 3]
      ,	coalesce(dw.d_koulutusluokitus.okmohjauksenala_sv,'Information saknas')		as [UKM-styrningsområde] 
      , coalesce(dw.d_koulutusluokitus.koulutusluokitus_sv,'Information saknas')	as [Utbildningsbenämning]
      ...
      , CASE 
		WHEN f1.kyselypohja='URASEURANTA-TOHTORI' and f1.Kyselyvuosi = 2021 THEN REPLACE(kysymykset.kysymys_en, '2019', '2018')
		WHEN f1.kyselypohja='URASEURANTA-TOHTORI' and f1.Kyselyvuosi = 2023 THEN REPLACE(kysymykset.kysymys_en, '2019', '2020')
		WHEN f1.kyselypohja='URASEURANTA-MAISTERI' and f1.Kyselyvuosi = 2021 THEN REPLACE(kysymykset.kysymys_en, '2017', '2016')
		WHEN f1.kyselypohja='URASEURANTA-MAISTERI' and f1.Kyselyvuosi = 2023 THEN REPLACE(kysymykset.kysymys_en, '2017', '2018')
		ELSE kysymykset.kysymys_en
	   END																	as [Question]
      ...
      ,CASE WHEN len(d8.Oppilaitoskoodi) < 5 THEN '0' + d8.Oppilaitoskoodi
			WHEN d8.Oppilaitoskoodi IS NULL THEN RIGHT('00000'+isnull(f1.organisaatio_koodi,''),5) 
			ELSE d8.Oppilaitoskoodi END as [Koodit Korkeakoulu]
	  ...
	  ,case 
		when f2.sukupuoli is null or f2.sukupuoli = ''  then 999 
      ...
      ,CASE WHEN kysymykset.kysymysryhma_fi in ('I  TYÖMARKKINATILANNE','I  TYÖMARKKINATILANTEESI','I  TAUSTAKYSYMYKSET', 'I TYÖMARKKINATILANNE')	then 1
			WHEN kysymykset.kysymysryhma_fi in ('II  NYKYINEN TYÖSUHDE JA SEN LAATU','II TYYTYVÄISYYS TUTKINTOON')			then 2
			WHEN kysymykset.kysymysryhma_fi in ('III  TOHTORIKOULUTUS JA SEN MERKITYS (1/4)','III TYÖURAN KOKONAISUUS','III  TOHTORIKOULUTUS JA SEN MERKITYS (1/5)')		then 3
			WHEN kysymykset.kysymysryhma_fi in ('III  TOHTORIKOULUTUS JA SEN MERKITYS (2/4)','IV TYÖTILANNE','III  TOHTORIKOULUTUS JA SEN MERKITYS (2/5)')				then 4
			WHEN kysymykset.kysymysryhma_fi in ('III  TOHTORIKOULUTUS JA SEN MERKITYS (3/4)','V TYÖLLISTYMISEN LAATU','III  TOHTORIKOULUTUS JA SEN MERKITYS (3/5)')		then 5
			WHEN kysymykset.kysymysryhma_fi in ('III  TOHTORIKOULUTUS JA SEN MERKITYS (4/4)','VI TYÖ JA OSAAMINEN (1/3)','III  TOHTORIKOULUTUS JA SEN MERKITYS (4/5)')	then 6
			WHEN kysymykset.kysymysryhma_fi in ('III  TOHTORIKOULUTUS JA SEN MERKITYS (4/4)','VI TYÖ JA OSAAMINEN (1/3)','III  TOHTORIKOULUTUS JA SEN MERKITYS (5/5)')	then 7
			WHEN kysymykset.kysymysryhma_fi in ('IV  TOHTORIOPINTOJEN RAHOITUS','VI TYÖ JA OSAAMINEN (2/3)')				then 8
			WHEN kysymykset.kysymysryhma_fi in ('VI TYÖ JA OSAAMINEN (3/3)')												then 9
			WHEN kysymykset.kysymysryhma_fi in ('VII OSAAMISEN KEHITTÄMINEN') then 10
			WHEN kysymykset.kysymysryhma_fi in ('Uraseurantakyselyn erityisteema 2020: Koronaviruspandemian vaikutukset')	then 11
  ...
  LEFT JOIN dw.d_ika on d_ika.ika_avain=coalesce(round(cast(f2.ika_valmistuessa as float),0),-1)							---IKÄ
  LEFT JOIN dw.d_sukupuoli on d_sukupuoli.sukupuoli_koodi=round(cast(f2.sukupuoli as float),0)								-- SUKUPUOLI
  LEFT JOIN dw.d_kieli on d_kieli.kieli_koodi=f2.aidinkieli													-- KIELI
  LEFT JOIN dw.d_maatjavaltiot2 on d_maatjavaltiot2.maatjavaltiot2_koodi=round(cast(f2.kansalaisuus as float),0)			-- KANSALAISUUS
  LEFT JOIN dw.d_koulutusluokitus on d_koulutusluokitus.koulutusluokitus_koodi= round(cast(f2.koulutusalakoodi as float),0)-- KOULUTUSLUOKITUS
  ...
  LEFT JOIN dw.d_arvokysymys kysymykset on kysymykset.kysymysid=f1.kysymysnumero and (SELECT MIN(kysymysversio) FROM dw.d_arvokysymys WHERE f1.kysymysnumero = kysymysid) = kysymykset.kysymysversio and kysymykset.kysymysryhmaid = (SELECT MIN(kysymysryhmaid) FROM dw.d_arvokysymys WHERE f1.kysymysnumero = kysymysid)
...
,'V  Kiitos paljon vastauksistasi!'
,'VII Kiitos paljon vastauksistasi!') 
and kysymykset.vastaustyyppi not in ('vapaateksti', 'luku') OR f1.Kysymysnumero in (8240, 8242, 8254, 6923, 6921, 6935, 10255, 10257, 10269,  10253, 6830, 8391, 10144)
...
,'14. Valitse edellisen kysymyksen vaihtoehdoista se, joka kuvaa parhaiten pääasiallisinta työtehtävääsi.'
)
/*not in 
