...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvolliset]    Script Date: 18.10.2024 14:15:54 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT * INTO [Koski_SA].[sa].[temp_oppivelvolliset_esirajaukset] FROM (
	...
	SELECT
		...
		,MAX(DATEADD(day,1,ov.loppuPvm)) OVER (partition by ov.oppija_oid) as oppivelvollisuus_paattynyt
		...
		,CASE
			...
			WHEN pk.oppija_oid is not null THEN 6
			ELSE -1
	...
	LEFT JOIN Koski_SA.sa.perusopetuksen_paattotodistuksen_keskiarvot pk on pk.oppija_oid = h.oppija_oid and pk.rnk = 1
...
LEFT JOIN [Koski_SA].[sa].[sa_koski_opiskeluoikeus_aikajakso] ooa on 
			ooa.opiskeluoikeus_oid = oo.opiskeluoikeus_oid and ooa.alku <= @maxPvm and ooa.loppu >= @alkuPvm
LEFT JOIN [Koski_SA].[sa].[sa_koski_paatason_suoritus] ps on ps.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
...
LEFT JOIN Koski_SA.sa.perusopetuksen_paattotodistuksen_keskiarvot pk on pk.oppija_oid = ov.oppija_oid and pk.rnk = 1
-- Vain alku- ja loppupäivän välisenä aikana oppivelvolliset 
INNER JOIN Koski_SA.sa.sa_oppivelvolliset ovv on ovv.oppija_oid = ov.oppija_oid and YEAR(ovv.alkuPvm) <= YEAR(@loppuPvm) AND YEAR(ovv.loppuPvm) >= YEAR(@alkuPvm)
...
SELECT DISTINCT
	ov.oppija_oid, kh.muutto_pvm, kh.poismuutto_pvm, kh.kotikunta, d.mannersuomessa
...
WHERE @alkuPvm <= kh.poismuutto_pvm and @maxPvm  >= kh.muutto_pvm
...
SELECT * INTO [Koski_SA].[sa].[temp_oppivelvollisuus_suoritettu] FROM (
	...
	FROM (
		...
		LEFT JOIN [Koski_SA].[sa].[temp_oppivelvolliset_kotikunta] kh on kh.oppija_oid = ov.oppija_oid and ov.oppivelvollisuus_paattynyt BETWEEN kh.muutto_pvm and kh.poismuutto_pvm
...
BEGIN
	...
	SELECT * INTO Koski_SA.sa.temp_oppivelvollisten_toiminta FROM (
		...
		FROM (
			SELECT DISTINCT
				...
				CASE
					WHEN @alkuPvm >= ov.tutkinto_suoritettu and ov.tutkinto_suoritettu <= DATEADD(year, 18, ov.syntymaaika) and ov.oppivelvollisuus_paattynyt >= ov.tutkinto_suoritettu THEN 1
					WHEN @alkuPvm >= DATEADD(year, 18, ov.syntymaaika) and ov.oppivelvollisuus_paattynyt >= DATEADD(year, 18, ov.syntymaaika) THEN 2
					WHEN ov.oppivelvollisuus_paattynyt >= vov.vapautettu THEN 9
					WHEN @alkuPvm >= ov.oppivelvollisuus_paattynyt THEN 3 
				...
				ovo.erityisopetus,
				CASE
					WHEN @alkuPvm >= ov.perusopetuksen_oppimaaran_yksilollistaminen_vahvistus_paiva THEN ov.perusopetuksen_oppimaaran_yksilollistaminen_hv
					WHEN @alkuPvm < ov.perusopetuksen_oppimaaran_yksilollistaminen_vahvistus_paiva THEN -1
					ELSE 6
				END as perusopetuksen_oppimaaran_yksilollistaminen,
				ov.perusopetuksen_oppimaaran_yksilollistaminen_hv,
				kh.kotikunta
			FROM [Koski_SA].[sa].[temp_oppivelvolliset_esirajaukset] ov
			LEFT JOIN [Koski_SA].[sa].[temp_oppivelvolliset_opiskelu] ovo on ov.oppija_oid = ovo.oppija_oid and @alkuPvm BETWEEN ovo.ooa_alku and ovo.ooa_loppu
			LEFT JOIN [Koski_SA].[sa].[temp_oppivelvolliset_kotikunta] kh on kh.oppija_oid = ov.oppija_oid and @alkuPvm BETWEEN kh.muutto_pvm and kh.poismuutto_pvm
			LEFT JOIN (
				SELECT 
					k.oppija_oid, 
			...
			INNER JOIN Koski_SA.sa.sa_oppivelvolliset ovp on ovp.oppija_oid = ov.oppija_oid and YEAR(@alkuPvm) BETWEEN YEAR(ovp.alkuPvm) AND YEAR(ovp.loppuPvm) 
			...
			AND NOT EXISTS (
				...
				AND tuvat.oppija_oid = ov.oppija_oid
	...
	FROM (
		...
		SELECT DISTINCT  
			...
			ov.aidinkieli,
			CASE 
				WHEN @alkuPvm >= ov.oppivelvollisuus_paattynyt THEN os.kotikunta
				ELSE ovt.kotikunta 
			END as kotikunta,
			os.koulutusmuoto,
		...
		FROM [Koski_SA].[sa].[temp_oppivelvolliset_esirajaukset] ov
		INNER JOIN [Koski_SA].[sa].[temp_oppivelvollisten_toiminta] ovt on ovt.oppija_oid = ov.oppija_oid
		LEFT JOIN [Koski_SA].[sa].[temp_oppivelvollisuus_suoritettu] os on os.oppija_oid = ov.oppija_oid
	...
	IF @alkuPvm = DATEADD(MONTH, DATEDIFF(MONTH, -1, GETDATE()), 0) AND DAY(GETDATE()) > 2
