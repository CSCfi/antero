USE [ANTERO]
GO
/****** Object:  StoredProcedure [sa].[p_lataa_sa_hakeneet_toisen_asteen_hakijat]    Script Date: 13.11.2019 19:17:13 ******/
SET ANSI_NULLS ON
GO
...
(
	case 
		when exists (select HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and (h.HakuOid = @haku1_oid or h.HakuOid = @haku2_oid))
			then 
				case 
					when exists (select HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid)) 
						then 
							case
								when exists (select HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) 
...
(
	case 
		when exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
			then 
				case
					when exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
						then
							case	
								when exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.ValinnanTila in (SELECT tila FROM @valintaLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) 
...
(
	case 
		when exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku2_oid or h.HakuOid = @haku1_oid))
			then
				case
					when exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku3_oid or h.HakuOid = @haku4_oid))
						then
							case 
								when exists (select distinct HakuOid from sa.sa_odw_hakeneet h where h.HenkiloOID=oid and h.VastaanotonTila in (SELECT tila FROM @vastaanottoLista) and (h.HakuOid = @haku5_oid or h.HakuOid = @haku6_oid)) 
