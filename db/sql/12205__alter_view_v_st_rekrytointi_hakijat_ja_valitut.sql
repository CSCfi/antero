...
/****** Object:  View [dw].[v_st_rekrytointi_hakijat_ja_valitut]    Script Date: 18.9.2026 15.33.36 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_st_rekrytointi_hakijat_ja_valitut] AS
 ...
 SELECT DISTINCT
	...
	'Ei' as 'Yhdistynyt henkilöstötiedonkeruuseen',
...
SELECT DISTINCT
	...
	d22.kytkin_fi as 'Yhdistynyt henkilöstötiedonkeruuseen',
	...
	null as hakijat2,
	'Valitut' as 'aineisto',
	HASHBYTES('SHA2_256', vs.salt + f.henkilonumero) as henkilonumero,
	f.sopimusnumero,
	f.hakunumero,
	CONCAT(f.hakunumero, HASHBYTES('SHA2_256', vs.salt + f.henkilonumero), f.d_korkeakoulu_id) as hakuavain,
	1 as mittari_avain,
	q3.vast_pros,

	--tohtorintutkinnon organisaatio
	case when d1.organisaatio_nykyinen_koodi = d16.organisaatio_nykyinen_koodi and d1.organisaatio_koodi != '-1' then 1 else 0 end as tohtori_sama_yo,
	case when d1.organisaatio_nykyinen_koodi != d16.organisaatio_nykyinen_koodi and d1.organisaatio_koodi != '-1' and d16.organisaatio_koodi!= '-1' then 1 else 0 end as tohtori_eri_yo,

	--ylemmän kk-tutkinnon organisaatio
	case when d1.organisaatio_nykyinen_koodi = d19.organisaatio_nykyinen_koodi and d1.organisaatio_koodi != '-1' then 1 else 0 end as ykk_sama_kk,
	case when d1.organisaatio_nykyinen_koodi != d19.organisaatio_nykyinen_koodi and d1.organisaatio_koodi != '-1' and d19.organisaatio_koodi!= '-1' then 1 else 0 end as ykk_eri_kk,
...
LEFT JOIN ANTERO.dw.d_kytkin d22 on d22.kytkin_koodi = coalesce(f.yhdistynyt_henkilostotiedonkeruuseen,0)
INNER JOIN ANTERO.dbo.view_salt vs on vs.[view] = 'v_st_rekrytointi_hakijat_ja_valitut'