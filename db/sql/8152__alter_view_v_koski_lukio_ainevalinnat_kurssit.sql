...
/****** Object:  View [dw].[v_koski_lukio_ainevalinnat_kurssit]    Script Date: 4.10.2023 19:04:12 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_koski_lukio_ainevalinnat_kurssit] AS
...
SELECT
  ...
  --lukion oppimäärä			
    ...
    ,[Lukion oppimäärän suorituskieli] = d3.suorituskieli_yleissivistava_fi
  ...
  --aine ja oppimäärä
    --,[osasuoritus_id]
	,[Oppiaineryhmä] = coalesce(d4.ryhma_nimi, 'Tieto puuttuu')
    ,[Oppiaine] = 
		case 
			when d4.ryhma_koodi = 'kie' and d4.nimi like '%,%' then left(d4.nimi, CHARINDEX(',', d4.nimi) - 1) --poistetaan lopusta ", väh./alle X kurssia"
			else coalesce(d4.nimi, 'Tieto puuttuu')
		end
    ,[Oppimäärä tai kieli] = coalesce(d5.nimi, 'Tieto puuttuu')
    --,[Oppiaineen suorituskieli] = d6.suorituskieli_yleissivistava_fi

  --kurssit
	,[Kurssin koodisto] = coalesce(kurssi_koodisto, 'Tieto puuttuu')
	,[Kurssikoodi] = case when kurssi_paikallinen_OPS_0_1 = 1 then 'Tieto puuttuu (paikallinen OPS)' else kurssi end
	,[Kurssi] = case when kurssi_paikallinen_OPS_0_1 = 1 then 'Tieto puuttuu (paikallinen OPS)' else kurssi + ' ' + coalesce(d13.nimi,'') end
    ...
    ,[Kurssin suorituskieli] = d8.suorituskieli_yleissivistava_fi
  ...
  --jarj
	...
	,[jarj_kurssin_koodisto] = coalesce(kurssi_koodisto, 'ööö')
	,[jarj_kurssi] = case when kurssi_paikallinen_OPS_0_1 = 1 then 'ööö' else coalesce(d13.jarjestys, 'ööö') end
