USE [Koski_SA]
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_loma]    Script Date: 27.8.2021 18:06:39 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT distinct
		...
		,alku = (select maxValue from dbo.Maximum (sa1.alku, dateadd(day, 1, coalesce(case when oo_l.valiaikainen_keskeytys != 1 then oo_l.ed_paattyi end,'1900-01-01')), @date))
	...
	LEFT JOIN (
		...
		group by opiskeluoikeus_oid,ed_paattyi,aiempi_koulutusmuoto,toisen_opiskeluoikeuden_sisalla,valiaikainen_keskeytys,alku,ed_paattynyt
