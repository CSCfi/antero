...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvolliset]    Script Date: 14.3.2023 19:11:55 ******/
SET ANSI_NULLS ON
GO
...
--exec [dw].[p_lataa_f_koski_oppivelvolliset]
	...
	SELECT * FROM (
		...
		FROM (
			...
			OUTER APPLY (
				...
				WHERE vapautettu <= CAST(CONCAT(YEAR(@alkuPvm),'-01-01') as date) and vov.oppija_oid = ov.oppija_oid and cast(vov.mitatoity as date) = cast('9999-01-01' as date)
