...
/****** Object:  View [dw].[v_ovara_haku_ja_valinta_toinen_aste_live_cultures]    Script Date: 5.2.2026 8.16.09 ******/
SET ANSI_NULLS ON
GO
...
--Suomi
	...
	,[pohjakoulutuksen_paattovuosi_fi] = case when f.pohjakoulutuksen_vuosi is null or f.pohjakoulutuksen_vuosi= 0 then 'Tieto puuttuu' else cast(f.pohjakoulutuksen_vuosi as varchar(4)) end 
...
--Ruotsi
	...
	,[pohjakoulutuksen_paattovuosi_sv] = case when f.pohjakoulutuksen_vuosi is null or f.pohjakoulutuksen_vuosi = 0 then 'Information saknas' else cast(f.pohjakoulutuksen_vuosi as char(4)) end 
...
--Englanti
	...
	,[pohjakoulutuksen_paattovuosi_en] = case when f.pohjakoulutuksen_vuosi is null or f.pohjakoulutuksen_vuosi = 0 then 'Missing data' else cast(f.pohjakoulutuksen_vuosi as char(4)) end 
...
/* Mittarit */
	...
	,[apusarake_Valitut3] = f.valittu
	,[apusarake_Paikan vastaanottaneet3] = f.vastaanottanut_paikan	
	,[apusarake_Aloittaneet3] = f.ilmoittautunut
...
/* Jarjestykset */
	...
	,[jarjestys_pohjakoulutuksen_paattovuosi] = case when f.pohjakoulutuksen_vuosi = 0 then 9999 else 2050-f.pohjakoulutuksen_vuosi end
...
LEFT JOIN dw.d_kytkin d12 ON d12.id = d10.ammatillinen_perustutkinto_erityisopetuksena
