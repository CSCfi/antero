...
/****** Object:  StoredProcedure [dw].[p_lataa_api_amm_opiskelijat_ja_tutkinnot_vuosi_tutkinto_esilataus]    Script Date: 6.9.2023 16:42:04 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,uudet_opiskelijat_lkm = count(distinct case when f.uudet_opiskelijat = 1 then f.oppija_oid end)
	,opiskelijat_lkm = count(distinct case when f.opiskelijat = 1 then f.oppija_oid end)
	,tutkinnon_suorittaneet_lkm = count(distinct case when f.tutkinnot = 1 then f.oppija_oid end)
