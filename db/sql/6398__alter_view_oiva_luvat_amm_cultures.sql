...
/****** Object:  View [dw].[v_oiva_luvat_amm_cultures]    Script Date: 7.9.2022 16:29:17 ******/
SET ANSI_NULLS ON
GO
...
SELECT --TOP 100000
	...
	,[toiminta_alueen_kunta_fi] = coalesce(nullif(d2.kunta_fi,ca.puuttuu_fi),'Ei määritelty')
	,[toiminta_alueen_maakunta_fi] = coalesce(nullif(d2.maakunta_fi,ca.puuttuu_fi),'Ei määritelty')
	...
	,[osaamisala_rajoite_fi] = coalesce(nullif(d4.selite_fi,ca.puuttuu_fi),'Ei määritelty')
	...
	,[toiminta_alueen_kunta_sv] = coalesce(nullif(d2.kunta_sv,ca.puuttuu_sv),'Ej angivet')
	,[toiminta_alueen_maakunta_sv] = coalesce(nullif(d2.maakunta_sv,ca.puuttuu_sv),'Ej angivet')
	...
	,[toiminta_alueen_kunta_en] = coalesce(nullif(d2.kunta_en,ca.puuttuu_en),'Not specified')
	,[toiminta_alueen_maakunta_en] = coalesce(nullif(d2.maakunta_en,ca.puuttuu_en),'Not specified')
