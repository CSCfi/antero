...
/****** Object:  StoredProcedure [sa].[p_lataa_virta_opettajakelpoisuus]    Script Date: 30.5.2022 11:16:47 ******/
SET ANSI_NULLS ON
GO
...
-- TEMP-tauluun kombinaatiot kelpoisuuskoodeista sekä kelpoisuuskoulujen korkeakoulu ja pvm
	...
	FROM (
		SELECT DISTINCT
			henkilo,
			CASE 
				WHEN kelpoisuuskoodi1 = 'OP' AND kelpoisuuskoodi2 = 'AI' THEN 'OP'
				WHEN kelpoisuuskoodi1 = 'ER' AND kelpoisuuskoodi2 = 'AI' THEN 'ER'
				WHEN kelpoisuuskoodi1 = 'AI' AND ainepatevyys in ('gf','ms') THEN 'LA'
				WHEN kelpoisuuskoodi1 = 'AI' AND ainepatevyys is not null THEN 'AI'
				WHEN kelpoisuuskoodi1 = 'PE' AND kelpoisuuskoodi2 = 'AI' THEN 'PE'
				WHEN kelpoisuuskoodi1 = 'EL' AND kelpoisuuskoodi2 = 'VA' THEN 'EL'
				WHEN kelpoisuuskoodi1 = 'VA' THEN 'VA'
				WHEN kelpoisuuskoodi1 = 'JA' THEN 'JA'
				WHEN kelpoisuuskoodi1 = 'JB' THEN 'JB'
				WHEN kelpoisuuskoodi1 = 'JC' THEN 'JC'
				WHEN kelpoisuuskoodi1 = 'IX' THEN 'IX'
				WHEN kelpoisuuskoodi1 = 'AI' AND ainepatevyys is null THEN 'PD'
			END as kelpoisuus,

			CASE 
				WHEN kelpoisuuskoodi1 = 'OP' AND kelpoisuuskoodi2 = 'AI' THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'ER' AND kelpoisuuskoodi2 = 'AI' THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'AI' AND ainepatevyys in ('gf','ms') THEN 
					CASE WHEN pvm1 < aine_pvm THEN aine_pvm ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'AI' AND ainepatevyys is not null THEN 
					CASE WHEN pvm1 < aine_pvm THEN aine_pvm ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'PE' AND kelpoisuuskoodi2 = 'AI' THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'EL' AND kelpoisuuskoodi2 = 'VA' THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'VA' THEN pvm1
				WHEN kelpoisuuskoodi1 = 'JA' THEN pvm1
				WHEN kelpoisuuskoodi1 = 'JB' THEN pvm1
				WHEN kelpoisuuskoodi1 = 'JC' THEN pvm1
			...
			CASE 
				...
				WHEN kelpoisuuskoodi1 = 'JA' THEN organisaatiokoodi1
				WHEN kelpoisuuskoodi1 = 'JB' THEN organisaatiokoodi1
				WHEN kelpoisuuskoodi1 = 'JC' THEN organisaatiokoodi1
		...
		SELECT DISTINCT
			...
			END as suoritustapa
		FROM #temp_ope_pat1
		WHERE kelpoisuuskoodi1 = 'AI' and ainepatevyys is not null and YEAR(aine_pvm) > YEAR(pvm1)) f
...
left join (
	select 
		henkilo
		,amk = min(case when kl.koulutusaste2002_koodi in (62,63,71,72,73,81,82) or kl.koulutusastetaso2_koodi in (62,63,71,72,73,81,82) then vuosi end)
		,alempi_kk = min(case when kl.koulutusaste2002_koodi in (63,72,73,81,82) or kl.koulutusastetaso2_koodi in (63,72,73,81,82) then vuosi end)
		,ylempi_kk = min(case when kl.koulutusaste2002_koodi in (72,73,81,82) or kl.koulutusastetaso2_koodi in (72,73,81,82) then vuosi end)
...
cross apply (
	select
		vuosi =
			case
				when kelpoisuus in ('JA','JB', 'JC', 'LA') then tutk_tyypit.amk
...
outer apply (
	...
	from (
		...
		where t2.vuosi <= kelp.vuosi and t2.henkilo=p.henkilo and (kl.koulutusaste2002_koodi in (62,63,71,72,73,81,82) or kl.koulutusastetaso2_koodi in (62,63,71,72,73,81,82))
	) q
	where rnk=1
) kork_tutk
	...
	from (
		...
		cross apply (
			select
				vuosi =
					case
						when up2.kelpoisuus in ('JA','JB', 'JC', 'LA') then tutk_tyypit2.amk
...
OUTER APPLY (
	Select
		CASE 
			WHEN p1.koodi = 'VA_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('VA','PE','AI') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'AI_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'ER_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','ER') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			...
			WHEN p1.koodi = 'OP' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('OP','AI') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'ER' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('ER','AI') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi in('PD', 'LA') THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','ER','OP') FOR XML PATH('')  ), 2 , 9999)),' ', '')
