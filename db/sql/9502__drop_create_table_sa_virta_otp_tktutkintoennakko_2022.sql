...
/****** Object:  Table [sa].[sa_virta_otp_tktutkintoennakko_2022]    Script Date: 17.1.2024 14:22:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[sa].[sa_virta_otp_tktutkintoennakko_2022]') AND type in (N'U') AND (SELECT COUNT(*) FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2022) > 0)
DROP TABLE [sa].[sa_virta_otp_tktutkintoennakko_2022]
GO
...
IF (SELECT COUNT(*) FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2022) > 0
...
INSERT INTO [sa].[sa_virta_otp_tktutkintoennakko_2022]
SELECT 
	[aikielir1x],
	[aineisto],
	[allk],
	...
	[source],
	[username]
FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2022
