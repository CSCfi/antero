...
/****** Object:  StoredProcedure [dw].[p_lataa_d_alueluokitus]    Script Date: 25.9.2024 8:43:06 ******/
SET ANSI_NULLS ON
GO
...
DROP INDEX IF EXISTS [NC__d_alueluokitus] ON [dw].[d_alueluokitus]
...
USING (
  SELECT DISTINCT
	...
	mannersuomessa = case when maakuntakoodi between 1 and 19 then 1 else 0 end,
    source
  ...
  WHERE kunta_koodi != '200' --ulkomaat käsitellään lopuksi
...
WHEN MATCHED THEN
  UPDATE SET
	...
	mannersuomessa=src.mannersuomessa,
...
--ulkomaat (tarvittaessa update manuaalisesti)
if not exists (select * from dw.d_alueluokitus where kunta_koodi='200')
begin
	insert into dw.d_alueluokitus (
		alueluokitus_avain,
		kunta_koodi,alkupvm,loppupvm,voimassa,
...
CREATE NONCLUSTERED INDEX [NC__d_alueluokitus]
ON [dw].[d_alueluokitus] ([kunta_koodi],[mannersuomessa])
