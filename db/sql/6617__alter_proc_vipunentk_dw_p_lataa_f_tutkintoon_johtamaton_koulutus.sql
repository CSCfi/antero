USE [VipunenTK_DW]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_f_tutkintoon_johtamaton_koulutus]    Script Date: 3.2.2022 7:48:36 ******/
SET ANSI_NULLS ON
GO
...
--ajetaan vst-oppilaitoksen tasolle tarkentuvat tiedot erikseen, huomioidaan myös näkymässä omana lukumäärätietona

EXEC [dbo].[p_lataa_f_2_11_Oppilaitosten_tutkintoon_johtamaton_vst_oppilaitostyyppi];
...
INSERT INTO [VipunenTK].[dbo].[f_TJK_2_11_tutkintoon_johtamaton_koulutus]
           ...
           ,[Osallistuneiden lukumäärä (netto)]
	...
	SELECT
		  ...
		  ,[Osallistuneiden lukumäärä (netto)] = Case when op.Mittari = 'N' then 
		  -- Lasketaan miesten määrä "yhteensä - naisten määrä", jos sukupuolitieto on tarjolla ja se on mies
				Case 
					when left(op.Sukupuoli,1)='M' and op.Naistenosuuskoodi<>'' 
					then isnull(fa.mittari_arvo,0) 
	...
	left join VipunenTK_lisatiedot.dbo.opetustuntien_sisalto_ja_tavoiteluokitus op
	  on op.Opetustuntikoodi = fa.mittari_nimi and fa.tilastointivuosi between year(alkaa) and year(paattyy)
