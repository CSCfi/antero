...
/****** Object:  View [dbo].[v_f_4_3_Tutkinnon_suorittaneiden_paaasiallinen_toiminta]    Script Date: 20.11.2020 8:42:08 ******/
SET ANSI_NULLS ON
GO
		...
		case
			when l.koulk in (select (kluok.koulutus_koodi )
					   ...
					   where 
							 ...
							 (kluok.koulutusaste2002_koodi   = '20')
							 and kluok.koulutus_koodi = l.koulk) then '1'
			when l.koulk in (select (kluok.koulutus_koodi )
					   from VipunenTK_DW.dbo.d_koulutusluokitus kluok
					   where 
					
							 (kluok.koulutusaste2002_koodi   = '31')
							 and kluok.koulutus_koodi = l.koulk) then '2'
			when l.koulk in (select (kluok.koulutus_koodi )
					   from VipunenTK_DW.dbo.d_koulutusluokitus kluok
					   where 
					
							 (kluok.koulutusaste2002_koodi   = '32')
							 and kluok.koulutus_koodi = l.koulk) then '3'
			when l.koulk in (select (kluok.koulutus_koodi )
					   from VipunenTK_DW.dbo.d_koulutusluokitus kluok
					   where 
					
							 (kluok.koulutusaste2002_koodi   in ('62', '71'))
							 and kluok.koulutus_koodi = l.koulk) then '4'
			when l.koulk in (select (kluok.koulutus_koodi )
					   from VipunenTK_DW.dbo.d_koulutusluokitus kluok
					   where 
					
							 (kluok.koulutusaste2002_koodi   in ('63','72','73','81','82'))
							 and kluok.koulutus_koodi = l.koulk) then '5'
			else '-1'
		end
	)
	as nvarchar(2))
