...
/****** Object:  View [dw].[v_haku_ja_valinta_toinen_aste_erityisopetus]    Script Date: 25.2.2026 12.46.02 ******/
SET ANSI_NULLS ON
GO
...
/* Koodimuuttujat */
	...
	,[Koodit Koulutusala, taso 1] = d20.koulutusalataso1_koodi
	,[Koodit Koulutusala, taso 2] = d20.koulutusalataso2_koodi
	,[Koodit Koulutusala, taso 3] = d20.koulutusalataso3_koodi


	/* Mittarit */

	--f.loadtime>=d1.hakijat vuosikellorajauksessa
	,[Hakijat3] = case when f.master_oid is not null then 1 else 0 end 
	,[Ensisijaiset hakijat3] = case when /*f.loadtime>=d1.hakijat and*/ hakutoive=1 then 1 else 0 end
	,[Valitut3] = case when f.loadtime>=d1.valitut then f.valittu else 0 end
	,[Paikan vastaanottaneet3] = case when f.loadtime>=d1.vastaanottaneet then f.vastaanottanut_paikan else 0 end		
	,[Aloittaneet3] = case when f.loadtime>=d1.aloittaneet then f.ilmoittautunut else 0 end
	...
	,[Pohjakoulutuksen päättövuosi järjestys] = case when f.pohjakoulutuksen_vuosi is null or f.pohjakoulutuksen_vuosi = 0 then 9999 else f.pohjakoulutuksen_vuosi end
...
WHERE f.koulutuksen_alkamisvuosi>2013 and f.koulutuksen_alkamisvuosi<2022 and d25.korkeakouluhaku=0 and d25.hakuryhma_koodi = '3'  