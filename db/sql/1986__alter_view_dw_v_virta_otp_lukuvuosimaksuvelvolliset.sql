-- FF check ok v_virta_otp_lukuvuosimaksuvelvolliset lisätty aloitusvuosi
USE [ANTERO]
GO
 
IF NOT EXISTS ( SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'dw.v_virta_otp_lukuvuosimaksuvelvolliset') )
	EXEC dbo.sp_executesql @statement = N'CREATE VIEW dw.v_virta_otp_lukuvuosimaksuvelvolliset AS SELECT 1 AS a'
GO
