USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_d_hakukohde]    Script Date: 30.6.2019 16:54:28 ******/
DROP PROCEDURE IF EXISTS [dw].[p_lataa_d_hakukohde]
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dw].[d_hakukohde]') AND type in (N'U'))
ALTER TABLE [dw].[d_hakukohde] DROP CONSTRAINT IF EXISTS [DF_d_hakukohde_username]
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dw].[d_hakukohde]') AND type in (N'U'))
ALTER TABLE [dw].[d_hakukohde] DROP CONSTRAINT IF EXISTS [DF_d_hakukohde_loadtime]
GO
/****** Object:  View [dw].[v_haku_ja_valinta_toinen_aste_lukio_amm]    Script Date: 30.6.2019 16:54:28 ******/
DROP VIEW IF EXISTS [dw].[v_haku_ja_valinta_toinen_aste_lukio_amm]
GO
/****** Object:  View [dw].[v_haku_ja_valinta_toinen_aste]    Script Date: 30.6.2019 16:54:28 ******/
DROP VIEW IF EXISTS [dw].[v_haku_ja_valinta_toinen_aste]
GO
/****** Object:  Table [dw].[d_hakukohde]    Script Date: 30.6.2019 16:54:28 ******/
DROP TABLE IF EXISTS [dw].[d_hakukohde]
GO
/****** Object:  Table [dw].[d_hakukohde]    Script Date: 30.6.2019 16:54:28 ******/
SET ANSI_NULLS ON
GO
...
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dw].[v_haku_ja_valinta_toinen_aste]'))
EXEC dbo.sp_executesql @statement = N'



CREATE VIEW [dw].[v_haku_ja_valinta_toinen_aste] AS
...
--Englanti
	...
	/* Koodimuuttujat */
	 ...
	 /* Mittarit */
	  ...
	  --Suomi
	  ,[Hakeutuminen] = d32.haut
	  ,[Valinta] = d32.valinnat
	  ,[Paikan vastaanotto] = d32.vastaanotto
	  --Ruotsi
	  ,[Sökning] = d32.haut_SV
...
SELECT distinct
	  ...
	  ,[Kansalaisuus] = NULL
	  ,[Kansalaisuus (maanosa)] = NULL
	  ,[Kiinnostunut oppisopimuskoulutuksesta] = NULL
	  ,[Kotipaikkakunta (hakija)] = NULL
	  ,[Kotipaikan maakunta (hakija)] = NULL
	  ,[Kotipaikan seutukunta (hakija)] = NULL
	  ,[Kotipaikan suuralue (OKM) (hakija)] = NULL
...
--Englanti
	...
	/* Koodimuuttujat */
	  ...
	  ,[Koodit Opintoala] = d6.opintoala2002_koodi
	  ,[Koodit Tutkinto] = d6.koulutusluokitus_koodi
	  ,[Koodit Pohjakoulutuksen järjestäjä] = NULL
	  ,[Koodit Pohjakoulutuksen oppilaitos] = NULL
	  ,[Koodit Pohjakoulutuksen maakunta] = NULL
	  ,[Koodit Pohjakoulutuksen kunta] = NULL
	  ...
	  ,[Hakijat3] = 0
	  ,[Hakijat3 perusopetuksen päättävät] = 0
	  ,[Ensisijaiset hakijat3] = 0
	  ,[Valitut3] = 0
	  ,[Valitut3 perusopetuksen päättävät] = 0
	  ,[Paikan vastaanottaneet3] = 0
	  ,[Paikan vastaanottaneet3 perusopetuksen päättävät] = 0
	  ,[Aloittaneet3] = 0
