...
/*
 Katsotaan master-oideittain samana päivänä alkaneet opiskeluoikeudet.
...
WITH suorituksen_tyyppi AS (
	...
	FROM (
		...
		where sa2.koulutusmuoto = 'ammatillinenkoulutus' or sa2.tuva_jarjestamislupa = 'ammatillinen'
	) q2
	WHERE rnk = 1
), 
...
opiskeluoikeudet AS (
	SELECT 
		...
		,alkamispaiva
		,rn = ROW_NUMBER() OVER (
			PARTITION BY master_oid, alkamispaiva 
	...
	FROM (
		SELECT DISTINCT
			...
			,min(coalesce(case when ooa.tila='lasna' then ra.sisaltyy_okm_perusrahoitus end, 0)) OVER(PARTITION BY oo.opiskeluoikeus_oid) as perusrahoitus
		...
		--läsnä-tilat tai suora valmistuminen
		inner join sa.sa_koski_opiskeluoikeus_aikajakso ooa 
			on ooa.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
			and (ooa.tila='lasna' or (ooa.tila = 'valmistunut' and oo.alkamispaiva = oo.paattymispaiva))
...
INSERT INTO sa.temp_opiskelijavuodet_samana_paivana_alkaneet_hylattavat (master_oid, opiskeluoikeus_oid_hyvaksytaan)
...
inner join (
    ...
    FROM opiskeluoikeudet 
	WHERE rn > 1 
) hyl on hyv.master_oid=hyl.master_oid and hyl.alkamispaiva=hyv.alkamispaiva
