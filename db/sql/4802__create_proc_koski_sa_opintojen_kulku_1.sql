...
/****** Object:  StoredProcedure [sa].[p_lataa_temp_koski_opintojen_kulku_1_perusjoukko]    Script Date: 30.8.2021 8:32:32 ******/
DROP PROCEDURE IF EXISTS [sa].[p_lataa_temp_koski_opintojen_kulku_1_perusjoukko]
GO
...
CREATE PROCEDURE [sa].[p_lataa_temp_koski_opintojen_kulku_1_perusjoukko] AS
...
SELECT perus.oppija_oid, 
    ...
    tutkinto_koodi = 
		...
		END,
	rnk = rank() over (partition by perus.oppija_oid, koulutus_kytkin, kal.kohortti_vp order by perus.alkamispaiva, oo.alkamispaiva, ps.suorituksen_alkamispaiva, ps.suorituksen_tyyppi, oo.opiskeluoikeus_oid)  FROM(
SELECT  oppija_oid,
		opiskeluoikeus_oid, 
		...
		koulutus_kytkin
					FROM (
								...
								alku,
								koulutus_kytkin = case
										WHEN ps.suorituksen_tyyppi = 'valma' THEN 'ammatillinen_valmentava'
										...
										WHEN ((ps.suorituksen_tyyppi IN ('ibtutkinto', 'preiboppimaara', 
											...
											OR
										(ps.suorituksen_tyyppi = 'internationalschoolmypvuosiluokka' AND ps.koulutusmoduuli_koodiarvo IN ('10', '11', '12'))) THEN 'yleissivistava2aste' END,
								...
								WHERE tila IN ('lasna', 'loma') AND ps.suorituksen_tyyppi != 'nayttotutkintoonvalmistavakoulutus' AND oo.sisaltyy_opiskeluoikeuteen_oid IS NULL) q1
		WHERE rnk = 1 AND koulutus_kytkin IN ('ammatillinen_valmentava', 'ammatillinen','yleissivistava2aste')) perus
