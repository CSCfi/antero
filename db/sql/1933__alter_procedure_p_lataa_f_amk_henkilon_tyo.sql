USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_amk_henkilon_tyo]    Script Date: 7.11.2018 9:59:30 ******/
SET ANSI_NULLS ON
GO
...
SELECT COALESCE(s2.id,-1)
	...
	,s1.Vuosi
	,s1.Henkilotyovuosi
	,s1.[Työajan käyttötiedot, opetus]
	,s1.[Työajan käyttötiedot, TKI-toiminta]
	,s1.[Työajan käyttötiedot, hallinto]
	...
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM([s16].[Sopimuksen alkupäivämäärä])), 104), -1) AS [sopimus_alkupvm]
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM([s16].[Sopimuksen loppupäivämäärä])), 104), - 1) AS [sopimus_loppupvm]
	,getdate()
...
LEFT JOIN [sa].sa_suorat_amk1_henkilokunta_TK s16 ON s16.vuosi = s1.vuosi AND s16.Ammattikorkeakoulu = s1.Ammattikorkeakoulu  AND s16.Henkilönumero= s1.Henkilönumero
