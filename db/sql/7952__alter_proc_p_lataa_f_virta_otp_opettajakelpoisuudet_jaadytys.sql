...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_opettajakelpoisuudet_jaadytys]    Script Date: 16.8.2023 10:20:13 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	' SELECT DISTINCT' +
		...
		' case when (pvm1 > pvm2 or pvm2 is null) and d1a.patevyys_fi like ' + '''' + '%erilliset%' + '''' + ' THEN 2 when pvm2 >= p.pvm1 and d1b.patevyys_fi like ' + '''' + '%erilliset%' + '''' + ' then 2 else 1 end as suoritustapa,' +
	...
	' WHERE p.Vuosi = ' + @alkuVuosi + ' or (' + @alkuVuosi + ' = 2022 and p.Vuosi > 2015)'
