USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_d_maatjavaltiot2]    Script Date: 26.5.2021 11:38:50 ******/
SET ANSI_NULLS ON
GO
...
begin
	...
	insert into dw.d_maatjavaltiot2 (
		...
		maanosa1_koodi,maanosa1_fi,maanosa1_sv,maanosa1_en,
	...
	select
		...
		koodi,nimi,nimi_sv,nimi_en, --maanosa1
	...
	set
		...
		maanosa1_koodi=s.koodi,maanosa1_fi=s.nimi,maanosa1_sv=s.nimi_sv,maanosa1_en=s.nimi_en,
...
USING (
  SELECT
	...
	--1
	CASE WHEN s.koodi = '246' THEN '1' WHEN s.koodi != '-1'  THEN '2' ELSE m.koodi END as maanosat1koodi,
	CASE WHEN s.koodi = '246' THEN s.nimi WHEN s.koodi != '-1'  THEN 'Muu' ELSE m.nimi END as  maanosat1nimi,
	CASE WHEN s.koodi = '246' THEN s.nimi_sv WHEN s.koodi != '-1'  THEN 'Övrigt' ELSE m.nimi_sv END as  maanosat1nimi_sv,
	CASE WHEN s.koodi = '246' THEN s.nimi_en WHEN s.koodi != '-1'  THEN 'Other' ELSE m.nimi_en END as  maanosat1nimi_en,
	--2
...
WHEN MATCHED THEN
	UPDATE SET
		...
		maanosa1_koodi=src.maanosat1koodi,maanosa1_fi=src.maanosat1nimi,maanosa1_sv=src.maanosat1nimi_sv,maanosa1_en=src.maanosat1nimi_en,
...
WHEN NOT MATCHED THEN
	...
	VALUES (
		...
		src.maanosat1koodi,src.maanosat1nimi,src.maanosat1nimi_sv,src.maanosat1nimi_en,
...
IF NOT EXISTS (
	SELECT * FROM INFORMATION_SCHEMA.COLUMNS
	WHERE TABLE_SCHEMA='dw' AND TABLE_NAME='d_maatjavaltiot2' AND COLUMN_NAME='jarjestys_maanosa1_koodi'
)

BEGIN
	ALTER TABLE dw.d_maatjavaltiot2 ADD jarjestys_maanosa1_koodi AS (case when maanosa1_koodi = '-1' then '999999' else cast(maanosa1_koodi as int) end)
END
