...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_ammatillinen_koulutus_tutkinnon_osat_ja_naytot]    Script Date: 20.5.2026 9.42.48 ******/
SET ANSI_NULLS ON
GO
...
SELECT
	...
	,vahvistus_paiva = case when pihvi.opiskeluoikeus_oid is not null and ps.vahvistus_paiva is null then null when pihvi.opiskeluoikeus_oid is not null then pihvi.vahvistus_paiva else ps.vahvistus_paiva end
...
FROM Koski_SA.sa.sa_koski_opiskeluoikeus oo
JOIN Koski_SA.sa.sa_amm_paatason_suoritus ps ON ps.opiskeluoikeus_oid = oo.opiskeluoikeus_oid
LEFT JOIN Koski_SA.sa.temp_amm_hankintakoulutus pihvi ON pihvi.sisaltyy_opiskeluoikeuteen_oid = oo.opiskeluoikeus_oid
...
SELECT
	...
	coalesce(d25.id, -1) as d_amos_spl_jarjestaja_linkki_id,
...
LEFT JOIN Koski_SA.sa.sa_amm_paatason_suoritus ps2 on ps.opiskeluoikeus_oid = ps2.opiskeluoikeus_oid and ps2.rnk = 1
...
LEFT JOIN ANTERO.dw.d_amos_spl_jarjestaja_linkki d25 on d25.oid_avain = ps.koulutustoimija_oid
WHERE tos.opiskeluoikeus_oid IS NOT NULL AND YEAR(tos.arviointi_paiva) <= @loppuVuosi

-- Koko tutkinnon suorittaneet, joilta ei löydy tutkinnon osia
INSERT INTO ANTERO.dw.f_koski_ammatillinen_koulutus_tutkinnon_osat_ja_naytot
SELECT DISTINCT

	NULL as arviointi_paiva,
	d1.kalenteri_avain as vahvistus_paiva,
	...
	coalesce(d3.id, -1) as d_maatjavaltiot2_kansalaisuus_id,
	coalesce(d4.id, -1) as d_ika_id,
	-1 as d_kytkin_oppivelvollinen_id,

	-1 as d_koski_tutkinnon_osat_id,
	-1 as d_kytkin_pakollinen_tutkinnon_osa_id,
	-1 as d_kytkin_paikallinen_tutkinnon_osa_id,
	-1 as d_kytkin_tunnustettu_tutkinnon_osa_id,
	-1 as d_kytkin_yhteinen_tutkinnon_osa_id,
	-1 as d_kytkin_yhteinen_ammatillinen_valinnainen_tutkinnon_osa_id,

	coalesce(d9.id, -1) as d_koulutusluokitus_id,
	coalesce(d10.id, -1) as d_koski_suorituksen_tyyppi_id,
	coalesce(d11.id, -1) as d_osaamisala_koodisto_id,
	coalesce(d12.id, -1) as d_tutkintonimike_id,
	coalesce(d13.id, -1) as d_suorituskieli_id,
	coalesce(d14.id, -1) as d_opintojenrahoitus_id,
	coalesce(d15.id, -1) as d_kytkin_hankintakoulutus_id,

	coalesce(d6.id, -1) as d_organisaatioluokitus_koulutuksen_jarjestaja_id,
	coalesce(d7.id, -1) as d_organisaatioluokitus_oppilaitos_id,
	coalesce(d8.id, -1) as d_organisaatioluokitus_toimipiste_id,

	-1 as d_kytkin_tyossaoppimisen_yhteydessa_id,
	-1 as d_nayton_arvioinnista_paattaneet_id,
	-1 as d_nayton_suorituspaikka_id,

	coalesce(d17.id, -1) as d_amos_spl_jarjestaja_linkki_di,
...
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d6 on d6.id = f.d_organisaatioluokitus_jarj_id
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d7 on d7.id = f.d_organisaatioluokitus_oppilaitos_id
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d8 on d8.id = f.d_organisaatioluokitus_toimipiste_id
LEFT JOIN ANTERO.dw.d_koulutusluokitus d9 on d9.id = f.d_koulutusluokitus_id
LEFT JOIN ANTERO.dw.d_koski_suorituksen_tyyppi d10 on d10.koodi = 'ammatillinentutkinto'
LEFT JOIN Koski_SA.sa.sa_amm_paatason_suoritus ps on ps.opiskeluoikeus_oid = f.opiskeluoikeus_oid and ps.rnk = 1
LEFT JOIN Koski_SA.sa.sa_koski_paatason_suoritus_osaamisala pso on pso.paatason_suoritus_id = ps.paatason_suoritus_id
LEFT JOIN Koski_SA.sa.sa_koski_paatason_suoritus_tutkintonimike pst on pst.paatason_suoritus_id = ps.paatason_suoritus_id
LEFT JOIN ANTERO.dw.d_osaamisala_koodisto d11 on d11.koodi = pso.osaamisala_koodiarvo
LEFT JOIN ANTERO.dw.d_tutkintonimike d12 on d12.tutkintonimike_koodi = pst.tutkintonimike_koodiarvo
LEFT JOIN ANTERO.dw.d_kieli d13 on d13.id = f.d_kieli_suorituskieli_id
LEFT JOIN ANTERO.dw.d_opintojenrahoitus d14 on d14.id = f.d_opintojenrahoitus_id
LEFT JOIN ANTERO.dw.d_kytkin d15 on d15.id = f.d_kytkin_hankintakoulutus_id
LEFT JOIN ANTERO.dw.d_suorituksen_tyyppi d16 on d16.id = f.d_suorituksen_tyyppi_id
LEFT JOIN ANTERO.dw.d_amos_spl_jarjestaja_linkki d17 on d17.oid_avain = d6.organisaatio_oid
WHERE NOT EXISTS (
	SELECT TOP 1 opiskeluoikeus_oid FROM Koski_SA.sa.temp_tutkinnon_osat tos
	WHERE tos.opiskeluoikeus_oid = f.opiskeluoikeus_oid
) and d16.koodi = 1
