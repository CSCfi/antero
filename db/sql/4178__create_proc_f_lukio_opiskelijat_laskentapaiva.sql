...
DROP PROCEDURE IF EXISTS[dw].[p_lataa_f_koski_lukio_opiskelijat_laskentapaiva]
GO

/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opiskelijat_laskentapaiva]    Script Date: 7.12.2020 14:50:52 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dw].[p_lataa_f_koski_lukio_opiskelijat_laskentapaiva] AS
...
SELECT [opiskeluoikeus_oid]
	...
	,CASE
		...
		AS tavoitetutkinto
		  ...
		  ,ps.[toimipiste_nimi]
		  ,Sukupuoli = (CASE WHEN CAST(SUBSTRING(henk.hetu,8,3) AS INT) % 2 = 1 THEN '2' WHEN CAST(SUBSTRING(henk.hetu,8,3) AS INT) % 2 = 0 THEN '1' ELSE '-1' END)
