...
/****** Object:  View [dbo].[v_sa_K3_10_Tutk_suor_osuus_aloittaneista_korkeakoulut_nykytila_1]    Script Date: 22.11.2024 14:39:27 ******/
SET ANSI_NULLS ON
GO
...
-- UUSI VUOSI
-- päivitä alle tietolähde ja lähdetaulut; lisää sarakkeet toiseen näkymään; poista alusta vanha vuosi

   ,[lkm]
      ,[tietolahde]='K3_10_sopv_24'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv, alvv, allk)
      ,[aineistorivinumero] = ROW_NUMBER() OVER(ORDER BY tilv, alvv, allk)
	  ,f1.[rivinro]
  FROM [TK_H9098_CSC].[dbo].[TK_K3_10_sopv_24_1] f1
  inner join [TK_H9098_CSC].[dbo].[TK_K3_10_sopv_24_2] as f2 on f1.rivinro=f2.rivinro 
