...
SELECT 
	...
	tuen_paatos = case when ooa2.tuen_paatoksen_jakso = 1 or ooa.tuen_paatoksen_jakso = 1 then 1 else 0 end
...
BEGIN
	...
	FROM (
		...
		LEFT JOIN sa.sa_oppivelvolliset_esi_ja_perusopetus ov on ov.master_oid = epo.master_oid and @alkuPvm BETWEEN ov.alkuPvm and ov.loppuPvm
		...
		AND NOT EXISTS (
			...
			WHERE tuvat.master_oid = epo.master_oid
...
--aikuisten perusopetuksen oppilaiden päätellyt vaiheet
EXEC [Koski_SA].[dbo].[p_lataa_esi_ja_perus_oppilaat_aipe_lukukausittain]
