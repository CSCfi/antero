USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_amk_rekrytointi_valitut]    Script Date: 24.9.2019 14:49:26 ******/
SET ANSI_NULLS ON
GO
...
LEFT JOIN [dw].[d_amk_toimipisteen_toimipaikka] s17 ON [s17].[koodi_toimipaikka] = [s1].[toimipaikka] AND [s17].[koodi_amk] = [s1].[korkeakoulu] AND [s17].[vuosi] =Tilastovuosi
...
LEFT JOIN [dw].[d_arvokysymys] s23 ON
--
--eliminate double rows (dublicate kysymysryhma for kysymys) when  kysymysryhmaid might vary
--  WHEN Finnish question
	([s23].[kysymys_fi] = [s1].[Kysymys] AND (select top 1 kysymysryhmaid from [dw].[d_arvokysymys] d1 where s1.kysymys= d1.kysymys_fi) = [s23].[kysymysryhmaid])
	 OR
--  WHEN Swedish question
	([s23].[kysymys_sv] = [s1].[Kysymys] AND (select top 1 kysymysryhmaid from [dw].[d_arvokysymys] d1 where s1.kysymys= d1.kysymys_sv) = [s23].[kysymysryhmaid])
--
LEFT JOIN [dw].[d_arvovastaus] s24 ON
--     WHEN Finnish question            OR    WHEN Swedish question
	[s24].[vastaus_fi] = [s1].[vastaus] OR [s24].[vastaus_sv] = [s1].[vastaus]
