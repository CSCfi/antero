...
DROP PROCEDURE IF EXISTS [dw].[p_lataa_f_amos_spl_vaikuttavuus]
GO
...
CREATE PROCEDURE [dw].[p_lataa_f_amos_spl_vaikuttavuus] AS
...
FROM 
  (
    SELECT 
      ...
      Sum([tyolliset_opiskelijat_ja_tyottomat])
       AS tyolliset_opiskelijat_ja_tyottomat_yhteensa
    FROM 
      ANTERO.sa.z_tmp_TK_K1_13_sopv
...
INTO ANTERO.sa.z_tmp_TK_K1_K13_sopv_jarjkerroin_fuusiot
  ...
  LEFT JOIN ANTERO.dw.d_amos_spl_jarjestaja_linkki d ON jk.jarj_opisk = d.ytunnus_avain 
	AND (jk.tilv + 3 <= YEAR(d.voimaan_alkaen) OR d.voimaan_alkaen IS NULL)
GROUP BY [tilv]
      ,d.ytunnus_amos_spl
	  ,d.oid_amos_spl
	  ,nimi_amos_spl
...
,CASE WHEN spl.nimi_amos_spl IS NOT NULL THEN 'Kyllä' ELSE 'Ei' END AS Suoritepäätös
,okm8.paa_asiallinen_toiminta_maakunta_koodi
      ...
      ,CASE
		...
		END
		AS [Koulutuksen järjestäjä (tutkinnon)]
	  ...
	  ,CASE	
		...
		WHEN ptoim1r2h = '22' THEN 'Päätoiminen opiskelija, koululainen'
	  ...
	  ,CASE	
		...
		WHEN ptoim1r2_tilvseur = '22' THEN 'Päätoiminen opiskelija, koululainen'
	  ...
	  ,statuskerroin * maak.aluekerroin * jk.jarjestajakerroin * k13.tyolliset AS [v Työllisten painotetut pisteet]
...
INTO ANTERO.dw.f_amos_spl_vaikuttavuus
  ...
  LEFT JOIN ANTERO.sa.z_tmp_TK_K1_K13_sopv_jarjkerroin_fuusiot jk
	ON jk.ytunnus_amos_spl = spl.ytunnus_amos_spl 
