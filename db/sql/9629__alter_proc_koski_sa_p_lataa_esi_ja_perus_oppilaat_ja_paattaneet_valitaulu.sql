...
/****** Object:  StoredProcedure [dbo].[p_lataa_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu]    Script Date: 20.2.2024 14:15:29 ******/
SET ANSI_NULLS ON
GO
...
DROP TABLE IF EXISTS sa.temp_esi_ja_peusopetus_oo
...
INTO sa.temp_esi_ja_peusopetus_oo		
...
BEGIN
	...
	FROM (
		...
		SELECT 
			...
			ranki = 
				case 
					when epo.suorituksen_tyyppi <> 'perusopetuksenvuosiluokka' then 1 
					else rank() over(partition by epo.oppija_oid, epo.suorituksen_tyyppi order by epo.koulutusmoduuli_koodiarvo desc)
				end
			
		FROM sa.temp_esi_ja_peusopetus_oo epo	
		WHERE 1=1 --@alkupvm >= '2020-08-01' 
		AND coalesce(epo.vahvistus_paiva, '9999-12-31') >= @alkuPvm
		AND (
			CASE 
				-- rankia varten
				WHEN epo.suorituksen_tyyppi = 'perusopetuksenvuosiluokka' and (epo.suorituksen_alkamispaiva > @loppuPvm or epo.suorituksen_alkamispaiva is null) THEN 0
				-- 1.8.2022 alkaen TUVA, jota ei raportoida perusopetuksen yhteydessä
				WHEN epo.suorituksen_tyyppi = 'perusopetuksenlisaopetus' and @alkupvm >= '2022-08-01' THEN 0
				ELSE 1
			END = 1
		)		 
		AND (
	...
	FROM (
		...
		WHERE epo.vahvistus_paiva between @alkuPvm and @loppuPvm
		AND CASE WHEN epo.suorituksen_tyyppi = 'perusopetuksenlisaopetus' and @alkupvm >= '2022-08-01' THEN 0 ELSE 1 END = 1 
	) q
