...
/****** Object:  View [dw].[v_arvo_amk_uraseuranta_uusi_api_testi]    Script Date: 16.3.2026 11.51.22 ******/
SET ANSI_NULLS ON
GO
...
, CONCAT(f.kyselyid,d4.kysymysid,d6.koodi,f.vastaajaid)
											AS 'vastausavain'
...
,CASE
	...
	WHEN f.vastaajaid is null and CAST(CONCAT(f.tilastovuosi,'-11-11') AS date) > CAST(GETDATE() AS date) THEN 0
	WHEN f.tilastovuosi is null THEN 0
	WHEN CAST(CONCAT(f.tilastovuosi,'-11-11') AS date) <= CAST(GETDATE() AS date) THEN 1
	ELSE 0
...
, d10.jarjestys								AS 'Järjestys pohjakoulutus'
, d11.koodi									AS 'Järjestys työkokemus'
, d12.jarjestys_rahoituslahde_koodi			AS 'Järjestys rahoituslähde'
...
LEFT JOIN dw.d_arvovastaus_monivalinta d10 ON d10.id =f.d_arvovastaus_taustakysymys_pohjakoulutus_id		-- pohjakoulutus
LEFT JOIN dw.d_arvovastaus_monivalinta d11 ON d11.id=f.d_arvovastaus_taustakysymys_tyokokemus_id			-- työkokemus																		
LEFT JOIN dw.d_virta_rahoituslahde d12 on d12.id = f.d_virta_rahoituslahde_id
LEFT JOIN dw.f_arvo_luodut_vastaajatunnukset f2 ON f2.kyselyid = f.kyselyid and 
