...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvollisten_opintojen_kulku]    Script Date: 26.9.2024 12:00:47 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	LEFT JOIN (
		...
		WHERE (
			ps.suorituksen_tyyppi in ('aikuistenperusopetuksenoppimaara', 'perusopetuksenoppimaara','perusopetuksenlisaopetus') 
		...
		AND YEAR(ps.vahvistus_paiva) < 2021 
		AND oo.oppivelvollisuuden_suorittamiseen_kelpaava = 1
	...
	WHERE YEAR(syntymaaika) >= 2004 AND d.mannersuomessa = 1 AND h.turvakielto = 0 AND oo.oppija_oid is null
) e
		...
		FROM (
			...
			FROM (
				SELECT DISTINCT
					...
					END as keskeyttanyt,
					CASE
						WHEN ov.oppija_oid is null THEN 1
						ELSE 0
