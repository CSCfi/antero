...
/****** Object:  View [dw].[v_amos_tyoelamapalaute]    Script Date: 7.4.2026 15.09.46 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,[Toimipiste] = d1b.organisaatio_fi
	,[Toimipisteen kunta] = d5b.kunta_fi
	,[Toimipisteen maakunta] = d5b.maakunta_fi

	,koodit_koulutuksen_jarjestaja = coalesce(d.ytunnus_amos_spl, f.ytunnus) --coalesce(d.ytunnus_amos_spl, d.ytunnus_avain)
	,[Koodit Oppilaitos] = f.oppilaitos_koodi --d1.organisaatio_koodi
	...
	,[jarjestys Toimipisteen maakunta] = d5b.jarjestys_maakunta_koodi
...
LEFT JOIN dw.d_organisaatioluokitus d1 ON d1.organisaatio_koodi = f.oppilaitos_koodi AND d1.organisaatio_avain like 'oppilaitos_%'
LEFT JOIN dw.d_organisaatioluokitus d1b ON d1b.organisaatio_oid = f.toimipiste_oid
...
LEFT JOIN dw.d_alueluokitus d5b ON d5b.kunta_koodi = coalesce(d1b.kunta_koodi, '-1')
