USE [Koski_SA]
GO
/****** Object:  StoredProcedure [sa].[p_lataa_opiskelijavuodet_paallekkaiset_opiskeluoikeudet]    Script Date: 27.8.2021 18:01:06 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT 
		...
		,ed_paattyi = case when aiempi_paattymispvm <= nyk_loppu_korjattu then cast(aiempi_paattymispvm as date) end
	...
	FROM (
		...
		SELECT distinct
			...
			,oo_la.tila
			,cast(oo_la.alku as date) ed_alku
			,cast(oo_la.loppu as date) ed_loppu
			,nyk_tila = ooa.tila
			,nyk_alku = ooa.alku
			,nyk_alku_korjattu = cast((select MaxValue from dbo.Maximum (@startDate, ooa.alku, oo_la.alku)) as date)
			,nyk_loppu_korjattu = cast(
					case
						when oo_la.opiskeluoikeus_paattynyt = 1 and oo_la.loppu <= ooa.loppu then (select MinValue From dbo.Minimum (@endDate, ooa.loppu, default))
			 ...
			 ,ed_paattynyt = oo_la.opiskeluoikeus_paattynyt
			 --,ed_jatkuu
		...
		inner join (
			...
			select 
				...
				,valiaikainen_kesk = case when tila = 'valiaikaisestikeskeytynyt' then 1 else 0 end
			...
			and oo.sisaltyy_opiskeluoikeuteen_oid is null
			   ...
			   and ((oo_la.loppu between ooa.alku and ooa.loppu) or (oo_la.alku between ooa.alku and ooa.loppu) or (ooa.alku between oo_la.alku and oo_la.loppu)) --aikajaksot menee päällekkäin
	...
	--AND nyk_opiskeluoikeus_oid = '1.2.246.562.15.79249625876'
