...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dw].[p_lataa_api_amm_opiskelijat_ja_tutkinnot_kuukausi_maakunta_esilataus]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dw].[p_lataa_api_amm_opiskelijat_ja_tutkinnot_kuukausi_maakunta_esilataus] AS' 
END
GO
...
LEFT JOIN dw.d_kalenteri d1 ON d1.id = f.d_kalenteri_id
LEFT JOIN dw.d_sukupuoli d2 ON d2.id = f.d_sukupuoli_id
LEFT JOIN dw.d_organisaatioluokitus d3 ON d3.id = f.d_organisaatioluokitus_oppilaitos_id
LEFT JOIN dw.d_alueluokitus d4 ON d4.kunta_koodi = d3.kunta_koodi 
LEFT JOIN dw.d_kieli d5 ON d5.id = f.d_kieli_aidinkieli_id
LEFT JOIN dw.d_ika d6 ON d6.id = f.d_ika_id
LEFT JOIN dw.d_maatjavaltiot2 d7 ON d7.id = f.d_maatjavaltiot2_kansalaisuus_id
GROUP BY 
	f.tilastovuosi,
	d1.kuukausi,
	COALESCE(d4.maakunta_fi, 'Tieto puuttuu'),
