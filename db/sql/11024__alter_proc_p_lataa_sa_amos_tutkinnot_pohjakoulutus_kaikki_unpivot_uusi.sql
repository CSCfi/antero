...
SELECT 
	...
	,pk_osuus_kaikki = cast(1.0 * sum(sum(pohjak_lkm)) over (partition by suorituskausi, tutktyyppi) / sum(sum(yhteensa_lkm)) over (partition by suorituskausi, tutktyyppi) as decimal(15,14))
	,ei_pk_osuus_kaikki = cast(1.0 * sum(sum(ei_pohjak_lkm)) over (partition by suorituskausi, tutktyyppi) / sum(sum(yhteensa_lkm)) over (partition by suorituskausi, tutktyyppi) as decimal(15,14))
