...
ALTER PROC [dbo].[p_lataa_TK_Koski_esi_ja_perusopetus_oppilaat] AS

/*
HUOM. Sisältää s24 alkaen kaksi poimintaa. 

Päivitysohje:
1. Kasvata alun delete-ehdon vuosirajausta yhdellä.
2. Kopioi edellisen vuoden kaksi insertiä ylimmäksi, kasvata näiden vuosirajauksia yhdellä ja poiskommentoi vanhat insertit.
*/

DELETE FROM dbo.TK_Koski_esi_ja_perusopetus_oppilaat
WHERE opisk = 1
AND 
	CASE 
		WHEN [tilastopaiva_1_0] = 1 THEN vuosi
		WHEN [tilastopaiva_1_0] = -1 THEN year(dateadd(month, 5, datefromparts(vuosi, kuukausi, 1)))
	END 
	= 2025 --huom. päivitä vvvv
;

--1/2: Tilastopäivä 20.9.
INSERT dbo.TK_Koski_esi_ja_perusopetus_oppilaat
SELECT 
	[opisk]
    ,[tutk]
    ,[vuosi]
    ...
    ,[kuljetusetu]
	,majoitusetu
	,jaa_luokalle = cast(null as int)
	,vahvistus_paiva = cast(null as date)
...
FROM [DWIVIPUARCH].[Koski_SA_02102025].[dbo].[esi_ja_perus_oppilaat_ja_paattaneet_valitaulu] vt --huom. päivitä kantaviite
WHERE opisk = 1
AND [tilastopaiva_1_0] = 1
AND vuosi = 2025 --huom. päivitä vvvv
;
...
FROM [DWIVIPUARCH].[Koski_SA_02102025].[dbo].[esi_ja_perus_oppilaat_ja_paattaneet_valitaulu] vt --huom. päivitä kantaviite
WHERE opisk = 1 
AND year(dateadd(month, 5, datefromparts(vuosi, kuukausi, 1))) = 2025 --huom. päivitä vvvv
;

--luokalle jääneet; poikkeuksena myös 2024 katsotaan 2025 jäädytyksestä (tätä pätkää ei kopioida)
INSERT dbo.TK_Koski_esi_ja_perusopetus_oppilaat
SELECT 
	[opisk]
    ,[tutk]
    ...
    ,[tilastopaiva_1_0] = -1 --tarvittaessa kuution mittareita tai raportin piilosuodatusta varten
    ,[aikuisten_alkuvaihe_1_0]
FROM [DWIVIPUARCH].[Koski_SA_02102025].[dbo].[esi_ja_perus_oppilaat_ja_paattaneet_valitaulu_2024_luokalle_jaaneet] vt
WHERE opisk = 1 
