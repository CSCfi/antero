...
/****** Object:  View [dbo].[v_f_2_10_Korkeakouluopiskelijoiden_aikaisempi_koulutus]    Script Date: 8.4.2021 20:15:57 ******/
SET ANSI_NULLS ON
GO
    ...
    as nvarchar(150))
      ...
      (
	   ...
	   ,ammtutk_pohjakoulutuksen_tila = cast(
		(
		case
        when (aikamm= '1' and ((cast(alvv_johdettu as int) - cast(suvuosiamm as int) >= 0))) then 'Alempi ammatillinen tutkinto suoritettu ennen koulutuksen aloittamista'
        when (aikamm = '1' and ((cast(alvv_johdettu as int) - cast(suvuosiamm as int) < 0))) then 'Alempi ammatillinen tutkinto suoritettu koulutuksen aloittamisen jälkeen'
        else  'Ei suoritettu alempaa ammatillista tutkintoa'
    end
       )
    as nvarchar(150))
