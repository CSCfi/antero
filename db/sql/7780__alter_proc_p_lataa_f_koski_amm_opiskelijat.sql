...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_amm_opiskelijat]    Script Date: 9.6.2023 10:03:42 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT
		 sa.vuosi
		 ,CASE 
			  WHEN (sa.kuukausi = 9 AND DATEFROMPARTS(sa.vuosi, sa.kuukausi, 20) > ooa.alku AND ooa.loppu IS NULL AND ps.vahvistus_paiva IS NULL) THEN 1
			  WHEN (sa.kuukausi = 9 AND DATEFROMPARTS(sa.vuosi, sa.kuukausi, 20) BETWEEN ooa.alku AND ooa.loppu) THEN 1
			  ELSE 0
		...
		,valmentava_koodi = 
			case
				...
				when sa.suorituksen_tyyppi = 'tuvakoulutuksensuoritus' then 3 --lähdetaulussa [tuva_jarjestamuslupa] = 'ammatillinen'
			end 
		,uusi
		,uusi_rahoitus
		,sa.oppija_oid
	...
	LEFT JOIN Koski_SA.sa.sa_koski_paatason_suoritus AS ps ON sa.opiskeluoikeus_oid = ps.opiskeluoikeus_oid				-- UUSI
	LEFT JOIN Koski_SA.sa.sa_koski_opiskeluoikeus_aikajakso AS ooa ON sa.opiskeluoikeus_oid = ooa.opiskeluoikeus_oid	-- UUSI
