USE [ANTERO]
GO
/****** Object:  View [dw].[v_ehoks_tutkinnon_suorittaneet_cultures]    Script Date: 27.1.2022 17:17:30 ******/
DROP VIEW IF EXISTS [dw].[v_ehoks_tutkinnon_suorittaneet_cultures]
GO
/****** Object:  View [dw].[v_ehoks_opiskelijat_cultures]    Script Date: 27.1.2022 17:17:30 ******/
DROP VIEW IF EXISTS [dw].[v_ehoks_opiskelijat_cultures]
GO
/****** Object:  View [dw].[v_ehoks_opiskelijat_cultures]    Script Date: 27.1.2022 17:17:30 ******/
SET ANSI_NULLS ON
GO
...
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dw].[v_ehoks_opiskelijat_cultures]'))
EXEC dbo.sp_executesql @statement = N'




CREATE VIEW [dw].[v_ehoks_opiskelijat_cultures] AS
...
SELECT 
	...
	,[jarjestys_tutkintotyyppi] = case when d1.koulutusluokitus_fi like ''%VALMA%'' then ''91'' when d1.koulutusluokitus_fi like ''%TELMA%'' then ''92'' when d24.kytkin_koodi = 1 then ''99'' else d1.jarjestys_tutkintotyyppi_koodi end
...
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dw].[v_ehoks_tutkinnon_suorittaneet_cultures]'))
EXEC dbo.sp_executesql @statement = N'


CREATE VIEW [dw].[v_ehoks_tutkinnon_suorittaneet_cultures] AS
...
SELECT 
	...
	--järjestykset
	,[jarjestys_tutkintotyyppi] = case when d24.kytkin_koodi =  1 then 98 else d1.jarjestys_tutkintotyyppi_koodi end
	,[jarjestys_koulutusala_taso_1] = d1.jarjestys_koulutusalataso1_koodi
	,[jarjestys_koulutusala_taso_2] = d1.jarjestys_koulutusalataso2_koodi
	,[jarjestys_koulutusala_taso_3] = d1.jarjestys_koulutusalataso3_koodi
	,[jarjestys_suorituksen_tyyppi] = d9.jarjestys
...
CROSS APPLY (
	SELECT 
		oppisopimuskoulutuksen_osuus = 
			case 
				when f.oppisopimuksen_osuus < 0.25 then ''0 - 25 %''
				when f.oppisopimuksen_osuus < 0.50 then ''25 - 50 %''
				when f.oppisopimuksen_osuus < 0.75 then ''50 - 75 %''
				else ''75 - 100 %''
			end
		,koulutussopimuskoulutuksen_osuus = 
			case 
				when f.koulutussopimuksen_osuus < 0.25 then ''0 - 25 %''
				when f.koulutussopimuksen_osuus < 0.50 then ''25 - 50 %''
				when f.koulutussopimuksen_osuus < 0.75 then ''50 - 75 %''
				else ''75 - 100 %''
			end
