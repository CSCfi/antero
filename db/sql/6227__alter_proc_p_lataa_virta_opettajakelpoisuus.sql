...
/****** Object:  StoredProcedure [sa].[p_lataa_virta_opettajakelpoisuus]    Script Date: 24.5.2022 9:06:57 ******/
SET ANSI_NULLS ON
GO
...
-- TEMP-tauluun kombinaatiot kelpoisuuskoodeista sekä kelpoisuuskoulujen korkeakoulu ja pvm
	...
	SELECT 
		...
		suoritustapa,
		rnk = row_number() over (
			partition by henkilo, kelpoisuus 
...
outer apply (
	...
	from (
		...
		where t2.vuosi <= kelp.vuosi and t2.henkilo=p.henkilo and t2.koulutuskoodi <> '999999'
	) q
	where rnk=1
) kork_tutk
...
OUTER APPLY (
	Select
		CASE 
			...
			WHEN p1.koodi = 'PD' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','ER','OP') FOR XML PATH('')  ), 2 , 9999)),' ', '')
