USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_yo_henkilon_tyo]    Script Date: 21.9.2018 10:24:07 ******/
SET ANSI_NULLS ON
GO
...
SELECT COALESCE(s2.id,-1)
	...
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM(s22.[Sopimuksen loppupäivämäärä])), 104), - 1) AS [sopimus_loppupvm]
	,getdate()
...
LEFT JOIN [sa].[sa_suorat_yo1_henkilokunta_tk] s22 ON s22.Henkilönumero = s1.YO_HENKILONRO AND s22.Vuosi = s1.VUOSI AND s22.Yliopisto = s1.YLIOPISTO_TUNNUS
