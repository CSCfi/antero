...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_henkilostotiedot_amk]    Script Date: 14.9.2026 8.19.47 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	FROM (
		SELECT 
			...
			CAST(SUM(CASE WHEN d3.maatjavaltiot2_fi NOT IN ('Suomi', 'Tieto puuttuu', 'Tuntematon') THEN f.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(f.henkilotyovuosi) as float) as arvo,
	...
	OUTER APPLY (
		SELECT 
			CAST(SUM(CASE WHEN d3b.maatjavaltiot2_fi NOT IN ('Suomi', 'Tieto puuttuu', 'Tuntematon') THEN fb.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(fb.henkilotyovuosi) as float) as arvo_sektori
...
-- Tohtorintutkinnon suorittaneiden osuus opetus- ja TKI-henkilöstössä (vuodesta 2021 alkaen)
...
USING (
	...
	FROM (
		SELECT 
			f.vuosi as tilastovuosi,
			d1.organisaatio_nykyinen_koodi as organisaatio_koodi,
			CAST(SUM(CASE WHEN f.suorittanut_tohtorintutkinnon  = 1 THEN f.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(f.henkilotyovuosi) as float) as arvo,
			'toht_otki_osuus' as indikaattori
		FROM [ANTERO].[dw].[f_amk_henkilon_tyo] f
		LEFT JOIN ANTERO.dw.d_organisaatioluokitus d1 on d1.id = f.d_amk_id 
		LEFT JOIN ANTERO.dw.d_amk_tehtavanjaottelu d2 on d2.id = f.d_tehtavanjaottelu
		WHERE vuosi >= 2021 and d2.selite_fi in ('Opetushenkilökunta', 'T&K&I-toiminnan henkilökunta')
		GROUP BY vuosi, d1.organisaatio_nykyinen_koodi
	) f
	OUTER APPLY (
		SELECT 
			CAST(SUM(CASE WHEN fb.suorittanut_tohtorintutkinnon  = 1 THEN fb.henkilotyovuosi ELSE 0 END) as float) / CAST(SUM(fb.henkilotyovuosi) as float) as arvo_sektori
		FROM [ANTERO].[dw].[f_amk_henkilon_tyo] fb
		LEFT JOIN ANTERO.dw.d_organisaatioluokitus d1b on d1b.id = fb.d_amk_id 
		LEFT JOIN ANTERO.dw.d_amk_tehtavanjaottelu d2b on d2b.id = fb.d_tehtavanjaottelu
		WHERE d2b.selite_fi in ('Opetushenkilökunta', 'T&K&I-toiminnan henkilökunta') and fb.vuosi = f.tilastovuosi
