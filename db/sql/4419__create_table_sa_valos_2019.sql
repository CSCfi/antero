...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[sa].[sa_valos_2019]') AND type in (N'U'))
BEGIN

CREATE TABLE [sa].[sa_valos_2019](
 ...
 CONSTRAINT [PK__sa_valos_2019] PRIMARY KEY CLUSTERED
(
	[id] ASC
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'sa.DF__sa_valos_2019__loadtime') AND type = 'D')
BEGIN
	ALTER TABLE [sa].[sa_valos_2019] ADD  CONSTRAINT [DF__sa_valos_2019__loadtime]  DEFAULT (getdate()) FOR [loadtime]
END
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'sa.DF__sa_valos_2019__username') AND type = 'D')
	BEGIN
ALTER TABLE [sa].[sa_valos_2019] ADD  CONSTRAINT [DF__sa_valos_2019__username]  DEFAULT (suser_name()) FOR [username]
END
GO
