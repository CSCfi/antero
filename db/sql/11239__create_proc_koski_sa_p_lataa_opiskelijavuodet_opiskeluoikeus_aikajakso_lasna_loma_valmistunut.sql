...
CREATE OR ALTER PROCEDURE [sa].[p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_lasna_loma_valmistunut] AS
...
FROM ( 
	...
	SELECT
		...
		,ed_alku = oo_l.alku
		,ed_loppu = oo_l.loppu
		,opintojen_rahoitus = coalesce(case when sa1.tila = 'lasna' then sa1.opintojen_rahoitus end, sa2.opintojen_rahoitus, sa3.opintojen_rahoitus, sa4.opintojen_rahoitus, sa1.opintojen_rahoitus)
		,maksuton
		,oikeutta_maksuttomuuteen_pidennetty
		,majoitus
		,sisaoppilaitosmainen_majoitus
		...
		,vaikeasti_vammainen
		,vammainen_ja_avustaja
		,osa_aikaisuus = coalesce(case when sa1.tila = 'lasna' then sa1.osa_aikaisuus end, sa2.osa_aikaisuus, sa3.osa_aikaisuus, sa4.osa_aikaisuus, sa1.osa_aikaisuus)
		,vankilaopetuksessa
		,oppisopimus_jossain_paatason_suorituksessa
		,opiskeluvalmiuksia_tukevat_opinnot
