...
SELECT DISTINCT
	...
	,CASE
		...
		WHEN [koulutusluokitus_fi] IN ('Lukion aineen oppimäärä', 'Lukiokoulutukseen valmistava koulutus', 'Tutkintokoulutukseen valmentava koulutus') THEN 4
	...
	,CASE
		WHEN ((d7.maakunta_koodi NOT IN ('21', '-1', '98') AND d7.maakunta_koodi IS NOT NULL) AND --Kotikunta Manner-Suomessa
			 ...
			 ((vt.vuosi-YEAR(henk.syntymaaika) >= 7 AND FLOOR(DATEDIFF(DAY, henk.syntymaaika, DATEFROMPARTS(vt.vuosi, vt.kuukausi, 1)) / 365.25)< 18) OR --Henkilö on yli 7-vuotias, mutta alle 18-vuotias
			 (vt.vuosi-YEAR(henk.syntymaaika) < 7 AND (ooa.pidennetty_oppivelvollisuus = 1 AND DATEFROMPARTS(vt.vuosi, vt.kuukausi, 1) BETWEEN ooa.alku AND ooa.loppu))) AND --Henkilö on alle 7-vuotias, mutta tarkastelupäivänä Koski-opiskeluoikeusdatan mukaan pidennetyn oppivelvollisuuden piirissä
			 (spvm.tutkinto_suoritettu > DATEFROMPARTS(vt.vuosi, vt.kuukausi, 1) OR spvm.tutkinto_suoritettu IS NULL)) THEN 1 --Henkilö ei ole suorittanut ylioppilastutkintoa tai ammatillista tutkintoa
		ELSE 0
