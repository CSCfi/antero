...
/****** Object:  View [dw].[v_virta_otp_tutkinnon_suorittaneet_vaihdossa]    Script Date: 18.9.2026 13.40.05 ******/
SET ANSI_NULLS ON
GO
...
SELECT   
	...
	[Koulutussektori] = CASE 
							...
							WHEN d8.koulutussektori_fi = 'Tieto puuttuu' and d8.koulutusastetaso2_fi in ('Ammattikorkeakoulututkinto', 'Ylempi ammattikorkeakoulututkinto') THEN 'Ammattikorkeakoulukoulutus'
	...
	[lkm_tutkinnot] = 0,
	HASHBYTES('SHA2_256', vs.salt + f.opiskelijaavain) as opiskelijaavain,
...
INNER JOIN ANTERO.dbo.view_salt vs on vs.[view] = 'v_virta_otp_tutkinnon_suorittaneet_vaihdossa'

WHERE f.tilastovuosi >= 2016 
and (d8.koulutusastetaso2_koodi in ('62', '63', '71', '72')) --amk, ylempi amk, alempi ja ylempi kk
...
WHERE CAST(CONCAT(cast(Tilastovuosi as nvarchar), '-04-01') as date) < cast(DATEADD(year, -1, GETDATE()) as date)