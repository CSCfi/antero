...
/****** Object:  StoredProcedure [sa].[p_lataa_ovara]    Script Date: 2.6.2026 14.31.35 ******/
SET ANSI_NULLS ON
GO
...
SELECT
	...
	MIN(
		CASE 
			WHEN vvt.alin_hyvaksytty_pistemaara <= 0 and js.Pisteet <= 0 THEN NULL 
			WHEN vvt.alin_hyvaksytty_pistemaara <= js.Pisteet THEN vvt.alin_hyvaksytty_pistemaara
			ELSE js.Pisteet 
		END
...
INNER JOIN ANTERO.sa.sa_ovara_valintarekisteri_valinnantulos vt on js.hakemus_hakukohde_valintatapa_id = vt.hakemus_hakukohde_valintatapa_id and vt.valinnan_tila in ('VARASIJALTA_HYVAKSYTTY','HYVAKSYTTY','PERUNUT')
LEFT JOIN ANTERO.sa.sa_ovara_gen_valintarekisteri_valintatapajono vvt on vvt.hakukohde_oid = js.hakukohde_oid and vvt.valintatapajono_id = js.valintatapajono_oid
...
SELECT
	...
	rnk = ROW_NUMBER() OVER (PARTITION BY hakemus_oid, hakukohde_oid ORDER BY valinnan_tila, prioriteetti, jonosija, pisteet desc, valinnantulos_id)
...
FROM (
	SELECT
		...
		js.pisteet,
		CASE
			WHEN vt.valinnan_tila in ('HYVAKSYTTY', 'VARASIJALTA_HYVAKSYTTY', 'PERUNUT') THEN 1
			ELSE 2
		END as valinnan_tila,
		vp.prioriteetti,
		js.jonosija
	FROM [ANTERO].[sa].[sa_ovara_valintarekisteri_valinnantulos] vt
	LEFT JOIN [ANTERO].[sa].[sa_ovara_valintarekisteri_jonosija] js on js.hakemus_hakukohde_valintatapa_id = vt.hakemus_hakukohde_valintatapa_id
	...
	LEFT JOIN [ANTERO].[sa].[sa_ovara_gen_valintaperuste_valintatapajono] vp on vp.valintatapajono_id = vt.valintatapajono_oid
...
SELECT 
	...
	CASE WHEN coalesce(vtj.alin_hyvaksytty_pistemaara,0) <= 0 and yhp.alinhyvaksyttypistemaara > 0 THEN yhp.alinhyvaksyttypistemaara WHEN vtj.alin_hyvaksytty_pistemaara <= 0 THEN NULL ELSE vtj.alin_hyvaksytty_pistemaara END as alinhyvaksyttypistemaara,
	...
	CASE
		...
		WHEN	vp.valintatapajono_tyyppi is null AND (vtj.valintatapajono_nimi like 'Avoim%' OR vtj.valintatapajono_nimi like '%Avoin%' OR 
		...
		THEN 'av'
		WHEN	vp.valintatapajono_tyyppi is null AND (khk.hakukohteen_nimi_fi like '%Rolling%admission%' OR khk.hakukohteen_nimi_fi like '%Jatkuva%haku%' OR
				kh.haun_nimi_fi like '%Rolling%admission%' OR kh.haun_nimi_fi like '%Jatkuva%haku') 
		THEN 'jh'
		ELSE RIGHT(vp.valintatapajono_tyyppi, LEN(vp.valintatapajono_tyyppi) - CHARINDEX('_', vp.valintatapajono_tyyppi))
...
LEFT JOIN [ANTERO].[sa].[sa_ovara_gen_valintarekisteri_valintatapajono] vtj on vtj.hakukohde_oid = aht.hakukohde_oid and vtj.valintatapajono_id = vt.valintatapajono_oid
LEFT JOIN [ANTERO].[sa].[sa_ovara_gen_valintaperuste_valintatapajono] vp on vp.valintatapajono_id = vtj.valintatapajono_id
LEFT JOIN [ANTERO].[sa].[temp_ylinhyvaksyttypisteraja] yhp on yhp.hakukohde_oid = aht.hakukohde_oid and yhp.valintatapajono_oid = vtj.valintatapajono_id
