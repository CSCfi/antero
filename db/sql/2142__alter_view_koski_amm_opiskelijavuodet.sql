...
/****** Object:  View [dw].[v_koski_amm_opiskelijavuodet]    Script Date: 11.3.2019 14:07:49 ******/
SET ANSI_NULLS ON
GO
...
SELECT        
	opiskelijavuodet AS Opiskelijavuosi, opiskelijavuodet AS Studentår, opiskelijavuodet AS [Student year]
    ,d_kytkin.kytkin_fi AS [Osa-aikaisuus], d_kytkin.kytkin_sv AS Deltid, d_kytkin.kytkin_en AS [Part Time]
	,f.vuosi AS Tilastovuosi, f.vuosi AS Statistikår, f.vuosi AS [Statistical year]
	...
	case d_kytkin_oppisopimuskoulutus.kytkin_fi when 'Kyllä' then 'Oppisopimuskoulutus' when 'Ei' then 'Ei oppisopimuskoulutus' end AS Oppisopimuskoulutus, 
	...
	case d_kytkin_vankilaopetus.kytkin_fi when 'Kyllä' then 'Vankilaopetus' when 'Ei' then 'Ei vankilaopetus' end AS Vankilaopetus, 
    d_kytkin_vankilaopetus.kytkin_sv AS FängELSEundervisning, d_kytkin_vankilaopetus.kytkin_en AS [Prison education], 
	case d_kytkin_henkilostokoulutus.kytkin_fi when 'Kyllä' then 'Henkilöstökoulutus' when 'Ei' then 'Ei henkilöstökoulutus' end AS Henkilöstokoulutus, 
    d_kytkin_henkilostokoulutus.kytkin_sv AS Personalutbildning, d_kytkin_henkilostokoulutus.kytkin_en AS [Personnel training], 
	case d_kytkin_tyovoimakoulutus.kytkin_fi when 'Kyllä' then 'Työvoimakoulutus' when 'Ei' then 'Ei työvoimakoulutus' end AS Työvoimakoulutus, 
    d_kytkin_tyovoimakoulutus.kytkin_sv AS Arbetskraftsutbildning, d_kytkin_tyovoimakoulutus.kytkin_en AS [Labour market training], d_organisaatioluokitus_oppilaitos.organisaatio_fi AS Oppilaitos, 
    ...
    d_alueluokitus_oppilaitos.ely_sv AS [Läroanstaltens NMT-område], d_alueluokitus_oppilaitos.ely_en AS [ELY area of the institution], d_alueluokitus_oppilaitos.jarjestys_ely_koodi AS jarj_oppilaitoksen_ely, 
    d_organisaatioluokitus_jarj.organisaatio_fi AS [Koulutuksen järjestäjä], d_organisaatioluokitus_jarj.organisaatio_sv AS Utbildningsanordnare, d_organisaatioluokitus_jarj.organisaatio_en AS [Education provider], 
...
FROM  dw.f_koski_amm_opiskelijavuodet f INNER JOIN
...
dw.d_kalenteri AS d_kalenteri ON d_kalenteri_id = d_kalenteri.id INNER JOIN
...
WHERE d_kalenteri.vuosi < YEAR(getdate()) OR (d_kalenteri.vuosi = YEAR(getdate()) AND d_kalenteri.kuukausi < MONTH(getdate())) AND d_opintojenrahoitus.koodi != 6
