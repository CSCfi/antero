...
/****** Object:  StoredProcedure [dw].[p_lataa_f_oiva_luvat_amm]    Script Date: 13.3.2026 10.59.42 ******/
SET ANSI_NULLS ON
GO
...
BEGIN 
	...
	DROP TABLE IF EXISTS dw.f_oiva_luvat_amm
		...
		,opv_enim_ei_oppivelvolliset AS (
			SELECT
				lupaId
				,arvo = CASE
				    WHEN koodisto='oivamuutoikeudetvelvollisuudetehdotjatehtavat' THEN -1
					WHEN koodisto='koulutussektori' THEN CAST(m.arvo AS int)
				END
			FROM maaraykset m
			WHERE 1=1
			AND ((m.koodisto='koulutussektori' AND m.koodiarvo='5') OR (m.koodisto='oivamuutoikeudetvelvollisuudetehdotjatehtavat' AND m.koodiarvo='61242492610412222'))
		)
		...
		SELECT distinct
			...
			,d_amos_spl_jarjestaja_linkki_id = coalesce(d14.id, -1)
			...
			,opv_enim_ei_oppivelvolliset = opv_enim_ei_oppivelvolliset
		INTO #temp	
		...
		FROM (
			SELECT DISTINCT 
				...
				,osaamisala_rajoite = am16.koodiarvo -- coalesce(am16.koodiarvo,m16.koodiarvo) siirrytty käyttämään pelkästään alimääräyksen rajoitetta 10.7.2025 Tiketti CSCVIPUNEN-4409
				,osaamisala_koodi= tol.osaamisala_koodi 
				,opiskelijavuodet = o.sekt
				,opiskelijavuodet_sisa = o.sisa
				,opiskelijavuodet_vet = o.vet
				,opv_enim_ei_oppivelvolliset = o2.arvo
			FROM luvat l 
			...
			LEFT JOIN alimaaraykset am16 ON am16.maaraysId=m3.maaraysId AND am16.koodisto='osaamisala' and am16.maaraystyyppi_tunniste='RAJOITE'
		--	LEFT JOIN maaraykset m16 ON m16.lupaId=l.lupaId AND m16.koodisto='osaamisala'
			...
			LEFT JOIN opv_enim_ei_oppivelvolliset o2 ON o2.lupaId = l.lupaId
		...
		LEFT JOIN dw.d_amos_spl_jarjestaja_linkki d14 on d14.oid_avain = d1.organisaatio_oid
