USE [ANTERO]
GO
/****** Object:  View [dw].[v_koski_amm_opiskelijavuodet]    Script Date: 17.9.2019 16:15:59 ******/
DROP VIEW IF EXISTS [dw].[v_koski_amm_opiskelijavuodet]
GO
/****** Object:  View [dw].[v_koski_amm_opiskelijavuodet]    Script Date: 17.9.2019 16:15:59 ******/
SET ANSI_NULLS ON
GO
...
SELECT        
	...
	,[Hyväksytyt/hylätyt opiskelijavuodet] = d22.selite_fi --postettu, koska tuotannon dimensiotaulu on tyhjä
...
FROM  dw.f_koski_amm_opiskelijavuodet_painotetut f 
INNER JOIN dw.d_sukupuoli d1 ON d1.id= f.d_sukupuoli_id
INNER JOIN dw.d_kieli d2 ON d2.id = f.d_kieli_aidinkieli_id
INNER JOIN dw.d_ika d3 ON d3.id = f.d_ika_id
...
INNER JOIN dw.d_kytkin d6 ON d6.id = f.d_kytkin_oppisopimuskoulutus_id
INNER JOIN dw.d_kytkin d7 ON d7.id = f.d_kytkin_vankilaopetus_id
INNER JOIN dw.d_kytkin d8 ON d8.id = f.d_kytkin_henkilostokoulutus_id
INNER JOIN dw.d_kytkin d9 ON d9.id = f.d_kytkin_tyovoimakoulutus_id 
INNER JOIN dw.d_alueluokitus d10 ON d10.kunta_koodi = d5.kunta_koodi 
INNER JOIN dw.d_organisaatioluokitus d11 ON d11.id = f.d_organisaatioluokitus_jarj_id
INNER JOIN dw.d_alueluokitus d12 ON d12.kunta_koodi = d11.kunta_koodi
INNER JOIN dw.d_erityisopetus d13 ON d13.id = f.d_erityisopetus_id
INNER JOIN dw.d_kalenteri d14 ON d14.id = f.d_kalenteri_tilastokuukausi_id
INNER JOIN dw.d_majoitus d15 ON d15.id = f.d_majoitus_id
INNER JOIN dw.d_kytkin d16 ON d16.id = f.d_osa_aikaisuus_id
INNER JOIN dw.d_suorituksen_tyyppi d17 ON d17.id = f.d_suorituksen_tyyppi_id
INNER JOIN dw.d_ammatillisen_tutkinnon_kustannusryhma d18 ON d18.id = f.d_kustannusryhma_id
INNER JOIN dw.d_opintojenrahoitus d19 ON d19.id = f.d_opintojenrahoitus_id
INNER JOIN dw.d_osaamisala d20 ON d20.id = f.d_osaamisala_id 
INNER JOIN dw.d_organisaatioluokitus d21 ON d21.id = f.d_organisaatioluokitus_toimipiste_id
INNER JOIN dw.d_amos_spl_opiskelijavuodet_hylkaamisperuste d22 ON d22.id = f.d_amos_spl_opiskelijavuodet_hylkaamisperuste_id


WHERE (d14.vuosi < YEAR(getdate()) OR (d14.vuosi = YEAR(getdate()) AND d14.kuukausi < MONTH(getdate())))
