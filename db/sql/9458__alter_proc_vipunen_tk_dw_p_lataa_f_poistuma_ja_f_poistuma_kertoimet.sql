...
/****** Object:  StoredProcedure [dbo].[p_lataa_f_poistuma_ja_f_poistuma_kertoimet]    Script Date: 8.1.2024 12:04:56 ******/
SET ANSI_NULLS ON
GO
...
BEGIN 
	...
	SET @loppuVuosiB = @alkuVuosi + 20

	IF (SELECT COUNT(*) FROM VipunenTK.dbo.v_f_tab_toimialaennuste) > 0
	BEGIN 
	SET @loppuVuosiB = (SELECT MAX(Tilastovuosi) FROM VipunenTK.dbo.v_f_tab_toimialaennuste)
	END

	WHILE @alkuVuosiB <= @loppuVuosiB
	BEGIN
