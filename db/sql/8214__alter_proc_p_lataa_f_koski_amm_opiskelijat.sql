...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_amm_opiskelijat]    Script Date: 18.10.2023 8:11:31 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT DISTINCT
		 sa.vuosi
		 ,CASE 
			  WHEN sa.kuukausi = 9 AND DATEFROMPARTS(sa.vuosi, sa.kuukausi, 20) BETWEEN ooa.alku AND COALESCE(ooa.loppu, '9999-01-01') THEN 1
			  ELSE 0
		...
		,kotikunta_manner_suomessa = coalesce(d26.mannersuomessa, 0)
		,oppivelvollinen = 
			CASE
				WHEN d26.mannersuomessa = 1 AND --Kotikunta Manner-Suomessa
					...
					((sa.vuosi-YEAR(henk.syntymaaika) > 7 AND FLOOR(DATEDIFF(DAY, henk.syntymaaika, DATEFROMPARTS(sa.vuosi, sa.kuukausi, 1)) / 365.25) < 18) OR --Henkilö täyttää yli 7-vuotta, mutta on alle 18-vuotias
					(sa.vuosi-YEAR(henk.syntymaaika) = 7 AND DATEFROMPARTS(sa.vuosi, sa.kuukausi, 1) >= DATEFROMPARTS(sa.vuosi, 8, 1))) AND --Henkilö täyttää 7 vuotta ja opintovelvollisuus alkaa kyseisen vuoden elokuun alusta
					(spvm.tutkinto_suoritettu > DATEFROMPARTS(sa.vuosi, sa.kuukausi, 1) OR spvm.tutkinto_suoritettu IS NULL) --Henkilö ei ole suorittanut ylioppilastutkintoa tai ammatillista tutkintoa
					THEN 1 
				ELSE 0
			END
		--CSCVIPUNEN-3345 15.3.2023
	...
	LEFT JOIN Koski_SA.sa.sa_koski_opiskeluoikeus_aikajakso AS ooa ON sa.opiskeluoikeus_oid = ooa.opiskeluoikeus_oid AND ooa.tila in ('lasna','loma') --UUSI
	...
	LEFT JOIN Koski_SA.dbo.toisen_asteen_keskeyttaneet ero on sa.opiskeluoikeus_oid = ero.opiskeluoikeus_oid AND sa.kuukausi = ero.kuukausi AND sa.vuosi = ero.vuosi
	--CSCVIPUNEN-3345 15.3.2023
