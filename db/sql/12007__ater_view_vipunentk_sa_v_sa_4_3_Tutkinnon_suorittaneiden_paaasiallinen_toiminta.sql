...
/****** Object:  View [dbo].[v_sa_4_3_Tutkinnon_suorittaneiden_paaasiallinen_toiminta]    Script Date: 23.4.2026 15.21.42 ******/
SET ANSI_NULLS ON
GO
...
SELECT [tilv]
      ...
      ,[tutklaja_tr]
	  ,[rahlahde_v2] = '-1'
      ,[ptoim1r2] =     /* sopv_15 sis‰lt‰‰ virheellisesti p‰‰asiallisen toiminnan arvoja maasta muuttaneille, sovittu tyˆjaostossa ett‰ korjataan vain n‰kym‰‰n arvoon muu tai tuntematon - Lasse R. 18.10.2016 */
		CASE
...
SELECT [tilv]
      ...
      ,[akk]
      ,[koulk]
      ,[kouljat] = case when koulk like '30110%' and kouljat = '9' /*havaintoja IB, EB, RP tilv21*/ then '1' else kouljat end
      ,[oppis]
      ,[tutklaja_tr]
	  ,[rahlahde_v2] = '-1'
      ,[ptoim1r2]
	  ...
	  ,aikielir1
	  ,mosa = CAST(mosa as varchar(2))
      ,[tietolahde]='K4_3_sopv_23'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K4_3_sopv_23]
...
SELECT [tilv]
	  ...
	  ,aikielir1
	  ,mosa = CAST(mosa as varchar(2))
      ,[tietolahde]='K4_3_sopv_24'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K4_3_sopv_24]
...
SELECT [tilv]
	  ...
	  ,aikielir1
	  ,mosa = CAST(mosa as varchar(2))
      ,[tietolahde]='K4_3_sopv_25'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K4_3_sopv_25]
...
SELECT [tilv]
	  ...
	  ,aikielir1
	  ,mosa = CAST(mosa as varchar(2))
      ,[tietolahde]='K4_3_sopv_26'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K4_3_sopv_26]
