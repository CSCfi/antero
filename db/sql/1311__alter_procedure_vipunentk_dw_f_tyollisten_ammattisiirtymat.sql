USE [VipunenTK_DW]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_f_tyollisten_ammattisiirtymat_ja_aiemmat_ammatit]    Script Date: 7.3.2018 15:19:34 ******/
SET ANSI_NULLS ON
GO
...
SELECT f.[tilastovuosi]
      ...
      ,aidinkieli_versio1_id = coalesce(d2.id, -2)
      ,f.[ika_avain]
      ,ika_id = coalesce(d3.id, -2)
      ,f.[tyopaikan_sijaintimaakunta_avain]
      ,tyopaikan_sijaintimaakunta_id = coalesce(d4.id, -2)
      ,tyopaikan_sijaintimaakunta_historia_id = coalesce(d4h.id, -2)      
--      ,f.[amm2001_m1]
      ,f.[mitenna_ammattiryhma2_koodi]
      ,mitenna_ammatti_id = coalesce(d5.id, -2)
      ,f.[paaasiallinen_toiminta_siirtyman_paassa_koodi]
      ,paaasiallinen_toiminta_siirtyman_paassa_id = coalesce(d6.id, -2)
      ,f.[tyopaikan_sijainti_v_tilastovuonna_avain]
      ,tyopaikan_sijainti_v_tilastovuonna_id = coalesce(d7.id, -2)
--      ,f.[amm2001_m1_2009]
      ,f.[mitenna_ammattiryhma2_siirtyman_paassa_koodi]
      ,mitenna_ammatti_siirtyman_paassa_id = coalesce(d8.id, -2)
	  ...
	  ,opintoala_erikoistumiskoulutus_siirtyman_paassa_id = coalesce(d27.id,'-1')


      ,f.[lkm]
      ,f.[suhteellinen_ajankohta_avain]
      ,suhteellinen_ajankohta_id = coalesce(d9.id, -2)
      ,f.[perusjoukko_tyolliset_avain]
      ,perusjoukko_tyolliset_id = coalesce(d10.id, -2)
      ,f.[siirtyman_tarkastelusuunta_avain]
      ,siirtyman_tarkastelusuunta_id = coalesce(d11.id, -2)
	  ,lahde = 'R4.19/R4.20'
...
SELECT f.[tilastovuosi]
      ...
      ,sukupuoli_id = coalesce(d1.id, -2)
      ,'-1'
      ,aidinkieli_versio1_id = coalesce(d2a.id, -2)
      ,f.[ika_avain]
      ,ika_id = coalesce(d3.id, -2)
      ,'-1'
      ,tyopaikan_sijaintimaakunta_id = coalesce(d4.id, -2)
      ...
      ,mitenna_ammatti_id = coalesce(d5.id, -2)
      ,'-1'
      ,paaasiallinen_toiminta_siirtyman_paassa_id = coalesce(d6a.id, -2)
	  ,'-1'
      ,tyopaikan_sijainti_v_tilastovuonna_id = coalesce(d7.id, -2)
      ,f.[mitenna_ammattiryhma2_siirtyman_paassa_koodi]
      ,mitenna_ammatti_siirtyman_paassa_id = coalesce(d8.id, -2)
