...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amk_julkaisut_f_i_tieteenalat]    Script Date: 7.6.2023 12:18:33 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_amk_julkaisut_f_i_tieteenalat]
AS
...
SELECT * INTO #temp_julkaisut FROM (
	...
	FROM (
		SELECT  
			...
			CONCAT('YO3B_', ROW_NUMBER() OVER(ORDER BY Vuosi ASC)) as julkaisunTunnus
		FROM [ANTERO].[sa].[sa_suorat_amk3b_julkaisut] s1
		JOIN master.dbo.spt_values s2 ON s2.type = 'P' AND s2.number < s1.[Julkaisujen määrä]
	) s
	UNPIVOT  
	...
	LEFT JOIN ANTERO.dw.d_organisaatioluokitus d4 ON Ammattikorkeakoulu = d4.organisaatio_koodi
) t

TRUNCATE TABLE dw.[f_amk_julkaisut_f_i_tieteenalat];

INSERT INTO dw.[f_amk_julkaisut_f_i_tieteenalat] (
	 [tilastovuosi]
...
SELECT
	...
	t2.d_tieteenala_id,
	t.julkaisunTunnus,
	'etl: p_lataa_f_amk_julkaisut_f_i_tieteenalat'
