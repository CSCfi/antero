...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_jarjestamismuoto]    Script Date: 19.8.2024 12:37:58 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER  PROCEDURE [dw].[p_lataa_f_varda_jarjestamismuoto]
AS
...
BEGIN
	...
	SELECT
	  ...
	  WHERE s.alkamis_pvm <= cast(concat(@alkuVuosi, '-12-31') as date) and (coalesce(s.paattymis_pvm, '9999-12-31') >= cast(concat(@alkuVuosi, '-12-31') as date) or s.paattymis_pvm = '')
