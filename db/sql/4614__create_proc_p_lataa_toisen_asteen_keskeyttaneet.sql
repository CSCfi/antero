...
/****** Object:  StoredProcedure [dbo].[p_lataa_toisen_asteen_keskeyttaneet_test]    Script Date: 25.5.2021 22:44:23 ******/
SET ANSI_NULLS ON
GO
...
DROP PROCEDURE IF EXISTS [dbo].[p_lataa_toisen_asteen_keskeyttaneet]
GO

CREATE PROCEDURE [dbo].[p_lataa_toisen_asteen_keskeyttaneet] AS
...
,CASE
	...
	WHEN oo.koulutusmuoto = 'ammatillinenkoulutus' THEN 'Ammatillinen'
	ELSE 'Ei tietoa' END
AS oppimaara
...
WHERE (koulutusmuoto IN ('ammatillinenkoulutus', 'diatutkinto', 'ibtutkinto', 'lukiokoulutus', 'luva')
...
,jatkaa_tutkintotyyppi = case when q1.opiskeluoikeus_oid is null then 0 when erot.tutkintotyyppi_koodi = q1.tutkintotyyppi_koodi then 1 else 0 end
,jatkaa_ammatillinen = case when q1.opiskeluoikeus_oid is null then 0 when q1.koulutusmuoto = 'ammatillinenkoulutus' then 1 else 0 end
,jatkaa_yleissivistava = case when q1.opiskeluoikeus_oid is null then 0 else yleissivistava end
,jatkaa_valmentava = case when q1.opiskeluoikeus_oid is null then 0 
	when amm_valmentava = 1 or lukio_valmentava = 1 then 1 else 0 end
...
,keskeyttaminen_vaiheittainen_koodi =
	case when
		erot.koulutusmoduuli_koodiarvo = q1.koulutusmoduuli_koodiarvo then 1
...
LEFT JOIN (SELECT oo.oppija_oid 
			,oo.opiskeluoikeus_oid
			,ooa.alku
			,oo.koulutusmuoto
			...
			,kl.tutkintotyyppi_koodi
			,yleissivistava = case when  koulutusmuoto IN ('diatutkinto', 'ibtutkinto', 'lukiokoulutus', 'luva')
							OR ps.suorituksen_tyyppi = 'internationalschooldiplomavuosiluokka'
							OR ps.suorituksen_tyyppi = 'internationalschoolmypvuosiluokka' AND ps.koulutusmoduuli_koodiarvo IN ('10', '11', '12')
							then 1 else 0 end
			,CASE
				...
				ELSE 'Ei tietoa' END
			AS oppimaara
		    ,amm_valmentava = case when ps.suorituksen_tyyppi in ('valma', 'telma') then 1 else 0 end
			,lukio_valmentava = case when koulutusmuoto = 'luva' then 1 else 0 end
...
--			,kytkin_uusi = 1
				...
				AND ps.suorituksen_tyyppi != 'nayttotutkintoonvalmistavakoulutus'
				AND oo.sisaltyy_opiskeluoikeuteen_oid IS NULL
				AND ooa.tila IN ('lasna', 'loma', 'valmistunut')) q1
...
GROUP BY oppija_oid, opiskeluoikeus_oid, viimeinen_lasna, vuosi, kuukausi) q3
