...
GO
ALTER PROCEDURE [dw].[p_lataa_d_amk_historia]
AS
IF NOT EXISTS (SELECT * FROM dw.d_amk_historia WHERE id=-1)

	BEGIN

	  SET IDENTITY_INSERT dw.d_amk_historia ON;

	  INSERT INTO dw.d_amk_historia (
		id
	...
	SELECT
		...
		koodi,nimi,nimi_sv,nimi_en,
		'','9999-12-31',
		'ETL: p_lataa_d_amk_historia',
		loadtime = GETDATE(),
		username = SUSER_NAME()
	FROM sa.sa_koodistot
	WHERE koodisto='vipunenmeta' AND koodi='-1';

	SET IDENTITY_INSERT dw.d_amk_historia OFF;
...
--
MERGE dw.d_amk_historia AS TARGET2
USING (
SELECT
	koodi
