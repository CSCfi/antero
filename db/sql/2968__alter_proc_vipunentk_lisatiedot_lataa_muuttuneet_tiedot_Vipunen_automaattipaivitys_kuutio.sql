...
/****** Object:  StoredProcedure [dbo].[lataa_muuttuneet_tiedot_Vipunen_automaattipaivitys_kuutio]    Script Date: 3.4.2020 11:09:26 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	JOIN dbo.Vipunen_aineisto_kuutio ak on
		...
		REPLACE(REPLACE(CASE 
							WHEN LEN(LEFT(Taulu, CHARINDEX('_sopv', Taulu) - 1)) > 9 
							THEN LEFT(Taulu, LEN(LEFT(Taulu, CHARINDEX('_sopv', Taulu) - 6))) 
