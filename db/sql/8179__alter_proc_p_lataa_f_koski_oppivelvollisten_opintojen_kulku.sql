...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvollisten_opintojen_kulku]    Script Date: 9.10.2023 13:33:39 ******/
SET ANSI_NULLS ON
GO
...
BEGIN	
	...
	BEGIN
		...
		SELECT * INTO Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku_tilanne FROM (
			SELECT 
				f.oppija_oid,
		...
		FROM (
			...
			FROM (
				...
				LEFT JOIN ANTERO.dw.d_kytkin d20 on d20.kytkin_koodi = case when s2.oppivelvollisen_toiminta > 10 then 1 else 0 end
				LEFT JOIN ANTERO.dw.d_kytkin d21 on d21.kytkin_koodi = case when s2.oppivelvollisen_toiminta = 9 then 1 else 0 end
			) s 
		) s 
