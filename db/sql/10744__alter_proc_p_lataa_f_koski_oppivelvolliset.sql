...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvolliset]    Script Date: 15.4.2025 8.39.25 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT * INTO [Koski_SA].[sa].[temp_oppivelvolliset_esirajaukset] FROM (
	...
	LEFT JOIN Koski_SA.sa.perusopetuksen_paattotodistuksen_keskiarvot pk on pk.oppija_oid = h.master_oid and pk.rnk = 1
	...
	LEFT JOIN (
		...
		(ps.suorituksen_tyyppi = 'europeanschoolofhelsinkivuosiluokkasecondarylower' and ps.koulutusmoduuli_koodiarvo = 'S4'))) po on po.master_oid = h.oppija_oid AND po.vahvistus_paiva = po.vahvistus_paiva_min		
) f
...
LEFT JOIN Koski_SA.sa.perusopetuksen_paattotodistuksen_keskiarvot pk on pk.oppija_oid = ov.master_oid and pk.rnk = 1
-- Vain alku- ja loppupäivän välisenä aikana oppivelvolliset 
INNER JOIN Koski_SA.sa.sa_oppivelvolliset ovv on ovv.oppija_oid = ov.master_oid and YEAR(ovv.alkuPvm) <= YEAR(@loppuPvm) AND YEAR(ovv.loppuPvm) >= @alkuVuosi
...
BEGIN
	...
	SELECT * INTO Koski_SA.sa.temp_oppivelvollisten_toiminta FROM (
		...
		FROM (
			...
			INNER JOIN Koski_SA.sa.sa_oppivelvolliset ovp on ovp.oppija_oid = ov.master_oid and @alkuVuosi BETWEEN YEAR(ovp.alkuPvm) AND YEAR(ovp.loppuPvm) 
