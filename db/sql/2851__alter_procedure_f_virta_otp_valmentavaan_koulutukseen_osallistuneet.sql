USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_valmentavaan_koulutukseen_osallistuneet]    Script Date: 2.3.2020 14:05:22 ******/
SET ANSI_NULLS ON
GO
...
INSERT INTO [dw].[f_virta_otp_valmentavaan_koulutukseen_osallistuneet]
           ...
           (select atunniste,
            ...
            d5.id,
            d6.id,
            YEAR(convert(date,dateadd(s, convert(int, sa.tooalkamispaivamaara/1000) , convert(datetime, '1-1-1970')))  ), 							 							 
           YEAR(convert(date,dateadd(s, convert(int, sa.vooalkamispaivamaara/1000) , convert(datetime, '1-1-1970')))  ),
           YEAR(convert(date,dateadd(s, convert(int, sa.voopaattymispaivamaara/1000) , convert(datetime, '1-1-1970')))  ),
			sa.vootyyppi,
           d1.id,
           YEAR(convert(date,dateadd(s, convert(int, sa.ttutkinnonsuorituspvm/1000) , convert(datetime, '1-1-1970')))  ),
		   xvuosi,
		   ...
		   from sa.sa_virta_otp_valmentavaan_koulutukseen_osallistuneet sa
                ...
                (CASE 
	             WHEN (YEAR(convert(date,dateadd(s, convert(int, sa.ttutkinnonsuorituspvm/1000), convert(datetime, '1-1-1970')))) >= YEAR(convert(date,dateadd(s, convert(int, sa.vooalkamispaivamaara / 1000), convert(datetime, '1-1-1970'))))) THEN 1 
	             ELSE 0
                 END)
...
END)
                 LEFT JOIN ANTERO.dw.d_kalenteri d9 on d9.vuosi = (YEAR(convert(date,dateadd(s, convert(int, sa.ttutkinnonsuorituspvm/1000), convert(datetime, '1-1-1970'))))	)	)	
