...
/****** Object:  StoredProcedure [dbo].[p_lataa_amm_opiskelijat_valitaulu]    Script Date: 21.4.2026 10.34.56 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT --distinct
		...
		,erityisopetus = 
			case				
				when YEAR(q.op_alkamispaiva) < 2026 and (ooa.erityinen_tuki = 1 or ooa.hojks = 1) and ooa.vaativan_erityisen_tuen_erityinen_tehtava=0 and q.suorituksen_tyyppi not in ('telma','valma','tuvakoulutuksensuoritus') then 2 --Erityisopiskelija ilman erit. tuen tehtävää; tutk. joht 
				when YEAR(q.op_alkamispaiva) < 2026 and (ooa.erityinen_tuki = 1 or ooa.hojks = 1) and ooa.vaativan_erityisen_tuen_erityinen_tehtava=0 and q.suorituksen_tyyppi in ('telma','valma','tuvakoulutuksensuoritus') then 3 --Erityisopiskelija ilman erit. tehtävää; valmentava
				...
				when YEAR(q.op_alkamispaiva) < 2026 and ooa.erityinen_tuki = 0 then 1 --Ei erityisopiskelija
				else -1
			end
