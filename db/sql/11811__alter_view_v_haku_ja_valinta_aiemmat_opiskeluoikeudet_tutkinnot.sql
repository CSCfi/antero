...
/****** Object:  View [dw].[v_haku_ja_valinta_aiemmat_opiskeluoikeudet_tutkinnot]    Script Date: 25.3.2026 10.27.27 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,f.[hakemus_oid]
	,d90.selite_fi as 'Valintatapajonon tyyppi'
	...
	,[Kansalaisuus] = d28.maatjavaltiot2_fi
	,d28.maatjavaltiot2_fi
	,[Kansalaisuus (ryhmä)] = d28.maanosa6_fi
	...
	,[Kansainvälinen opiskelija] = d91.kytkin_fi
	...
	,[Vastaanottaa korkeakoulupaikan 3 vuoden aikana] = 
		case
			when f2.vastaanottaa_paikan_3v = 1 then 'Kyllä'
			else 'Ei'
		end

	,f2.vastaanottaa_paikan_3v 
	...
	,[Muun opiskeluoikeuden aloitusvuosi] = coalesce(cast(year(f2.muu_alkamispvm) as varchar(50)),'Ei aiempaa opiskeluoikeutta')
	...
	,[Muu opiskeluoikeus voimassa] =  case 
										when (f2.muu_paattymispvm > f.hakemus_luotu) or (f2.muu_alkamispvm is not null and f2.muu_paattymispvm is null) then 'On voimassa' 
										when (f2.muu_paattymispvm <= f.hakemus_luotu) then 'Ei voimassa' 
										else 'Ei aiempaa opiskeluoikeutta' 
									end
	,[Muu opiskeluoikeus voimassa (priorisoitu)] =  case 
										when (f2.muu_paattymispvm_prio > f.hakemus_luotu) or (f2.muu_alkamispvm is not null and f2.muu_paattymispvm_prio is null) then 'On voimassa' 
										when (f2.muu_paattymispvm_prio <= f.hakemus_luotu) then 'Ei voimassa' 
										else 'Ei aiempaa opiskeluoikeutta' 
									end
	,[Muun opiskeluoikeuden alkupvm] = f2.muu_alkamispvm
	,[Tuorein muun opiskeluoikeuden aloitusvuosi] = coalesce(cast(year(f2.tuorein_muu_oo_alkamispvm) as varchar(50)),'Ei aiempaa opiskeluoikeutta')
	...
	,[Aiemman tutkinnon suoritusvuosi] = coalesce(cast(YEAR(f2.aiempi_tutkinto_suorituspvm) as varchar(50)),'Ei aiempaa tutkintoa')
	,[Aiemman tutkinnon korkeakoulu] = coalesce(d52.organisaatio_fi,'Ei aiempaa tutkintoa')
	,[Aiemman tutkinnon suorituspvm] = f2.aiempi_tutkinto_suorituspvm
	,[Tuorein aiemman tutkinnon suoritusvuosi] = coalesce(cast(YEAR(f2.tuorein_aiempi_tutkinto_suorituspvm) as varchar(50)),'Ei aiempaa tutkintoa')
	...
	,[Hakenut ensikertaa korkeakouluun] = 
		case
			when f2.ensi_kertaa_hakeva = 0 then d56.kytkin_fi
			when f.koulutuksen_alkamisvuosi < 2018 then 'Tieto puuttuu'
			ELSE d56.kytkin_fi
		end
	
	,[aidosti_ensikertainen]
	,[Ensikertainen hakija] =
		case
			when aidosti_ensikertainen = 1 THEN 'Kyllä'
			else 'Ei'
		end
	,[Ensikertaisuuden lähde] =
		case 
			when f.ensikertalainen = 0 then 'ODW'
			else 'VIRTA-OTP'
		end
	,ensi_kertaa_hakeva as ensikertaa_hakeva
	,f.ensikertalainen as ensikertaisena_kohdeltava
	,[Ensikertaisena kohdeltava hakija] =	
		case 
			when f.ensikertalainen <> 1 then 'Ei' 
			else 'Kyllä' 
		end
	/* Mittarit */
	,f.valittu as Valitut2
	,f.vastaanottanut_paikan as 'Paikan vastaanottaneet2'
	,f.ilmoittautunut as Aloittaneet2
	...
	,[Kansalaisuus ryhmä järjestys] = d28.jarjestys_maanosa6_koodi
	...
	,[Toisen asteen koulutuksen suoritusvuosi järjestys] = case when f.toisen_asteen_koulutuksen_suoritusvuosi = 0 then '9999' else 9999-CAST(f.toisen_asteen_koulutuksen_suoritusvuosi AS int) end
	...
	,[Aiemman tutkinnon vertailu järjestys] = f2.aiemman_tutkinnon_vertailu
	,[Muun opiskeluoikeuden vertailu järjestys] = f2.muun_opintooikeuden_vertailu
...
LEFT JOIN dw.f_haku_ja_valinta_korkea_aste_hakijat_aiemmat_opiskeluoikeudet_ja_tutkinnot f2 on f.hakemus_oid = f2.hakemus_oid and f.d_hakukohde_id = f2.d_hakukohde_id
...
LEFT JOIN dw.d_organisaatioluokitus d51 on d51.id = f2.d_organisaatioluokitus_muu_korkeakoulu_id
LEFT JOIN dw.d_organisaatioluokitus d52 on d52.id = f2.d_organisaatioluokitus_aiempi_tutkinto_korkeakoulu_id
LEFT JOIN dw.d_koulutusluokitus d53 on d53.id = f2.d_koulutusluokitus_muu_tutkinto_id
LEFT JOIN dw.d_koulutusluokitus d54 on d54.id = f2.d_koulutusluokitus_aiempi_tutkinto_id
LEFT JOIN dw.d_kytkin d56 on d56.kytkin_koodi = f2.ensi_kertaa_hakeva
--taustatiedot
...
LEFT JOIN dw.d_kytkin d11 ON d11.id=f.d_kytkin_pohjakoulutus_ulkomaat_id
LEFT JOIN dw.d_ika d26 ON d26.id=f.d_ika_id
LEFT JOIN dw.d_maatjavaltiot2 d28 ON d28.id=f.d_maatjavaltiot2_kansalaisuus_id
--hakukohde
LEFT JOIN dw.d_hakukohde d10 ON d10.id = f.d_hakukohde_id
LEFT JOIN dw.d_organisaatioluokitus d16a ON d16a.id = f.d_organisaatioluokitus_koulutustoimija_id
...
LEFT JOIN dw.d_tutkinnon_taso_kk d18 ON d18.id = f.d_tutkinnon_taso_kk_id
LEFT JOIN dw.d_alueluokitus d19 ON d19.id = f.d_alueluokitus_hakukohde_id
LEFT JOIN dw.d_koulutusluokitus d20 ON d20.id = f.d_koulutusluokitus_id
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d23 ON d23.id = f.d_tutkinnon_aloitussykli_kk_id
LEFT JOIN dw.d_oppilaitoksenopetuskieli d27 ON d27.id=f.d_oppilaitoksenopetuskieli_koulutuksen_kieli_id
--hakutoiveet
--1
LEFT JOIN dw.d_hakukohde d32 ON d32.id=f.d_hakukohde_hakukohde1_id
LEFT JOIN dw.d_koulutusluokitus d33 ON d33.id=f.d_koulutusluokitus_hakukohde1_id
LEFT JOIN dw.d_organisaatioluokitus d33a ON d33a.id=f.d_organisaatioluokitus_hakukohde1_id 
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d33b ON d33b.id=f.d_tutkinnon_aloitussykli_kk_hakukohde1_id
--2
LEFT JOIN dw.d_hakukohde d34 ON d34.id=f.d_hakukohde_hakukohde2_id
LEFT JOIN dw.d_koulutusluokitus d35 ON d35.id=f.d_koulutusluokitus_hakukohde2_id
LEFT JOIN dw.d_organisaatioluokitus d35a ON d35a.id=f.d_organisaatioluokitus_hakukohde2_id  
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d35b ON d35b.id=f.d_tutkinnon_aloitussykli_kk_hakukohde2_id
--3
LEFT JOIN dw.d_hakukohde d36 ON d36.id=f.d_hakukohde_hakukohde3_id
LEFT JOIN dw.d_koulutusluokitus d37 ON d37.id=f.d_koulutusluokitus_hakukohde3_id
LEFT JOIN dw.d_organisaatioluokitus d37a ON d37a.id=f.d_organisaatioluokitus_hakukohde3_id 
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d37b ON d37b.id=f.d_tutkinnon_aloitussykli_kk_hakukohde3_id
--4
LEFT JOIN dw.d_hakukohde d38 ON d38.id=f.d_hakukohde_hakukohde4_id
LEFT JOIN dw.d_koulutusluokitus d39 ON d39.id=f.d_koulutusluokitus_hakukohde4_id
LEFT JOIN dw.d_organisaatioluokitus d39a ON d39a.id=f.d_organisaatioluokitus_hakukohde4_id 
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d39b ON d39b.id=f.d_tutkinnon_aloitussykli_kk_hakukohde4_id
--5
LEFT JOIN dw.d_hakukohde d40 ON d40.id=f.d_hakukohde_hakukohde5_id
LEFT JOIN dw.d_koulutusluokitus d41 ON d41.id=f.d_koulutusluokitus_hakukohde5_id
LEFT JOIN dw.d_organisaatioluokitus d41a ON d41a.id=f.d_organisaatioluokitus_hakukohde5_id 
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d41b ON d41b.id=f.d_tutkinnon_aloitussykli_kk_hakukohde5_id
--6
LEFT JOIN dw.d_hakukohde d42 ON d42.id=f.d_hakukohde_hakukohde6_id
LEFT JOIN dw.d_koulutusluokitus d43 ON d43.id=f.d_koulutusluokitus_hakukohde6_id
LEFT JOIN dw.d_organisaatioluokitus d43a ON d43a.id=f.d_organisaatioluokitus_hakukohde6_id  
LEFT JOIN dw.d_tutkinnon_aloitussykli_kk d43b ON d43b.id=f.d_tutkinnon_aloitussykli_kk_hakukohde6_id
LEFT JOIN dw.d_kytkin d70 on d70.kytkin_koodi = f2.muu_oo_koulutusala_taso_1_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d71 on d71.kytkin_koodi = f2.muu_oo_koulutusala_taso_2_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d72 on d72.kytkin_koodi = f2.muu_oo_koulutusala_taso_3_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d73 on d73.kytkin_koodi = f2.muu_oo_koulutusaste_taso_2_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d74 on d74.kytkin_koodi = f2.muu_oo_tavoitetutkinto_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d75 on d75.kytkin_koodi = f2.muu_oo_korkeakoulu_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d76 on d76.kytkin_koodi = f2.aiempi_tutk_koulutusala_taso_1_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d77 on d77.kytkin_koodi = f2.aiempi_tutk_koulutusala_taso_2_sama_kuin_hakukohde
LEFT JOIN dw.d_kytkin d78 on d78.kytkin_koodi = f2.aiempi_tutk_koulutusala_taso_3_sama_kuin_hakukohde
...
LEFT JOIN dw.d_kytkin d91 on f.kansainvalinen_opiskelija = d91.kytkin_koodi
