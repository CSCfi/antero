...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvollisten_opintojen_kulku]    Script Date: 19.12.2024 10.54.15 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT
		...
		CASE WHEN kesk.oppija_oid is null THEN 0 ELSE 1 END as keskeyttanyt_valpas
	...
	LEFT JOIN Koski_SA.sa.sa_oppivelvolliset_esi_ja_perusopetus ov on ov.oppija_oid = f.oppija_oid and f.tarkastelu_pvm between ov.alkuPvm and ov.loppuPvm
	LEFT JOIN Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku_kotikunta_mannersuomessa km on km.oppija_oid = f.oppija_oid and f.tarkastelu_pvm between km.muutto_pvm and km.poismuutto_pvm
	LEFT JOIN [Koski_SA].[sa].[sa_valpas_oppivelvollisuuden_keskeytys] kesk on kesk.oppija_oid = f.oppija_oid and kesk.peruttu = 0 and f.tarkastelu_pvm between kesk.alku and coalesce(kesk.loppu, '9999-01-01')
	...
	*/
	...
	FROM (
		...
		GROUP BY f.oppija_oid, tarkastelu_pvm
	) f
	...
	FROM (
		SELECT
			...
			s.tilanne,
			tilanne_prio = MIN(s.tilanne) OVER(PARTITION BY s.oppija_oid, s.aika_vuosina),
			s.aika_vuosina,
			...
			ranki_st = 
				RANK() OVER (
					PARTITION BY s.oppija_oid, s.opiskeluoikeus_oid, s.tilanne, s.aika_vuosina
					ORDER BY 
						CASE 
		...
		FROM (
			SELECT DISTINCT
				...
				CASE
					...
					-- Vapautettu
					WHEN v.oppija_oid is not null THEN 13
			...
			-- Tilanteet ilman priorisointia
			LEFT JOIN Koski_SA.sa.temp_oppivelvollisten_opintojen_kulku_tilanne s3 on s1.oppija_oid = s3.oppija_oid and s3.tarkastelu_pvm = tj.tarkastelu_pvm
