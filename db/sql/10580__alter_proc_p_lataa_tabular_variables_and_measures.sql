...
/****** Object:  StoredProcedure [dw].[p_lataa_tabular_variables_and_measures]    Script Date: 19.2.2025 11.16.12 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dw].[p_lataa_tabular_variables_and_measures] AS
...
SELECT DISTINCT * INTO #temp FROM (
	SELECT 
		CASE
			WHEN perspective like '%sv' THEN coalesce(concat('Kod ', d.selite_sv), [name])
			WHEN perspective like '%en' THEN coalesce(concat('Codes ', d.selite_en), [name])
