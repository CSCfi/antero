...
/****** Object:  StoredProcedure [dbo].[p_lataa_amm_tutkinnot_valitaulu]    Script Date: 17.10.2023 16:17:49 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT 
		...
		,hankintakoulutus = case when oo.sisaltyy_opiskeluoikeuteen_oid is not null and oo.koulutustoimija_oid != oo2.koulutustoimija_oid then 1 else 0 end
	...
	--läsnäolopäivät
	LEFT JOIN (select * from lasna_paivat) hyv_pv on hyv_pv.opiskeluoikeus_oid = coalesce(oo2.opiskeluoikeus_oid, oo.opiskeluoikeus_oid)
	--lomapäivät
	LEFT JOIN (select * from hylatyt_loma_paivat) hyl_pv on hyl_pv.opiskeluoikeus_oid = coalesce(oo2.opiskeluoikeus_oid, oo.opiskeluoikeus_oid)
	--opso- ja koso-päivät
	LEFT JOIN (select * from opso_koso_paivat) os_ks on os_ks.opiskeluoikeus_oid = coalesce(oo2.opiskeluoikeus_oid, oo.opiskeluoikeus_oid)
	...
	AND NOT EXISTS (
		select 1 from sa.sa_koski_opiskeluoikeus sa 
		where (koulutusmuoto = 'ammatillinenkoulutus' OR tuva_jarjestamislupa = 'ammatillinen')
