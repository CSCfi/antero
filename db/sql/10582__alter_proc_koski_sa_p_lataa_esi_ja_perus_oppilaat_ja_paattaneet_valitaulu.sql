...
/****** Object:  StoredProcedure [dbo].[p_lataa_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu]    Script Date: 19.2.2025 17.14.45 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT DISTINCT
		...
		,majoitusetu --jyräytyvä, tyhjätään oppilailta muut paitsi kuluva lukuvuosi
	...
	FROM (
		...
		OUTER APPLY (
			...
			select top 1
				ps.paatason_suoritus_id,
				ps.jaa_luokalle,
				vahvistus_paiva_muokattu = case when ps.vahvistus_paiva >= epo.paattymispaiva then dateadd(day, -1, epo.paattymispaiva) else ps.vahvistus_paiva end,
				suorituskieli = ps.suorituskieli_koodiarvo, 
