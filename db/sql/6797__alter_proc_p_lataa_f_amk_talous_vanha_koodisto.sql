...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amk_talous_vanha_koodisto]    Script Date: 21.12.2022 10:56:55 ******/
SET ANSI_NULLS ON
GO
...
INSERT INTO dw.f_amk_talous_vanha_koodisto
           (d_amk_id
		   ,d_ohjauksenala_id
		   ,d_tili_id
		   ,d_aineistotyyppi_id
...
SELECT d1.id as d_amk_id
  ...
  FROM (select tilik
    ...
    ,'-1' as erittely
    ,arvo
      ,'ETL: p_lataa_f_amk_talous_vanha_koodisto - SA_SUORAT_AMK_TALOUS_1_tuloslaskelma' AS src
from
(select tilik
		,amk_koodi
  ...
  (arvo for tili in
    (	liikevaihto
	  ...
	  --rajoittaa uusien talouskoodistojen luvut fakta-taulusta
	   WHERE aineistotyyppi_osa_1 = case when (select count(*) from sa.sa_suorat_amk_talous_1_tuloslaskelma where tilik < 2018 and tilik = unpvt.tilik and aineistotyyppi_osa_1 = 'L' and amk_koodi = unpvt.amk_koodi) > 0 then 'L' else 'E' end
	   AND TILIK < 2019
...
select tilik
    ...
    ,'-1' as erittely
    ,arvo
      ,'ETL: p_lataa_f_amk_talous_vanha_koodisto - SA_SUORAT_AMK_TALOUS_2_tase' AS src
from
(select amk_koodi
	  ,COALESCE(Emo_konserni, 'E') as konserni
      ,tilik
  ...
  ,tkjaama
    ...
    --rajoittaa uusien talouskoodistojen luvut fakta-taulusta
	   WHERE aineistotyyppi_osa_1 = case when (select count(*) from sa.sa_suorat_amk_talous_2_tase where tilik < 2018 and tilik = unpvt.tilik and aineistotyyppi_osa_1 = 'L' and amk_koodi = unpvt.amk_koodi) > 0 then 'L' else 'E' end
	   AND TILIK < 2019
union all
select	tilik
		,amk_koodi
		...
		,erittely
		,arvo
		 ,'ETL: p_lataa_f_amk_talous_vanha_koodisto - SA_SUORAT_AMK_TALOUS_3_tuloslaskelman_toiminnot' AS src
from
(
 select tilik
      ,amk_koodi
...
select tilik
	  ...
	  ,'-1' as toiminto
 	  ,'-1' as erittely
      ,arvo
      ,'ETL: p_lataa_f_amk_talous_vanha_koodisto - SA_SUORAT_AMK_TALOUS_4_tutkimusmenot' AS src
from
(select amk_koodi
    ,tilik
      ,opm95opa_koodi
  ...
  unpivot (arvo for tili in
    (tutmenot
      ...
      ,ulkomuur
      ,amkomarah)) as unpvt
	   --otetaan faktatauluun mukaan myös uuden talouskoodiston luvut
...
select tilik
      ...
      ,'-1' as erittely
      ,arvo
      ,'ETL: p_lataa_f_amk_talous_vanha_koodisto - SA_SUORAT_AMK_TALOUS_5_kustannustiedot' AS src
from
(select amk_koodi
		,tilik
		,ohjala
...
select tilik
      ...
      ,'-1' as erittely
      ,arvo
      ,'ETL: p_lataa_f_amk_talous_vanha_koodisto - SA_SUORAT_AMK_TALOUS_6_liiketoiminnan_tuloslaskelma' AS src
from
(select tilik
      ,amk_koodi
      ,aineistotyyppi
