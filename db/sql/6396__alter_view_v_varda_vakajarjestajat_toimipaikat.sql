...
/****** Object:  View [dw].[v_varda_vakajarjestajat_toimipaikat]    Script Date: 7.9.2022 9:17:55 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_varda_vakajarjestajat_toimipaikat] AS
		...
		LEFT JOIN (
			SELECT 
				...
				count(distinct henkilo_id) as lapsia
			FROM ANTERO.dw.f_varda_vakasuhteet_vakapaatokset
			GROUP BY tilastovuosi, vakasuhde_toimipaikka_id
		) c2 on tp.tilastovuosi = c2.tilastovuosi and tp.toimipaikka_id = c2.vakasuhde_toimipaikka_id
		--Järjestämismuodot
		LEFT JOIN (
			SELECT 
