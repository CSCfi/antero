...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_kvliikkuvuus]    Script Date: 21.2.2023 9:56:56 ******/
SET ANSI_NULLS ON
GO
...
/* muutosloki:
	...
	15.2.2023 VV: Lisätty d_kalenteri_alkupvm_id
	21.2.2023 VV: Lisätty lyhyen liikkuvuuden tauluihin join liikkuvuusohjelmaa varten sekä lisätty d_kytkin_blended_id
*/
...
SELECT
  ...
  (select d from dbo.BigintToDate(jaksoAlkupvm)) as jaksoAlkupvm,

  (select d from dbo.BigintToDate(jaksoLoppupvm)) as jaksoLoppupvm,

  case when isnumeric(f.kesto)=1 then cast(f.kesto as int) end as kesto,

  (select d from dbo.BigintToDate(luontipaivamaara)) as tilannepvm,
  YEAR((select d from dbo.BigintToDate(jaksoAlkupvm))) as tilastovuosi,
  xxaloitusvuosi,
  cast(1 as int) AS lukumaara,
  coalesce(d13.id, -1) as d_kalenteri_alkupvm_id,
  d14.id as d_kytkin_blended_id,
...
LEFT JOIN dw.d_kalenteri d13 on d13.kalenteri_avain = (select d from dbo.BigintToDate(f.jaksoAlkupvm))
LEFT JOIN dw.d_kytkin d14 on d14.kytkin_koodi = case when YEAR((select d from dbo.BigintToDate(jaksoAlkupvm))) < 2022 then -1 when f.liikkuvuudenluokittelu = 'b' then 1 else 0 end 
...
select
		   ...
		   ,-1 as d_kalenteri_alkupvm_id
		   ,-1 as d_kytkin_blended_id
