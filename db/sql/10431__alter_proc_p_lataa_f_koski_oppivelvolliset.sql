...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvolliset]    Script Date: 17.12.2024 8.19.19 ******/
SET ANSI_NULLS ON
GO
...
INNER JOIN Koski_SA.sa.sa_oppivelvolliset ovv on ovv.oppija_oid = ov.oppija_oid and YEAR(ovv.alkuPvm) <= YEAR(@loppuPvm) AND YEAR(ovv.loppuPvm) >= @alkuVuosi
		...
		SELECT DISTINCT 
			...
			oppivelvollisen_toiminta_min = 
				MIN(f.oppivelvollisen_toiminta) OVER (
					PARTITION BY f.oppija_oid, f.tilastopaiva)
