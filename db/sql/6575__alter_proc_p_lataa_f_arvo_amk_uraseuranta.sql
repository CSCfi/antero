...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amk_uraseuranta]    Script Date: 18.10.2022 11:22:53 ******/
SET ANSI_NULLS ON
GO
...
INSERT INTO dw.f_arvo_amk_uraseuranta (
	...
	SELECT DISTINCT
		...
		,'-1' as d_kalenteri_tunnus_luotu_id
		...
		,CAST(LEFT(vt.kuukausi,4) as int) - 5 as valmistumisvuosi
		,'ETL: p_lataa_f_arvo_amk_uraseuranta' as source
		,CAST(LEFT(vt.kuukausi,4) as int) as tilastovuosi
		...
		WHERE oppilaitoskoodi is not null and kk.tyyppi = 'amk-uraseuranta' and kk.kyselykerta_vuosi is not null and concat(d4.id, '_', d3.id) not in (SELECT DISTINCT concat(d_arvokyselykerta_id,'_',d_koulutusluokitus_id) FROM ANTERO.dw.f_arvo_amk_uraseuranta)
