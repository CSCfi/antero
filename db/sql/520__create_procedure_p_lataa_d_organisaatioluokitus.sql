if not exists (select * from sys.objects where object_id = object_id('dw.p_lataa_d_organisaatioluokitus') and type in ('p', 'pc'))
begin
...
if not exists (select * from dw.d_organisaatioluokitus where id=-1) begin
  ...
  latitude=s.koodi,longitude=s.koodi,
  source=s.source
...
USING (
  ...
  SELECT
    ...
    t.koodi as latitude,t.koodi as longitude,
    s.source
