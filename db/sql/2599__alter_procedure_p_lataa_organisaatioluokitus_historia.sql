...
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dw].[p_lataa_d_organisaatioluokitus_historia] AS'
END
GO
...
if not exists (select * from dw.d_organisaatioluokitus_historia where id=-1) begin
  ...
  insert into dw.d_organisaatioluokitus_historia (
    id,
    organisaatio_avain,
  ...
  and koodi='-1'
  ;
  set identity_insert dw.d_organisaatioluokitus_historia off;
end else begin
  update d
...
--
-- update new organisation names for old universities and universities of applied science
--
--
-- update new organisation names for old universities and universities of applied science
--
MERGE dw.d_organisaatioluokitus_historia AS TARGET2
USING (
SELECT
	koodi
	,(SELECT COALESCE(nimi, nimi_sv,nimi_en,'Tuntematon') FROM sa.sa_organisaatioluokitus WHERE koodi=oh.organisaation_uusi_koodi ) AS uusi_nimi
	,(SELECT COALESCE(nimi_sv, nimi,nimi_en,'Okänd') FROM sa.sa_organisaatioluokitus WHERE koodi=oh.organisaation_uusi_koodi ) AS uusi_nimi_sv
	,(SELECT COALESCE(nimi_en, nimi,nimi_sv,'Unknownn') FROM sa.sa_organisaatioluokitus WHERE koodi=oh.organisaation_uusi_koodi ) AS uusi_nimi_en
	,(SELECT koodi FROM sa.sa_organisaatioluokitus WHERE koodi=oh.organisaation_uusi_koodi ) AS nykyinen_tunnus
 ...
 FROM sa.sa_organisaatioluokitus  s
LEFT JOIN sa.sa_organisaatio_historia oh ON oh.organisaation_koodi=s.koodi
WHERE koodi =  oh.organisaation_koodi ) AS SRC2
...
WHEN MATCHED
	THEN
		...
		SET target2.organisaatio_fi = src2.uusi_nimi
			...
			,target2.loadtime=getdate()
			,target2.username=suser_name();
...
-- UPDATE OLD name for a current universities and universities of applied science which has changed  name
--
MERGE dw.d_organisaatioluokitus_historia AS TARGET3
USING (
SELECT
	 organisaation_koodi as koodi
	...
	,loppupvm
	,loadtime
 FROM sa.sa_organisaatio_historia oh
WHERE oh.organisaation_koodi=oh.organisaation_uusi_koodi ) AS SRC3
