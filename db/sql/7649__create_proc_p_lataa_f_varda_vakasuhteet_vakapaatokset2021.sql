...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_vakasuhteet_vakapaatokset2021]    Script Date: 3.5.2023 12:33:40 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE  [dw].[p_lataa_f_varda_vakasuhteet_vakapaatokset2021]
AS
...
BEGIN
	...
	FROM varda_sa.sa.sa_varda_henkilo2021 s
	INNER JOIN varda_sa.sa.sa_varda_lapsi2021 s2 on s2.henkilo_id = s.henkilo_id
	INNER JOIN varda_sa.sa.sa_varda_varhaiskasvatuspaatos2021 s3 on s3.lapsi_id = s2.lapsi_id
	INNER JOIN varda_sa.sa.sa_varda_varhaiskasvatussuhde2021 s4 on s4.varhaiskasvatuspaatos_id = s3.varhaiskasvatuspaatos_id
	INNER JOIN varda_sa.sa.sa_varda_toimipaikka2021 s5 on s5.toimipaikka_id = s4.toimipaikka_id
	INNER JOIN varda_sa.sa.sa_varda_vakajarjestaja2021 s6 on s5.vakajarjestaja_id = s6.vakajarjestaja_id
