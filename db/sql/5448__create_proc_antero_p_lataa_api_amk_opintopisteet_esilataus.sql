...
DROP PROCEDURE IF EXISTS [dw].[p_lataa_api_amk_opintopisteet_esilataus]
GO
...
CREATE PROCEDURE [dw].[p_lataa_api_amk_opintopisteet_esilataus] AS

TRUNCATE TABLE Antero.dw.[api_amk_opintopisteet];

INSERT INTO Antero.dw.[api_amk_opintopisteet]
...
FROM (
	SELECT
		...
		,[tietojen_ajankohta] = concat('5.3.', f.vuosi+1)
	...
	LEFT JOIN ANTERO.dw.d_amk d3 on d3.id=f.d_amk_id
	...
	SELECT 
		  ...
		  [koodit_ammattikorkeakoulu] = d3.amk_tunnus,
		  [aineistotyyppi] = 'Live',
		  [tietojen ajankohta] = right('0'+cast(day(dateadd(day, -1, sa.loadtime))-1 as nvarchar(2)), 2) + '.' + right('0'+cast(month(dateadd(day, -1, sa.loadtime)) as nvarchar(2)), 2) + '.' + cast(year(dateadd(day, -1, sa.loadtime)) as nvarchar(4))
