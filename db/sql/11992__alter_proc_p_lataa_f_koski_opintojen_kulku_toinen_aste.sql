...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_opintojen_kulku_toinen_aste]    Script Date: 21.4.2026 8.35.37 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dw].[p_lataa_f_koski_opintojen_kulku_toinen_aste] AS
...
BEGIN	
	...
	-- Ladataan temp-tauluun master_oid-kohtaisesti tiedot aikajaksoista, jolloin henkilöt ovat asuneet Manner-Suomessa
	...
	SELECT DISTINCT
		...
		-- CSCVIPUNEN-4899: ei tietoa erityisestä tuesta vuoden 2025 jälkeen alkavista koulutuksista
		CASE WHEN YEAR(f.alkamispaiva) >= 2026 and f.erityisopetus in (1,2,3) then -1 else f.erityisopetus end erityisopetus,
		f.ika,
		f.aidinkieli,
		f.kansalaisuus,
		f.sukupuoli
	...
	FROM (
		...
		SELECT DISTINCT 
			...
			oppivelvollinen_alkamispaivana = case when f.alkamispaiva < '2021-08-01' then -1 when ov.master_oid is null then 0 else 1 end,
			eo.erityisopetus,
			f.ika,
			f.aidinkieli,
			f.kansalaisuus,
		...
		LEFT JOIN Koski_SA.sa.perusopetuksen_paattotodistuksen_keskiarvot pk on pk.master_oid = f.master_oid and pk.rnk = 1
		LEFT JOIN Koski_SA.sa.sa_oppivelvolliset ov on ov.master_oid = f.master_oid and f.alkamispaiva between ov.alkuPvm and ov.loppuPvm
		OUTER APPLY (
			SELECT
				erityisopetus =
					MAX(
	...
	SELECT DISTINCT
		...
		aloitettu_koulutus = 
			CASE
				...
				WHEN (lps.opiskeluoikeus_oid is not null and lps.tavoitetutkinto <> 'Ei tutkintotavoitetta') or ooy.master_oid is not null THEN 2
	...
	*/
	...
	FROM (
		...
		FROM (
			...
			SELECT
				...
				*/
				CASE 
					WHEN ot.valmistunut = 1 and ot.aloitettu_koulutus = 4 and tuvat.master_oid is null
						THEN 1 
					ELSE 0
			...
			LEFT JOIN Koski_SA.sa.sa_oppivelvolliset ov on tj.master_oid = ov.master_oid and tj.tarkastelu_pvm between ov.alkuPvm and ov.loppuPvm
			LEFT JOIN Koski_SA.sa.temp_opintojen_kulku_toinen_aste_kotikunta_mannersuomessa km on km.master_oid = tj.master_oid and tj.tarkastelu_pvm between km.muutto_pvm and km.poismuutto_pvm
			LEFT JOIN Koski_SA.sa.tuva_lasna_loma_jaksot tuvat on tuvat.master_oid = tj.master_oid and ot.alku between tuvat.alku and tuvat.loppu
			WHERE 
			...
			NOT EXISTS (
				...
				AND tuvat.master_oid = tj.master_oid
			)
		) f
	...
	FROM (
		...
		SELECT
			...
			tilanne_prio = MIN(s.tilanne) OVER(PARTITION BY s.master_oid, s.aloitettu_koulutus, s.aika_vuosina),
			s.aika_vuosina,
		...
		FROM (
			SELECT DISTINCT
				...
				CASE
					...
					*/	
					...
					-- Valmistunut ammatillisen tutkinnon osan/osien koulutuksesta
					WHEN s2.aloitettu_koulutus = 4 and s2.valmistunut = 1 and tuvat.master_oid is null THEN 7
					-- Jatkaa ammatillisen tutkinnon osan/osien koulutuksessa
			...
			LEFT JOIN Koski_SA.sa.tuva_lasna_loma_jaksot tuvat on tuvat.master_oid = s1.master_oid and s2.alku between tuvat.alku and tuvat.loppu
			-- Ei huomioida koulutuksia, jos henkilö on samaan aikaan tuva-koulutuksessa.
			WHERE
			NOT EXISTS (
				SELECT 1 
