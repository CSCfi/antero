...
/****** Object:  View [dw].[v_haku_ja_valinta_toinen_aste]    Script Date: 25.2.2026 12.45.27 ******/
SET ANSI_NULLS ON
GO
...
--Englanti
	...
	/* Koodimuuttujat */
	 ...
	 --f.loadtime>=d1.hakijat vuosikellorajauksessa
	  ,[Hakijat3] = case when f.master_oid is not null then 1 else 0 end --case when f.loadtime>=d1.hakijat then 1 else 0 end
	  ...
	  ,[Pohjakoulutuksen päättövuosi järjestys] = case when f.pohjakoulutuksen_vuosi is null or f.pohjakoulutuksen_vuosi = 0 then 9999 else 2050-f.pohjakoulutuksen_vuosi end
...
LEFT JOIN sa.sa_haku_ja_valinta_hakijat_toinen_aste d32 ON d32.oid=f.master_oid and d32.vuosi=f.koulutuksen_alkamisvuosi
LEFT JOIN dw.d_maatjavaltiot2 d33 on d33.id=f.d_maatjavaltiot_kansalaisuus_id
WHERE f.koulutuksen_alkamisvuosi BETWEEN 2014 and 2021 and d25.korkeakouluhaku=0