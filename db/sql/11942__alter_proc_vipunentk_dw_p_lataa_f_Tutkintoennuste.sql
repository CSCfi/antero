...
/****** Object:  StoredProcedure [dbo].[p_lataa_f_Tutkintoennuste]    Script Date: 7.4.2026 7.56.15 ******/
SET ANSI_NULLS ON
GO
...
-- 1. Aloittaneet: 15-64-vuotiaat
-- Ammatillinen koulutus: vain ops-perusteiset ammatilliset perustutkinnot, ei työvoimakoulutusta
-- Ammattikorkeakoulutus: ei yamk
-- Yliopistokoulutuksessa: ylempi korkeakoulututkinto ja alempi korkeakoulututkinto

-- 2. Tutkinnon suorittaneiden työvoimaosuus : 25-49-vuotiaat
-- Ammatillinen koulutus: ammatti- ja erikoisammattitutkinnot + vain ops-perusteiset ammatilliset perustutkinnot
-- Ammattikorkeakoulutus: ei yamk
-- Yliopistokoulutuksessa: ylempi korkeakoulututkinto, lääkäreiden erikoistumiskoulutus, alemmat korkeakoulututkinnot farmaseuteilta ja varhaiskasvatuksen opettajilta
-- Työlliset opiskelijat huomioidaan lääketieteen, eläinlääketieteen ja suun terveydenhuollon tapauksessa (vain yliopistokoulutus)

-- 3. Alkuperäisen ja myöhemmän tutkinnon suorittaneet: 7 vuoden tarkastelu
-- Ammatillinen koulutus: vain ops-perusteiset ammatilliset perustutkinnot
-- Ammattikorkeakoulutus: ei yamk
-- Yliopistokoulutuksessa: ylempi korkeakoulututkinto, alemmat korkeakoulututkinnot farmaseuteilta ja varhaiskasvatuksen opettajilta

-- 4a. Aloittaneiden läpäisy: 
-- Ammatillinen koulutus: 4 vuoden tarkastelu, vain ops-perusteiset ammatilliset perustutkinnot
-- Ammattikorkeakoulutus: 7 vuoden tarkastelu, ei yamk
-- Yliopistokoulutuksessa: 9 vuoden tarkastelu, ylempi korkeakoulututkinto, alemmat korkeakoulututkinnot farmaseuteilta ja varhaiskasvatuksen opettajilta

-- 4b. Läpäisy (keskeyttäneet)
-- Ammatillinen koulutus
-- Ammattikorkeakoulutus: Ei yamk
-- Yliopistokoulutuksessa: Ylempi korkeakoulututkinto, alemmat korkeakoulututkinnot farmaseuteilta ja varhaiskasvatuksen opettajilta

-- 5. Tutkintotuotoksen viitearvot
-- Ammatillinen koulutus: 4 vuoden tarkastelu, vain ops-perusteiset ammatilliset perustutkinnot, ammattitutkinnot ja erikoisammattitutkinnot, ei työvoimakoulutusta
-- Ammattikorkeakoulutus: 7 vuoden tarkastelu
-- Yliopistokoulutuksessa: ylempi korkeakoulututkinto, lääkäreiden erikoistumiskoulutus, alemmat korkeakoulututkinnot farmaseuteilta ja varhaiskasvatuksen opettajilta, tohtorin ja lisensiaatin tutkinnot
	...
	DROP TABLE IF EXISTS #tutkintoennuste_h
	DROP TABLE IF EXISTS #tutkintoennuste_i
	...
	SELECT 
		...
		NULL as ika2,
		NULL as lapaisseet,
		NULL as lapaisseet_ja_keskeyttaneet,
		NULL as laskentalogiikka,
		d8.aidinkieli_versio1 as 'Äidinkieli'
	...
	LEFT JOIN [VipunenTK].[dbo].d_aidinkieli_versio1 d8 on d8.id = f.aidinkieli_versio1_id
	...
	AND ((d7.koulutustyyppi in ('Ammattikorkeakoulututkinto, päivätoteutus', 'Ammattikorkeakoulututkinto, monimuotototeutus') and d16.Koulutusaste_taso2_koodi = '62') or d16.Koulutusaste_taso2_koodi in ('72', '63'))
	AND f.aineisto = 'L'
	...
	GROUP BY f.tilv,d16.iscfi2013,d16.Koulutusaste_taso2,CASE WHEN d5.ika_int BETWEEN 15 and 34 THEN '15-34' ELSE '35-64' END,d16.iscfi2013_koodi,d4.aidinkieli_versio1
	...
	SELECT
		...
		,NULL as lapaisseet_ja_keskeyttaneet
		,NULL as laskentalogiikka
		,d3.aidinkieli_versio1 as 'Äidinkieli'
	...
	LEFT JOIN d_koulutusmuoto d2 on d2.id = f.koulutusmuoto_id
	LEFT JOIN d_aidinkieli_versio1 d3 on d3.id = f.aidinkieli_versio1_id
	WHERE aineisto <> 'R' and tilastointivuosi >= 2018 and d18.ika_int between 25 and 49 and ((Koulutusaste_taso2_koodi = 32 and d2.koulutusmuoto_koodi in ('1','3')) or Koulutusaste_taso2_koodi in (33,41,62,72,73) or (Koulutusaste_taso2_koodi = 63 and iscfi2013_koodi in ('0112', '0916')))
	...
	LEFT JOIN d_aidinkieli_versio1 d3 on d3.id = f.aidinkieli_versio1_id
	WHERE aineisto <> 'R' and tilastointivuosi >= 2018 and d18.ika_int between 25 and 49 and ((Koulutusaste_taso2_koodi = 32 and d2.koulutusmuoto_koodi in ('1','3')) or Koulutusaste_taso2_koodi in (33,41,62,72,73) or (Koulutusaste_taso2_koodi = 63 and iscfi2013_koodi in ('0112', '0916')))
	and (d14.paaasiallinen_toiminta in ('Päätoiminen työllinen', 'Työtön') or (d14.paaasiallinen_toiminta = 'Työllinen opiskelija' and d1.iscfi2013_koodi in ('0841','0911','0912') and Koulutusaste_taso2_koodi in (72,73)))
	...
	FROM (
		...
		LEFT JOIN VipunenTK.dbo.d_opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus d2 on d2.id = f.opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus_id
		LEFT JOIN d_aidinkieli_versio1 d4 on d4.id = f.aidinkieli_id
		WHERE d1.tarkastelujakso_koodi = '7' and suorv >= 2010 and ((d3.Koulutusaste_taso2_koodi = ('32') and d2.opetussuunnitelmaperusteinen_koulutus_nayttotutkintoon_valmistava_koulutus = 'opetussuunnitelmaperusteinen koulutus/tutkinto') or d3.Koulutusaste_taso2_koodi in ('62', '72') or (d3.Koulutusaste_taso2_koodi = '63' and d3.iscfi2013_koodi in ('0112','0916')))
		GROUP BY [suorv] ,d3.Koulutusaste_taso2 ,d3.iscfi2013,d3.iscfi2013_koodi, CASE 
				WHEN d3.Koulutusaste_taso2_koodi = '32' THEN 'Ammatillinen koulutus'
				WHEN d3.Koulutusaste_taso2_koodi = '62' THEN 'Ammattikorkeakoulukoulutus'
				ELSE 'Yliopistokoulutus'
			END, d4.aidinkieli_versio1
	) f
	UNPIVOT  
	...
	SELECT  
		...
		CASE
			WHEN f.aineistokoodi = 'K3.9' THEN CONCAT(CAST(CAST(LEFT(d1.lukuvuosi,4) as int) + 4 as varchar), '/', CAST(CAST(RIGHT(d1.lukuvuosi,4) as int) + 4 as varchar))
			WHEN d6.koulutuslaji2_koodi = '51b' or (COALESCE(NULLIF(d7.Koulutusaste_taso2_koodi,-1),d7b.Koulutusaste_taso2_koodi) = '63' and COALESCE(NULLIF(d7.iscfi2013_koodi,-1),d7b.iscfi2013_koodi) in ('0112','0916')) THEN CONCAT(CAST(CAST(LEFT(d1.lukuvuosi,4) as int) + 9 as varchar), '/', CAST(CAST(RIGHT(d1.lukuvuosi,4) as int) + 9 as varchar))
			ELSE CONCAT(CAST(CAST(LEFT(d1.lukuvuosi,4) as int) + 6 as varchar), '/', CAST(CAST(RIGHT(d1.lukuvuosi,4) as int) + 6 as varchar))
		END as Lukuvuosi,
		...
		CASE 
			...
			WHEN d6.koulutuslaji2_koodi = '51b' or (COALESCE(NULLIF(d7.Koulutusaste_taso2_koodi,-1),d7b.Koulutusaste_taso2_koodi) = '63' and COALESCE(NULLIF(d7.iscfi2013_koodi,-1),d7b.iscfi2013_koodi) in ('0112','0916')) THEN 'Yliopistokoulutus'
		...
		NULL as lapaisseet_ja_keskeyttaneet,
		CASE 
			WHEN f.aineistokoodi = 'K3.9' THEN 'Kohorttipohjainen (K3.9)'
			ELSE 'Kohorttipohjainen (K3.10)'
		END as laskentalogiikka,
		d11.aidinkieli_versio1 as 'Äidinkieli'
	...
	LEFT JOIN dbo.d_aidinkieli_versio1 d11 on d11.id = f.aikielir1_id
	...
	AND f.ophal <> '3' -- Työvoimakoulutus
	GROUP BY COALESCE(NULLIF(d7.Koulutusaste_taso2,'Tuntematon'),d7b.koulutusaste_taso2), COALESCE(NULLIF(d7.iscfi2013_koodi, -1), d7b.iscfi2013_koodi),
			COALESCE(NULLIF(d7.iscfi2013,'Tuntematon'),d7b.iscfi2013),
			CASE
			WHEN f.aineistokoodi = 'K3.9' THEN CONCAT(CAST(CAST(LEFT(d1.lukuvuosi,4) as int) + 4 as varchar), '/', CAST(CAST(RIGHT(d1.lukuvuosi,4) as int) + 4 as varchar))
			WHEN d6.koulutuslaji2_koodi = '51b' or (COALESCE(NULLIF(d7.Koulutusaste_taso2_koodi,-1),d7b.Koulutusaste_taso2_koodi) = '63' and COALESCE(NULLIF(d7.iscfi2013_koodi,-1),d7b.iscfi2013_koodi) in ('0112','0916')) THEN CONCAT(CAST(CAST(LEFT(d1.lukuvuosi,4) as int) + 9 as varchar), '/', CAST(CAST(RIGHT(d1.lukuvuosi,4) as int) + 9 as varchar))
			ELSE CONCAT(CAST(CAST(LEFT(d1.lukuvuosi,4) as int) + 6 as varchar), '/', CAST(CAST(RIGHT(d1.lukuvuosi,4) as int) + 6 as varchar))
			END, d6.koulutuslaji2, CASE 
			WHEN f.aineistokoodi = 'K3.9' THEN 'Ammatillinen koulutus'
			WHEN d6.koulutuslaji2_koodi = '51b' or (COALESCE(NULLIF(d7.Koulutusaste_taso2_koodi,-1),d7b.Koulutusaste_taso2_koodi) = '63' and COALESCE(NULLIF(d7.iscfi2013_koodi,-1),d7b.iscfi2013_koodi) in ('0112','0916')) THEN 'Yliopistokoulutus'
			ELSE 'Ammattikorkeakoulukoulutus'
			END,
	...
	SELECT
		...
		,NULL as lapaisseet_ja_keskeyttaneet
		,CASE
			WHEN [Ennusteen koulutusaste] = 'Ammatillinen koulutus' THEN 'Kohorttipohjainen (K3.9)' 
			ELSE 'Kohorttipohjainen (K3.10)'
		END as laskentalogiikka
		,[Äidinkieli]
	...
	FROM (
		...
		LEFT JOIN dbo.d_koulutusluokitus d7b ON d7b.koulutusluokitus_avain=coalesce(d7.korvaava_koulutus_koodi,'-1')
		LEFT JOIN dbo.d_lahde AS d10 ON d10.id = f.lahde_id 
		LEFT JOIN dbo.d_aidinkieli_versio1 as d11 on d11.id = f.aikielir1_id 
		WHERE (f.aineistokoodi = 'K3.9' AND d10.lahde_koodi in ('31','33','34','35','36') AND f.[1a Aloittaneiden opintojen kulku koulutuslajin mukaan prioriteettina mikä tahansa tutkinto] <> 'Virhetilanne'
		AND (d17.tarkastelujakso_koodi = '4' AND CAST(LEFT(d1.lukuvuosi,4) as int) >= 2012 AND d6.jarjestamistapa = 'opetussuunnitelmaperusteinen koulutus/tutkinto'
		AND coalesce(d7.Koulutusaste_taso2_koodi, d7b.Koulutusaste_taso2_koodi) = '32')) OR
		(f.aineistokoodi = 'K3.10' AND d10.lahde_koodi = '51' AND f.[1a Aloittaneiden opintojen kulku koulutuslajin mukaan prioriteettina mikä tahansa tutkinto] <> 'Virhetilanne'
	...
	-- Läpäisy (keskeyttäneet)

	SELECT
		NULL as Tilastovuosi,
		Lukuvuosi,
		f.iscfi2013 as 'Koulutusala, taso 3',
		f.iscfi2013_koodi as koulutusala_koodi,
		Koulutustyyppi,
		...
		NULL as suoritetut_tutkinnot,
		NULL as 'alkuperaisen_tutkinnon_aloittaneet',
		f.[Ennusteen koulutusaste],
		NULL as tutkinnot,
		NULL as ika2,
		f.lapaisseet,
		f.lapaisseet_ja_keskeyttaneet,
		'Odotusarvo (K3.11)' as laskentalogiikka,
		[Äidinkieli]
	INTO #tutkintoennuste_j
	FROM (
		SELECT
			[tilastovuosi] AS Lukuvuosi
			,SUM(
				CASE 
					WHEN d16.Koulutusaste_taso2_koodi in ('32','33','41') AND d3_39.opintojen_kulku_kala_t3n_mukaan_koodi = 1 THEN f.opiskelijat 
					WHEN d16.Koulutusaste_taso2_koodi in ('32','33','41') THEN 0 
					WHEN d3_75.opintojen_kulku_kala_t3n_mukaan_versio2_koodi = 1 THEN f.opiskelijat
					ELSE 0
				END
			) as lapaisseet
			,SUM(
				CASE 
					WHEN d16.Koulutusaste_taso2_koodi in ('32','33','41') AND d3_39.opintojen_kulku_kala_t3n_mukaan_koodi not in (2,3,4) THEN f.opiskelijat 
					WHEN d16.Koulutusaste_taso2_koodi in ('32','33','41') THEN 0 
					WHEN d3_75.opintojen_kulku_kala_t3n_mukaan_versio2_koodi not in (2,3,4) THEN f.opiskelijat
					ELSE 0
				END
			) as lapaisseet_ja_keskeyttaneet
			,d16.iscfi2013_koodi
			,d16.iscfi2013
			,d16.Koulutusaste_taso2
			,CASE 
			...
			,CASE 
				...
				ELSE 'Ammatillinen koulutus'
			END as 'Ennusteen koulutusaste',
			d1.aidinkieli_versio1 as 'Äidinkieli'
		FROM VipunenTK.dbo.f_keskeyttaneet f
		LEFT JOIN VipunenTK.dbo.d_oppilaitoksen_taustatiedot d15 ON d15.id= oppilaitoksen_taustatiedot_id
		LEFT JOIN VipunenTK.dbo.d_koulutusluokitus d16 ON d16.id = koulutusluokitus_id
		LEFT JOIN VipunenTK.dbo.d_hallinnonala2 d19b ON d19b.id=hallinnonala2_id
		LEFT JOIN VipunenTK.dbo.d_koulutustyyppi d20 ON d20.id= koulutustyyppi_id
		LEFT JOIN VipunenTK.dbo.d_opintojen_kulku_kala_t3n_mukaan d3_39 ON d3_39.id = [2a_opiskelijoiden_opintojen_kulku_kala_t3_mukaan_id]
		LEFT JOIN VipunenTK.dbo.d_opintojen_kulku_kala_t3n_mukaan_versio2 d3_75 ON d3_75.id = [2bb_opiskelijoiden_opintojen_kulku_kala_t3_mukaan_id]
		LEFT JOIN VipunenTK.dbo.d_aidinkieli_versio1 d1 on d1.id = f.aidinkieli_versio1x_id
		WHERE
		((d16.Koulutusaste_taso2_koodi in ('32','33','41') AND CAST(LEFT([tilastovuosi],4) as int) >= 2016) OR ((d16.Koulutusaste_taso2_koodi in ('62','72','73') or (d16.Koulutusaste_taso2_koodi = '63' and d16.iscfi2013_koodi in ('0916', '0112')))
		AND CAST(LEFT([tilastovuosi],4) as int) >= 2017))
		AND d15.oppilaitoskoodi not in ('02557','10031')
		AND d19b.hallinnonala_koodi <> 2 -- Työvoimakoulutus
		...
		,CASE 
			...
			WHEN d16.Koulutusaste_taso2_koodi in ('63','72','73') THEN 'Yliopistokoulutus'
			ELSE 'Ammatillinen koulutus'
		END, d1.aidinkieli_versio1
		) f
		WHERE f.lapaisseet_ja_keskeyttaneet <> 0

	IF (SELECT COUNT(*) FROM #tutkintoennuste_j WHERE [Ennusteen koulutusaste] = 'Ammatillinen koulutus') > 0
	BEGIN
	DELETE VipunenTK.dbo.f_Tutkintoennuste WHERE [Ennusteen koulutusaste] = 'Ammatillinen koulutus' and lapaisseet_ja_keskeyttaneet is not null
	INSERT INTO VipunenTK.dbo.f_Tutkintoennuste
	SELECT * FROM #tutkintoennuste_j WHERE [Ennusteen koulutusaste] = 'Ammatillinen koulutus'
	END

	IF (SELECT COUNT(*) FROM #tutkintoennuste_j WHERE [Ennusteen koulutusaste] = 'Ammattikorkeakoulukoulutus') > 0
	BEGIN
	DELETE VipunenTK.dbo.f_Tutkintoennuste WHERE [Ennusteen koulutusaste] = 'Ammattikorkeakoulukoulutus' and lapaisseet_ja_keskeyttaneet is not null
	INSERT INTO VipunenTK.dbo.f_Tutkintoennuste
	SELECT * FROM #tutkintoennuste_j WHERE [Ennusteen koulutusaste] = 'Ammattikorkeakoulukoulutus'
	END

	IF (SELECT COUNT(*) FROM #tutkintoennuste_j WHERE [Ennusteen koulutusaste] = 'Yliopistokoulutus') > 0
	BEGIN
	DELETE VipunenTK.dbo.f_Tutkintoennuste WHERE [Ennusteen koulutusaste] = 'Yliopistokoulutus' and lapaisseet_ja_keskeyttaneet is not null
	INSERT INTO VipunenTK.dbo.f_Tutkintoennuste
	SELECT * FROM #tutkintoennuste_j WHERE [Ennusteen koulutusaste] = 'Yliopistokoulutus'
	END
	...
	AND coalesce(d9.oppilaitoksen_korvaavakoodi,d9.oppilaitoskoodi) not in ('01029','01569','10102') 
	AND f.tilastointivuosi >= 2017 and d5.ika_int BETWEEN 15 and 64
	AND ((d7.koulutustyyppi in ('Ammattikorkeakoulututkinto, päivätoteutus', 'Ammattikorkeakoulututkinto, monimuotototeutus') and d16.Koulutusaste_taso2_koodi = '62') or (d16.Koulutusaste_taso2_koodi in ('72', '73') or (d16.Koulutusaste_taso2_koodi = '63' and d16.iscfi2013_koodi in ('0916', '0112'))))
	AND f.aineisto = 'L'
	...
	,CASE 
		WHEN d7.koulutustyyppi in ('Ammattikorkeakoulututkinto, päivätoteutus', 'Ammattikorkeakoulututkinto, monimuotototeutus') THEN 'Ammattikorkeakoulukoulutus'
		ELSE 'Yliopistokoulutus'
	END, d8.aidinkieli_versio1

	IF (SELECT COUNT(*) FROM #tutkintoennuste_h WHERE [Ennusteen koulutusaste] = 'Ammattikorkeakoulukoulutus') > 0
	BEGIN
	DELETE VipunenTK.dbo.f_Tutkintoennuste WHERE [Ennusteen koulutusaste] = 'Ammattikorkeakoulukoulutus' and tutkinnot is not null
	INSERT INTO VipunenTK.dbo.f_Tutkintoennuste
	SELECT * FROM #tutkintoennuste_h WHERE [Ennusteen koulutusaste] = 'Ammattikorkeakoulukoulutus'
	END

	IF (SELECT COUNT(*) FROM #tutkintoennuste_h WHERE [Ennusteen koulutusaste] = 'Yliopistokoulutus') > 0
	BEGIN
	DELETE VipunenTK.dbo.f_Tutkintoennuste WHERE [Ennusteen koulutusaste] = 'Yliopistokoulutus' and tutkinnot is not null
	INSERT INTO VipunenTK.dbo.f_Tutkintoennuste
	SELECT * FROM #tutkintoennuste_h WHERE [Ennusteen koulutusaste] = 'Yliopistokoulutus'
	END
	...
	LEFT JOIN VipunenTK.dbo.d_koulutuksen_tavoite_toteuma d27 on d27.id=f.koulutuksen_tavoite_toteuma_id
	LEFT JOIN VipunenTK.dbo.d_rahoituslahde_amm d3 on d3.id = f.rahoituslahde_amm_id
	LEFT JOIN VipunenTK.dbo.d_aidinkieli_versio1 d2 on d2.id = f.aidinkieli_versio1_id
	...
	AND d27.koulutuksen_tavoite_toteuma = 'Koko tutkinto' 
	AND d3.rahoituslahde_amm_koodi not in ('2', '3', '10', '13') -- Ei työvoimakoulutusta
	GROUP BY f.tilastointivuosi,d5.ika_int,d16.iscfi2013,d16.Koulutusaste_taso2,CASE WHEN d5.ika_int BETWEEN 15 and 34 THEN '15-34' ELSE '35-64' END,d16.iscfi2013_koodi,d2.aidinkieli_versio1
	
	IF (SELECT COUNT(*) FROM #tutkintoennuste_i WHERE [Ennusteen koulutusaste] = 'Ammatillinen koulutus') > 0
	BEGIN
	DELETE VipunenTK.dbo.f_Tutkintoennuste WHERE [Ennusteen koulutusaste] = 'Ammatillinen koulutus' and tutkinnot is not null
	INSERT INTO VipunenTK.dbo.f_Tutkintoennuste
	SELECT * FROM #tutkintoennuste_i WHERE [Ennusteen koulutusaste] = 'Ammatillinen koulutus'
	END
