...
/****** Object:  View [dw].[v_arvo_kandi]    Script Date: 22.4.2026 11.57.08 ******/
SET ANSI_NULLS ON
GO
...
, CASE 
	WHEN f.kyselyvuosi is null THEN 0
	WHEN f.vastaajaid is null THEN 0
	WHEN SUBSTRING(@@SERVERNAME,4,1) = 'T' THEN 1
	WHEN CAST(CONCAT(f.kyselyvuosi+1,'-02-01') AS date) <= CAST(GETDATE() AS date) THEN 1
	ELSE 0
...
LEFT JOIN dw.d_arvovastaus_monivalinta d10 ON d10.id = f.d_arvovastaus_taustakysymys_pohjakoulutus_id
LEFT JOIN dw.d_arvovastaus_monivalinta d11 ON d11.id = f.d_arvovastaus_taustakysymys_ika_id
...
WHERE f.kyselyvuosi is not null and d4.vastaustyyppi <> 'NPS'