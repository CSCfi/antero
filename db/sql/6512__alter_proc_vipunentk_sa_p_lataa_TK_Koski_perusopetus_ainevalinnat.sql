USE [VipunenTK_SA]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_TK_Koski_perusopetus_ainevalinnat]    Script Date: 4.10.2022 0:52:58 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[p_lataa_TK_Koski_perusopetus_ainevalinnat] --declare
...
DECLARE @pvmPaate2 nvarchar(9) = case when @tilv < 2022 then '_03102022' else @pvmPaate end --oman äidinkielen opinnot
...
select Lukuvuosi
	...
	,aidinkieli
	,kansalaisuus
	,osasuoritus_id = concat(f1.paatason_suoritus_id, ''_OA'', cast(oao.arviointipaiva as date))
