USE [Koski_SA]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_tutkinnot_ja_tutkinnonosat_valitaulu_duplikaatit]    Script Date: 13.4.2023 15:16:49 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE OR ALTER PROCEDURE [dbo].[p_lataa_tutkinnot_ja_tutkinnonosat_valitaulu_duplikaatit] AS
...
SELECT 
	...
	,rnk_paiva = row_number () over (
		partition by oppija_oid,oa.jarj_oid,tutkinto_koodi,vahvistus_paiva,tutkintonimike_koodiarvo,osaamisala_koodiarvo 
		order by vt.id 
	)
	,cnt_tutknimike = count(tn.tutkintonimike_koodiarvo) over (
...
OUTER APPLY (select avain = concat(oppija_oid,'|',oa.jarj_oid,'|',vuosi,'|',tutkinto_koodi)) oa2
WHERE paataso=1 and suorittaa_koko_tutkintoa=1
and oa2.avain in (
	select concat(oppija_oid,'|',coalesce(spl.oid_amos_spl,spl.oid_avain),'|',vuosi,'|',tutkinto_koodi)
	...
	group by oppija_oid,coalesce(spl.oid_amos_spl,spl.oid_avain),vuosi,tutkinto_koodi 
	having count(*)>1
)
...
INSERT INTO #cte (id,oppija_oid,opiskeluoikeus_oid,paatason_suoritus_id,vuosi,koulutustoimija_oid,vahvistus_paiva,tutkinto_koodi,osaamisala_koodiarvo,tutkintonimike_koodiarvo,rnk_paiva,cnt_tutknimike)
...
--Testiopiskelija
select * from #cte where oppija_oid='1.2.246.562.24.94592415178'
*/
...
SELECT distinct
	id
	,oppija_oid,opiskeluoikeus_oid,koulutustoimija_oid,tutkinto_koodi,vuosi,vahvistus_paiva,rnk_paiva
	,dupl =
		--Ei duplikaatti
...
/* 
--Tietty oppija
SELECT d.id,d.oppija_oid,d.opiskeluoikeus_oid,c.paatason_suoritus_id,d.vuosi,d.koulutustoimija_oid,d.vahvistus_paiva,d.tutkinto_koodi,c.osaamisala_koodiarvo,c.tutkintonimike_koodiarvo,d.dupl 
FROM #dupl d
JOIN #cte c on c.id = d.id and c.rnk_paiva=d.rnk_paiva
WHERE 1=1
and d.oppija_oid = '1.2.246.562.24.10480571181'
ORDER BY vuosi desc,oppija_oid,koulutustoimija_oid,vahvistus_paiva,tutkinto_koodi,d.rnk_paiva

--Sellaiset, joilla kaikki suoritukset menee duplikaateiksi
SELECT d.*,c.osaamisala_koodiarvo,c.tutkintonimike_koodiarvo FROM (
	SELECT *,min_dupl=min(dupl) over (partition by oppija_oid,vuosi,koulutustoimija_oid,tutkinto_koodi) FROM #dupl
) d
JOIN #cte c on c.id = d.id and c.rnk_paiva=d.rnk_paiva
WHERE min_dupl=1
ORDER BY oppija_oid
