...
/****** Object:  View [dw].[v_horizon]    Script Date: 8.5.2020 14:12:04 ******/
SET ANSI_NULLS ON
GO
...
SELECT
	...
	CASE 
		WHEN [name] like '%<%>%' and (shortName like '%<%>%' or shortName is null) THEN 'Unknown'
		WHEN ([name] like '%<%>%' or [name] is null) and shortName not like '%<%>%' and shortName is not null THEN shortName
		ELSE coalesce([name], 'Missing data') 
	END AS 'Organisation',

	CASE 
		WHEN [shortName] like '%<%>%' and ([name] like '%<%>%' or [name] is null) THEN 'Unknown'
		WHEN ([shortName] like '%<%>%' or shortName is null) and [name] not like '%<%>%' and [name] is not null THEN [name]
	...
	CASE 
		WHEN [name] like '%<%>%' and (shortName like '%<%>%' or shortName is null) THEN 'ÖÖÖ'
		WHEN ([name] like '%<%>%' or [name] is null) and shortName not like '%<%>%' and shortName is not null THEN shortName
		ELSE coalesce([name], 'ÖÖÄ') 
	END AS jarjestys_organisation,

	CASE 
		WHEN [shortName] like '%<%>%' and ([name] like '%<%>%' or [name] is null) THEN 'ÖÖÖ'
		WHEN ([shortName] like '%<%>%' or shortName is null) and [name] not like '%<%>%' and [name] is not null THEN [name]
