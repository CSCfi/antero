...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_henkilosto_maaraaikainen]    Script Date: 30.3.2023 13:59:11 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_varda_henkilosto_maaraaikainen]
AS
...
TRUNCATE TABLE dw.f_varda_henkilosto_maaraaikainen
...
BEGIN
	...
	INSERT INTO dw.f_varda_henkilosto_maaraaikainen (
		tilastovuosi
		,[tyontekija_id]
		,henkilo_id
	...
	SELECT
		...
		'p_lataa_f_varda_henkilosto_maaraaikainen' as source
	
	FROM varda_sa.sa.sa_varda_henkilo_palvelussuhde2022 s
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo_tyontekija2022] s2 on s.tyontekija_id = s2.id
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo2022] s3 on s2.henkilo_id = s3.id
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo_tyoskentelypaikka2022] s4 on s.id = s4.palvelussuhde_id
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo_taydennyskoulutustyontekija2022] s5 on s2.id = s5.tyontekija_id
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo_taydennyskoulutus2022] s6 on s5.taydennyskoulutus_id = s6.id and YEAR(s6.suoritus_pvm) = @alkuVuosi
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo_toimipaikka2022] s7 on s4.toimipaikka_id = s7.id
	LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo_organisaatio2022] s8 on s2.vakajarjestaja_id = s8.id
	
	LEFT JOIN dw.d_ika d on d.ika_avain = DATEDIFF(YEAR, s3.syntyma_pvm, CAST(CONCAT(@alkuVuosi, '-01-01') as date))
	...
	LEFT JOIN dw.f_varda_kielipainotus f on f.tilastovuosi = @alkuVuosi and f.toimipaikka_id = s7.id
	...
	LEFT JOIN dw.d_sukupuoli d11 on d11.sukupuoli_koodi = s3.sukupuoli_koodi
	...
	OUTER APPLY (
		SELECT d0.id FROM (
			...
			WHERE s02.alkamis_pvm <= cast(concat(@alkuVuosi, '-01-01') as date) and s02.paattymis_pvm >= cast(concat(@alkuVuosi, '-12-31') as date)
			GROUP BY s0.id) lkm
