...
/****** Object:  View [dw].[v_st_henkilosto]    Script Date: 14.9.2026 8.30.11 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	 [Tilastovuosi] = vuosi
	 ,[Tohtorintutkinnon vuosi] = CASE WHEN tohtorintutkinto_vuosi = -1 THEN 'Tieto puuttuu' WHEN suorittanut_tohtorintutkinnon is not null THEN coalesce(cast(tohtorintutkinto_vuosi as varchar), 'Ei tohtorintutkintoa') ELSE 'Ei tohtorintutkintoa' END
	 ,[Pedagogisten opintojen (60 op) vuosi] = CASE WHEN suorittanut_pedagogiset_opinnot is not null THEN coalesce(cast(pedagogisten_opintojen_vuosi as varchar), 'Ei pedagogisia opintoja (60 op)') ELSE 'Tieto puuttuu' END
	...
	,[Merkittävimmän tutkinnon taso (henkilöstötiedonkeruu)] = d8.selite_fi
	...
	,[Suoritettu tohtorintutkinto] = COALESCE(d21.koulutusluokitus_fi, 'Ei tohtorintutkintoa')
	,[Suoritetut pedagogiset opinnot (60 op)] = coalesce(d22.patevyys_fi, 'Ei pedagogisia opintoja (60 op)')
	,[Suorittanut pedagogiset opinnot (60 op)] = d19.kytkin_fi
	,[Suorittanut tohtorintutkinnon] = d20.kytkin_fi
	...
	,[Pedagogisten opintojen (60 op) korkeakoulu] = COALESCE(d17.organisaatio_fi, 'Ei pedagogisia opintoja (60 op)')
	,[Tohtorintutkinnon korkeakoulu] = COALESCE(d18.organisaatio_fi, 'Ei tohtorintutkintoa')
	,[Tohtorintutkinnon lähde] = COALESCE(REPLACE(tohtorintutkinnon_lahde,'-1','Tieto puuttuu'), 'Ei tohtorintutkintoa')
	--ruotsi
	,[Statistikår] = vuosi
	...
	,[Den mest betydande examens nivå (insamling av personaldata)] = d8.selite_sv
	...
	,[Level of the most significant degree (personnel data collection)] = d8.selite_en
	...
	--muut apusarakkeet
	,q.suorittanut_pedagogiset_opinnot
	,q.suorittanut_tohtorintutkinnon
	...
	,[Koodit Merkittävimmän tutkinnon taso (henkilöstötiedonkeruu)] = d8.koodi
	...
	,[jarj Tohtorintutkinnon vuosi] = CASE WHEN tohtorintutkinto_vuosi = -1 THEN 9999 WHEN suorittanut_tohtorintutkinnon is not null THEN coalesce(9999 - tohtorintutkinto_vuosi, 9998) ELSE 9998 END
	,[jarj Pedagogisten opintojen (60 op) vuosi] = CASE WHEN suorittanut_pedagogiset_opinnot is not null THEN coalesce(9999 - pedagogisten_opintojen_vuosi, 9998) ELSE 9999 END
	,[jarj Tohtorintutkinnon korkeakoulu] = CASE WHEN suorittanut_tohtorintutkinnon is not null THEN coalesce(d18.organisaatio_fi, 'ÖÖÄ') ELSE 'ÖÖÖ' END
	,[jarj Pedagogisten opintojen (60 op) korkeakoulu] = CASE WHEN suorittanut_pedagogiset_opinnot is not null THEN coalesce(d17.organisaatio_fi, 'ÖÖÄ') ELSE 'ÖÖÖ' END
	,[jarj Suoritettu tohtorintutkinto] = CASE WHEN suorittanut_tohtorintutkinnon is not null THEN coalesce(d21.koulutusluokitus_fi, 'ÖÖÄ') ELSE 'ÖÖÖ' END
	,[jarj Suoritetut pedagogiset opinnot (60 op)] = CASE WHEN suorittanut_pedagogiset_opinnot is not null THEN coalesce(d22.patevyys_fi, 'ÖÖÄ') ELSE 'ÖÖÖ' END

FROM (
	--amk
	SELECT
		vuosi
		,f.pedagogisten_opintojen_vuosi
		,f.tohtorintutkinto_vuosi
		,f.d_sukupuoli_id
		,f.d_ika_id
		...
		,f.d_ohjauksenala_id
		,f.d_tieteenala_id
		,f.suorittanut_pedagogiset_opinnot
		,f.suorittanut_tohtorintutkinnon
		,f.d_virta_patevyydet_pedagogiset_opinnot_id
		,f.d_koulutusluokitus_tohtorintutkinto_id
		,f.tohtorintutkinnon_lahde

		,d_amk_id AS d_organisaatioluokitus_id
		,f.d_organisaatioluokitus_pedagogiset_opinnot_id
		,f.d_organisaatioluokitus_tohtorintutkinto_id
	...
	SELECT
		...
		,d_yliopisto_id AS d_organisaatioluokitus_id
		,f.d_organisaatioluokitus_pedagogiset_opinnot_id
		,f.d_organisaatioluokitus_tohtorintutkinto_id
...
LEFT JOIN dw.d_organisaatioluokitus d7 on d7.id = q.d_organisaatioluokitus_id
...
LEFT JOIN dw.d_organisaatioluokitus d17 on d17.id = q.d_organisaatioluokitus_pedagogiset_opinnot_id
LEFT JOIN dw.d_organisaatioluokitus d18 on d18.id = q.d_organisaatioluokitus_tohtorintutkinto_id
LEFT JOIN dw.d_kytkin d19 on d19.kytkin_koodi = coalesce(q.suorittanut_pedagogiset_opinnot,-1)
LEFT JOIN dw.d_kytkin d20 on d20.kytkin_koodi = coalesce(q.suorittanut_tohtorintutkinnon,-1)
LEFT JOIN dw.d_koulutusluokitus d21 on d21.id = q.d_koulutusluokitus_tohtorintutkinto_id
LEFT JOIN dw.d_virta_patevyydet d22 on d22.id = q.d_virta_patevyydet_pedagogiset_opinnot_id