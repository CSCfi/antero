...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste_hakijat_aiemmat_opiskeluoikeudet_ja_tutkinnot]    Script Date: 9.3.2023 9:12:08 ******/
SET ANSI_NULLS ON
GO
...
) AS OP ON OP.henkilo_oid = f.henkilo_oid AND ((OP.opisk_alkamisPvm <= coalesce(d4.alkuPvm, d4b.alkuPvm) and OP.tavoitetutkinto <> d5.koulutusluokitus_koodi) or (YEAR(OP.opisk_alkamisPvm) < YEAR(coalesce(d4.alkuPvm, d4b.alkuPvm)) AND OP.tavoitetutkinto = d5.koulutusluokitus_koodi))
--tutkinnot
LEFT JOIN (
