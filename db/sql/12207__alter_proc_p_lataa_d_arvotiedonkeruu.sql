...
/****** Object:  StoredProcedure [dw].[p_lataa_d_arvotiedonkeruu]    Script Date: 21.9.2026 13.23.27 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	SELECT DISTINCT
		coalesce(o.kyselyid,k.kyselyid) as kyselyid,
		COALESCE(o.osio_nimi_fi,nimi_fi,nimi_sv,nimi_en) kysely_fi,
		COALESCE(o.osio_nimi_sv,nimi_sv,nimi_fi,nimi_en) kysely_sv,
		COALESCE(o.osio_nimi_en,nimi_en,nimi_fi,nimi_sv) kysely_en,
	...
	-- Digikyvykkyyden osiot omiksi kyselyikseen
	OUTER APPLY (
		SELECT DISTINCT 
			-- 999, jotta lähitulevaisuudessa ei tule duplikaatteja
			CONCAT(k2.kyselyid,999,k2.osio_id) as kyselyid,
			k2.osio_nimi_fi,
			k2.osio_nimi_en,
			k2.osio_nimi_sv
		FROM Arvo_SA.sa.sa_arvo_v2_kysymykset k2
		WHERE k2.osio_id is not null and k.kyselytyyppi = 'digikyvykkyys' and k2.kyselyid = k.kyselyid
	) o
...
-- AMK-uraseurannan visualisointia varten: CSCVIPUNEN-5132
INSERT INTO ANTERO.dbo.view_salt
([view])
SELECT DISTINCT CONCAT('v_arvo_amk_uraseuranta_palkka_ja_tyoura_', CAST(d1.kysymysid as varchar)) as 'view'
FROM ANTERO.dw.d_arvokysymys d1
LEFT JOIN ANTERO.dw.d_arvotiedonkeruu d2 on d2.kyselyid = d1.kyselyid
WHERE d2.kyselytyyppi = 'amk-uraseuranta' and NOT EXISTS (
	SELECT TOP 1 1 FROM ANTERO.dbo.view_salt vs
	WHERE vs.[view] = CONCAT('v_arvo_amk_uraseuranta_palkka_ja_tyoura_', CAST(d1.kysymysid as varchar))
)

INSERT INTO ANTERO.dbo.view_salt
([view])
SELECT DISTINCT CONCAT('v_arvo_amk_uraseuranta_sijoittuminen_tyoelamaan_', CAST(d1.kysymysid as varchar)) as 'view'
FROM ANTERO.dw.d_arvokysymys d1
LEFT JOIN ANTERO.dw.d_arvotiedonkeruu d2 on d2.kyselyid = d1.kyselyid
WHERE d2.kyselytyyppi = 'amk-uraseuranta' and NOT EXISTS (
	SELECT TOP 1 1 FROM ANTERO.dbo.view_salt vs
	WHERE vs.[view] = CONCAT('v_arvo_amk_uraseuranta_sijoittuminen_tyoelamaan_', CAST(d1.kysymysid as varchar))
)

INSERT INTO ANTERO.dbo.view_salt
([view])
SELECT DISTINCT CONCAT('v_arvo_amk_uraseuranta_yrittajyys_', CAST(d1.kysymysid as varchar)) as 'view'
FROM ANTERO.dw.d_arvokysymys d1
LEFT JOIN ANTERO.dw.d_arvotiedonkeruu d2 on d2.kyselyid = d1.kyselyid
WHERE d2.kyselytyyppi = 'amk-uraseuranta' and NOT EXISTS (
	SELECT TOP 1 1 FROM ANTERO.dbo.view_salt vs
	WHERE vs.[view] = CONCAT('v_arvo_amk_uraseuranta_yrittajyys_', CAST(d1.kysymysid as varchar))
)