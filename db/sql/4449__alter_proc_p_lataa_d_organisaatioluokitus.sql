USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_d_organisaatioluokitus]    Script Date: 23.2.2021 17:05:02 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	left join sa.sa_organisaatioluokitus s2 on s2.oid = s1.liitosoid and s1.liitosoid is not null and s2.alkupvm <= getdate()
	left join sa.sa_organisaatioluokitus s3 on s3.oid = s2.liitosoid and s2.liitosoid is not null and s3.alkupvm <= getdate()
	left join sa.sa_organisaatioluokitus s4 on s4.oid = s3.liitosoid and s3.liitosoid is not null and s4.alkupvm <= getdate()
	left join sa.sa_organisaatioluokitus s5 on s5.oid = s4.liitosoid and s4.liitosoid is not null and s5.alkupvm <= getdate()
	left join sa.sa_organisaatioluokitus s6 on s6.oid = s5.liitosoid and s5.liitosoid is not null and s6.alkupvm <= getdate()
	left join sa.sa_organisaatioluokitus s7 on s7.oid = s6.liitosoid and s6.liitosoid is not null and s7.alkupvm <= getdate()
	...
	inner join (select koodi,oid,ROW_NUMBER()OVER(PARTITION BY koodi ORDER BY koodi) AS nro from sa.sa_organisaatioluokitus group by koodi,oid) AS rnk on rnk.koodi=s1.koodi and rnk.oid = s1.oid
	where 1=1
	and t.koodisto='vipunenmeta' and t.koodi='-1'
	and s1.koodi in (
	select koodi from sa.sa_organisaatioluokitus
...
/*
WHEN NOT MATCHED BY source and id != '-1' THEN DELETE --too risky
*/
;

--to avoid misjoins
UPDATE dw.d_organisaatioluokitus
SET organisaatio_koodi = nullif(nullif(organisaatio_koodi,''),'-1')
,organisaatio_nykyinen_koodi = nullif(nullif(organisaatio_nykyinen_koodi,''),'-1')
,organisaatio_oid = nullif(nullif(organisaatio_oid,''),'-1')
WHERE id != '-1' 
