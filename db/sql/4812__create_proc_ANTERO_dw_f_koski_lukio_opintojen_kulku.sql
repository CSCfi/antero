...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opintojen_kulku]    Script Date: 30.8.2021 9:00:37 ******/
DROP PROCEDURE IF EXISTS [dw].[p_lataa_f_koski_lukio_opintojen_kulku]
GO
...
CREATE PROCEDURE [dw].[p_lataa_f_koski_lukio_opintojen_kulku] AS
...
--	d_osaamisala_id = COALESCE(d5.id, -1),
	...
	d_opintojen_kulku_vaihdokset_jarjestaja_id = COALESCE(d17.id, -1),
	d_opintojen_kulku_vaihdokset_oppilaitos_id = COALESCE(d18.id, -1),
...
LEFT JOIN (SELECT DISTINCT opiskeluoikeus_oid, 
						   oppimaara = CASE
							...
							WHERE suorituksen_tyyppi != 'luva' AND suorituksen_tyyppi != 'lukionaineenoppimaara' AND (suorituksen_tyyppi IN ('ibtutkinto', 'preiboppimaara', 
																	'lukionoppimaara', 'diatutkintovaihe', 'diavalmistavavaihe', 'internationalschooldiplomavuosiluokka'))
											OR
										(suorituksen_tyyppi = 'internationalschoolmypvuosiluokka' AND koulutusmoduuli_koodiarvo IN ('10', '11', '12'))) ps 
...
LEFT JOIN ANTERO.dw.d_lukio_oppimaara_man d15 ON d15.oppimaara_tunniste = ps.oppimaara
LEFT JOIN ANTERO.dw.d_alueluokitus d16 ON d11.kunta_koodi = d16.kunta_koodi
LEFT JOIN ANTERO.dw.d_opintojen_kulku_vaihdokset d17 ON q.vaihtanut_jarjestajaa = d17.vaihdos_koodi AND d17.vaihdos_tyyppi = 'jarjestaja'
LEFT JOIN ANTERO.dw.d_opintojen_kulku_vaihdokset d18 ON q.vaihtanut_oppilaitosta = d18.vaihdos_koodi AND d18.vaihdos_tyyppi = 'oppilaitos'



WHERE q.koulutus_kytkin = 'yleissivistava2aste' AND oppimaara IN ('nuortenops', 'aikuistenops', 'ulk_tutkinto')
