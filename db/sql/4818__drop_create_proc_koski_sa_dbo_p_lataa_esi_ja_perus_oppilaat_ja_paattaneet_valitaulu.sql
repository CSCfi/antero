...
/****** Object:  StoredProcedure [dbo].[p_lataa_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu]    Script Date: 31.8.2021 20:22:24 ******/
DROP PROCEDURE IF EXISTS [dbo].[p_lataa_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu]
GO
...
CREATE PROCEDURE [dbo].[p_lataa_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu]
AS
...
- Osa jyräytyvistä tiedoista raportoidaan oppilaille vain kuluvalta lukuvuodelta; rajaukset alustetaan faktalatauksessa ja toteutetaan näkymässä.
- Kotiopetus-, ulkomailla- sekä tehostetun tuen jaksot liitetään toistaiseksi 1/0-tietona oo:n aikajaksotauluun (1 jaksojen leikatessa, muuten 0); 
  eivät siis reunatapauksissa näy 0-ryhmässä kuten muut jaksotiedot. Näistä vain kotiopetus käytössä raportilla, mietitään kuitenkin optimaalinen ratkaisu tähän.
*/

DECLARE @alkuPvm as date, @loppuPvm as date, @tilastoPvm as date

SET @alkuPvm = '2019-08-01'
		...
		*/
		...
		,suorituskieli = 
			case 
				...
				when ps.suorituskieli_koodiarvo is not null then 'XX'
			end	
