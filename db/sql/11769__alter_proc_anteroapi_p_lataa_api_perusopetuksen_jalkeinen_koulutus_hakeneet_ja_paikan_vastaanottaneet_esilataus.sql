...
/****** Object:  StoredProcedure [dw].[p_lataa_api_perusopetuksen_jalkeinen_koulutus_hakeneet_ja_paikan_vastaanottaneet_esilataus]    Script Date: 16.2.2026 8.25.12 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	f.d_hakukohde_id,
	f.d_haku_id,
	f.d_oppilaitoksenopetuskieli_id,
	f.d_koulutusluokitus_hakukohde_id,
	f.d_alueluokitus_hakukohde_id,
	f.d_organisaatioluokitus_koulutuksen_jarjestaja_id,
	f.d_organisaatioluokitus_oppilaitos_id,
	f.d_organisaatioluokitus_toimipiste_id,
	MIN(f.aloituspaikat) as aloituspaikat_lkm,
	COUNT(DISTINCT master_oid) as kaikki_hakijat_lkm,
	COUNT(DISTINCT CASE WHEN hakutoive = 1 THEN master_oid ELSE NULL END) as ensisijaiset_hakijat_lkm,
	COUNT(DISTINCT CASE WHEN f.valittu = 1 THEN master_oid ELSE NULL END) as valitut_lkm,
	COUNT(DISTINCT CASE WHEN f.vastaanottanut_paikan = 1 THEN master_oid ELSE NULL END) as paikan_vastaanottaneet_lkm,
	COUNT(DISTINCT CASE WHEN f.ilmoittautunut = 1 THEN master_oid ELSE NULL END) as aloittaneet_lkm
...
GROUP BY f.koulutuksen_alkamisvuosi,f.d_kausi_koulutuksen_alkamiskausi_id,f.d_hakukohde_id,f.d_haku_id,f.d_oppilaitoksenopetuskieli_id,f.d_koulutusluokitus_hakukohde_id,f.d_alueluokitus_hakukohde_id,f.d_organisaatioluokitus_koulutuksen_jarjestaja_id,f.d_organisaatioluokitus_oppilaitos_id,f.d_organisaatioluokitus_toimipiste_id
...
SELECT 
	...
	d27.selite_fi as koulutuksen_kieli,
...
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d16a ON d16a.id = f.d_organisaatioluokitus_koulutuksen_jarjestaja_id
...
LEFT JOIN ANTERO.dw.d_oppilaitoksenopetuskieli d27 ON d27.id=f.d_oppilaitoksenopetuskieli_id
