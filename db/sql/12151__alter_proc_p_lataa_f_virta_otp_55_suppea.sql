...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_55_suppea]    Script Date: 25.8.2026 12.03.55 ******/
SET ANSI_NULLS ON
GO
...
SELECT
  ...
  coalesce(d3.id,-1) AS d_tk_koulutustyyppi_id,
  f.vuosi,
...
LEFT JOIN VipunenTK.dbo.d_koulutustyyppi d3 on d3.koulutustyyppi_koodi = f.koulutustyyppi

WHERE (f.suorittanut55ilmanPankkia = 1 or f.suorittanut55pankinAvulla = 1 or f.suorittanut27 = 1)
GROUP BY f.vuosi, d1.id, d2.id, f.source, d3.id