...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_ammatillisen_koulutuksen_arvosanat]    Script Date: 21.4.2026 8.39.13 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT
	...
	CASE				
		WHEN YEAR(oo.alkamispaiva) < 2026 and (ooa_ed.erityinen_tuki = 1 or ooa_ed.hojks = 1) and ooa_ed.vaativan_erityisen_tuen_erityinen_tehtava=0 and ps.suorituksen_tyyppi not in ('telma','valma','tuvakoulutuksensuoritus') THEN 2 --Erityisopiskelija ilman erit. tuen tehtävää; tutk. joht 
		WHEN YEAR(oo.alkamispaiva) < 2026 and(ooa_ed.erityinen_tuki = 1 or ooa_ed.hojks = 1) and ooa_ed.vaativan_erityisen_tuen_erityinen_tehtava=0 and ps.suorituksen_tyyppi in ('telma','valma','tuvakoulutuksensuoritus') THEN 3 --Erityisopiskelija ilman erit. tehtävää; valmentava
		...
		WHEN YEAR(oo.alkamispaiva) < 2026 and ooa_ed.erityinen_tuki = 0 THEN 1 --Ei erityisopiskelija
		ELSE -1
