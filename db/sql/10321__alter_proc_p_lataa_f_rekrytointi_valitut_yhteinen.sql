...
/****** Object:  StoredProcedure [dw].[p_lataa_f_rekrytointi_valitut_yhteinen]    Script Date: 28.10.2024 13:19:47 ******/
SET ANSI_NULLS ON
GO
...
DROP TABLE IF EXISTS ANTERO.sa.temp_rekytiedonkeruu

SELECT 
	s2.* 
INTO ANTERO.sa.temp_rekytiedonkeruu
FROM (
	SELECT 
		[Korkeakoulu], 
		[Hakunumero], 
		[Henkilönumero], 
		MIN([Tilastovuosi]) AS Tilastovuosi
	FROM [ANTERO].[sa].[sa_suorat_amk7d_rekrytointi_valitun_kaikki_tiedot]
	GROUP BY [Korkeakoulu], [Hakunumero], [Henkilönumero]
) s
...
INSERT INTO dw.f_rekrytointi_valitut_yhteinen
	(
      ...
      ,[d_nimike_id]
	  ,[yhdistynyt_henkilostotiedonkeruuseen]
      ,[tunnus_sektori]
	  ,[loadtime]
	  ,[source]
	  ,[username]
...
SELECT DISTINCT
	  ...
	  ,s.yhdistynyt_hlostokeruuseen as yhdistynyt_henkilostotiedonkeruuseen
...
FROM ANTERO.sa.temp_rekytiedonkeruu s
...
OUTER APPLY (
	SELECT 
		MAX(id) as id
	FROM ANTERO.[dw].[d_arvokysymys] ak
	WHERE ak.kysymys_fi = s.Kysymys OR ak.kysymys_sv = s.Kysymys
) d19
LEFT JOIN [dw].[d_arvovastaus] d20 ON d20.vastaus_fi = s.Vastaus OR d20.vastaus_sv = s.Vastaus
LEFT JOIN ANTERO.dw.d_ika d21 on d21.ika_avain = s.Ikä
OUTER APPLY (
	SELECT MAX([Haun kansainvälisyys]) as id 
	FROM ANTERO.sa.sa_suorat_amk7a_rekrytointihaut hka
	WHERE hka.tilastovuosi = s.tilastovuosi and  hka.Korkeakoulu = s.Korkeakoulu and hka.Hakunumero = s.Hakunumero
) d22a
OUTER APPLY (
	SELECT MAX([Haun kansainvälisyys]) as id 
	FROM ANTERO.sa.sa_suorat_amk7a_rekrytointihaut hkb
	WHERE hkb.tilastovuosi + 1 = s.tilastovuosi and  hkb.Korkeakoulu = s.Korkeakoulu and hkb.Hakunumero = s.Hakunumero
) d22b
...
LEFT JOIN ANTERO.dw.d_organisaation_alayksikot d12 ON d12.[alayksikko_koodi]  = s.[Laitos tai vastaava] AND d12.korkeakoulu_koodi = s.Korkeakoulu and s.Tilastovuosi = d12.vuosi
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d13 on d13.organisaatio_koodi = s.[Tohtorintutkinnon yliopisto]
-- CSCVIPUNEN-3461
LEFT JOIN ANTERO.dw.d_maatjavaltiot2 d14 on d14.maatjavaltiot2_koodi = CASE WHEN s.yhdistynyt_hlostokeruuseen = 1 THEN COALESCE(s.[Tohtorintutkinnon suoritusmaa], '998') ELSE s.[Tohtorintutkinnon suoritusmaa] END
OUTER APPLY (
	SELECT 
		MAX(id) as id
	FROM ANTERO.[dw].[d_arvokysymys] ak
...
LEFT JOIN ANTERO.dw.d_ika d21 on d21.ika_avain = s.Ikä
OUTER APPLY (
	SELECT MAX([Haun kansainvälisyys]) as id1, MAX(Yhteisprofessuuri) as id2, MAX(Kutsuhaku) as id3
	FROM ANTERO.sa.sa_suorat_yo8a_rekrytointihaut hka
	WHERE hka.tilastovuosi = s.tilastovuosi and  hka.Korkeakoulu = s.Korkeakoulu and hka.Hakunumero = s.Hakunumero
) d22a
OUTER APPLY (
	SELECT MAX([Haun kansainvälisyys]) as id1, MAX(Yhteisprofessuuri) as id2, MAX(Kutsuhaku) as id3
	FROM ANTERO.sa.sa_suorat_yo8a_rekrytointihaut hkb
	WHERE hkb.tilastovuosi + 1 = s.tilastovuosi and  hkb.Korkeakoulu = s.Korkeakoulu and hkb.Hakunumero = s.Hakunumero
) d22b
