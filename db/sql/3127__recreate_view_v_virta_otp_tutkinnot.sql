-- Virta OTP kaikki tutkinnot ilman rajauksia näkymä raporttia varten
USE ANTERO
GO 
IF NOT EXISTS ( SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'dw.v_virta_otp_tutkinnot') )
	EXEC dbo.sp_executesql @statement = N'CREATE VIEW dw.v_virta_otp_tutkinnot AS select 1 AS a'
GO
...
AS
	SELECT   		
			...
			, MONTH(dateadd(S, f.[tutkinnonsuoritusvuosi]/1000, '1970-01-01 03:00:00'))  Suorituskuukausi 
