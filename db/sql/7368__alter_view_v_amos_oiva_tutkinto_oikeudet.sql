...
/****** Object:  View [sa].[v_amos_oiva_tutkinto_oikeudet]    Script Date: 15.2.2023 19:46:54 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	FROM (
		...
		--tuvat mukaan. JS 10.2.2023
		SELECT 
			jarjestajaYtunnus
			,alkupvm_tod = alkupvm
			,alkupvm
			,loppupvm = coalesce(loppupvm, '9999-12-31')	
			,tutkintokoodi = '999908'
			,generoitu = 0
		FROM [sa].[sa_oiva_luvat] sa
		WHERE koulutustyyppi = '5' and koulutusmuodonPaasivunOtsikko = 'Ammatillinen koulutus'

		UNION

		--tuvat myös fuusioituneelle ennen voimaantuloa. JS 15.2.2023
		SELECT 
			sa1.jarjestajaYtunnus
			,alkupvm_tod = sa2.alkupvm
			,alkupvm = cast(dateadd(yy, datediff(yy, 0, sa2.loppupvm)-1, 0) as date)
			,loppupvm = sa2.loppupvm	
			,tutkintokoodi = '999908'
			,generoitu = 0
		FROM [sa].[sa_oiva_luvat] sa1
		left join dw.d_amos_spl_jarjestaja_linkki a on a.ytunnus_amos_spl = sa1.jarjestajaYtunnus
		left join sa.sa_oiva_luvat sa2 on sa2.jarjestajaYtunnus = a.ytunnus_avain and sa2.alkupvm < sa1.alkupvm and datediff(day, sa2.loppupvm, sa1.alkupvm) = 1
		WHERE 1=1
		and a.ytunnus_avain != a.ytunnus_amos_spl
		and datediff(day,dateadd(day, -1, a.voimaan_alkaen), sa1.alkupvm) = 1
		and sa1.koulutustyyppi = '5' and sa1.koulutusmuodonPaasivunOtsikko = 'Ammatillinen koulutus'
		and sa2.koulutustyyppi = '5' and sa2.koulutusmuodonPaasivunOtsikko = 'Ammatillinen koulutus'
