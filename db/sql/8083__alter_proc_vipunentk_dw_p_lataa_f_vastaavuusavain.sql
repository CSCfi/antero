...
/****** Object:  StoredProcedure [dbo].[p_lataa_f_vastaavuusavain]    Script Date: 21.9.2023 13:27:17 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	SELECT * INTO VipunenTK.dbo.temp_koulutukset FROM (
		...
		WHERE opetushallinnon_koulutus in (-1,1) AND tilv = @alkuVuosi and f.aloittaneet is not null
		...
		left join VipunenTK.dbo.d_koulutusluokitus d4 on d4.id=f.koulutusluokitus_id
		where opetushallinnon_koulutus_id <> 2 and tutkinnon_toteuma_id <> 47 and tilastointivuosi = @alkuVuosi and f.lukumaara is not null
		...
		where f.aineisto <> 'E' and d8.ensisijainen_opintooikeus_koodi <>  2 and f.koulutusluokitus not in ('-1', '999999') and tilastointivuosi = @alkuVuosi and f.aloittaneet is not null
		...
		left join VipunenTK.dbo.d_koulutusluokitus d4 on d4.id=f.koulutusluokitus_id
		where f.aineisto <> 'E' and f.koulutusluokitus not in ('-1', '999999') and tilastointivuosi = @alkuVuosi and f.lukumaara is not null
	) f
