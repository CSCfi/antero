...
DROP PROCEDURE IF EXISTS [dw].[p_lataa_api_henkilosto_esilataus]
GO
...
CREATE PROCEDURE [dw].[p_lataa_api_henkilosto_esilataus] AS
...
SELECT 
	  ...
	  ,f.[Henkilöstöryhmä (harjoittelukoulut)] as [henkilostoryhma(harjoittelukoulut)]
...
FROM (
	...
	LEFT JOIN ANTERO.dw.d_amk d7 on d7.id=f.d_amk_id
	...
	LEFT JOIN ANTERO.dw.d_ohjauksenala d5 on d5.id=f.d_ohjauksenala_id
	LEFT JOIN ANTERO.dw.d_tieteenala d6 on d6.id=f.d_tieteenala_id
	LEFT JOIN ANTERO.dw.d_yo d7 on d7.id=f.d_yliopisto_id
