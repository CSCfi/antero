...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_move]    Script Date: 16.3.2026 11.47.41 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT
	CAST(RIGHT(v.tiedonkeruu,4) as int) as tilastovuosi,
	v.vastaajaid,
	k.oppilaitoskoodi
INTO ANTERO.dbo.temp_move
FROM Arvo_SA.sa.sa_arvo_v2_vastaukset_muut v
LEFT JOIN Arvo_SA.[sa].[sa_arvo_v2_kyselyt] kk on kk.kyselyid = v.kyselyid
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_vastaajatunnukset k on v.vastaajatunnusid = k.vastaajatunnusid
WHERE kk.kyselytyyppi = 'move' and YEAR(v.vastausaika) >= 2024
...
SELECT DISTINCT
	...
	kysymys_fi,
	CASE
		WHEN m.monivalintavaihtoehto_fi = 'Onnistui' THEN 1
		WHEN m.monivalintavaihtoehto_fi = 'Ei onnistunut' THEN 0
		WHEN m.monivalintavaihtoehto_fi = 'Ei suorittanut' THEN NULL
		WHEN m.kysymysid is not null THEN m.arvo
		ELSE coalesce(cast(luku as decimal(10,2)), cast(replace(replace(vaihtoehto, 'kylla', '1'), 'ei', '0') as int))
...
FROM Arvo_SA.sa.sa_arvo_v2_vastaukset_muut v
LEFT JOIN  Arvo_SA.[sa].[sa_arvo_v2_kyselyt] kk on kk.kyselyid = v.kyselyid
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_kysymykset k on k.kysymysid = v.kysymysid and k.kysymysversio = 1 --v.kysymysversio
LEFT JOIN Arvo_SA.sa.sa_arvo_v2_monivalintavaihtoehdot m on m.kysymysid = v.kysymysid and v.kysymysversio = m.kysymysversio and m.arvo = v.numerovalinta
WHERE kk.kyselytyyppi = 'move' and YEAR(v.vastausaika) >= 2024
