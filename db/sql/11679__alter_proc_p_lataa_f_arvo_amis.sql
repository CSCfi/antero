...
SELECT 
	...
	,toimipiste_oid = 
		case 
			when (f6.[taustatiedot.heratepvm] > '2024-06-30' and f6.vastaajatunnusid != 1224658)
				or f6.vastaajatunnus_alkupvm between '2021-07-01' and '2024-06-30' 
			then f8.toimipiste_oid
		end 
	,f2.tutkintokoodi
	...
	,tunnus_pvm = 
		case 
			when f6.[taustatiedot.heratepvm] > '2024-06-30' and f6.vastaajatunnusid != 1224658 then f6.[taustatiedot.heratepvm] 
			when f6.vastaajatunnus_alkupvm <= '2024-06-30' then f6.vastaajatunnus_alkupvm 
		end 
	,f.vastausaika
...
) AS tk_ak ON tk_ak.vastaajaid = f.vastaajaid AND tk_ak.kyselyid = f.kyselyid
...
) AS tk_hv ON tk_hv.vastaajaid = f.vastaajaid AND tk_hv.kyselyid = f.kyselyid
...
) AS tk_arvio ON tk_arvio.vastaajaid = f.vastaajaid AND tk_arvio.kyselyid = f.kyselyid
...
LEFT JOIN dw.d_arvokyselykerta d7 ON d7.kyselyid = MQ.kyselyid
