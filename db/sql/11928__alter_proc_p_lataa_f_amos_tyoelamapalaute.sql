...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amos_tyoelamapalaute]    Script Date: 7.4.2026 14.52.43 ******/
SET ANSI_NULLS ON
GO
...
BEGIN 
	...
	SELECT 
		...
		,ytunnus
		,oppilaitos_koodi = d1.organisaatio_koodi
		,toimipiste_oid = d1b.organisaatio_oid
	...
	FROM (
		...
		FROM (
			...
			CROSS APPLY (
				select 
					generoitava_rahoituskausi = concat('01.07.', year(getdate()) - 1, ' - ', '30.06.', year(getdate())) /*
						case 
	...
	LEFT JOIN dw.d_organisaatioluokitus d1b ON d1b.id = q2.d_organisaatio_toimipiste_id
