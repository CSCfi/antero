...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_digikyvykkyys]    Script Date: 27.5.2026 7.38.27 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,f.vastaajaid
	,CASE 
		WHEN YEAR(f.vastausaika) < 2024 THEN 'ammatillinen' 
		WHEN f6.taustatiedot like '%ammatillinen%' THEN 'ammatillinen'
		ELSE 'lukio' END as sektori
...
SELECT 
	...
	,coalesce(d6.id,-1) AS d_arvokysymys_id
	,coalesce(d8.id,-1) AS d_arvokysymys_2_id
	-- Osiot omiksi kyselyikseen. d_arvotiedonkeruu-dimensiossa vastaava käsittely
	,case when f3.osio_id is not null then concat(f.kyselyid,999,f3.osio_id) else f.kyselyid end as kyselyid
	,coalesce(d15.id,-1) AS d_arvovastaus_monivalinta_id
	,f.numerovalinta
	,f.vastaajaid
	,f6.koulutussektori as sektori
	,'ETL: p_lataa_f_arvo_digikyvykkyys' as source
...
AND CAST(RIGHT(f5.tiedonkeruu,4) as int) >= 2025