...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvolliset]    Script Date: 25.7.2023 8:15:14 ******/
SET ANSI_NULLS ON
GO
...
LEFT JOIN (
	...
	WHERE ((ps.suorituksen_tyyppi in('aikuistenperusopetuksenoppimaara', 'perusopetuksenoppimaara','perusopetuksenlisaopetus')) OR
		   ...
		   (ps.suorituksen_tyyppi = 'europeanschoolofhelsinkivuosiluokkasecondarylower' and ps.koulutusmoduuli_koodiarvo = 'S5')) AND 
...
LEFT JOIN (
	...
	WHERE (oo.paattymispaiva is not null AND oo.viimeisin_tila = 'valmistunut' AND
	      ((ps.suorituksen_tyyppi in('ammatillinentutkinto','ibtutkinto','diatutkintovaihe','ebtutkinto')))) OR
...
LEFT JOIN (
	...
	(ps.suorituksen_tyyppi = 'internationalschoolmypvuosiluokka' and ps.koulutusmoduuli_koodiarvo = '9') OR
	(ps.suorituksen_tyyppi = 'europeanschoolofhelsinkivuosiluokkasecondarylower' and ps.koulutusmoduuli_koodiarvo = 'S5'))) po on po.oppija_oid = h.oppija_oid AND po.vahvistus_paiva = po.vahvistus_paiva_min
-- Rajaukset
...
FROM (
	SELECT DISTINCT
		...
		CASE 
			...
			WHEN ps.suorituksen_tyyppi in ('europeanschoolofhelsinkivuosiluokkasecondaryupper', 'ebtutkinto') and ooa.tila in ('lasna', 'loma','valmistunut') and oo.oppivelvollisuuden_suorittamiseen_kelpaava = 1 THEN 1
			WHEN ((ps.suorituksen_tyyppi in ('perusopetuksenoppimaara','aikuistenperusopetuksenoppimaara','aikuistenperusopetuksenoppimaaranalkuvaihe')) or 
				 ...
				 (ps.suorituksen_tyyppi in ('europeanschoolofhelsinkivuosiluokkaprimary', 'europeanschoolofhelsinkivuosiluokkasecondarylower'))) and ooa.tila in ('lasna', 'loma') and oo.oppivelvollisuuden_suorittamiseen_kelpaava = 1 THEN 2
...
BEGIN
		...
		FROM (
			SELECT DISTINCT
				...
				CASE
					...
					WHEN ps.suorituksen_tyyppi in ('europeanschoolofhelsinkivuosiluokkasecondaryupper', 'ebtutkinto') and ooa.tila in ('lasna', 'loma','valmistunut') and oo.oppivelvollisuuden_suorittamiseen_kelpaava = 1 THEN 3
					WHEN ((ps.suorituksen_tyyppi in ('perusopetuksenoppimaara','aikuistenperusopetuksenoppimaara','aikuistenperusopetuksenoppimaaranalkuvaihe')) or 
						 (ps.suorituksen_tyyppi in('internationalschoolpypvuosiluokka', 'internationalschoolmypvuosiluokka') and ps.koulutusmoduuli_koodiarvo in ('1','2','3','4','5','6','7','8','9')) or
						 (ps.suorituksen_tyyppi in ('europeanschoolofhelsinkivuosiluokkaprimary', 'europeanschoolofhelsinkivuosiluokkasecondarylower') and ps.koulutusmoduuli_koodiarvo in ('P1','P2','P3','P4','P5','S1','S2','S3','S4','S5'))) and ooa.tila in ('lasna', 'loma') and oo.oppivelvollisuuden_suorittamiseen_kelpaava = 1 THEN 5
	...
	FROM (
		...
		SELECT 
			...
			CASE
				WHEN ovt.oppivelvollisen_toiminta in (8,11,12,13) THEN NULL
			...
			ov.kotikunta,
			CASE 
				WHEN ovt.oppivelvollisen_toiminta in (8,11,12,13) THEN '-1'
			...
			--hv
			koulutusmuoto_hv = case when ovt.oppivelvollisen_toiminta between 1 and 10 and ovt.oppivelvollisen_toiminta <> 8 then ovt.koulutusmuoto end,
			koulutusmoduuli_koodiarvo_hv = case when ovt.oppivelvollisen_toiminta between 1 and 10 and ovt.oppivelvollisen_toiminta <> 8 then ovt.koulutusmoduuli_koodiarvo end,
			suorituksen_tyyppi_hv = case when ovt.oppivelvollisen_toiminta between 1 and 10 and ovt.oppivelvollisen_toiminta <> 8 then ovt.suorituksen_tyyppi end,
