...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvollisten_opintojen_kulku]    Script Date: 21.4.2026 8.36.26 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	FROM (
		SELECT DISTINCT 
			...
			perusopetuksen_oppimaaran_yksilollistaminen =
				CASE
					...
					WHEN f.alkamispaiva < pk.vahvistus_paiva or pk.master_oid is null THEN -1
					ELSE 6
				END,
			eo.erityisopetus,
		...
		FROM (
			...
			INNER JOIN Koski_SA.sa.sa_oppivelvolliset ov2 on ov2.master_oid = ov.master_oid and aika.alku BETWEEN LEAST(ov2.alkuPvm, @alkuPvm) and ov2.loppuPvm
	...
	SELECT
		...
		CASE WHEN ov.master_oid is null THEN 1 ELSE 0 END as oppivelvollisuus_paattynyt,
	...
	LEFT JOIN Koski_SA.sa.sa_oppivelvolliset_esi_ja_perusopetus ov on ov.master_oid = f.master_oid and f.tarkastelu_pvm between ov.alkuPvm and ov.loppuPvm
	...
	*/
	SELECT DISTINCT
		...
		CASE
			...
			WHEN oo.oppivelvollisuuden_suorittamiseen_kelpaava = 1 and lps.tavoitetutkinto <> 'Ei tutkintotavoitetta' or ooy.master_oid is not null THEN 2
	...
	FROM (
		...
		FROM (
			...
			-- Henkilötiedot
			LEFT JOIN Koski_SA.sa.sa_koski_henkilo_master h on h.master_oid = s1.master_oid
