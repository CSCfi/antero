-- 2018 freezed data with age (ikä)
 

IF NOT EXISTS (
  SELECT * FROM INFORMATION_SCHEMA.TABLES
  WHERE TABLE_SCHEMA='sa' AND TABLE_NAME='sa_virta_otp_viisviis_2018'
) BEGIN

CREATE TABLE sa.sa_virta_otp_viisviis_2018(
  ...
  CONSTRAINT PK__sa_virta_otp_viisviis_2018 PRIMARY KEY CLUSTERED (id ASC)
)
;

ALTER TABLE sa.sa_virta_otp_viisviis_2018 ADD  CONSTRAINT DF__sa_virta_otp_viisviis_2018__loadtime  DEFAULT (getdate()) FOR loadtime
;
ALTER TABLE sa.sa_virta_otp_viisviis_2018 ADD  CONSTRAINT DF__sa_virta_otp_viisviis_2018__username  DEFAULT (suser_name()) FOR username
;
