...
DROP VIEW IF EXISTS [dw].[v_koski_amm_opiskelijavuodet_2023]
GO
...
CREATE VIEW [dw].[v_koski_amm_opiskelijavuodet_2023] AS

SELECT
	[Opiskelijavuosi] = sum(opiskelijavuodet) + SUM(case when d_amos_spl_opiskelijavuodet_hylkaamisperuste.koodi in ('2','3','4') then opiskelijavuodet_hylatyt else 0 end)
...
HAVING sum(opiskelijavuodet) + SUM(case when d_amos_spl_opiskelijavuodet_hylkaamisperuste.koodi in ('2','3','4') then opiskelijavuodet_hylatyt else 0 end) > 0.0
