...
CREATE OR ALTER VIEW [dw].[v_amos_koonti_2028] AS
...
FROM (  
	...
	SELECT  
		...
		,[Oppilaitoksen kunta]
		,[Oppilaitoksen maakunta]
		,jarj_oppilaitoksen_maakunta = [jarjestys Oppilaitoksen maakunta]
		,Toimipiste --= 'Tieto puuttuu'
		,[Toimipisteen kunta] --= 'Tieto puuttuu'
		,[Toimipisteen maakunta] --= 'Tieto puuttuu'
		,jarjestys_toimipisteen_maakunta = [jarjestys Toimipisteen maakunta]
	...
	SELECT 
		...
		,[Tilastokausi]
		,[Tilastovuosi]
		,Tilastokuukausi = 'Tieto puuttuu'
		,[jarjestys Tilastokuukausi] = 99
		,[Järjestäjärakenne]
		,[Toiminnanohjauksen kokeilu]
