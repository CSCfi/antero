...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_yo_uraseuranta]    Script Date: 7.8.2026 11.46.24 ******/
SET ANSI_NULLS ON
GO
...
SELECT * FROM (
	...
	LEFT JOIN ANTERO.dw.d_arvovastaus_monivalinta d10 on d10.kysymysid = s.kysymysid and d10.koodi = 
		CASE 
			WHEN s.kysymysid = 2930 THEN 
				CASE 
					WHEN CAST(LEFT(s.vapaateksti, CHARINDEX('.', s.vapaateksti + '.') - 1) AS INT) > 3 
...
SELECT 
	...
	case when d2.kyselypohja_tarkenne_fi = 'URASEURANTA-TOHTORI' THEN -1 ELSE f.d_koulutusluokitus_id END as d_koulutusluokitus_id,
	d.okmohjauksenala_koodi,
	d.koulutusastetaso2_koodi,
...
LEFT JOIN ANTERO.dw.d_arvotiedonkeruu d2 on d2.id = f.d_arvotiedonkeruu_id
GROUP BY f.tilastovuosi,f.d_organisaatioluokitus_id,d2.kyselypohja_tarkenne_fi,case when d2.kyselypohja_tarkenne_fi = 'URASEURANTA-TOHTORI' THEN -1 ELSE f.d_koulutusluokitus_id END,
d.okmohjauksenala_koodi,d.koulutusastetaso2_koodi
...
LEFT JOIN ANTERO.dw.d_koulutusluokitus d on d.id = f.d_koulutusluokitus_id
LEFT JOIN ANTERO.dw.d_arvotiedonkeruu d2 on d2.id = f.d_arvotiedonkeruu_id
LEFT JOIN #uraseuranta_temp t on t.tilastovuosi = f.tilastovuosi and t.d_organisaatioluokitus_id = f.d_organisaatioluokitus_id and
case when d2.kyselypohja_tarkenne_fi = 'URASEURANTA-TOHTORI' THEN -1 ELSE f.d_koulutusluokitus_id END = t.d_koulutusluokitus_id and
d.okmohjauksenala_koodi = t.okmohjauksenala_koodi and d.koulutusastetaso2_koodi = t.koulutusastetaso2_koodi

DROP TABLE IF EXISTS #uraseuranta_temp


-- Raporteille ARVO:n rajapinnan rahoitusmallikysymysten tiedot uusimman vuoden osalta, kunnes lopulliset tiedot ovat saatavilla

DECLARE @max_vuosi as int

SET @max_vuosi =  (SELECT MAX(Kyselyvuosi) FROM [ANTERO].SA.sa_arvo_yo_uraseuranta_tohtorit_ja_maisterit)
...
SELECT
	YEAR(f5.voimassa_loppupvm) as tilastovuosi,
	f.vastaajaid,
	f.numerovalinta,
	CASE
		WHEN f4.nimi_fi like '%TYÖ JA OSAAMINEN%' THEN 1.000000 / 6.000000
		ELSE 1
	END as rahoituskysymys_paino,
	d1.id as d_arvotiedonkeruu_id,
	d2.id as d_arvokysymys_id,
	d3.id as d_organisaatioluokitus_id,
	d9.id as d_koulutusluokitus_id,
	d4.id as d_sukupuoli_id,
	d5.id as d_ika_id,
	d6.id as d_kieli_aidinkieli_id,
	d7.id as d_maatjavaltiot2_kansalaisuus_id,
	d8.id as d_arvovastaus_monivalinta_id,
	NULL as d_kytkin_vaihtoehto_id,
	d10.id as d_virta_rahoituslahde_id,
	0 as visualisointiin,
	'ETL: p_lataa_f_arvo_yo_uraseuranta' as source
FROM Arvo_SA.sa.sa_arvo_v2_vastaukset_muut f
...
LEFT JOIN ANTERO.dw.d_arvotiedonkeruu d1 on d1.kyselyid = f.kyselyid
LEFT JOIN ANTERO.dw.d_arvokysymys d2 on d2.kysymysid = f.kysymysid and d2.kyselyid = f.kyselyid and d2.kysymysversio = f.kysymysversio and d2.rnk = 1
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d3 on d3.organisaatio_koodi = f2.oppilaitoskoodi
LEFT JOIN ANTERO.dw.d_sukupuoli d4 on d4.sukupuoli_koodi = f2.sukupuoli
LEFT JOIN ANTERO.dw.d_ika d5 on d5.ika_avain = f2.ika_valmistuessa
LEFT JOIN ANTERO.dw.d_kieli d6 on d6.kieli_koodi = f2.aidinkieli
LEFT JOIN ANTERO.dw.d_maatjavaltiot2 d7 on d7.maatjavaltiot2_koodi = f2.kansalaisuus
LEFT JOIN ANTERO.dw.d_arvovastaus_monivalinta d8 on d8.koodi = f.numerovalinta and d8.kysymysid = f.kysymysid and d8.kysymysversio = f.kysymysversio
LEFT JOIN ANTERO.dw.d_koulutusluokitus d9 on d9.koulutusluokitus_koodi = f2.tutkinto
LEFT JOIN ANTERO.dw.d_virta_rahoituslahde d10 on d10.rahoituslahde_koodi = f2.rahoituslahde
WHERE YEAR(f5.voimassa_loppupvm) > @max_vuosi and f5.kyselytyyppi = 'yo-uraseuranta' and f3.rahoitusmallikysymys = 1