...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_move_aida]    Script Date: 20.12.2024 13.31.01 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dw].[p_lataa_f_arvo_move_aida] as
...
FROM (
	SELECT 
		...
		CASE 
			...
			WHEN ROUND(CAST(REPLACE(vauhditon_5_loikka, ',', '.') as float),1) >= p7.alaraja3 THEN 3 
			WHEN ROUND(CAST(REPLACE(vauhditon_5_loikka, ',', '.') as float),1) BETWEEN p7.alaraja2 and p7.ylaraja2 THEN 2
			WHEN ROUND(CAST(REPLACE(vauhditon_5_loikka, ',', '.') as float),1) <= p7.ylaraja1 THEN 1
			ELSE NULL
		END as vauhditon_5_loikka_pisteet,
