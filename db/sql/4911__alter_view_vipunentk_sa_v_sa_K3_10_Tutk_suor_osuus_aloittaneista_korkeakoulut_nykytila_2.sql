...
/****** Object:  View [dbo].[v_sa_K3_10_Tutk_suor_osuus_aloittaneista_korkeakoulut_nykytila_2]    Script Date: 21.9.2021 16:15:41 ******/
SET ANSI_NULLS ON
GO
...
-- lisää sarakkeet ylle ja päivitä tietolähde sekä lähdetaulut alle
    ...
    ,[lkm]
    ,[tietolahde]='K3_10_sopv_21'
    ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv, alvv, allk)
    ,[aineistorivinumero] = ROW_NUMBER() OVER(ORDER BY tilv, alvv, allk)
	,f1.[rivinro]
  FROM [TK_H9098_CSC].[dbo].[TK_K3_10_sopv_21_1] f1
  inner join [TK_H9098_CSC].[dbo].[TK_K3_10_sopv_21_2] as f2 on f1.rivinro=f2.rivinro 
