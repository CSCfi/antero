		...
		case 
			when d21.kytkin_koodi = 0 then 1
				...
				case
					...
					when d5.jarjestys_erityisopetus_ryhma = '3' and v2b.erityinen_vaativa_tuki = 0 then 4
					else 1
				end
		end
	,lupa_suorituskieli = 
...
--vaativa erityinen tuki hankintakoulutuksen tapauksessa hankkivan järjestäjän luvasta
LEFT JOIN ANTERO.dw.d_amos_spl_jarjestaja_linkki sjl2 ON sjl2.oid_avain = coalesce(q.jarj_oid_hist, q.jarj_oid)
CROSS APPLY (
	SELECT jarj =
		case 
			when tilv_date < sjl2.voimaan_alkaen then sjl2.ytunnus_avain
			else coalesce(sjl2.ytunnus_amos_spl, sjl2.ytunnus_avain)
		end
) ca2
--tapaukset joita verrataan lupatietoihin
LEFT JOIN ANTERO.dw.d_kytkin d21 ON d21.kytkin_koodi = 
	case 
		when q.suorituksen_tyyppi = '3' then 0 --tutkinnon osaa pienemmistä kokonaisuuksista koostuva suoritus
		when (d11.tutkintotyyppi_koodi in ('02','19','20') and q.vuosi > 2021) 
...
--muut oikeudet pl. vaativa erityinen tuki
OUTER APPLY (
	select top 1 *
...
--muut oikeudet, vaativa erityinen tuki
OUTER APPLY (
	select top 1 *
	from ANTERO.sa.v_amos_oiva_muut_oikeudet v2b
	where tilv_date between v2b.alkupvm and v2b.loppupvm 
	and v2b.jarjestajaYtunnus = ca2.jarj
) v2b
--kielet, opetus
OUTER APPLY (
	select top 1 jarjestajaYtunnus
