...
/****** Object:  StoredProcedure [dw].[p_lataa_api_perusopetus_oppilaat_lukuvuosi_oppilaitos_esilataus]    Script Date: 18.11.2024 11:41:29 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE  [dw].[p_lataa_api_perusopetus_oppilaat_lukuvuosi_oppilaitos_esilataus] AS
...
FROM (
	SELECT 
		...
		oppilaat_lukuvuosi_lkm = COUNT(DISTINCT CASE WHEN opisk = 1 THEN oppija_oid ELSE NULL END),
		oppilaat_syyslukukausi_lkm = COUNT(DISTINCT CASE WHEN opisk = 1 and d14.koodi = 'S' THEN oppija_oid ELSE NULL END),
		oppilaat_kevatlukukausi_lkm = COUNT(DISTINCT CASE WHEN opisk = 1 and d14.koodi = 'K' THEN oppija_oid ELSE NULL END)
