...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amos_opiskelijapalaute]    Script Date: 7.4.2026 14.19.00 ******/
SET ANSI_NULLS ON
GO
	...
	FROM (
		...
		FROM (
			...
			SELECT 
				...
				,oppilaitos_koodi = d2.organisaatio_koodi
				,toimipiste_oid = d2b.organisaatio_oid
				,d5.kyselyid
			...
			LEFT JOIN dw.d_organisaatioluokitus d2b ON d2b.id = f.d_organisaatio_toimipiste_id
			...
			LEFT JOIN dw.d_arvotiedonkeruu d5 ON d5.id = f.d_arvokysely_id
			LEFT JOIN dw.d_koulutusluokitus d7 ON d7.id = f.d_koulutusluokitus_id
			LEFT JOIN dw.d_arvovastaus_monivalinta p ON p.id = f.d_arvovastaus_koulutustausta_id
			LEFT JOIN dw.f_arvo_luodut_vastaajatunnukset f2 
				ON f2.kyselyid = d5.kyselyid
				AND concat(f2.toimipiste_oid, '|', f2.oppilaitoskoodi, '|', f2.ytunnus) 
					= concat(nullif(d2b.organisaatio_oid,'-1'), '|', nullif(d2.organisaatio_koodi,'-1'), '|', d1.ytunnus_avain)
