...
/****** Object:  View [dw].[v_oiva_luvat_amm_cultures]    Script Date: 13.3.2026 11.03.13 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT --TOP 1000
	...
	--fi
	,[luvan_paattymisvuosi_fi] = coalesce(nullif(cast(d6.vuosi as varchar(20)),'-1'),'Ei p‰‰ttynyt')
	,[luvan_alkamiskuukausi_fi] = d5.kuukausi_fi
	,[luvan_paattymiskuukausi_fi] = coalesce(nullif(d6.kuukausi_fi,ca.puuttuu_fi),'Ei p‰‰ttynyt')
	,[tarkastelupaiva_fi] = concat('Voimassa 1.', d14.kuukausi, '.')  -- Lupa voimassa muutos tarkastelup‰iv‰ksi JK
	...
	,[toiminta_alueen_kunta_fi] = coalesce(nullif(d2a.kunta_fi,ca.puuttuu_fi),'Ei m‰‰ritelty')
	,[toiminta_alueen_kunnan_hyvinvointialue_fi] = coalesce(nullif(d2a.hyvinvointialue_fi,ca.puuttuu_fi),'Ei m‰‰ritelty')
	,[toiminta_alueen_maakunta_fi] = coalesce(nullif(d2b.maakunta_fi,ca.puuttuu_fi),'Ei m‰‰ritelty')
	,[toiminta_alueena_koko_maa_fi] = d11.kytkin_fi
	,[toiminta_alueena_koko_maakunta_fi] = d12.kytkin_fi
	,[tutkinto_fi] = coalesce(nullif(d3.koulutusluokitus_fi,ca.puuttuu_fi),'Ei m‰‰ritelty')
	,[osaamisala_fi] = CASE d13.koodi WHEN -1 THEN NULL ELSE d13.selite_fi END -- Tieto puuttuu n‰kym‰‰n tyhj‰n‰ JK 25.3.2025
	...
	,[osaamisala_rajoite_fi] =  CASE d4.koodi WHEN -1 THEN NULL ELSE d4.selite_fi END -- Tieto puuttuu n‰kym‰‰n tyhj‰n‰
	,[luvanmukaiset_koulutukset_fi] = coalesce(nullif(d9.selite_fi,ca.puuttuu_fi),'Ei m‰‰ritelty')
	,[muut_oikeudet_velvollisuudet_ehdot_ja_tehtavat_fi] = coalesce(nullif(d10.selite_fi,ca.puuttuu_fi),'Ei m‰‰ritelty')
	,[tutkintokieli1_fi] = coalesce(nullif(d8.kieli_fi,ca.puuttuu_fi), 'opetuskieli')
	,[tutkintokieli2_fi] = coalesce(nullif(case d8b.kieli_fi when 'pohjoissaame' then 'saame' else d8b.kieli_fi end,ca.puuttuu_fi), 'ei m‰‰ritelty')
	,[opetuskieli_fi] = d7.selite_fi
	,[toiminnanohjauksen_kokeilu_fi] = d15.[Toiminnanohjauksen kokeilu]
	,[koul_jarjestajan_pilke_alue_kokeilu_fi] = d15.[Koul. j‰rjest‰j‰n PILKE-alue (kokeilu)]

	--sv
	,[luvan_paattymisvuosi_sv] = coalesce(nullif(cast(d6.vuosi as varchar(20)),'-1'),'Ej lˆpt ut')
	,[luvan_alkamiskuukausi_sv] = d5.kuukausi_sv
	,[luvan_paattymiskuukausi_sv] = coalesce(nullif(d6.kuukausi_sv,ca.puuttuu_sv),'Ej lˆpt ut')
	,[tarkastelupaiva_sv] = concat('Giltigt 1.', d14.kuukausi, '.') -- Lupa voimassa muutos tarkastelup‰iv‰ksi JK
	...
	,[osaamisala_sv] = CASE d13.koodi WHEN -1 THEN NULL ELSE d13.selite_sv END -- Tieto puuttuu n‰kym‰‰n tyhj‰n‰ JK 25.3.2025
	...
	,[osaamisala_rajoite_sv] = CASE d4.koodi WHEN -1 THEN NULL ELSE d4.selite_sv END -- Tieto puuttuu n‰kym‰‰n tyhj‰n‰
	...
	,[tutkintokieli1_sv] = coalesce(nullif(d8.kieli_sv,ca.puuttuu_sv), 'undervisningssprÂk')
	,[tutkintokieli2_sv] =  coalesce(nullif(d8b.kieli_sv,ca.puuttuu_sv),'ej angivet')
	,[opetuskieli_sv] = d7.selite_sv
	,[toiminnanohjauksen_kokeilu_sv] = d15.[Toiminnanohjauksen kokeilu]
	,[koul_jarjestajan_pilke_alue_kokeilu_sv] = d15.[Koul. j‰rjest‰j‰n PILKE-alue (kokeilu)]
	...
	,[tarkastelupaiva_en] = concat('Valid 1.', d14.kuukausi, '.') -- Lupa voimassa muutos tarkastelup‰iv‰ksi JK
	...
	,[osaamisala_en] = CASE d13.koodi WHEN -1 THEN NULL ELSE d13.selite_en END -- Tieto puuttuu n‰kym‰‰n tyhj‰n‰ JK 25.3.2025
	...
	,[tutkintokieli1_en] = coalesce(nullif(d8.kieli_en,ca.puuttuu_en), 'undervisningssprÂk')
	,[tutkintokieli2_en] =  coalesce(nullif(d8b.kieli_en,ca.puuttuu_en),'Not specified')
	,[opetuskieli_en] = d7.selite_en
	,[toiminnanohjauksen_kokeilu_en] = d15.[Toiminnanohjauksen kokeilu]
	,[koul_jarjestajan_pilke_alue_kokeilu_en] = d15.[Koul. j‰rjest‰j‰n PILKE-alue (kokeilu)]

	--apusarakkeet
	,apusarake_opiskelijavuodet = CASE d14.vuosi WHEN 2026 THEN NULL ELSE opiskelijavuodet END
	,apusarake_opiskelijavuodet_sisa = opiskelijavuodet_sisa
	,apusarake_opiskelijavuodet_vet = opiskelijavuodet_vet
	,apusarake_opv_enim_ei_oppivelvolliset = opv_enim_ei_oppivelvolliset

	,[koodit_koulutuksen_jarjestaja] = d1.organisaatio_koodi
	,[koodit_tutkinto] = d3.koulutusluokitus_koodi
	,[koodit_osaamisala] = CASE d13.koodi WHEN -1 THEN NULL ELSE d13.koodi END -- Tieto puuttuu n‰kym‰‰n tyhj‰n‰ JK 25.3.2025
	...
	,[jarjestys_toiminnanohjauksen_kokeilu] = d15.[jarjestys Toiminnanohjauksen kokeilu]
	,[jarjestys_koul_jarjestajan_pilke_alue_kokeilu] = d15.[jarjestys Koul. j‰rjest‰j‰n PILKE-alue (kokeilu)]
...
LEFT JOIN dw.d_kalenteri d14 on d14.id = f.d_kalenteri_tarkastelupaiva -- Lupa voimassa muutos tarkastelup‰iv‰ksi JK
LEFT JOIN dw.d_amos_spl_jarjestaja_linkki d15 on d15.id = f.d_amos_spl_jarjestaja_linkki_id
