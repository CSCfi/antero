...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_opiskelija_ja_tutkinto_ennakko]    Script Date: 21.9.2026 8.55.31 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT
		...
		coalesce(d10.id, -1) as d_kytkin_ensikertalainen_id,
		...
		coalesce(d11.id, -1) as d_virta_rahoituslahde_id,
		...
		ensikertaiset_aloittaneet = CAST(case when aloitusvuosi = tilv then lkm else null end as int),
		s.op55,
		s.opyht0,
		s.opyht104,
		s.opyht119,
		...
		CASE WHEN coalesce(s.rahlahde,-1) <> 5 or s.tilv < 2023 THEN NULL ELSE COALESCE(s.maksuvelk,0) END as maksuvelk,
		CASE WHEN coalesce(s.rahlahde,-1) <> 5 or s.tilv < 2023 THEN NULL ELSE COALESCE(s.maksuvels,0) END as maksuvels,
		CASE WHEN coalesce(s.rahlahde,-1) <> 5 or s.tilv < 2023 THEN NULL ELSE COALESCE(s.maksuk,0) END as maksuk,
		CASE WHEN coalesce(s.rahlahde,-1) <> 5 or s.tilv < 2023 THEN NULL ELSE COALESCE(s.maksus,0) END as maksus,
		CASE WHEN coalesce(s.rahlahde,-1) <> 5 or s.tilv < 2023 THEN NULL ELSE COALESCE(s.apurahak,0) END as apurahak,
		CASE WHEN coalesce(s.rahlahde,-1) <> 5 or s.tilv < 2023 THEN NULL ELSE COALESCE(s.apurahas,0) END as apurahas
	FROM ' + @taulu + ' s
	...
	LEFT JOIN ANTERO.dw.d_kytkin d10 on d10.kytkin_koodi = s.ensikertalainen
	LEFT JOIN ANTERO.dw.d_virta_rahoituslahde d11 on d11.rahoituslahde_koodi = s.rahlahde'
	...
	SELECT
		tilv as tilastovuosi,
		null as aloitusvuosi,
		alvv as kirjoihintulovuosi,
		coalesce(d1.id, -1) as d_organisaatioluokitus_id,
		coalesce(d2.id, -1) as d_maatjavaltiot2_kansalaisuus_id,
		coalesce(d3.id, -1)as d_sukupuoli_id,
		-1 as d_kytkin_ensikertalainen_id,
		coalesce(d4.id, -1) as d_koulutuksen_kieli_id,
		coalesce(d5.id, -1) as d_aidinkieli_id,
		coalesce(d6.id, -1) as d_koulutusluokitus_id,
		coalesce(d7.id, -1) as d_ika_id,
		...
		NULL as aloittaneet,
		NULL as ensikertaiset_aloittaneet,
		NULL as op55,
		NULL as opyht0,
		NULL as opyht104,
		NULL as opyht119,
