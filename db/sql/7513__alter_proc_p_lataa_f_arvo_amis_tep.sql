USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amis_tep]    Script Date: 29.3.2023 21:10:14 ******/
SET ANSI_NULLS ON
GO
...
SELECT distinct
	...
	--,coalesce(d21.id,-1) AS d_rahoitusryhma_id
	...
	,tyopaikkaohjaajakysely_vastaajaid = case when MAIN_QUERY.tyyppi = 'tyopaikkaohjaajakysely' then vastaajaid end
	,tyopaikkaohjaajakysely_vastaajatunnus = case when MAIN_QUERY.tyyppi = 'tyopaikkaohjaajakysely' then vastaajatunnus end
...
FROM (
	...
	WHERE 1=1
	AND f5.tyyppi in ('tyopaikkaohjaajakysely', 'tyopaikkakysely')
...
--poistettu ks. CSCVIPUNEN-3260
--LEFT JOIN dw.d_rahoitusryhma d21 ON d21.koodi = MAIN_QUERY.[taustatiedot.rahoitusryhma]
