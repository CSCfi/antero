...
/****** Object:  StoredProcedure [dbo].[p_lataa_amm_tutkinnot_valitaulu]    Script Date: 9.2.2021 10:21:14 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[p_lataa_amm_tutkinnot_valitaulu] @alkuPvm as date = '2018-01-01', @loppuPvm as date = NULL AS

DECLARE @year int
SET @year = YEAR(@alkuPvm)

IF @loppuPvm is null SET @loppuPvm = EOMONTH(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0))
...
BEGIN
	...
	AND year(coalesce(ps.vahvistus_paiva,'9999-12-31')) = @year AND coalesce(ps.vahvistus_paiva,'9999-12-31') <= @loppuPvm
