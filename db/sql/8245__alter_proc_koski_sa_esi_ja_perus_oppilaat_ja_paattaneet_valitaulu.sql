...
/****** Object:  StoredProcedure [dbo].[p_lataa_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu]    Script Date: 27.10.2023 17:52:51 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	FROM (
		...
		AND (
			...
			--1.8.2022 alkaen TUVA, joka raportoidaan perusopetuksen osalta vain erikseen
			OR
			(
		...
		AND (
			(YEAR(@alkuPvm) > YEAR(COALESCE(ebib.alku, ooa2.alku))) OR (YEAR(@alkuPvm) > YEAR(COALESCE(ebib.alku, ooa.alku)))
			OR (YEAR(@alkuPvm) = YEAR(COALESCE(ebib.alku, ooa2.alku)) and MONTH(@alkuPvm) >= MONTH(COALESCE(ebib.alku, ooa2.alku)))
			OR (YEAR(@alkuPvm) = YEAR(COALESCE(ebib.alku, ooa.alku)) and MONTH(@alkuPvm) >= MONTH(COALESCE(ebib.alku, ooa.alku)))
		)
	...
	WHERE ranki = 1 
	AND kotiopetus = 0 --jatkossa erillisellä raportilla. JS 27.10.2023
