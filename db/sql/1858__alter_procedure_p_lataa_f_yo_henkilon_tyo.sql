...
SELECT COALESCE(s2.id,-1)
	...
	,[s1].VUOSI
	,HENKILOTYOVUOSI_LKM
	,-1 AS [sopimus_alkupvm] --COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM(s1.[Sopimuksen alkupäivämäärä])), 104), -1) AS [sopimus_alkupvm]
	,- 1 AS [sopimus_loppupvm] --COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM(s1.[Sopimuksen loppupäivämäärä])), 104), - 1) AS [sopimus_loppupvm]
	,getdate()
