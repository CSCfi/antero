...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_oppivelvolliset]    Script Date: 25.9.2026 9.48.01 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT
	...
	CASE				
		WHEN YEAR(oo.alkamispaiva) < 2026 and (ooa.erityinen_tuki = 1 or ooa.hojks = 1) and ooa.vaativan_erityisen_tuen_erityinen_tehtava=0 and ps.suorituksen_tyyppi not in ('telma','valma','tuvakoulutuksensuoritus') THEN 2 --Erityisopiskelija ilman erit. tuen tehtävää; tutk. joht 
		WHEN YEAR(oo.alkamispaiva) < 2026 and (ooa.erityinen_tuki = 1 or ooa.hojks = 1) and ooa.vaativan_erityisen_tuen_erityinen_tehtava=0 and ps.suorituksen_tyyppi in ('telma','valma','tuvakoulutuksensuoritus') THEN 3 --Erityisopiskelija ilman erit. tehtävää; valmentava
		...
		WHEN YEAR(oo.alkamispaiva) < 2026 and ooa.erityinen_tuki = 0 THEN 1 --Ei erityisopiskelija
		ELSE -1
...
BEGIN
	...
	FROM (
		...
		SELECT DISTINCT
			...
			CASE 
				WHEN ovt.oppivelvollisen_toiminta in (13,14,15) and tilastovuosi >= 2026 THEN -1
				WHEN ovt.oppivelvollisen_toiminta in (13,14,15) THEN 1
	...
	FROM (
		...
		SELECT DISTINCT  
			...
			ovt.perusopetuksen_oppimaaran_yksilollistaminen_hv,
			CASE 
				WHEN ovt.oppivelvollisen_toiminta = 9 and tilastovuosi >= 2026 THEN -1
...
([view])
SELECT DISTINCT 
	CONCAT(ds.dataset, d.paivays)
FROM [ANTERO].[dw].[f_koski_oppivelvolliset] f
INNER JOIN [ANTERO].[dw].[d_kalenteri] d on d.kalenteri_avain = f.ajankohta and d.kuukausi between 8 and 12 and RIGHT(f.ajankohta,2) = '01'
OUTER APPLY (
	SELECT 'f_koski_oppivelvolliset_1_' as dataset
	UNION ALL
	SELECT 'f_koski_oppivelvolliset_2_' as dataset
	UNION ALL
	SELECT 'f_koski_oppivelvolliset_3_' as dataset
) ds
WHERE NOT EXISTS (
	SELECT TOP 1 1 
	FROM ANTERO.dbo.view_salt vs
	WHERE CONCAT(ds.dataset, d.paivays) = vs.[view]
)