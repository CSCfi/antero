USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_d_maatjavaltiot2]    Script Date: 23.4.2020 10:42:29 ******/
SET ANSI_NULLS ON
GO
...
USING (
  SELECT
	...
	case when s.koodi = '246' then s.nimi when s.maanosatkoodi = '1' then concat(maanosatnimi,' (pl. Suomi)')  else 'Muu' end as maanosat4nimi,
	case when s.koodi = '246' then s.nimi_sv when s.maanosatkoodi = '1' then concat(maanosatnimi_sv,' (exkl. Finland)') else 'Övrigt' end as maanosat4nimi_sv,
	case when s.koodi = '246' then s.nimi_en when s.maanosatkoodi = '1' then concat(maanosatnimi_en,' (excl. Finland)') else 'Other' end as maanosat4nimi_en,
