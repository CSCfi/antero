...
/****** Object:  StoredProcedure [dbo].[p_lataa_sa_oppivelvolliset]    Script Date: 26.8.2024 10:58:59 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	MIN(
		CASE 
			WHEN ts.vahvistus_paiva < DATEADD(YEAR,18,h.syntymaaika) and (ts.vahvistus_paiva < vov.vapautettu or vov.vapautettu is null) THEN cast(ts.vahvistus_paiva as date)
...
GROUP BY h.[oppija_oid], COALESCE(pov.pidennetty_oppivelvollisuus,0)
