...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_ammatillisen_koulutuksen_arvosanat]    Script Date: 24.1.2025 9.43.27 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_koski_ammatillisen_koulutuksen_arvosanat] AS
...
SELECT
	...
	TRIM(os2.koulutusmoduuli_koodiarvo) as tutkinnonosan_osaalue,
...
FROM (
	SELECT
		os.*,
		rnk = RANK() OVER(partition by os.opiskeluoikeus_oid, os.tutkinnonosa, koulutusmoduuli_koodisto, os.tutkinnonosan_osaalue ORDER BY arvosana desc, osasuoritus_id)
