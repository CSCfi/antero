...
/****** Object:  View [dbo].[v_f_4_2_Vaeston_koulutusrakenne_ja_paaasiallinen_toiminta]    Script Date: 13.4.2026 12.11.30 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,[koulutusluokitus_korkein_viimeisin_aloitettu_sama_ka] =
		case 
		    when cast(tilv as int) < 2020 then '9'  -- korjaus tuntemattomaksi, koska ka_opiskkoulk alkaen tilv 2020
			when nullif(ka_opiskkoulk,'') != '999999' and ka_opiskkoulk = koulk then '1'
			else '0'
		end
	,[koulutusluokitus_korkein_viimeisin_aloitettu_sama_ta] =
		case
		    when cast(tilv as int) < 2020 then '9'  -- korjaus tuntemattomaksi, koska ta_opiskkoulk alkaen tilv 2020
			when nullif(ta_opiskkoulk,'') != '999999' and ta_opiskkoulk = koulk then '1'
			else '0'
		end
