...
/****** Object:  View [dw].[v_haku_ja_valinta_kk]    Script Date: 10.12.2019 9:39:47 ******/
SET ANSI_NULLS ON
GO
...
SELECT --top 10
	  ...
	  ,[Valintatapajono] = 
		case
			when d47.JonoNimi like '%avoimen%' or d47.JonoNimi like '%ÖPU%' then 'Avoin väylä'
			when d47.JonoNimi like '%todistus%' or d47.JonoNimi like '%betygs%' then 'Todistusvalinta'
			when d47.JonoNimi like '%kilpailu%' then 'Kilpailumenestys'
			when d47.JonoNimi like '%yhteispiste%' or d47.JonoNimi like '%totalpoäng%' then 'Yhteispisteet'
			when d47.JonoNimi like '%koepiste%' or d47.JonoNimi like '%provpoäng%' 
			  or d47.JonoNimi like '%valintakoe%' or d47.JonoNimi like '%urvals%' or d47.JonoNimi like '%examination%' then 'Koepisteet'
			else 'Muu'
		end
...
--Apusarakkeet mittareille	    
	  ...
	  ,[Valintatapajono järjestys] = 
		case
			when d47.JonoNimi like '%avoimen%' or d47.JonoNimi like '%ÖPU%' then '1'
			when d47.JonoNimi like '%todistus%' or d47.JonoNimi like '%betygs%' then '2'
			when d47.JonoNimi like '%kilpailu%' then '3'
			when d47.JonoNimi like '%yhteispiste%' or d47.JonoNimi like '%totalpoäng%' then '4'
			when d47.JonoNimi like '%koepiste%' or d47.JonoNimi like '%provpoäng%' 
			  or d47.JonoNimi like '%valintakoe%' or d47.JonoNimi like '%urvals%' or d47.JonoNimi like '%examination%' then '5'
			else '9'
		end
...
FROM [dw].[f_haku_ja_valinta_aloituspaikat_ja_pistemaarat] f
	  ...
	  LEFT JOIN ANTERO.dw.d_opintojenlaajuus d2 on d2.id=f.d_opintojen_laajuus_id
	  ...
	  LEFT JOIN ANTERO.dw.d_koulutusluokitus d6 on d6.id=f.d_koulutusluokitus_hakukohde_id
	  LEFT JOIN ANTERO.dw.d_kieli d7 on d7.id=f.d_kieli_koulutus_id
	  LEFT JOIN ANTERO.dw.d_organisaatioluokitus d8a on d8a.id=d_organisaatio_koulutuksen_jarjestaja_id
	  ...
	  LEFT JOIN ANTERO.dw.d_hakuaika d13 on d13.id=f.d_hakuaika_id
	  LEFT JOIN ANTERO.dw.d_tutkinnon_aloitussykli_kk d44 on d44.id=f.d_tutkinnon_aloitussykli_kk_id
	  LEFT JOIN ANTERO.dw.d_tutkinnon_taso_kk d45 on d45.id=f.d_tutkinnon_taso_kk_id
