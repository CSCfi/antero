...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_kandi]    Script Date: 18.8.2025 9.25.56 ******/
SET ANSI_NULLS ON
GO
...
DROP TABLE IF EXISTS sa.sa_arvo_temp_uudet_kysymykset
...
SELECT 
	...
	1 as kysymysversio,
	rank() OVER(PARTITION BY k.kysymysid, k.uusi_kysymysid ORDER BY kysymysryhmaid, kysymysryhmajarjestys) as rnk
INTO sa.sa_arvo_temp_uudet_kysymykset 
FROM (
	SELECT DISTINCT 
		kysymysid,
		CASE
			WHEN kysymysid in('1088', '4069') THEN '11238'
			WHEN kysymysid in('1115', '4084') THEN '11253'
			WHEN kysymysid in('1117', '4086') THEN '11279'
			WHEN kysymysid in('1120', '4089') THEN '11282'
			WHEN kysymysid in('1143', '4093') THEN '11257'
			WHEN kysymysid in('1132', '4111') THEN '11265'
			ELSE kysymysid
		END as uusi_kysymysid
	FROM ANTERO.dw.d_arvokysymys 
	WHERE kysymysid in ('1088', '4069','1115', '4084','1117', '4086','1120', '4089','1143', '4093','1132', '4111')
) k
LEFT JOIN ANTERO.dw.d_arvokysymys d on d.kysymysid = k.uusi_kysymysid
...
SELECT k.oppilaitoskoodi
	...
	,f.kyselyid
	,f5.kyselykertaid
	,COALESCE(uk.uusi_kysymysid, f3.kysymysid) as kysymysid
	,COALESCE(uk.kysymysversio, f3.kysymysversio) as kysymysversio
	,f3.kysymysid as kysymysid_rahoitusmalli
	,f3.kysymysversio as kysymysversio_rahoitusmalli
	,COALESCE(uk.kysymysryhmaid, f3.kysymysryhmaid) as kysymysryhmaid
	,f3.kysymysryhmaid as kysymysryhmaid_rahoitusmalli
	,COALESCE(uk.kysymysryhmajarjestys, f4.jarjestys) as kysymysryhmajarjestys
...
LEFT JOIN ANTERO.sa.sa_arvo_temp_uudet_kysymykset uk on uk.kysymysid = f.kysymysid and uk.rnk = 1 
