...
/****** Object:  View [dw].[v_amos_spl_koski_paatason_suoritukset_ja_kustannusryhmat]    Script Date: 16.9.2019 13:06:21 ******/
SET ANSI_NULLS ON
GO
...
SELECT distinct
		...
		-- 12.5 diaarinumero ja yhdistelmäavaimet lisätty
		 ...
		 NULL /* t.diaarinumero+'_'+t.koulutusmoduuli_koodiarvo */ as diaarinumero_tutkinto_koodi,
		 NULL /* t.diaarinumero+'_'+t.tutkinto_ja_osaamisala_koodi */ as diaarinumero_tutkinto_ja_osaamisala_koodi,
...
--						 ltrim(rtrim(replace(replace(replace(substring(data,CHARINDEX('"perusteenDiaarinumero": ',data)+26,15),'}',''),'"',''),',',''))) as diaarinumero_space,
			...
			CASE 
				WHEN year(vahvistus_paiva)=2018 /* year(getdate())-1 */ THEN 1 
				ELSE 0 
