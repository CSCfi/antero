...
/****** Object:  View [dbo].[v_f_2_3_Lukiokoulutuksen_ja_ammatillisen_koulutuksen_opiskelijat]    Script Date: 5.8.2021 19:44:58 ******/
SET ANSI_NULLS ON
GO
	...
	,koulutusryhma_avain =
		CASE
		  ...
		  --yhdistettävän 2.17 luokituksessa muutoksia tilastovuosina 2018 ja 2019
			...
			WHEN tutklaja = 1 AND koso = 1 AND opso = 2 THEN '74' --amm. pk., k-e
			WHEN tutklaja in (2,3) AND koso = 1 AND opso = 2 THEN '75' --amm. lk., k-e
			WHEN tutklaja = 1 AND koso = 2 AND opso = 1 THEN '77' --amm. pk., e-k
			WHEN tutklaja in (2,3) AND koso = 2 AND opso = 1 THEN '78' --amm. lk., e-k
			WHEN tutklaja = 1 AND koso = 1 AND opso = 1 THEN '80' --amm. pk., k-k
			WHEN tutklaja in (2,3) AND koso = 1 AND opso = 1 THEN '81' --amm. lk., k-k
