...
/****** Object:  View [dw].[v_koski_amm_opiskelijat_ja_tutkinnot_cultures]    Script Date: 25.8.2021 14:59:53 ******/
SET ANSI_NULLS ON
GO
...
SELECT --TOP 1000
	...
	,[aidinkieli_fi] = d2.kieliryhma1_fi
	,[ika_fi] = d3.ikaryhma6_fi
	,[ikaryhma_fi] = d3.ikaryhma3_fi
	,[kansalaisuus_fi] = d22.maanosa1_fi
	,[kansalaisuus_maanosa_fi] = d22.maanosa0_fi
	,[tutkintotyypin_ryhma_fi] = case when d24.kytkin_koodi = 1 then (select distinct tutkintotyypin_ryhma_fi from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi = '09') else d4.tutkintotyypin_ryhma_fi end  
	,[tutkintotyyppi_fi] = case when d4.koulutusluokitus_fi like '%VALMA%' then 'VALMA' when d4.koulutusluokitus_fi like '%TELMA%' then 'TELMA' when d24.kytkin_koodi =  1 then (select distinct tutkintotyypin_ryhma_fi from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi = '09') else d4.tutkintotyyppi_fi end
	...
	,[aidinkieli_sv] = d2.kieliryhma1_sv
	,[ika_sv] = d3.ikaryhma6_sv
	,[ikaryhma_sv] = d3.ikaryhma3_sv
	,[kansalaisuus_sv] = d22.maanosa1_sv
	,[kansalaisuus_maanosa_sv] = d22.maanosa0_sv
	,[tutkintotyypin_ryhma_sv] = case when d24.kytkin_koodi =  1 then (select distinct tutkintotyypin_ryhma_sv from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi = '09') else d4.tutkintotyypin_ryhma_sv end  
	,[tutkintotyyppi_sv] = case when d4.koulutusluokitus_sv like '%VALMA%' then 'VALMA' when d4.koulutusluokitus_sv like '%TELMA%' then 'TELMA' when d24.kytkin_koodi =  1 then (select distinct tutkintotyypin_ryhma_sv from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi = '09') else d4.tutkintotyyppi_sv end
	...
	,[aidinkieli_en] = d2.kieliryhma1_en
	,[ika_en] = d3.ikaryhma6_en
	,[ikaryhma_en] = d3.ikaryhma3_en
	,[kansalaisuus_en] = d22.maanosa1_en
	,[kansalaisuus_maanosa_en] = d22.maanosa0_en
	,[tutkintotyypin_ryhma_en] = case when d24.kytkin_koodi =  1 then (select distinct tutkintotyypin_ryhma_en from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi = '09') else d4.tutkintotyypin_ryhma_en end  
	,[tutkintotyyppi_en] = case when d4.koulutusluokitus_en like '%VALMA%' then 'VALMA' when d4.koulutusluokitus_en like '%TELMA%' then 'TELMA' when d24.kytkin_koodi =  1 then (select distinct tutkintotyypin_ryhma_en from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi = '09') else d4.tutkintotyyppi_en end
	...
	,d2.jarjestys_kieliryhma1 as jarjestys_aidinkieli
	...
	,case when d4.koulutusluokitus_fi like '%VALMA%' then '97' when d4.koulutusluokitus_fi like '%TELMA%' then '98' when d24.kytkin_koodi = 1 then '99' /*(select distinct jarjestys_tutkintotyyppi_koodi from [ANTERO].[dw].[d_koulutusluokitus] where tutkintotyypin_ryhma_koodi='09')*/ else d4.jarjestys_tutkintotyyppi_koodi end as jarjestys_tutkintotyyppi
...
WHERE tilastovuosi > year(getdate())-4
