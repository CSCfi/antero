...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dw].[p_lataa_api_lukio_opiskelijat_kuukausi_maakunta_esilataus]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dw].[p_lataa_api_lukio_opiskelijat_kuukausi_maakunta_esilataus] AS' 
END
GO
...
LEFT JOIN dw.d_sukupuoli AS d1 ON f.Sukupuoli = d1.sukupuoli_koodi
LEFT JOIN dw.d_organisaatioluokitus d2 ON d2.organisaatio_oid = f.koulutustoimija_oid
LEFT JOIN dw.d_alueluokitus d3 ON d3.kunta_koodi = d2.kunta_koodi 
LEFT JOIN dw.d_kieli AS d4 ON f.aidinkieli = d4.kieli_koodi
LEFT JOIN dw.d_ika AS d5 ON f.Ika = d5.ika_avain
