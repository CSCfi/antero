...
SELECT DISTINCT
	...
	,kr = 
		coalesce(
			...
			,'2' --tutk. osaa pienemmistä kokonaisuuksista koostuvassa suorituksessa ei aina tutk. osaa pienempää kokonaisuutta
		)
...
FROM (
	...
	--kustannusryhmä niille joilta ei löydy osasuorituksia (tai vain tutk. osaa pien. paikalliseen liittyvä)
	outer apply (
		select top 1
		...
		order by coalesce(uusi_eat_koodi, koulutusluokitus_koodi)
	) k3
	...
	left join ANTERO.dw.d_amos_tutkinto_kustannusryhma tkr ON tkr.tutkinto_koodi = k3.koulk and year(oo.pvm) between year(tkr.alkupvm) and year(coalesce(tkr.loppupvm,'9999-12-31'))
...
FROM (
	...
	left join ANTERO.dw.d_amos_tutkinto_kustannusryhma_uusi tkr ON tkr.tutkinto_koodi = k3.koulk and year(oo.pvm) between year(tkr.alkupvm) and year(coalesce(tkr.loppupvm,'9999-12-31'))
