...
/****** Object:  View [dw].[v_amos_opiskelijapalaute_uusi]    Script Date: 7.4.2026 15.08.08 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,[Oppilaitos] = coalesce(d0.organisaatio_fi, ca.organisaatio_fi)
	,[Oppilaitoksen kunta] = d3.kunta_fi
	,[Oppilaitoksen maakunta] = d3.maakunta_fi
	,[Toimipiste] = coalesce(d0b.organisaatio_fi, ca.organisaatio_fi)
	,[Toimipisteen kunta] = d3b.kunta_fi
	,[Toimipisteen maakunta] = d3b.maakunta_fi

	--koodit
	,[Koodit Koulutuksen järjestäjä] = coalesce(d.ytunnus_amos_spl, f.ytunnus)
	,[Koodit Oppilaitos] = f.oppilaitos_koodi
	,[Koodit Toimipiste (OID)] = f.toimipiste_oid
	...
	,[jarjestys Koulutuksen järjestäjän maakunta] = d2.jarjestys_maakunta_koodi
	,[jarjestys Oppilaitoksen maakunta] = d3.jarjestys_maakunta_koodi
	,[jarjestys Toimipisteen maakunta] = d3b.jarjestys_maakunta_koodi
...
LEFT JOIN dw.d_organisaatioluokitus d0 ON d0.organisaatio_koodi = f.oppilaitos_koodi AND d0.organisaatio_avain like 'oppilaitos_%'
LEFT JOIN dw.d_organisaatioluokitus d0b ON d0b.organisaatio_oid = f.toimipiste_oid
...
LEFT JOIN dw.d_alueluokitus d3b ON d3b.kunta_koodi = coalesce(d0b.kunta_koodi, '-1')
LEFT JOIN dw.d_arvotiedonkeruu d4 ON d4.kyselyid = f.kyselyid
LEFT JOIN dw.d_kalenteri d5 ON d5.kalenteri_avain = datefromparts(f.vuosi, f.kuukausi, 1)
CROSS APPLY (select organisaatio_fi, organisaatio_sv, organisaatio_en from dw.d_organisaatioluokitus where id = -1) ca