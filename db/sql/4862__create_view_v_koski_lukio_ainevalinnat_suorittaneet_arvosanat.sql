...
/****** Object:  View [dw].[v_koski_lukio_ainevalinnat_suorittaneet_arvosanat]    Script Date: 20.9.2021 18:47:28 ******/
DROP VIEW IF EXISTS [dw].[v_koski_lukio_ainevalinnat_suorittaneet_arvosanat]
GO
...
CREATE VIEW [dw].[v_koski_lukio_ainevalinnat_suorittaneet_arvosanat] AS
...
--tehdään oma lataus seuraavan jäädytyksen yhteydessä tai kun alkaa tökkiä
  ...
  --lkm
	...
	,aine_S_arvosana_0_1
	,pitka_ma_opiskelija_0_1 
	= max(case when d5.ryhma_koodi = 'luo' and d5.koodi = 'MAA' then 1 else 0 end) 
		over(partition by [paatason_suoritus_id])
