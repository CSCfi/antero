...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_luodut_vastaajatunnukset]    Script Date: 16.10.2024 12:47:29 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT  
		k.kyselykertaid,
		coalesce(k.oppilaitoskoodi, d2.organisaatio_koodi) as oppilaitoskoodi,
	...
	LEFT JOIN ANTERO.dw.d_organisaatioluokitus d on d.organisaatio_koodi = kk.koulutustoimija
	LEFT JOIN ANTERO.dw.d_organisaatioluokitus d2 on d2.ylaorganisaatio_oid = d.organisaatio_oid AND d2.loppupvm is null
	WHERE (kk.tyyppi = 'avop' OR (kk.tyyppi in ('amk-uraseuranta', 'kandipalaute') AND YEAR(vastaajatunnus_alkupvm) > 2017)) AND coalesce(k.oppilaitoskoodi, d2.organisaatio_koodi) is not null
	GROUP BY
	k.kyselykertaid,
	coalesce(k.oppilaitoskoodi, d2.organisaatio_koodi),
	k.tutkintokoodi,
