...
/****** Object:  StoredProcedure [dw].[p_lataa_d_erityisopetus]    Script Date: 21.4.2026 8.29.03 ******/
SET ANSI_NULLS ON
GO
...
USING (
	SELECT TOP 100 PERCENT  koodi as koodi
		...
		,CASE
			WHEN koodi in (1) THEN 'Ei erityisen tuen opiskelija (vuoden 2025 loppuun)'
			WHEN koodi in (2,3) THEN 'Erityisen tuen opiskelija (vuoden 2025 loppuun)'
			WHEN koodi in (4,5,6,7,8,9) THEN 'Vaativan erityisen tuen opiskelija'
			ELSE 'Tieto puuttuu'
		END as ryhma
		,CASE
			WHEN koodi in (1) THEN 'Studerande som inte fått särskilt stöd (till slutet av år 2025)'
			WHEN koodi in (2,3) THEN 'Studerande som fått särskilt stöd (till slutet av år 2025)'
			WHEN koodi in (4,5,6,7,8,9) THEN 'Studerande som fått krävande särskilt stöd'
			ELSE 'Information saknas'
		END AS ryhma_sv
		,CASE
			WHEN koodi in (1) THEN 'Students with no special needs support'
			WHEN koodi in (2,3) THEN 'Students with special needs support'
			WHEN koodi in (4,5,6,7,8,9) THEN 'Students with intensive special needs support'
			ELSE 'Missing data'
		END AS ryhma_en
		,CASE
