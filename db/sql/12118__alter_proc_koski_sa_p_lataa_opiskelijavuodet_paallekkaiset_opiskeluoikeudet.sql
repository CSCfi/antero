...
FROM (
	...
	SELECT 
		...
		,rnk = rank() over (
			partition by nyk_opiskeluoikeus_oid 
			order by opintojen_rahoitus_vos_max desc, voimassa_jakson_alkaessa desc, valiaikainen_kesk_max, aiempi_paattymispvm desc, aiempi_alkamispvm, aiempi_tuva_jarjestamislupa, opiskeluoikeus_oid
		) 
	...
	FROM (
		...
		left join sa.sa_koski_henkilo_master h on h.master_oid = oo_a.master_oid
		...
		inner join (
			...
			select 
				...
				,koulutusmuoto = 
					case 
						when oo.koulutusmuoto in ('ibtutkinto', 'diatutkinto', 'internationalschool', 'europeanschoolofhelsinki') then 'lukiokoulutus'
						else coalesce(oo.koulutusmuoto,'') 
					end
			...
			and (
				...
				or oo.koulutusmuoto = 'perusopetukseenvalmistavaopetus'
				or (oo.koulutusmuoto = 'lukiokoulutus' and ps.suorituksen_tyyppi = 'lukionoppimaara') 
				or oo.koulutusmuoto = 'ibtutkinto'
				or oo.koulutusmuoto = 'diatutkinto'
				or (oo.koulutusmuoto = 'internationalschool' and (ps.suorituksen_tyyppi = 'internationalschooldiplomavuosiluokka' or (ps.suorituksen_tyyppi = 'internationalschoolmypvuosiluokka' and ps.koulutusmoduuli_koodiarvo in ('10', '11', '12'))))
				or (oo.koulutusmuoto = 'europeanschoolofhelsinki' and (ps.suorituksen_tyyppi = 'europeanschoolofhelsinkivuosiluokkasecondaryupper' or (ps.suorituksen_tyyppi = 'europeanschoolofhelsinkivuosiluokkasecondarylower' and ps.koulutusmoduuli_koodiarvo = 'S5')))
			)
			...
			(														--ammatillinen sis. ammatillisen TUVA alkanut ennen nykyistä tai samana päivänä
				...
				or													--perusopetukseen valmistava alkanut ennen nykyistä tai samana päivänä ja ikä alle 17 v.
				(oo_la.alkamispaiva <= oo_a.alkamispaiva and oo_la.koulutusmuoto = 'perusopetukseenvalmistavaopetus' and alkanut_samana_paivana = 0 
					and eomonth(oo_a.pvm) < dateadd(year, 17, h.syntymaaika)
				)
				or
	...
	WHERE opintojen_rahoitus_vos_max = 1
