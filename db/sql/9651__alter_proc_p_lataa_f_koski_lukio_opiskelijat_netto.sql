...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opiskelijat_netto]    Script Date: 23.2.2024 8:18:24 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dw].[p_lataa_f_koski_lukio_opiskelijat_netto] AS
...
SELECT
	...
	,CASE
		...
		WHEN tavoitetutkinto = 'EB-tutkinto (European Baccalaureate)' THEN 3
		WHEN tavoitetutkinto = 'IB-tutkinto (International Baccalaureate)' THEN 4
		WHEN tavoitetutkinto = 'Ei tutkintotavoitetta' THEN 5
		END
	...
	,d_kytkin_oppivelvollinen_id = COALESCE(d23.id, -1)
	,opiskelijat = SUM(ka)
	,loadtime = GETDATE()
...
LEFT JOIN d_kytkin d23 on d23.kytkin_koodi = k.oppivelvollinen
