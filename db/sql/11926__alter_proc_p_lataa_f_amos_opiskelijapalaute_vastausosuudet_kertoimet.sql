...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amos_opiskelijapalaute_vastausosuudet_kertoimet]    Script Date: 7.4.2026 14.27.47 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,katokorjauskerroin_rahoitus = 
		CASE 
			WHEN Rahoituskausi = '01.07.2018 - 30.06.2019' THEN
				...
				END
			WHEN Rahoituskausi in ('01.07.2019 - 30.06.2020', '01.07.2020 - 30.06.2021', '01.07.2021 - 30.06.2022', '01.07.2022 - 30.06.2023', '01.07.2023 - 30.06.2024') THEN
				CASE
			...
			ELSE 
				CASE
					WHEN vastausosuus_rahoitus >= 0.6 THEN 1.0 / vastausosuus_rahoitus
					ELSE (1.0 / 0.60000000000000) + 2.0 * (0.6 - vastausosuus_rahoitus)
				END
		END
	...
	,katokorjauskerroin_arvo =
		CASE 
			...
			ELSE
				CASE
					WHEN vastausosuus_arvo >= 0.6 THEN 1.0 / vastausosuus_arvo
					ELSE (1.0 / 0.60000000000000) + 2.0 * (0.6 - vastausosuus_arvo)
				END
		END
	...
	FROM (
		...
		FROM (
			...
			FROM (
				...
				--arvon tiedot
				SELECT
					coalesce(jl.ytunnus_amos_spl, d1.organisaatio_koodi)	AS 'ytunnus_amos_spl'
					,jl.nimi_amos_spl
				...
				LEFT JOIN dw.d_organisaatioluokitus d1 ON d1.organisaatio_oid = f.koulutustoimija
				LEFT JOIN dw.d_kalenteri d3 ON d3.id = f.d_kalenteri_tunnus_luotu_id
				LEFT JOIN dw.d_amos_spl_jarjestaja_linkki jl ON jl.oid_avain = f.koulutustoimija
				...
				LEFT JOIN (
					...
					LEFT JOIN dw.d_arvotiedonkeruu akk ON akk.kyselyid = f.kyselyid
				...
				and f2.ytunnus_amos_spl = coalesce(jl.ytunnus_amos_spl, d1.organisaatio_koodi) 
				...
				GROUP BY coalesce(jl.ytunnus_amos_spl, d1.organisaatio_koodi), jl.nimi_amos_spl, d3.rahoituskausi_amm, d5.kyselypohja_tarkenne_amis_fi, d5.kyselypohja_tarkenne_fi, f2.kyselyn_kohteet_arvo
