...
SELECT 
  ...
  --FI
	...
	,[aikuisten_perusopetuksen_vaihe_fi] =  
		case vaihe_koodi_priorisoitu 
			when 1.0 then 'Lukutaitovaihe' 
			when 2.0 then 'Alkuvaihe' 
			when 2.5 then 'Lukutaitovaihe tai alkuvaihe (ei tiedossa)'
			when 3.0 then 'Päättövaihe' 
			else 'Vaihe ei tiedossa' 
		end
  ...
  --SV
	...
	,[aikuisten_perusopetuksen_vaihe_sv] =  
		case vaihe_koodi_priorisoitu 
			when 1.0 then 'Läskunnighetsskedet' 
			when 2.0 then 'Inledningsskedet' 
			when 2.5 then 'Läskunnighets- eller inledningsskedet (inte känt)'
			when 3.0 then 'Slutskedet' 
			else 'Skede inte känt' 
		end
  ...
  --KOODIT
	,koodit_opetuksen_jarjestaja = coalesce(nullif(d11.organisaatio_koodi, ''), '-1')
	,koodit_oppilaitos = coalesce(nullif(d9.organisaatio_koodi, ''), '-1')
  ...
  --JARJESTYS
	...
	,jarjestys_aikuisten_perusopetuksen_vaihe = coalesce(vaihe_koodi_priorisoitu, 9.0)
