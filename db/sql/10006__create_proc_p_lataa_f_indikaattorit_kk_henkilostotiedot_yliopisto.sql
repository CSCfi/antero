...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_henkilostotiedot_yliopisto]    Script Date: 23.8.2024 8:07:35 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	LEFT JOIN ANTERO.dw.d_yo d1 ON f.d_yo_id = d1.id
...
USING (
	...
	FROM (
		SELECT
			f.tilastovuosi,
			f.organisaatio_koodi,
			SUM(CAST(f.ulkopuolinen_rahoitus as float)) / SUM(CAST(f.htv as float)) as arvo,
			f.indikaattori
	...
	OUTER APPLY (
		SELECT 
			SUM(CAST(fb.ulkopuolinen_rahoitus as float)) / SUM(CAST(fb.htv as float)) as arvo_sektori
