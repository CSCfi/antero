...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_kvliikkuvuus]    Script Date: 27.4.2018 15:28:41 ******/
SET ANSI_NULLS ON
GO
...
select 
           ...
           case when  AMK like 'Arcada%' then (select id from [ANTERO].[dw].[d_organisaatioluokitus] where organisaatio_koodi = '02535')
		         ...
		         when  not exists (select id from [ANTERO].[dw].[d_organisaatioluokitus] where amk = organisaatio_fi) then '-1'
		         else (select id from [ANTERO].[dw].[d_organisaatioluokitus] where amk = organisaatio_fi)
...
select  
		...
		case when Yliopisto like 'Aalto yli%' then (select id from antero.dw.d_organisaatioluokitus where organisaatio_koodi = '10076')
		  ...
		  when  not exists (select id from [ANTERO].[dw].[d_organisaatioluokitus] where yliopisto = organisaatio_fi and oppilaitostyyppi_koodi = '42') then '-1'
		  else (select id from [ANTERO].[dw].[d_organisaatioluokitus] where yliopisto = organisaatio_fi and oppilaitostyyppi_koodi = '42')
		...
		,'-1' as d_koulutusala_2002_id
		,coalesce(d2.id,'-1') as d_koulutusala_1995_id 
