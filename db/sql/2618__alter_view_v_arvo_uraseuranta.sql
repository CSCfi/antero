...
,case when 
	f.d_arvokysymys_id in (2922, 2924, 2953, 2954, 3060, 3061, 3079, 3081, 3084, 3085) then 'Kyllä' 
...
,case					
			when f.kysymysryhma = 'VI  TYÖ JA OSAAMINEN (3/3)' then 1 / 6.0
			else 1.0
...
,case 
		when f.d_arvokysymys_id in (2922, 2924, 2953, 2954, 3060, 3061, 3079, 3081, 3084, 3085) 
		then numerovalinta+1 
		else numerovalinta							end numerovalinta

,case when f.kysymysryhma in ('II  TYYTYVÄISYYS TUTKINTOON','V  TYÖLLISTYMISEN LAATU') then 'arvosana6' else f.vastaustyyppi  end as vastaustyyppi
,vaihtoehto
,valtakunnallinen
...
,case when f.kyselypohja = 'URASEURANTA-MAISTERI' and  f.d_arvokysymys_id in (2922, 2924, 2953, 2954, 3060, 3061, 3079, 3081, 3084, 3085) and f.numerovalinta=6 then 0 --"Ei rahoitusmallikysymyksiä joissa vastauksena numerovalinta 6 (=Kysymys ei koske minua)
	  when f.kyselypohja = 'URASEURANTA-MAISTERI' then 1 else 0 end as ura_maisteri
...
--and not (kysymysid between 2958 and 2967 and f.kyselypohja='URASEURANTA-MAISTERI') --poistetaan 17.10.2019 koska Jukka haluaa rahoitusmallikysymykset mys vuoden 2017 raportille. 
--and  f.numerovalinta+1<7 		--jos korotettu numerovalinta on 7 kyseessä on vastaus "Kysymys ei koske minua", nämä pois
union all
