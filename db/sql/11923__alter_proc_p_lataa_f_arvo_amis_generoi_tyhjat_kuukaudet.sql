...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amis_generoi_tyhjat_kuukaudet]    Script Date: 7.4.2026 14.56.43 ******/
SET ANSI_NULLS ON
GO
...
select distinct
		...
		,k.kysymysryhmaid
		,kkr.jarjestys
   INTO dw.f_arvo_amis_generoi_tyhjat_kuukaudet_temp
	from Arvo_SA.sa.sa_arvo_v2_kysymysryhmat kkr
	join Arvo_SA.sa.sa_arvo_v2_kyselyt kk on kk.kyselyid = kkr.kyselyid
	join Arvo_SA.sa.sa_arvo_v2_kysymykset k on k.kysymysryhmaid = kkr.kysymysryhmaid
	join (
		...
		from Arvo_SA.sa.sa_arvo_v2_vastaukset_amis v
		join Arvo_SA.sa.sa_arvo_v2_kysymykset k on k.kyselyid=v.kyselyid and k.kysymysid = v.kysymysid and k.kysymysversio = v.kysymysversio
		join Arvo_SA.sa.sa_arvo_v2_kysymysryhmat kkr on kkr.kyselyid = v.kyselyid and kkr.kysymysryhmaid = k.kysymysryhmaid
	) v on v.kysymysid = k.kysymysid and v.kysymysversio = k.kysymysversio and v.kysymysryhmaid = k.kysymysryhmaid
	where kk.kyselytyyppi = 'ammatillisen-opiskelijapalaute'
	and kk.tiedonkeruu not like '%pilotti%'
	and kkr.kategoria = 'valtakunnallinen'
	and k.kysymystyyppi != 'vapaateksti'
	and (k.taustakysymyksen_tyyppi is null or k.taustakysymyksen_tyyppi IN ('kiusaaminen','kiusaamiseen_puuttuminen'))

INSERT INTO dw.f_arvo_amis_generoi_tyhjat_kuukaudet_temp
		select distinct
		kk.kyselyid
		,k.kysymysid
		,k.kysymysversio
	...
	left join dw.f_arvo_amis_generoi_tyhjat_kuukaudet_temp t on t.kyselyid=kk.kyselyid and t.kysymysid=k.kysymysid and t.kysymysversio=k.kysymysversio and t.kysymysryhmaid=k.kysymysryhmaid and t.jarjestys=kkr.jarjestys
...
SELECT
    coalesce(koulutustoimija, d13.organisaatio_oid)
	,coalesce(f1.d_organisaatio_toimipiste_id, d8b.id, -1)
    ...
    ,coalesce(f1.d_alueluokitus_toimipiste_id, d10b.id, -1)
    ...
    ,coalesce(f1.[d_arvokysely_id], d6.id)
    ,'-1'
    ,[vastaajaid]
    ,'ETL: p_lataa_f_arvo_amis_generoi_tyhjat_kuukaudet'
    ,[numerovalinta]
    ,'-1' --suodattuminen estetty kuutiossa. JS 18.2.2026
	,null --suodattuminen estetty kuutiossa. JS 3.12.2025
	,'-1' --suodattuminen estetty kuutiossa. JS 21.1.2025

FROM [ANTERO].[dw].[f_arvo_amis] f1
inner join dw.d_kalenteri d1 on d1.id = f1.d_kalenteri_tunnus_luotu_id
inner join dw.d_arvotiedonkeruu d2 on d2.id = f1.d_arvokysely_id
...
inner join dw.d_organisaatioluokitus d4b on d4b.id = f1.d_organisaatio_toimipiste_id
right join dw.f_arvo_luodut_vastaajatunnukset f2 
	on f2.kyselyid = d2.kyselyid
	and concat(f2.toimipiste_oid, '|', f2.oppilaitoskoodi, '|', f2.ytunnus) = concat(nullif(d4b.organisaatio_oid,'-1'), '|', nullif(d4.organisaatio_koodi,'-1'), '|', d4a.organisaatio_koodi)
...
inner join dw.d_arvotiedonkeruu d6 on d6.kyselyid = f2.kyselyid
...
left join dw.d_organisaatioluokitus d8b on d8b.organisaatio_oid = f2.toimipiste_oid
...
left join dw.d_alueluokitus d10b on d10b.kunta_koodi = d8b.kunta_koodi
...
left join dw.d_arvokysymys d12 on d12.kyselyid=k1.kyselyid and d12.kysymysid = k1.kysymysid and d12.kysymysversio = k1.kysymysversio and d12.kysymysryhmaid = k1.kysymysryhmaid and d12.kysymysryhmajarjestys = k1.jarjestys
left join dw.d_organisaatioluokitus d13 on d13.organisaatio_koodi=f2.ytunnus

WHERE (
	d2.kyselyid is null 
	or coalesce(nullif(d4b.organisaatio_oid,'-1'), nullif(d4.organisaatio_koodi,'-1'), d4a.organisaatio_koodi) is null 
	or d3.koulutusluokitus_koodi is null 
	or d1.vuosi is null 
	or d1.kuukausi is null
)
and d6.kyselytyyppi = 'ammatillisen-opiskelijapalaute'
