...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_korkea_aste_valintatapajonot]    Script Date: 13.8.2024 14:12:53 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	SELECT
		CASE 
			WHEN f.JonoTyyppi is null AND (JonoNimi like 'Avoim%' OR f.JonoNimi like '%Avoin%' OR 
				 d1.haun_nimi_fi like '%Avoi%' OR d1.haun_nimi_fi like '%Öppna%' OR d1.haun_nimi_fi like '%Öppen%' OR
				 d3.selite_fi like '%Avoi%' OR d3.selite_fi like '%Öppen%' OR d3.selite_fi like '%Öppna%' OR d3.selite_fi like '%Open%')
			THEN 'av'
			WHEN f.JonoTyyppi is null AND (d3.selite_fi like '%Rolling%admission%' OR d1.haun_nimi_fi like '%Rolling%admission%' OR
				 d3.selite_fi like '%Rolling%admission%' OR d1.haun_nimi_fi like '%Jatkuva%haku') 
			THEN 'jh'
			ELSE f.JonoTyyppi
		END AS JonoTyyppi,
		f.JonoNimi,
