...
/****** Object:  StoredProcedure [dw].[p_lataa_f_amos_tase_ja_tunnusluvut_kustannuskysely]    Script Date: 8.1.2026 11.09.14 ******/
SET ANSI_NULLS ON
GO
...
SET @query = '
	...
	FROM (
		...
		FROM (
			SELECT * FROM [sa].[sa_amos_kustannuskysely_jarjestajakohtainen_kaikki_vuodet] WHERE omistaja = ''1 Yksityinen'') sa1
		UNPIVOT (
		...
		FROM (
			SELECT * FROM [sa].[sa_amos_kustannuskysely_jarjestajakohtainen_kaikki_vuodet] WHERE omistaja != ''1 Yksityinen'') sa2
		UNPIVOT (
			[maara_euroa] FOR [tilitunnus]
			IN (' + @cols92_eitase + ')
		) tp2
