...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_vakasuhteet_vakapaatokset]    Script Date: 2.6.2026 10.49.48 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE  [dw].[p_lataa_f_varda_vakasuhteet_vakapaatokset]
AS
...
BEGIN
	...
	SELECT
		...
		CASE 
			WHEN (s.vakatoimija_id IS NULL or s.vakatoimija_id = 0) and s.oma_organisaatio_id is null THEN s5.vakajarjestaja_id
	  		WHEN (s.vakatoimija_id IS NULL or s.vakatoimija_id = 0) and s.oma_organisaatio_id is not null THEN s.oma_organisaatio_id
			ELSE s.vakatoimija_id
	...
	FROM varda_sa.sa.sa_varda_lapsi_master_oid s
--	INNER JOIN varda_sa.sa.sa_varda_lapsi s2 on s2.henkilo_id = s.henkilo_id
	INNER JOIN varda_sa.sa.sa_varda_varhaiskasvatuspaatos s3 on s3.lapsi_id = s.lapsi_id
	...
	INNER JOIN varda_sa.sa.sa_varda_organisaatio s6 on s5.vakajarjestaja_id = s6.id
	LEFT JOIN dw.d_sukupuoli d1 on d1.sukupuoli_koodi = s.sukupuoli_koodi
	LEFT JOIN dw.d_alueluokitus d2 on d2.kunta_koodi = case when len(s.kotikunta_koodi) < 3 then concat ('0', s.kotikunta_koodi) else s.kotikunta_koodi end
	...
	LEFT JOIN dw.d_varda_yritysluokitus d9 on d9.koodi = s6.yritysmuoto
	OUTER APPLY (
		SELECT
	...
	WHERE 
	cast(concat(@alkuVuosi,'-12-31') as date) between s3.alkamis_pvm and coalesce(s3.paattymis_pvm, '9999-12-31') AND 
	cast(concat(@alkuVuosi,'-12-31') as date) between s4.alkamis_pvm and coalesce(s4.paattymis_pvm, '9999-12-31') AND
	cast(concat(@alkuVuosi,'-12-31') as date) between s5.alkamis_pvm and coalesce(s5.paattymis_pvm, '9999-12-31') AND
	cast(concat(@alkuVuosi,'-12-31') as date) between s6.alkamis_pvm and coalesce(s6.paattymis_pvm, '9999-12-31') AND
	s.action <> 'DELETED' AND s3.action <> 'DELETED' AND s4.action <> 'DELETED' AND s5.action <> 'DELETED' AND s6.action <> 'DELETED' AND
	s.sukupuoli_koodi is not null AND
	-- Ei kuolinpesiä
	s6.yritysmuoto <> '53'
