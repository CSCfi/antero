...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste]    Script Date: 27.4.2026 14.07.02 ******/
SET ANSI_NULLS ON
GO
...
OUTER APPLY (
	...
	FROM Koski_SA.dbo.ylioppilastutkinnon_suorittaneiden_arvosanat a
...
SELECT 
	...
	NULL as pisteraja,
	h.hakemus_luotu,
	-- updatet insertin jälkeen
	0 as ensi_kertaa_hakeva,
	0 as vastaanottaa_paikan_3v,
...
SELECT
	...
	NULL as ensi_kertaa_hakeva,
	NULL as vastaanottaa_paikan_3v,
	'etl: p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste' as source,
	SUSER_SNAME() as username,
	@loadtime as loadtime
...
-- Ensi kertaa hakevien päättely
SELECT
	h.hakemus_oid,
	h.master_oid,
	RANK() OVER (PARTITION BY master_oid ORDER BY h.koulutuksen_alkamisvuosi, h.koulutuksen_alkamiskausi, h.hakemus_luotu) as hakukerta_kk
INTO ANTERO.sa.temp_ensi_kertaa_hakeneet
FROM ANTERO.sa.sa_ovara_hakeneet h
WHERE h.kk_haku = 1

UPDATE f
SET ensi_kertaa_hakeva = 1
FROM ANTERO.dw.f_haku_ja_valinta_hakeneet_korkea_aste f
WHERE f.aidosti_ensikertainen = 1 and EXISTS (
	SELECT TOP 1 hakemus_oid 
	FROM ANTERO.sa.temp_ensi_kertaa_hakeneet ekh 
	WHERE ekh.hakukerta_kk = 1 and ekh.hakemus_oid = f.hakemus_oid
)

DROP TABLE IF EXISTS ANTERO.sa.temp_ensi_kertaa_hakeneet

-- Vastaanottaa korkeakoulupaikan kolmen vuoden sisällä
UPDATE f 
SET vastaanottaa_paikan_3v = 1
FROM ANTERO.dw.f_haku_ja_valinta_hakeneet_korkea_aste f
LEFT JOIN ANTERO.dw.d_kausi d on d.id = f.d_kausi_koulutuksen_alkamiskausi_id
OUTER APPLY (
	SELECT TOP 1 vastaanottanut_paikan
	FROM ANTERO.sa.sa_ovara_hakeneet vkkp3
	WHERE vkkp3.kk_haku = 1 and vkkp3.vastaanottanut_paikan = 1 and vkkp3.master_oid = f.master_oid
	and cast(concat(vkkp3.koulutuksen_alkamisvuosi,'.', case when vkkp3.koulutuksen_alkamiskausi = 's' then '5' else 0 end) as float) between cast(concat(f.koulutuksen_alkamisvuosi,'.', case when d.koodi = 's' then '5' else 0 end) as float) and cast(concat(f.koulutuksen_alkamisvuosi,'.', case when d.koodi = 's' then '5' else 0 end) as float) + 2.5
) AS vkkp3 
