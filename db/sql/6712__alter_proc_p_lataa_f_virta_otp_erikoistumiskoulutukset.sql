...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_erikoistumiskoulutukset]    Script Date: 1.12.2022 12:40:58 ******/
SET ANSI_NULLS ON
GO
...
TRUNCATE TABLE [ANTERO].[dw].[f_virta_otp_erikoistumiskoulutukset]

WHILE @alkuVuosi <= YEAR(GETDATE())

BEGIN

	IF (SELECT COUNT(*) FROM ANTERO.INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = concat('sa_virta_otp_erikoistumiskoulutukset_', cast(@alkuVuosi as nvarchar))) >= 1

	BEGIN

	SET @taulu = concat('ANTERO.sa.sa_virta_otp_erikoistumiskoulutukset_', cast(@alkuVuosi as nvarchar))
	...
	SET @sql = 
		...
		' year((select d from dbo.BigintToDate(sa.tutkinnonsuorituspvm))) as muun_tutkinnon_suoritusvuosi,' +
		' 1 as lkm,' +
		...
		' LEFT JOIN ANTERO.dw.d_kalenteri d6 on d6.kalenteri_avain = (select d from dbo.BigintToDate(sa.opiskeluoikeudenalkamispaivamaara))' +
		' LEFT JOIN ANTERO.dw.d_kalenteri d7 on d7.kalenteri_avain = (select d from dbo.BigintToDate(sa.tutkinnonsuorituspvm))' +
		...
		' (CASE ' +
		' WHEN (select d from dbo.BigintToDate(sa.tutkinnonsuorituspvm)) > (select d from dbo.BigintToDate(sa.opiskeluoikeudenalkamispaivamaara)) THEN 1' +
		' ELSE 0' +
		' END)' +
		...
		' ELSE 0' +
		' END)' + 
		' WHERE (cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) = 2021 and d6.vuosi <= 2021) or (cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int) <> 2021 and d6.vuosi = cast(' + cast(@alkuVuosi as nvarchar(4)) + ' as int))'
