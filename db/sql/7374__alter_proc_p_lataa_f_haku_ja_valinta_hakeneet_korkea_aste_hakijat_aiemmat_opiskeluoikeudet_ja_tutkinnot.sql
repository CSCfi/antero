...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste_hakijat_aiemmat_opiskeluoikeudet_ja_tutkinnot]    Script Date: 20.2.2023 8:27:45 ******/
SET ANSI_NULLS ON
GO
...
) AS OP ON OP.henkilo_oid = f.henkilo_oid AND ((OP.opisk_alkamisPvm <= coalesce(d4.loppuPvm, d4b.loppuPvm) and OP.tavoitetutkinto <> d5.koulutusluokitus_koodi) or (DATEDIFF(month, OP.opisk_alkamisPvm, coalesce(d4.loppuPvm, d4b.loppuPvm)) <= 2 AND OP.tavoitetutkinto = d5.koulutusluokitus_koodi))
--tutkinnot
LEFT JOIN (
