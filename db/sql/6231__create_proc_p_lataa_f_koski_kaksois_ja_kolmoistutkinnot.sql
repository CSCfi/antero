...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dw].[p_lataa_f_koski_kaksois_ja_kolmoistutkinnot]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dw].[p_lataa_f_koski_kaksois_ja_kolmoistutkinnot] AS' 
END
GO
...
BEGIN
	...
	FROM (
		...
		LEFT JOIN [Koski_SA].[sa].[sa_ytr_update] ytr on ytr.oppija_oid = oo.oppija_oid and left(ytr.certificate,4) = YEAR(@alkuPvm) and 
	...
	SELECT  
		...
		coalesce(d6a.id,'-1') as d_organisaatioluokitus_amm_id,
		coalesce(d6b.id,'-1') as d_organisaatioluokitus_lukio_id,
		coalesce(d6c.id,'-1') as d_organisaatioluokitus_yo_id,
		coalesce(d7a.id,'-1') as d_organisaatioluokitus_koulutustoimija_amm_id,
		coalesce(d7b.id,'-1') as d_organisaatioluokitus_koulutustoimija_lukio_id,
		coalesce(d7c.id,'-1') as d_organisaatioluokitus_koulutustoimija_yo_id,
	...
	) amm on amm.oppija_oid = h.oppija_oid
	LEFT JOIN (
		SELECT DISTINCT
			oo.oppija_oid,
			oo.opiskeluoikeus_oid,
	...
	) luk on luk.oppija_oid = h.oppija_oid
	LEFT JOIN (
		SELECT DISTINCT
	...
	) yot on yot.oppija_oid = h.oppija_oid
	...
	LEFT JOIN [ANTERO].[dw].[d_kalenteri] d1d on d1d.paivays = yot.vahvistus_paiva
	...
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d6a on d6a.organisaatio_oid = amm.oppilaitos_oid
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d6b on d6b.organisaatio_oid = luk.oppilaitos_oid
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d6c on d6c.organisaatio_oid = yot.school_oid and d6c.organisaatio_avain not like 'koulutustoimija%'
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d7a on d7a.organisaatio_oid = amm.koulutustoimija_oid
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d7b on d7b.organisaatio_oid = luk.koulutustoimija_oid
	LEFT JOIN [ANTERO].[dw].[d_organisaatioluokitus] d7c on (d7c.organisaatio_oid = d6c.ylaorganisaatio_oid and d7c.organisaatio_avain like 'koulutustoimija_%') or (d6c.organisaatio_oid = yot.school_oid and d6c.organisaatio_avain like 'koulutustoimija%')
	LEFT JOIN [Koski_SA].[sa].[p_lataa_temp_koski_lukio_kurssit_summa] kurssit on kurssit.oppija_oid = h.oppija_oid
