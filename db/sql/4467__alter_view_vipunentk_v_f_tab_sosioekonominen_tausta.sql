...
/****** Object:  View [dbo].[v_f_tab_sosioekonominen_tausta]    Script Date: 6.3.2021 0:09:48 ******/
SET ANSI_NULLS ON
GO
...
LEFT JOIN d_koulutusluokitus d2 on d2.id = koulutusluokitus_aiti_id
LEFT JOIN d_koulutusluokitus d3 on d3.id = koulutusluokitus_isa_id
LEFT JOIN d_koulutuslaji d4 on d4.id = koulutuslaji_id
LEFT JOIN d_sukupuoli d5 on d5.id = sukupuoli_id
LEFT JOIN d_opetushallinnon_koulutus d6 on d6.id = opetushallinnon_koulutus2_id
LEFT JOIN d_aidinkieli_versio1 d7 on d7.id = aidinkieli_versio1x_id
LEFT JOIN d_kausi d8 on d8.kausi_id = syntymavuosi_id
LEFT JOIN d_kieli d9 on d9.id = koulutuksen_kieli_id
LEFT JOIN d_alueluokitus d10 on d10.id = koulutuksen_maakunta_id
LEFT JOIN d_alueluokitus d11 on d11.id = kotikunta_id
LEFT JOIN d_alueluokitus d12 on d12.id = kotikunta1x_id
LEFT JOIN d_tulodesiili d13 on d13.id = tulodesiili_aiti_id
LEFT JOIN d_tulodesiili d14 on d14.id = tulodesiili_isa_id
LEFT JOIN d_sosioekonominen_asema d15 on d15.id = sosioekonominen_asema_1_aiti_id
LEFT JOIN d_sosioekonominen_asema d16 on d16.id = sosioekonominen_asema_2_aiti_id
LEFT JOIN d_sosioekonominen_asema d17 on d17.id = sosioekonominen_asema_1_isa_id
LEFT JOIN d_sosioekonominen_asema d18 on d18.id = sosioekonominen_asema_2_isa_id
LEFT JOIN d_ika d19 on d19.id = ika_1v_id
--LEFT JOIN d_ika d20 on d20.id = ika_5v_id
LEFT JOIN d_opiskelijan_olo d21 on d21.id = opiskelijan_olo_syys_id
LEFT JOIN d_kausi d22 on d22.kausi_id = kirjoihintulovuosi_id
