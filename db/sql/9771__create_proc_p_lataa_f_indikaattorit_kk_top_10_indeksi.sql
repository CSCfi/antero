...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_top_10_indeksi]    Script Date: 17.4.2024 12:27:20 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SET @sql2 = 'SELECT @n2 = COUNT(*) FROM Bibliometriikka.INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = ''FactSuomi4Vuotta'' and TABLE_SCHEMA = CONCAT(''SCKuutio'',' + @alkuVuosi + ')'
	...
	USING (
		...
		FROM (
			SELECT 
				CAST(RIGHT(d2.Nelivuotisjakso,4) as int) + 3 as tilastovuosi,
				d3.organisaatio_nykyinen_koodi as organisaatio_koodi,
				SUM(f.[Top10]) / SUM(f.[JulkaisutT10]) as arvo,
				''top10'' as Indikaattori
			FROM [Bibliometriikka].[SCKuutio' + @alkuvuosi + '].[FactSuomi4Vuotta] f
			INNER JOIN [Bibliometriikka].[ScKuutio' + @alkuvuosi + '].[DimOrg] d1 on d1.Organisaationumero = f.Organisaationumero
			INNER JOIN [Bibliometriikka].[ScKuutio' + @alkuvuosi + '].[Dim4Vuosi] d2 ON f.ScVuosikoodi4 = d2.ScVuosikoodi4
			LEFT JOIN ANTERO.dw.d_organisaatioluokitus  d3 on d3.organisaatio_koodi = d1.Oppilaitostunnus
			WHERE CAST(RIGHT(d2.Nelivuotisjakso,4) as int) + 3 >= 2018 and d3.oppilaitostyyppi_koodi = ''42'' and d2.ScVuosikoodi4 < (' + @alkuvuosi + ' - 2004)
			GROUP BY CAST(RIGHT(d2.Nelivuotisjakso,4) as int) + 3, d3.organisaatio_nykyinen_koodi
		) f
		OUTER APPLY (
			SELECT 
				SUM(fb.[Top10]) / SUM(fb.[JulkaisutT10]) as arvo_sektori
			FROM [Bibliometriikka].[SCKuutio' + @alkuvuosi + '].[FactSuomi4Vuotta] fb
			INNER JOIN [Bibliometriikka].[ScKuutio' + @alkuvuosi + '].[DimOrg] d1b on d1b.Organisaationumero = fb.Organisaationumero
			INNER JOIN [Bibliometriikka].[ScKuutio' + @alkuvuosi + '].[Dim4Vuosi] d2b ON fb.ScVuosikoodi4 = d2b.ScVuosikoodi4
			LEFT JOIN ANTERO.dw.d_organisaatioluokitus  d3b on d3b.organisaatio_koodi = d1b.Oppilaitostunnus
			WHERE d3b.oppilaitostyyppi_koodi = ''42'' and d2b.ScVuosikoodi4 < (' + @alkuvuosi + ' - 2004) and
			CAST(RIGHT(d2b.Nelivuotisjakso,4) as int) + 3 = f.tilastovuosi
		) s
	) AS src
