...
/****** Object:  StoredProcedure [dw].[p_lataa_api_korkeakoulutus_hakeneet_ja_paikan_vastaanottaneet_esilataus]    Script Date: 16.7.2026 8.54.32 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT
	...
	,[valintatapajonon_tyyppi] = d47.selite_fi
	...
	,[koulutuksen_kieli] = d27.selite_fi
	...
	,case when f.loadtime>=d1.valitut and f.valittu = 1 then f.pisteet ELSE null end kokonaispisteet
	,f.master_oid as henkilo_oid
	,case when hakutoive='1' then 1 else 0 end as ensisijainen_hakija						
	,case when f.loadtime>=d1.valitut then f.valittu else 0 end as valittu
	,case when f.loadtime>=d1.vastaanottaneet then f.vastaanottanut_paikan else 0 end as paikan_vastaanottanut			
	,case when f.loadtime>=d1.aloittaneet then f.ilmoittautunut else 0 end as aloittanut
...
SELECT
	...
	,[opintoala_2002] = d20.opintoala2002_fi
	,[paaasiallinen_tutkinto_hakukohde] = d20.koulutusluokitus_fi
	,[koulutuksen_kieli] = d7.selite_fi
...
LEFT JOIN ANTERO.dw.d_koulutusluokitus d20 on d20.id=f.d_koulutusluokitus_id
LEFT JOIN ANTERO.dw.d_oppilaitoksenopetuskieli d7 on d7.id=f.d_oppilaitoksenopetuskieli_koulutuksen_kieli_id
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d8b on d8b.id=d_organisaatioluokitus_oppilaitos_id
LEFT JOIN ANTERO.dw.d_organisaatioluokitus d8c on d8c.id=d_organisaatioluokitus_toimipiste_id
LEFT JOIN ANTERO.dw.d_tutkinnon_aloitussykli_kk d44 on d44.id=f.d_tutkinnon_aloitussykli_kk_id
where (f.koulutuksen_alkamisvuosi > 2016 OR (f.koulutuksen_alkamisvuosi = 2016 and d9.selite_fi = 'Syksy')) and d1.korkeakouluhaku = 1
and f.master_oid is null
