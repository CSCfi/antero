...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_ainekelpoisuudet]    Script Date: 19.9.2022 18:31:08 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT DISTINCT
		...
		case when d.patevyys_fi like '%musiikki%' then 'Musiikkipedagogi' else left(d.patevyys_fi, len(d.patevyys_fi) - charindex(',', reverse(d.patevyys_fi))) end as aine,
		coalesce(nullif(ltrim(rtrim(replace(reverse(substring(reverse(d.patevyys_fi),0,charindex(',',reverse(d.patevyys_fi)))),'op.',''))),''), 'Tieto puuttuu') as laajuus,
		tut.suorituspaivamaara as paivamaara_tutkinto,
		kl.koulutusluokitus_koodi,
		case 
