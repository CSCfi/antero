...
/****** Object:  StoredProcedure [dw].[p_lataa_d_koskiyokokeet]    Script Date: 3.6.2026 12.42.08 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dw].[p_lataa_d_koskiyokokeet]
AS
	...
	FROM (
		SELECT DISTINCT
			...
			CASE
				...
				WHEN nimi like '%oppimäärä%' THEN LEFT(replace(nimi,' erivapaudella',''), CHARINDEX(',', replace(nimi,' erivapaudella','')) - 1)
				WHEN nimi like '%äidinkiel%' THEN SUBSTRING(replace(nimi,' erivapaudella',''), CHARINDEX(', ', replace(nimi,' erivapaudella','')) + 2, LEN(replace(nimi,' erivapaudella','')))
				WHEN nimi like '%toisena kielenä%' THEN replace(nimi, ' toisena kielenä','') 
			...
			CASE
				WHEN nimi like '%äidinkiel%' THEN 'äidinkieli'
				WHEN nimi like '%keskipitkä%' THEN 'keskipitkä'
				WHEN nimi like '%pitkä%' THEN 'pitkä'
				WHEN nimi like '%lyhyt%' THEN 'lyhyt'
				WHEN nimi like '%laajempi%' THEN 'laajempi'
				WHEN nimi like '%toisena%' THEN 'toisena kielenä'
				ELSE NULL 
			END laajuus_fi,
			NULL as laajuus_sv,
			NULL as laajuus_en,
			CASE
				...
				WHEN nimi like '%oppimäärä%' or nimi like '%kiel%' THEN 'Kieli'
				ELSE 'Reaali'
...
WHEN MATCHED
	THEN
		...
		SET aine_selite_fi = src.aine_selite_fi
			...
			,aineryhma_selite_fi = src.aineryhma_selite_fi
			,aineryhma_selite_sv = src.aineryhma_selite_sv
			,aineryhma_selite_en = src.aineryhma_selite_en
