IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.f_amk_talous_6_liiketoiminnan_tuloslaskelma') AND type in (N'U'))
BEGIN
CREATE TABLE dbo.f_amk_talous_6_liiketoiminnan_tuloslaskelma(
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__loadt__3493CFA7') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_6_liiketoiminnan_tuloslaskelma ADD  DEFAULT (getdate()) FOR loadtime
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__usern__3587F3E0') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_6_liiketoiminnan_tuloslaskelma ADD  DEFAULT (suser_sname()) FOR username
END