IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.d_tili') AND type in (N'U'))
BEGIN
CREATE TABLE dbo.d_tili(
...
INSERT dbo.d_tili (tili_koodi, tili_taso0_fi, tili_taso1_fi, tili_taso2_fi, tili_taso3_fi, tili_taso4_fi, tili_taso0_sv, tili_taso1_sv, tili_taso2_sv, tili_taso3_sv, tili_taso4_sv, tili_taso0_en, tili_taso1_en, tili_taso2_en, tili_taso3_en, tili_taso4_en, loadtime, source, username) VALUES (N'-1', N'Tuntematon', N'Tuntematon', N'Tuntematon', N'Tuntematon', N'Tuntematon', N'Okänd', N'Okänd', N'Okänd', N'Okänd', N'Okänd', N'Unknown', N'Unknown', N'Unknown', N'Unknown', N'Unknown', CAST(N'2016-11-22T00:00:00.000' AS DateTime), N'manual', N'dwi\jrouhiai')
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__d_tili__loadtime__4D2A7347') AND type = 'D')
BEGIN
ALTER TABLE dbo.d_tili ADD  CONSTRAINT DF__d_tili__loadtime__4D2A7347  DEFAULT (getdate()) FOR loadtime
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__d_tili__username__4E1E9780') AND type = 'D')
BEGIN
ALTER TABLE dbo.d_tili ADD  CONSTRAINT DF__d_tili__username__4E1E9780  DEFAULT (suser_sname()) FOR username
END