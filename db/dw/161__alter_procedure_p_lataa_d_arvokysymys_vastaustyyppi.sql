ALTER PROCEDURE [dbo].[p_lataa_d_arvokysymys] AS
if not exists (select * from dbo.d_arvokysymys where id=-1) begin
  set identity_insert dbo.d_arvokysymys on;
  insert into dbo.d_arvokysymys (
    id,
  ...
  and koodi='-1'
  ;
  set identity_insert dbo.d_arvokysymys off;
end else begin
  update d
  set
