...
if not exists (select * from dbo.d_alueluokitus where id=-1) begin
  set identity_insert dbo.d_alueluokitus on;
  insert into dbo.d_alueluokitus (
    id,
    alueluokitus_avain,
    kunta_koodi,
    kunta_fi,
  ...
  and koodi='-1'
  ;
  set identity_insert dbo.d_alueluokitus off;
end else begin
  update d
  ...
  from dbo.d_alueluokitus d
  join ANTERO_SA.dbo.sa_koodistot s on s.koodi=d.alueluokitus_avain
