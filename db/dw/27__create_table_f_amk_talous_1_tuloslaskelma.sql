IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.f_amk_talous_1_tuloslaskelma') AND type in (N'U'))
BEGIN
CREATE TABLE dbo.f_amk_talous_1_tuloslaskelma(
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__loadt__236943A5') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_1_tuloslaskelma ADD  DEFAULT (getdate()) FOR loadtime
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__usern__245D67DE') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_1_tuloslaskelma ADD  DEFAULT (suser_sname()) FOR username
END