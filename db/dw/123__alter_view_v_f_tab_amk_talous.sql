...
SELECT d1.amk_tunnus AS 'Ammattikorkeakoulun tunnus'
  ...
  ,f.arvo
  FROM f_amk_talous f
INNER JOIN ANTERO_DW.dbo.d_amk d1 ON f.d_amk_id = d1.id
INNER JOIN ANTERO_DW.dbo.d_ohjauksenala d2 ON f.d_ohjauksenala_id = d2.id
INNER JOIN ANTERO_DW.dbo.d_tili d3 ON f.d_tili_id = d3.id
INNER JOIN ANTERO_DW.dbo.d_aineistotyyppi d4 ON f.d_aineistotyyppi_id = d4.id
INNER JOIN ANTERO_DW.dbo.d_toiminto d5 ON f.d_toiminto_id = d5.id
INNER JOIN ANTERO_DW.dbo.d_erittely d6 ON f.d_erittely_id = d6.id
