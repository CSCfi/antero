IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.f_amk_talous_3_kulut_ja_tuotot') AND type in (N'U'))
BEGIN
CREATE TABLE dbo.f_amk_talous_3_kulut_ja_tuotot(
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__loadt__2BFE89A6') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_3_kulut_ja_tuotot ADD  DEFAULT (getdate()) FOR loadtime
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__usern__2CF2ADDF') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_3_kulut_ja_tuotot ADD  DEFAULT (suser_sname()) FOR username
END