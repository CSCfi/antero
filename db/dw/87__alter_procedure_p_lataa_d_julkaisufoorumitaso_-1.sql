...
if exists (select * from dbo.d_julkaisufoorumitaso where id>=0 and julkaisufoorumitaso_koodi='-1') begin
  delete from dbo.d_julkaisufoorumitaso where id>=0 and julkaisufoorumitaso_koodi='-1'
end
if not exists (select * from dbo.d_julkaisufoorumitaso where id=-1) begin
  set identity_insert dbo.d_julkaisufoorumitaso on;
  insert into dbo.d_julkaisufoorumitaso (
    id,
  ...
  and koodi='-1'
  ;
  set identity_insert dbo.d_julkaisufoorumitaso off;
end else begin
  update d
  ...
  from dbo.d_julkaisufoorumitaso d
  join ANTERO_SA.dbo.sa_koodistot s on s.koodi=d.julkaisufoorumitaso_koodi
