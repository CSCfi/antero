IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.f_amk_talous_2_tase') AND type in (N'U'))
BEGIN
CREATE TABLE dbo.f_amk_talous_2_tase(
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__loadt__2645B050') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_2_tase ADD  DEFAULT (getdate()) FOR loadtime
END

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.DF__f_amk_tal__usern__2739D489') AND type = 'D')
BEGIN
ALTER TABLE dbo.f_amk_talous_2_tase ADD  DEFAULT (suser_sname()) FOR username
END