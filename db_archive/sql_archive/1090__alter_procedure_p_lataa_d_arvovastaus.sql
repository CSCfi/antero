...
if not exists (select * from dw.d_arvovastaus where id=-1) begin
  ...
  and koodi='-1'
  ;
  set identity_insert dw.d_arvovastausys off;
end else begin
  update d
  set
    vuosi = s.koodi,
...
USING (
  ...
  vuosi
  vastaus_koodi,
  COALESCE(vastaus_fi,vastaus_sv,vastaus_en) vastaus_fi,
  COALESCE(vastaus_sv,vastaus_fi,vastaus_en) vastaus_sv,
  COALESCE(vastaus_en,vastaus_fi,vastaus_sv) vastaus_en,
  kysymysryhmaid,
  vastaustyyppi,
  'ETL: p_lataa_d_arvovastaus' AS source
  FROM sa.sa_suorat_kooodistot_arvo_vastaukset
  ) AS src
...
WHEN MATCHED THEN
  UPDATE SET
    ...
    vastaus_fi = src.nimi,
    vastaus_sv = src.nimi_sv,
    vastaus_en = src.nimi_en,
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    src.vuosi,
    src.koodi, src.vastaus_fi, src.vastaus_sv, src.vastaus_en,
    src.kysymysryhmaid,
    src.vastaustyyppi,
    src.source
  );
