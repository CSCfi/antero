...
/****** Object:  View [dbo].[v_sa_2_4efg_Lukio_ja_amm_koulutuksen_tutkinnot]    Script Date: 25.8.2023 14:57:29 ******/
SET ANSI_NULLS ON
GO
...
/*
JS 7.10.2022:
Pyrit‰‰n sopv23 tullessa lis‰‰m‰‰n ik‰ tilastovuonna kaikkiin sa-n‰kymiin (2.4h, ja 2.4cg sopv22-) ja muokkaamaan jatkolataukset k‰ytt‰m‰‰n sit‰. 
Nyt 2.4h:ssa iki‰ ohjautuu jonkin verran tuntemattomaksi alvv puuttumisen takia, ja 2.4g k‰ytet‰‰n teknisen‰ arvona tutalv = tilv - 3 kirjoihintuloi‰n puuttuessa.
*/
...
SELECT
	...
	,ophal = cast(
		case 
			...
			when lahde < 30 then 
				case 
					when coalesce(coalesce(d.koulutuksen_jarjestajan_korvaavakoodi,d.koulutuksen_jarjestajakoodi), '') = '0145076-7' --'≈lands Landskapsregering'
					then '3' --Muiden hallinnonalojen alainen koulutus
					else '1' --Opetushallinto, pl. tyˆvoimakoulutus
				end
			--amm
			when ophal = '1' then '1'
			when ophal = '3' then '2' --Opetushallinto, tyˆvoimakoulutus
			when ophal = '2' 
				and tilv < 2021
				and (
					rahoitus = '10'
					or (
						...
						and coalesce(coalesce(d.koulutuksen_jarjestajan_korvaavakoodi, d.koulutuksen_jarjestajakoodi), '') not in (
							'0145076-7', '2153105-4', '150622-3', '0909306-3' --'≈lands Landskapsregering','Poliisiammattikorkeakoulu','Rikosseuraamusalan Koulutuskeskus','Pelastusopisto'
						)
					)
				)
				then '2'
	...
	,tavoite =  cast(coalesce(nullif(kaikkivuodet.tavoite,''),'-1')  as nvarchar(2))   -- HUOM! t‰ss‰ aineistossa tulkitaan tyhj‰t tutkinnon tavoitteet tarkoituksella arvoon "1 - koko tutkinnon suorittaminen" kuutiom‰‰ritelm‰n mukaan -- Lasse
...
SELECT
      ...
      ,[lkm]
      ,[aikoul]
      ,[tutalv] = alvv --ik‰laskentaa varten
      ,[tutklaja]
      ,[tunim]
      ,[sutunim] = NULL
      ,[rutunim] = NULL
...
SELECT
	  ...
	  ,[tietolahde]='K2_4h_sopv_21'
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K2_4h_2020_sopv_21] --t‰m‰kin mukaan
WHERE toteuma_tutk is not null
UNION ALL
SELECT
	   [tilv]
...
SELECT
      ...
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K2_4h_sopv_22] --t‰m‰kin mukaan
WHERE toteuma_tutk is not null 
UNION ALL
SELECT
	   [tilv]
      ...
      ,[lkm]
      ,[aikoul]
      ,[tutalv] = case when ikar2_kirjv = 'XX' then cast(tilv - 3 as varchar) else cast(0 + tilv + ikar2_kirjv - ikar2 as varchar) end --ik‰laskentaa varten
      ,[tutklaja] = NULL
      ,[tunim] = NULL
      ,[sutunim] = NULL
      ,[rutunim] = NULL
...
SELECT
      ...
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
FROM [TK_H9098_CSC].[dbo].[TK_K2_4h_sopv_23] --t‰m‰kin mukaan
WHERE toteuma_tutk is not null 
UNION ALL
SELECT
	   [tilv]
