...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_henkilosto]    Script Date: 19.8.2024 12:34:24 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER   PROCEDURE [dw].[p_lataa_f_varda_henkilosto]
AS
...
BEGIN
	...
	FROM varda_sa.sa.sa_varda_tyoskentelypaikka s
/*LEFT*/ JOIN [Varda_SA].[sa].[sa_varda_palvelussuhde] s4 on  s.palvelussuhde_id = s4.id
/*LEFT*/ JOIN [Varda_SA].[sa].[sa_varda_tyontekija] s2 on s4.tyontekija_id = s2.id
...
/*LEFT*/ JOIN [Varda_SA].[sa].[sa_varda_organisaatio] s8 on s2.vakajarjestaja_id = s8.id
	LEFT JOIN [Varda_SA].[sa].[sa_varda_toimipaikka] s7 on s.toimipaikka_id = s7.toimipaikka_id

	
	LEFT JOIN dw.d_ika d on d.ika_avain = DATEDIFF(YEAR, s2.syntyma_pvm, CAST(CONCAT(@alkuVuosi, '-01-01') as date))
	LEFT JOIN dw.d_varda_tehtavanimike d2 on d2.koodi = s.tehtavanimike_koodi
	LEFT JOIN dw.d_kytkin d3 on d3.kytkin_koodi = replace(replace(s.kelpoisuus_kytkin, 'f', 0), 't', 1)
	LEFT JOIN dw.d_varda_tyosuhde d4 on d4.koodi = s4.tyosuhde_koodi
	LEFT JOIN dw.d_varda_tyoaika d5 on d5.koodi = s4.tyoaika_koodi
	...
	LEFT JOIN dw.d_varda_maaraaikaisuuden_kesto_luokitus d12 on d12.koodi = 
		case 
			when s4.tyosuhde_koodi <> '2' then '-1'
	...
	AND s4.alkamis_pvm <= CAST(CONCAT(@alkuVuosi, '-12-31') as date) 
	and (s4.paattymis_pvm >= CAST(CONCAT(@alkuVuosi, '-12-31') as date) 
	or s4.paattymis_pvm is NULL)
...
--	AND s7.action <> 'DELETED'
--	AND s8.action <> 'DELETED'

	AND NOT EXISTS 
	(
	SELECT id FROM Varda_sa.sa.sa_varda_pidemmatpoissaolot 
	WHERE palvelussuhde_id = s4.id 
--	AND s.tyosuhde_koodi = '1' 
	AND alkamis_pvm <= CAST(CONCAT(@alkuVuosi, '-12-31') as date) AND paattymis_pvm >= CAST(CONCAT(@alkuVuosi, '-12-31') as date)
	)	
