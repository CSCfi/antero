...
/****** Object:  Table [sa].[sa_virta_otp_tktutkintoennakko_2018]    Script Date: 17.1.2024 14:22:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[sa].[sa_virta_otp_tktutkintoennakko_2018]') AND type in (N'U') AND (SELECT COUNT(*) FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2018) > 0)
DROP TABLE [sa].[sa_virta_otp_tktutkintoennakko_2018]
GO
...
IF (SELECT COUNT(*) FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2018) > 0

BEGIN

CREATE TABLE [sa].[sa_virta_otp_tktutkintoennakko_2018](
...
INSERT INTO [sa].[sa_virta_otp_tktutkintoennakko_2018]
SELECT 
	[aikielir1x],
	[aineisto],
	[allk],
	...
	[source],
	[username]
FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2018
