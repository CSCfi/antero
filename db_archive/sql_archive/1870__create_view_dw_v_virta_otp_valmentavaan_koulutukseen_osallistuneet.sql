...
AS
		SELECT   			 		   
			...
			, [Valmentava oikeus aloitusvuosi]   =  YEAR(convert(date,dateadd(s, convert(bigint, f.vooalkamispaivamaara) , convert(datetime, '1-1-1970')))  )
			...
			, [Tutkinto oikeus aloitusvuosi]   =  YEAR(convert(date,dateadd(s, convert(bigint, f.tooalkamispaivamaara)  , convert(datetime, '1-1-1970')))  )
			...
			--, [Tutkinnon suorittamisvuosi] = f.ttutkintosuorituspvm		-- date			
		 	, [Tutkinnon suorittamisvuosi]   =  YEAR(convert(date,dateadd(s, convert(bigint, f.ttutkintosuorituspvm) , convert(datetime, '1-1-1970')))  ) 							 
