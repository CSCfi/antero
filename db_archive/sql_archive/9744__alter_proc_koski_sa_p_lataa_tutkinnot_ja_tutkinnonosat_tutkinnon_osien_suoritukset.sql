...
SELECT
	...
	COALESCE(
		CASE
			...
			when laajuus_puuttuu_eperusteista=1 and [koulutusmoduuli_laajuus_arvo] > 20 then 20
		...
		END
		,ep_tutkinnonosa_laajuus
		,0
	) AS koulutusmoduuli_laajuus_arvo
		...
		laajuus_puuttuu_eperusteista = 
			CASE
				...
				WHEN os.koulutusmoduuli_koodisto_ylempi = 'tutkinnonosatvalinnanmahdollisuus' 
					AND os.suorituksen_tyyppi = 'ammatillinenkorkeakouluopintoja' THEN 1
				WHEN os.koulutusmoduuli_koodisto_ylempi = 'tutkinnonosatvalinnanmahdollisuus' 
					AND os.suorituksen_tyyppi in ('ammatillinenlukionopintoja','ammatillinenmuitaopintovalmiuksiatukeviaopintoja','ammatillisentutkinnonosanosaalue') THEN 2
				*/
				--kaatoluokka 
...
INTO #temp_to_kr
FROM [sa].[temp_tutkinnot_ja_tutkinnonosat_tutkinnon_osien_suoritukset] ost
