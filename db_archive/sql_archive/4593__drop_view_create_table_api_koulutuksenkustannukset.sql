...
/****** Object:  Table [dw].[api_koulutuksenkustannukset]    Script Date: 7.5.2021 1:03:59 ******/

if exists
(
  select 1 from sys.views v 
  inner join sys.[schemas] s on v.schema_id = s.schema_id 
  where s.name = 'api' and v.name = 'koulutuksenkustannukset' and v.type = 'v'
)
drop view api.koulutuksenkustannukset
go

DROP TABLE IF EXISTS [api].[koulutuksenkustannukset]
GO
...
CREATE TABLE [api].[koulutuksenkustannukset](
 ...
 CONSTRAINT [PK__api_koulutuksenkustannukset_defaultorder] PRIMARY KEY CLUSTERED 
(
	[defaultorder] ASC
