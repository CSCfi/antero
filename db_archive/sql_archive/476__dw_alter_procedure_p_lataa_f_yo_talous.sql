...
(select yliop_koodi
  ...
  unpivot (arvo for tili in
      ...
      ,aptemka
      ,aptever)) as unpvt
	  where aineistotyyppi_osa_1 = case when tilik = (select max(tilik) from sa.sa_suorat_yo_talous_1_tuloslaskelma) 
		then case when (select count(*) from sa.sa_suorat_yo_talous_1_tuloslaskelma where tilik = (select max(tilik) from sa.sa_suorat_yo_talous_1_tuloslaskelma) and aineistotyyppi_osa_1 = 'L' and yliop_koodi = unpvt.yliop_koodi) > 0 then 'L' else 'E' end
		else 'L'
	  end
union all
select tilik
...
(select tilik
  ...
  unpivot (arvo for tili in
      ...
      ,okrrjaps
      ,okrmuutv)) as unpvt
	  where aineistotyyppi_osa_1 = case when tilik = (select max(tilik) from sa.sa_suorat_yo_talous_2_tase) 
		then case when (select count(*) from sa.sa_suorat_yo_talous_2_tase where tilik = (select max(tilik) from sa.sa_suorat_yo_talous_2_tase) and aineistotyyppi_osa_1 = 'L' and yliop_koodi = unpvt.yliop_koodi) > 0 then 'L' else 'E' end
		else 'L'
	  end
union all
select tilik
...
(select tilik
  ...
  (varastojen_lisays_tai_vahennys
	  ...
	  where aineistotyyppi = case when tilik = (select max(tilik) from sa.sa_suorat_yo_talous_6_liiketoiminnan_tuloslaskelma) 
		then case when (select count(*) from sa.sa_suorat_yo_talous_6_liiketoiminnan_tuloslaskelma where tilik = (select max(tilik) from sa.sa_suorat_yo_talous_6_liiketoiminnan_tuloslaskelma) and aineistotyyppi = 'L' and yliop_koodi = unpvt.yliop_koodi) > 0 then 'L' else 'E' end
		else 'L'
	  end) f
