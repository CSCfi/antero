...
SELECT 
	...
	,p.patevyyskoodi
	,p.patevyysnimi
	,kelpoisuus_koodi_alku = p.kelpoisuuskoodi
	...
	,rnk = row_number() over (
		partition by h.henkilo, (case when p.patevyysryhma='opettaja' then p1.koodi else p.patevyysnimi end) 
		...
		vuosi =
			case 
				when p.patevyysryhma='opettaja' then 
					case
						when kelpoisuuskoodi in ('VA','EL') then tutk_tyypit.alempi_kk
...
OUTER APPLY (
	...
	from (
		...
		left join dw.d_virta_patevyydet vp2 on vp2.patevyys_koodi=up2.patevyyskoodi
		left join (
			select 
				henkilo
				,amk = min(case when kl.koulutusastetaso2_koodi >= 62 then vuosi end)
		...
		cross apply (
			select
				vuosi =
					case 
						when vp.patevyysryhma1_fi ='opettaja' then 
						case
							when map.kelpoisuuskoodi in ('VA','EL') then tutk_tyypit2.alempi_kk
							else tutk_tyypit2.ylempi_kk
						end
						when vp.patevyysryhma1_fi='opettajaaineet' then 
						case
							when ol.oppilaitostyyppi_koodi='41' then tutk_tyypit2.amk
							else tutk_tyypit2.ylempi_kk
						end
					end		
		) tutk2
...
WHERE 1=1 and (p1.koodi like concat('%',p.kelpoisuuskoodi, '%') or p.patevyysryhma <> 'opettaja')
