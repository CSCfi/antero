...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste]    Script Date: 10.2.2020 15:09:27 ******/
SET ANSI_NULLS ON
GO
...
left join (
	...
	and f.HakutoiveNumero = 1) d35 on d35.HenkiloOID = f.HenkiloOID and d35.HakuOid = f.HakuOid
left join (
	SELECT
		f.HenkiloOID,
		f.HakuOid,
		d2.koulutusalataso1_koodi,
		MIN(f.HakutoiveNumero) as hakutoive,
		CASE WHEN MIN(f.HakutoiveNumero) = '1' THEN '1' ELSE d3.id END as id
	FROM sa.sa_odw_hakeneet f
	...
	WHERE f.ValinnanTila in ('HYVAKSYTTY','VARASIJALTA_HYVAKSYTTY','PERUNUT')
	GROUP BY f.HenkiloOID, f.HakuOid, d2.koulutusalataso1_koodi, d3.id) d36 on d36.HenkiloOID = f.HenkiloOID and d36.HakuOid = f.HakuOid
