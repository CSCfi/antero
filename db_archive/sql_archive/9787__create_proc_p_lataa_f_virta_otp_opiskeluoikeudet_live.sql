...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_opiskeluoikeudet_live]    Script Date: 17.4.2024 14:56:04 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_virta_otp_opiskeluoikeudet_live] AS

TRUNCATE TABLE [ANTERO].[dw].[f_virta_otp_opiskeluoikeudet_live] 

INSERT INTO [ANTERO].[dw].[f_virta_otp_opiskeluoikeudet_live] 
SELECT
	...
	coalesce(d3.id, -1) as d_kytkin_usea_oo_samalta_alalta,
	coalesce(d4.id, -1) as d_kytkin_usea_oo_samasta_ammattikorkeakoulusta,
	coalesce(d5.id, -1) as d_kytkin_usea_oo_samasta_yliopistosta,
	coalesce(d6.id, -1) as d_kytkin_yliopisto_ja_ammattikorkeakoulu,
	...
	getdate() as loadtime,
	'ETL: p_lataa_f_virta_otp_opiskeluoikeudet_live' as source,
