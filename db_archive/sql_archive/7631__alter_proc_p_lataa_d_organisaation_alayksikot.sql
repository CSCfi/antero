...
/****** Object:  StoredProcedure [dw].[p_lataa_d_organisaation_alayksikot]    Script Date: 24.4.2023 15:34:00 ******/
SET ANSI_NULLS ON
GO
...
USING (
	...
	GROUP BY [vuosi] ,[Korkeakoulun_koodi] ,COALESCE([Paayksikon_koodi], '-1'), [Alayksikon_koodi], 'etl: p_lataa_d_organisaation_alayksikot (' + [IMP_CREATED_BY] +')'
	) AS src
...
MERGE dw.d_organisaation_alayksikot AS target
		USING (
			...
			,'etl: p_lataa_d_organisaation_alayksikot (' + [IMP_CREATED_BY] +')'AS source
			FROM sa.sa_suorat_amk8a_alayksikkokoodisto where [IMP_CREATED_BY] != 'Initial load'
			GROUP BY [vuosi],[Korkeakoulun_koodi],COALESCE([Paayksikon_koodi], '-1'), COALESCE([Alayksikon_koodi], '-1'), 'etl: p_lataa_d_organisaation_alayksikot (' + [IMP_CREATED_BY] +')'
		) AS src
		ON target.vuosi = src.vuosi
			AND target.korkeakoulu_koodi = src.korkeakoulun_koodi
			AND target.alayksikko_koodi = src.alayksikon_koodi
