...
/****** Object:  StoredProcedure [dbo].[p_lataa_opiskelijavuodet_valitaulu]    Script Date: 26.9.2023 14:06:02 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	EXEC sa.p_lataa_opiskelijavuodet_opiskeluoikeus_aikajakso_opva 
	...
	SELECT 
		...
		,kustannusryhma = 
			CASE 
				...
				WHEN ps.suorituksen_tyyppi = 'tutkinnonosaapienemmistäkokonaisuuksistakoostuvasuoritus' THEN 
					CASE WHEN liittyy_tutkinnonosaan_1_0 = 1 THEN otkr.kr ELSE '2' END
		...
		,tutkosaa_pienempi_tt = 
			CASE 
				WHEN liittyy_tutkinnonosaan_1_0 = 1 THEN coalesce(tp.tutkintotyyppi, '02') --eperusteissa ei ainakaan kaikkia yhteisiä
	...
	CROSS APPLY (
		/*TUVA-koulutuksesta päällekkäisissä huomioidaan alustavasti vain [tuva_jarjestamislupa]='ammatillinen', siksi alla riittää [aiempi_koulutusmuoto]='tuva'. JS 27.9.2022*/
		SELECT 
			hylatty_opiskelijavuosi_loma =					case
																when coalesce(nullif(ooa.aiempi_koulutusmuoto,''),'ammatillinenkoulutus') in ('ammatillinenkoulutus','tuva') and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 and ooa.alkanut_samana_paivana_hylataan = 1 then 0.0
			/* samana päivänä alkaneet, kokonaan hylättävät */	when ooa.aiempi_koulutusmuoto in ('ammatillinenkoulutus','tuva') and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 and valiaikainen_keskeytys = 0 then 0.0
			/* aikaisemmin alkaneet, päättyvät */				when ooa.aiempi_koulutusmuoto in ('ammatillinenkoulutus','tuva') and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 and valiaikainen_keskeytys = 0 then 0.0
			...
			,hylatty_opiskelijavuosi_paallekkainen_amm =	case
																...
																when coalesce(nullif(ooa.aiempi_koulutusmuoto,''),'ammatillinenkoulutus') in ('ammatillinenkoulutus','tuva') and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 and ooa.alkanut_samana_paivana_hylataan = 1 then (ooa.lasna_pv_hyv_ei_muokattu + ooa.loma_pv_hyv_ei_muokattu + opva_pv_hyv_ei_muokattu) 
			/* samana päivänä alkaneet, kokonaan hylättävät */	when ooa.aiempi_koulutusmuoto in ('ammatillinenkoulutus','tuva') and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 then ((ooa.lasna_pv_hyv_ei_muokattu - ooa.lasna_pv_hyv) + ooa.loma_pv_hyv_ei_muokattu + (ooa.opva_pv_hyv_ei_muokattu - ooa.opva_pv_hyv))  
			/* aikaisemmin alkaneet, päättyvät */				when ooa.aiempi_koulutusmuoto in ('ammatillinenkoulutus','tuva') and ooa.sisaltyy_toiseen_opiskeluoikeuteen = 0 then ((ooa.lasna_pv_hyv_ei_muokattu - ooa.lasna_pv_hyv) + (ooa.loma_pv_hyv_ei_muokattu - ooa.loma_pv_hyv) + (ooa.opva_pv_hyv_ei_muokattu - ooa.opva_pv_hyv)) 
																else 0.0 
															end
