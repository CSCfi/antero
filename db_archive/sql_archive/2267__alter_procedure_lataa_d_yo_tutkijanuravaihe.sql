...
BEGIN
	SET IDENTITY_INSERT dw.d_yo_tutkijanuravaihe ON;
		INSERT INTO dw.d_yo_tutkijanuravaihe (
			id,
			koodi,
			selite_fi,
			selite_sv,
		...
		SELECT
			...
			'etl: p_lataa_d_yo_tutkijanuravaihe'
...
ELSE
	BEGIN
		...
		SET
			...
			loadtime = GETDATE(),
			username = SUSER_NAME(),
			source = 'etl: p_lataa_d_yo_tutkijanuravaihe'
		FROM dw.d_yo_tutkijanuravaihe d
...
USING (
	...
	FROM sa.sa_koodistot
	WHERE koodisto = 'suorattutkijanuravaihe'
	order by koodi
	) AS src
