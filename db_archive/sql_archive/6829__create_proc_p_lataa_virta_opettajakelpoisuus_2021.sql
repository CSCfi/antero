...
/****** Object:  StoredProcedure [sa].[p_lataa_virta_opettajakelpoisuus_2021]    Script Date: 4.1.2023 13:36:51 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [sa].[p_lataa_virta_opettajakelpoisuus_2021] AS
...
-- TEMP-tauluun kombinaatiot kelpoisuuskoodeista sekä kelpoisuuskoulujen korkeakoulu ja pvm
	...
	into #temp_ope_pat1
	from [sa].[sa_virta_otp_ope_patevyydet_2021] pat
	...
	left join (
		...
		from [sa].[sa_virta_otp_ope_patevyydet_2021] s
	...
	SELECT 
		...
		rnk = row_number() over (
			...
			order by pvm, pvm1, pvm2, aine_pvm)
	into #temp_ope_pat2
	FROM (
		SELECT DISTINCT
			henkilo,
			CASE 
				WHEN kelpoisuuskoodi1 = 'OP' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','EL') THEN 'OP'
				WHEN kelpoisuuskoodi1 = 'ER' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','EL') THEN 'ER'
				WHEN kelpoisuuskoodi1 in ('AI','JA','JB','JC','EL') AND ainepatevyys is not null AND ainepatevyys not in ('gf','ms') THEN 'AI'
				WHEN kelpoisuuskoodi1 = 'PE' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','EL') THEN 'PE'
			...
			CASE 
				WHEN kelpoisuuskoodi1 = 'OP' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','EL') THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'ER' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','EL') THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
				WHEN kelpoisuuskoodi1 in ('AI','JA','JB','JC','EL') AND ainepatevyys is not null AND ainepatevyys not in ('gf','ms') THEN 
					CASE WHEN pvm1 < aine_pvm THEN aine_pvm ELSE pvm1 END
				WHEN kelpoisuuskoodi1 = 'PE' AND kelpoisuuskoodi2 in ('AI','JA','JB','JC','EL') THEN
					CASE WHEN pvm1 < pvm2 THEN pvm2 ELSE pvm1 END
			...
			aine_pvm,
			CASE
				WHEN pvm1 > pvm2 and nimi1 like '%erilliset%' THEN 2
				WHEN pvm2 >= pvm1 and nimi2 like '%erilliset%' THEN 2
				ELSE 1
			END as suoritustapa
		FROM #temp_ope_pat1 
		...
		WHERE ainepatevyys in ('gf','ms') and kelpoisuuskoodi1 in ('AI', 'JA', 'JB', 'JC', 'EL')) f
...
INTO sa.virta_opettajakelpoisuus_2021

FROM #temp_ope_pat2 p
left join dw.d_organisaatioluokitus ol on ol.organisaatio_koodi=p.organisaatiokoodi
left join sa.sa_virta_otp_ope_henkilotiedot_2021 h on h.henkilo=p.henkilo
...
left join (
	...
	from [sa].[sa_virta_otp_ope_tutkinnot_2021] t
...
outer apply (
	...
	from (
		...
		from [sa].[sa_virta_otp_ope_tutkinnot_2021] t2
...
OUTER APPLY (
	Select
		CASE 
			WHEN p1.koodi = 'VA_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('VA','PE','AI','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'AI_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'ER_PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','ER','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'VA_ER' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('EL','VA', 'ER') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'EL' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('EL','VA', 'ER') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'OP' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('OP','AI','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'ER' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('ER','AI','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'PE' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi = 'AI' THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('PE','AI','ER','OP','JA','JB','JC','EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
			WHEN p1.koodi in('PD', 'LA') THEN REPLACE((SUBSTRING((SELECT '_' + patevyyskoodi AS 'data()' FROM ANTERO.sa.virta_opettajapatevyys_mappaus WHERE kelpoisuuskoodi in ('VA','PE','AI','ER','OP','JA','JB','JC', 'EL') FOR XML PATH('')  ), 2 , 9999)),' ', '')
...
OUTER APPLY (
	...
	FROM (
		...
		FROM ANTERO.sa.sa_virta_otp_ope_patevyydet_2021 ya_pat
