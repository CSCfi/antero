...
SELECT
	...
	,d_organisaatioluokitus_hk_oppilaitos_id = COALESCE(d12b.id,-1)
	...
	,d_organisaatioluokitus_hk_toimipiste_id = COALESCE(d18b.id,-1)
...
FROM (
	...
	SELECT DISTINCT
		...
		,hankintakoulutuksen_toimipiste_oid = case when hankintakoulutus = 1 then toimipiste_oid_tt end
		,hankintakoulutuksen_oppilaitos_oid = case when hankintakoulutus = 1 then oppilaitos_oid_tt end
		...
		,toimipiste_oid = case when oo_tt.tutk_tt = 1 then toimipiste_oid_tt else toimipiste_oid end
...
LEFT JOIN dw.d_organisaatioluokitus d12b ON d12b.organisaatio_oid = k.hankintakoulutuksen_oppilaitos_oid
...
LEFT JOIN dw.d_organisaatioluokitus d18b ON d18b.organisaatio_oid = k.hankintakoulutuksen_toimipiste_oid
