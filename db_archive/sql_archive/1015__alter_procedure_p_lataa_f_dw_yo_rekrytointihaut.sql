...
INSERT INTO dw.f_yo_rekrytointihaut
	...
	,[d_hakijan_maanosa_id]
	,[d_hakijan_maanosa2_id]
	,[vuosi]
	,[hakijat_lkm]
	,[loadtime]
	,[source]
...
LEFT JOIN [dw].[d_kytkin] s8 ON s8.kytkin_koodi = (
		SELECT CASE
				WHEN [kansalaisuus] = '0'
					THEN '1'
				WHEN [kansalaisuus] = '1'
						THEN '0'
				ELSE '-1'
				END AS kansalaisuus_id)
LEFT JOIN [dw].[d_maatjavaltiot2] s9 ON  s9.maanosa0_koodi = (
		SELECT CASE
				...
				ELSE '-1'
			END AS maanosa_id)
LEFT JOIN [dw].[d_maatjavaltiot2] s10 ON  s10.maanosa_koodi = (
			SELECT CASE
					WHEN [ei-suomalaisen hakijan maanosa] = '1'
						THEN '1'
					WHEN [ei-suomalaisen hakijan maanosa] = '2'
