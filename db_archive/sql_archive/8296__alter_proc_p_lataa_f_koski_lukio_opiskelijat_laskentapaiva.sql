...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opiskelijat_laskentapaiva]    Script Date: 2.11.2023 17:50:34 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	FROM (
		...
		SELECT  
			...
			,CASE 
				WHEN (kuukausi = 1 AND paivays > CASE WHEN ooa.alku > lps.alku THEN ooa.alku ELSE lps.alku END AND ooa.loppu IS NULL AND ps.vahvistus_paiva IS NULL) THEN 1
			...
			,CASE 
				WHEN (kuukausi = 9 AND paivays > CASE WHEN ooa.alku > lps.alku THEN ooa.alku ELSE lps.alku END AND ooa.loppu IS NULL AND ps.vahvistus_paiva IS NULL) THEN 1
