...
/****** Object:  StoredProcedure [dbo].[p_lataa_TK_Koski_lukio_ainevalinnat_suorittaneet]    Script Date: 4.10.2023 17:57:07 ******/
SET ANSI_NULLS ON
GO
...
,[aine] = --kielet luokitellaan "vähimmäislaajuuden" mukaan
	case 
		...
		when (aine = ''B1'' AND ((aine_laajuus_yksikko = ''4'' and coalesce(aine_laajuus,0) < 5 and kurssit_lkm_total < 5) or (aine_laajuus_yksikko = ''2'' and aine_laajuus < 10))) 
		  OR (aine not in (''B1'', ''AOM'') AND ((aine_laajuus_yksikko = ''4'' and coalesce(aine_laajuus,0) < 6 and kurssit_lkm_total < 6) or (aine_laajuus_yksikko = ''2'' and aine_laajuus < 12)))
		then aine + ''X''
		else aine
	end
,aineenoppimaara_koodisto
