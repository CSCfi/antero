USE [ANTERO]
GO
/****** Object:  View [dw].[v_varda_varhaiskasvatustiedot]    Script Date: 15.9.2022 16:23:54 ******/
DROP VIEW IF EXISTS  [dw].[v_varda_varhaiskasvatustiedot]
GO
/****** Object:  View [dw].[v_varda_vakajarjestajat_toimipaikat]    Script Date: 15.9.2022 16:23:54 ******/
DROP VIEW IF EXISTS  [dw].[v_varda_vakajarjestajat_toimipaikat]
GO
/****** Object:  View [dw].[v_varda_vakajarjestajat_toimipaikat]    Script Date: 15.9.2022 16:23:54 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dw].[v_varda_vakajarjestajat_toimipaikat] AS
		...
		SELECT
			...
			coalesce(vtpa.nimi_fi, 'Ei painotusta') as toimipaikan_toiminnallinen_painotus_fi,
			coalesce(vtpa.nimi_sv, 'Ingen betoning') as toimipaikan_toiminnallinen_painotus_sv,
			coalesce(vtpa.nimi_en, 'No emphasis') as toimipaikan_toiminnallinen_painotus_en,
...
CREATE VIEW [dw].[v_varda_varhaiskasvatustiedot] AS
 ...
 select * from (
	...
	case when sukup.sukupuoli_koodi = 1 then 'Male'
	  ...
	  tunti_ast.jarj_nro as jarjestys_tuntimaara_asteikko,
	  ikar.jarj_nro as jarjestys_ikaryhma,
	  tm.jarjestys as jarjestys_toimintamuoto,
	  ...
	  ml.jarjestysnumero as jarjestys_maksun_luokka,

	  case 
		when left(ml.kuvaus_fi, 6) in ('0-290 '/*€*/, '291-40'/*0 €*/, '401-50'/*0 €*/, 'Yli 50'/*0 €*/) 
		then 1 
		else 0 
...
select
	...
	where va.ika < 11 and va.tilastointivuosi > 2020 and al.maakunta not like 'Ahvenanmaa%'
	group by
