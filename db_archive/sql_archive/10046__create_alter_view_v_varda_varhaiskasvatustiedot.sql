...
/****** Object:  View [dw].[v_varda_varhaiskasvatustiedot]    Script Date: 19.8.2024 12:46:27 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER VIEW [dw].[v_varda_varhaiskasvatustiedot] AS
 ...
 select * from (
	  ...
	  ikar.ikaryhma7_sv as ikaryhma_sv,	  
	   ...
	   case when ikar.ikaryhma7_fi = '3 - 5 vuotta' then '3 vuotta täyttäneet'
	  when ikar.ikaryhma7_fi = '6 vuotta' then '3 vuotta täyttäneet'
	  when ikar.ikaryhma7_fi = '7 vuotta ja yli' then '3 vuotta täyttäneet'
	  else ikar.ikaryhma7_fi 
	  end as ikaryhma_suppea_fi,
	  case when ikar.ikaryhma7_en = '3 - 5 years' then '3 years and up'
	  when ikar.ikaryhma7_en = '6 years' then '3 years and up'
	  when ikar.ikaryhma7_en = 'Older than 6 years' then '3 years and up'
	  else ikar.ikaryhma7_en 
	  end as ikaryhma_suppea_en,
	   case when ikar.ikaryhma7_sv = '3 - 5 år' then '3 år och över'
	  when ikar.ikaryhma7_sv = '6 år' then '3 år och över'
	  when ikar.ikaryhma7_sv = '7 år och över' then '3 år och över'
	  else ikar.ikaryhma7_sv 
	  end as ikaryhma_suppea_sv, 
	...
	case when sukup.sukupuoli_koodi = 1 then 'Male'
	  ...
	  case when va_kytkin2.kytkin_fi = 'Tieto puuttuu' then 'Vuorohoito'
	  else va_kytkin2.kytkin_fi 
	  end as kokopaivainen_varhaiskasvatus_fi,

	  case when va_kytkin2.kytkin_sv = 'Information saknas' then 'Skiftomsorg'
	  else va_kytkin2.kytkin_sv 
	  end as kokopaivainen_varhaiskasvatus_sv,

	  case when va_kytkin2.kytkin_en = 'Information missing' then 'Vuorohoito_EN'
	  else va_kytkin2.kytkin_en 
	  end as kokopaivainen_varhaiskasvatus_en,
...
--	  va_kytkin2.kytkin_sv as kokopaivainen_varhaiskasvatus_sv,
	  ...
	  case when va_kytkin3.kytkin_fi = 'Tieto puuttuu' then 'Vuorohoito'
	  else va_kytkin3.kytkin_fi 
	  end as paivittainen_varhaiskasvatus_fi,

	  case when va_kytkin3.kytkin_sv = 'Information saknas' then 'Skiftomsorg'
	  else va_kytkin3.kytkin_sv 
	  end as paivittainen_varhaiskasvatus_sv,

	  case when va_kytkin3.kytkin_en = 'Information missing' then 'Vuorohoito_EN'
	  else va_kytkin3.kytkin_en 
	  end as paivittainen_varhaiskasvatus_en,
...
--	  va_kytkin3.kytkin_sv as paivittainen_varhaiskasvatus_sv,
	  ...
	  tp_alue.kuntaryhma_fi as toimipaikan_kuntaryhma_fi,
	  tp_alue.kuntaryhma_sv as toimipaikan_kuntaryhma_sv,
	  tp_alue.kuntaryhma_en as toimipaikan_kuntaryhma_en,
	  ...
	  case when tm.nimi_fi = 'Ryhmäperhepäivähoito' then 'Perhepäivähoito (sis. ryhmäperhepäivähoito)'
	  when tm.nimi_fi = 'Perhepäivähoito' then 'Perhepäivähoito (sis. ryhmäperhepäivähoito)'
	  else tm.nimi_fi
	  end as toimintamuoto_suppea_fi,
	  case when tm.nimi_en = 'Ryhmäperhepäivähoito' then 'Perhepäivähoito_EN'
	  when tm.nimi_en = 'Perhepäivähoito' then 'Perhepäivähoito_EN'
	  else tm.nimi_en
	  end as toimintamuoto_suppea_en,
	  case when tm.nimi_sv = 'Gruppfamiljedagvård' then 'Familjedagvård (inkl. Gruppfamiljedagvård)'
	  when tm.nimi_sv = 'Familjedagvård' then 'Familjedagvård (inkl. Gruppfamiljedagvård)'
	  else tm.nimi_sv
	  end as toimintamuoto_suppea_sv,
	  ...
	  case when ikar.jarjestys_ikaryhma7 = 2 then 2
	  when ikar.jarjestys_ikaryhma7 = 3 then 2
	  when ikar.jarjestys_ikaryhma7 = 4 then 2
	  else ikar.jarjestys_ikaryhma7
	  end as jarjestys_ikaryhma_suppea,
	  tm.jarjestys as jarjestys_toimintamuoto,
	  case when tm.jarjestys = 3 then 2
	  else tm.jarjestys
	  end as jarjestys_toimintamuoto_suppea,
	  ...
	  ht.cum_sum_asiakasmaksu as apusarake_kumulatiivinen_summa_asiakasmaksu,
	  ht.cum_sum_palveluseteli_arvo as apusarake_kumulatiivinen_summa_palveluseteli,
	  ...
	  --varhaiskasvatustiedot
		...
		--Filters not NULL
		where vaka.tilastovuosi >= 2021 and vaka.tilastovuosi <= 2023 
...
select
	...
	case
		when va.ika < 3 then				(select	kuvaus_fi from dw.d_varda_ikaryhma where id = 1)
		when va.ika >= 3 then				'3 vuotta täyttäneet'	
	end as ikaryhma_suppea_fi,
		case
		when va.ika < 3 then				(select	kuvaus_en from dw.d_varda_ikaryhma where id = 1)
		when va.ika >= 3 then				'3 years and up'		
	end as ikaryhma_suppea_en,
		case
		when va.ika < 3 then				(select	kuvaus_sv from dw.d_varda_ikaryhma where id = 1)	
		when va.ika >= 3 then				'3 år och över'		
	end as ikaryhma_suppea_sv,
	...
	al.maakunta_FI		as	  vakajarjestaja_maakunta_fi,
	al.maakunta_EN		as	  vakajarjestaja_maakunta_sv,
	al.maakunta_SV		as	  vakajarjestaja_maakunta_en,
	...
	al.kuntaryhma_FI as toimipaikka_kuntaryhma_fi,
	al.kuntaryhma_SV as toimipaikka_kuntaryhma_sv,
	al.kuntaryhma_EN as toimipaikka_kuntaryhma_en,
	...
	'Päiväkoti' as	  toimintamuoto_suppea_fi,
	'Päiväkoti' as	  toimintamuoto_suppea_en,
	'Daghem' as	  toimintamuoto_suppea_sv,
	...
	case	when va.ika < 3				then (select jarj_nro from dw.d_varda_ikaryhma where id = 1)		
			when va.ika >= 3				then (select jarj_nro from dw.d_varda_ikaryhma where id = 2)
	end as jarjestys_ikaryhma_suppea,
	1 as jarjestys_toimintamuoto,
	1 as jarjestys_toimintamuoto_suppea,
	...
	null as apusarake_kumulatiivnen_summa_asiakasmaksu,
	null as apusarake_kumulatiivnen_summa_palveluseteli,
	null as tilastovuosi_oid,
	sum(lukumaara) as vaesto,
	0 as tilastointi,
	apusarake_yritystyyppi_1_0 =  1,
	case when va.tilastointivuosi = 2021 then 0 else 1 end as apusarake_vuosi
	...
	group by
	va.tilastointivuosi, al.kunta_fi, al.kunta_en, al.kunta_sv, al.maakunta_fi, al.maakunta_en, al.maakunta_sv, al.kuntaryhma_fi, al.kuntaryhma_en, al.kuntaryhma_sv, al.kunta_koodi, va.ika, d.aidinkieli_versio2, d.aidinkieli_versio2_EN, d.aidinkieli_versio2_SV,
	case when d.aidinkieli_versio2 not in ('suomi', 'ruotsi') then 'muu kieli'
