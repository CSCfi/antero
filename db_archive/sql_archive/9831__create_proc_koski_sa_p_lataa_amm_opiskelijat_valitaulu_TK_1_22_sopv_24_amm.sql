...
/****** Object:  StoredProcedure [dbo].[p_lataa_amm_opiskelijat_valitaulu_TK_1_22_sopv_24_amm]    Script Date: 10.5.2024 18:08:38 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dbo].[p_lataa_amm_opiskelijat_valitaulu_TK_1_22_sopv_24_amm] AS

--Ajetaan kertaluonteisesti talteen aineiston 1.22 sopv24 poimintaa varten, tässä versiossa päällekkäinen tuva-opiskelu ei rajattu vaan liputettu.
--Varsinainen aineistopoiminta latauksessa sa.p_lataa_TK_1_22_sopv_24_amm.

DECLARE @alkuPvm as date, @loppuPvm as date, @endOfLoop as date

SET @alkuPvm = '2023-01-01' --'2018-01-01'
SET @loppuPvm = EOMONTH(@alkuPvm)
SET @endOfLoop = '2023-12-31'

DROP TABLE IF EXISTS sa.temp_amm_koski_opiskeluoikeus_TK_1_22_sopv_24_amm
DROP TABLE IF EXISTS sa.temp_amm_eka_lasna_jakso_TK_1_22_sopv_24_amm
DROP TABLE IF EXISTS sa.temp_amm_naytto_opisk_TK_1_22_sopv_24_amm
...
SELECT 
	...
	oo.alkamispaiva
INTO sa.temp_amm_koski_opiskeluoikeus_TK_1_22_sopv_24_amm
...
INTO sa.temp_amm_eka_lasna_jakso_TK_1_22_sopv_24_amm
...
INTO sa.temp_amm_naytto_opisk_TK_1_22_sopv_24_amm
FROM sa.sa_koski_opiskeluoikeus_aikajakso
GROUP BY opiskeluoikeus_oid


TRUNCATE TABLE [dbo].[amm_opiskelijat_valitaulu_TK_1_22_sopv_24_amm]

WHILE @alkuPvm <= @endOfLoop --DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0)

BEGIN

	DROP TABLE IF EXISTS sa.temp_amm_opiskelijat_sopimusjaksot_TK_1_22_sopv_24_amm
	
	SELECT DISTINCT 
		 opiskeluoikeus_oid
		,sopimusmuoto
	INTO sa.temp_amm_opiskelijat_sopimusjaksot_TK_1_22_sopv_24_amm
	...
	BEGIN
		...
		DROP TABLE IF EXISTS sa.temp_amm_aiempi_oikeus_sama_jarj_TK_1_22_sopv_24_amm
		...
		INTO sa.temp_amm_aiempi_oikeus_sama_jarj_TK_1_22_sopv_24_amm
		FROM (
			...
			LEFT JOIN sa.temp_amm_naytto_opisk_TK_1_22_sopv_24_amm n_o on n_o.opiskeluoikeus_oid = oo3.opiskeluoikeus_oid and n_o.naytto = 0
	...
	INSERT INTO [dbo].[amm_opiskelijat_valitaulu_TK_1_22_sopv_24_amm] 
	...
	SELECT --distinct
		...
		,samanaikainen_tuva = case when tuvat.oppija_oid is null then 0 else 1 end
	...
	FROM (
		...
		SELECT 
			...
			,oppivelvollinen = case when ov.alkuPvm <= @alkuPvm and ov.loppuPvm > @alkuPvm then 1 else 0 end
	
		FROM sa.temp_amm_koski_opiskeluoikeus_TK_1_22_sopv_24_amm oo
		JOIN sa.sa_koski_henkilo h on h.oppija_oid = oo.oppija_oid
		...
		LEFT JOIN sa.temp_amm_opiskelijat_sopimusjaksot_TK_1_22_sopv_24_amm os on os.sopimusmuoto like '%oppisopimu%' and os.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
		LEFT JOIN sa.temp_amm_opiskelijat_sopimusjaksot_TK_1_22_sopv_24_amm ks on ks.sopimusmuoto = 'Koulutussopimus' and ks.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
	...
	--muut opiskeluoikeudet samalla järjestäjällä
	LEFT JOIN sa.temp_amm_aiempi_oikeus_sama_jarj_TK_1_22_sopv_24_amm oo2 on oo2.opiskeluoikeus_oid = q.opiskeluoikeus_oid and oo2.r_per = 1 and oo2.aiempi_perus = 1

	--muut opiskeluoikeudet samalla järjestäjällä (rahoitus)
	LEFT JOIN sa.temp_amm_aiempi_oikeus_sama_jarj_TK_1_22_sopv_24_amm oo3 on oo3.opiskeluoikeus_oid = q.opiskeluoikeus_oid and oo3.r_rah = 1 and oo3.aiempi_rahoitus = 1

	--suoraan näyttöön
	LEFT JOIN sa.temp_amm_naytto_opisk_TK_1_22_sopv_24_amm n_o on n_o.opiskeluoikeus_oid = q.opiskeluoikeus_oid

	--eka läsnäjakso
	LEFT JOIN sa.temp_amm_eka_lasna_jakso_TK_1_22_sopv_24_amm ooa2 on ooa2.opiskeluoikeus_oid = q.opiskeluoikeus_oid

	LEFT JOIN sa.tuva_lasna_loma_jaksot tuvat 
		on q.suorituksen_tyyppi != 'tuvakoulutuksensuoritus'
...
DROP TABLE sa.temp_amm_koski_opiskeluoikeus_TK_1_22_sopv_24_amm
DROP TABLE sa.temp_amm_opiskelijat_sopimusjaksot_TK_1_22_sopv_24_amm
DROP TABLE sa.temp_amm_eka_lasna_jakso_TK_1_22_sopv_24_amm
DROP TABLE sa.temp_amm_naytto_opisk_TK_1_22_sopv_24_amm
DROP TABLE sa.temp_amm_aiempi_oikeus_sama_jarj_TK_1_22_sopv_24_amm
