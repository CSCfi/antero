...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste_hakijat_aiemmat_opiskeluoikeudet_ja_tutkinnot]    Script Date: 17.10.2022 11:06:13 ******/
SET ANSI_NULLS ON
GO
...
) AS OP ON OP.henkilo_oid = f.henkilo_oid AND OP.opisk_alkamisPvm <= coalesce(d4.loppuPvm, d4b.loppuPvm) AND NOT (OP.tavoitetutkinto = d5.koulutusluokitus_koodi AND (OP.opisk_alkamisPvm >= coalesce(d4.alkuPvm,d4b.alkuPvm)))
--tutkinnot
LEFT JOIN (
