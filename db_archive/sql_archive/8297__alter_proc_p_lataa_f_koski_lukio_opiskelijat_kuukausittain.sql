...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opiskelijat_kuukausittain]    Script Date: 2.11.2023 17:50:29 ******/
SET ANSI_NULLS ON
GO
...
LEFT JOIN (
	...
	ON ( 
		--ALKU
		((temp_cal.vuosi = q1.alku_vuosi AND temp_cal.kuukausi >= q1.alku_kuukausi)	OR (temp_cal.vuosi > q1.alku_vuosi))
		AND 
		--LOPPU
		((temp_cal.vuosi = q1.loppu_vuosi AND temp_cal.kuukausi <= q1.loppu_kuukausi) OR (temp_cal.vuosi < q1.loppu_vuosi))
	)
WHERE kytkin = 1
AND NOT EXISTS (
	SELECT * FROM Koski_SA.sa.tuva_lasna_loma_jaksot tuvat 
	WHERE q1.koulutusmuoto != 'tuva'
	and tuvat.oppija_oid = q1.oppija_oid
...
LEFT JOIN Koski_SA.sa.sa_koski_henkilo AS henk ON oo.oppija_oid = henk.oppija_oid
LEFT JOIN dw.d_koulutusluokitus AS d5 ON ps.koulutusmoduuli_koodiarvo = d5.koulutusluokitus_koodi
...
LEFT JOIN (
	...
	WHERE ooy.tila_koodiarvo = 'valmistunut'
	OR (
		ps.vahvistus_paiva is not null
		AND (
			ps.suorituksen_tyyppi in ('ammatillinentutkinto','ibtutkinto','diatutkintovaihe')
