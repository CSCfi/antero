USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_ainevalinnat_suorittaneet]    Script Date: 4.12.2020 19:10:25 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dw].[p_lataa_f_koski_lukio_ainevalinnat_suorittaneet] 
AS
...
-- 3) Päivitä arch-palvelimen näkymä ja prosessoi kuutio
...
--aputaulut
	...
	,[aine] = --kielet luokitellaan "vähimmäislaajuuden" mukaan
		case 
			...
			when (aine = 'B1' AND ((aine_laajuus_yksikko = '4' and coalesce(aine_laajuus,0) < 5 and kurssit_lkm_total < 5)
									OR (aine_laajuus_yksikko = '2' and aine_laajuus < 10))
				 ) 
				 OR 
				 (aine <> 'B1' AND ((aine_laajuus_yksikko = '4' and coalesce(aine_laajuus,0) < 6 and kurssit_lkm_total < 6)
