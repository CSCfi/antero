...
/****** Object:  StoredProcedure [dw].[p_lataa_api_yo_opintopisteet_kuukausi]    Script Date: 22.11.2022 12:43:08 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_api_yo_opintopisteet_kuukausi] AS

BEGIN
  EXEC [dw].[p_lataa_api_yo_opintopisteet_kuukausi_esilataus]
END

IF ((SELECT COUNT(DISTINCT tilastovuosi) FROM [dw].[api_yo_opintopisteet_kuukausi]) >= 3 AND  (SELECT MAX(tilastovuosi) FROM [dw].[api_yo_opintopisteet_kuukausi]) = YEAR(GETDATE()))
