...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_amm_tutkinnot]    Script Date: 6.9.2023 12:08:10 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT
		...
		,toimipiste_oid_tt
		,oppilaitos_oid --_tt
		,koulutustoimija_oid --_tt
		,hankintakoulutuksen_koulutustoimija_oid = null
		,hankintakoulutus
		,henkilostokoulutus
		,suorituksen_tyyppi = 
			case
	...
	WHERE vt.hankintakoulutus = 0

	UNION ALL

	--hankintakoulutus, oo (kuori)
	SELECT
		 vt.vuosi
		,kuukausi
		,sukupuoli
		...
		,oppilaitos_oid --_tt
		,koulutustoimija_oid --_tt
		,hankintakoulutuksen_koulutustoimija_oid = koulutustoimija_oid_tt
		,hankintakoulutus
		,henkilostokoulutus
		,suorituksen_tyyppi = 
			case
	...
	WHERE vt.hankintakoulutus = 1

	UNION ALL

	--hankintakoulutus, tt (pihvi)
	SELECT
		 vt.vuosi
		,kuukausi
		,sukupuoli
		...
		,valmentava_koodi = 
			case
				when suorituksen_tyyppi = 'valma' then 1
				when suorituksen_tyyppi = 'telma' then 2
				when suorituksen_tyyppi = 'tuvakoulutuksensuoritus' then 3 --lähdetaulussa [tuva_jarjestamislupa] = 'ammatillinen'
			end 
		,oppisopimus_osuus = 
			case 
				when oppis_ja_koulutus.oppis_osuus <= 0 then '0 %'
...
LEFT JOIN d_organisaatioluokitus d18 ON d18.organisaatio_oid = k.toimipiste_oid_tt
