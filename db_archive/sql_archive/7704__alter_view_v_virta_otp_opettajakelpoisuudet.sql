...
/****** Object:  View [dw].[v_virta_otp_opettajakelpoisuudet]    Script Date: 10.5.2023 13:10:15 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT
	...
	,[Ik‰] = d2.ika_fi
	,[Ik‰ryhm‰] = d2.ikaryhma3_fi
	,[Opettajakelpoisuus] = d3.selite_fi
	,[Kelpoisuuden suoritustapa] =
		CASE
			WHEN suoritustapa = '1' THEN 'Kelpoisuus tutkinnon yhteydess‰'
			WHEN suoritustapa = '2' THEN 'Kelpoisuus suoritettu erillisin‰ opintoina'
			ELSE 'Tieto puuttuu'
		END
	...
	,CASE 
		...
		ELSE 'Kyll‰'
	 END AS [Suorittanut korkeakoulututkinnon]
	,CASE 
		...
		WHEN d3.selite_fi in('Luovien alojen opettaja', 'Opettajan pedagogiset opinnot (ei merkint‰‰ ainekelpoisuudesta)') THEN 'Muu'
		ELSE 'Yleissivist‰v‰'
	END as raportti

	,[Korkeakoulu] = d5.organisaatio_fi

	,d16.kytkin_fi as 'Kelpoisuus suoritettu yhdess‰ korkeakoulussa'
	...
	,[jarjestys Ik‰ryhm‰] = d2.jarjestys_ikaryhma3
...
OUTER APPLY (
	SELECT 1 as erityisopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'ER' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) erityisopettaja
OUTER APPLY (
	SELECT 1 as varhaiskasvatuksen_erityisopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi in ('VA_ER', 'EL') and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) varhaiskasvatuksen_erityisopettaja
OUTER APPLY (
	SELECT 1 as erityisluokanopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'ER_PE' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) erityisluokanopettaja
OUTER APPLY (
	SELECT 1 as luokanopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
...
OUTER APPLY (
	SELECT 1 as luokanopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'PE' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) luokanopettaja_prio
OUTER APPLY (
	SELECT 1 as varhaiskasvatuksen_opettaja_luokanopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'VA_PE' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) varhaiskasvatuksen_opettaja_luokanopettaja
OUTER APPLY (
	SELECT 1 as aineenopettaja_luokanopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'AI_PE' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) aineenopettaja_luokanopettaja
OUTER APPLY (
	SELECT 1 as opintoohjaaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'OP' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) opintoohjaaja
OUTER APPLY (
	SELECT 1 as aineenopettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'AI' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) aineenopettaja
OUTER APPLY (
	SELECT 1 as varhaiskasvatuksen_opettaja_amk FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'VA' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) varhaiskasvatuksen_opettaja_amk
OUTER APPLY (
	SELECT 1 as varhaiskasvatuksen_opettaja_yo FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'VY' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) varhaiskasvatuksen_opettaja_yo
OUTER APPLY (
	SELECT 1 as varhaiskasvatuksen_sosionomi FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'VS' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) varhaiskasvatuksen_sosionomi
OUTER APPLY (
	SELECT 1 as ammatillinen_opettajankoulutus FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'JA' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) ammatillinen_opettajankoulutus
OUTER APPLY (
	SELECT 1 as ammatillinen_erityisopettajankoulutus FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'JB' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) ammatillinen_erityisopettajankoulutus
OUTER APPLY (
	SELECT 1 as ammatillinen_opintoohjaajakoulutus FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'JC' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) ammatillinen_opintoohjaajakoulutus
OUTER APPLY (
	SELECT 1 as ruotsinkielinen_ammatillinen_opettajakoulutus FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'IX' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) ruotsinkielinen_ammatillinen_opettajakoulutus
OUTER APPLY (
	SELECT 1 as luovien_alojen_opettaja FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'LA' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) luovien_alojen_opettaja
OUTER APPLY (
	SELECT 1 as pedagogiset_opinnot FROM ANTERO.dw.f_virta_otp_opettajakelpoisuudet f0
	LEFT JOIN ANTERO.dw.d_opettajapatevyys d on d.id = f0.d_kelpoisuus_id
	WHERE d.koodi = 'PD' and f0.henkilo = f.henkilo and f0.Vuosi = f.Vuosi and f0.Status = 'Raportilla n‰kyv‰ kelpoisuus'
) pedagogiset_opinnot
WHERE  f.Vuosi <= YEAR(getdate()) AND [Status] = 'Raportilla n‰kyv‰ kelpoisuus'
