...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amk_uraseuranta]    Script Date: 3.4.2023 9:22:34 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,numerovalinta = 
		case 
			when f3.kysymysid in (
						...
						9979, 9980, 9981, 9982, 10733, 10867)	--muut kysymykset asteikolla 0-5
	...
	Arvo_SA.sa.sa_arvo_kohteet_taustatiedot f 
...
SELECT DISTINCT concat(d_arvokyselykerta_id,'_',d_koulutusluokitus_id) as avain INTO ANTERO.dbo.arvo_amk_uraseuranta_temp
...
INSERT INTO dw.f_arvo_amk_uraseuranta (
	...
	SELECT DISTINCT
		...
		LEFT JOIN ANTERO.dw.d_arvokyselykerta d4 on d4.kyselykertaid = vt.kyselykertaid
		LEFT JOIN ANTERO.dw.d_kalenteri d5 on d5.kalenteri_avain = vt.vastaajatunnus_alkupvm
		WHERE oppilaitoskoodi is not null and kk.tyyppi = 'amk-uraseuranta' and kk.kyselykerta_vuosi is not null and concat(d4.id, '_', d3.id) not in (SELECT avain FROM ANTERO.dbo.arvo_amk_uraseuranta_temp)
