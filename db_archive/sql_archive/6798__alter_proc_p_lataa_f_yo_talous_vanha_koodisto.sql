...
/****** Object:  StoredProcedure [dw].[p_lataa_f_yo_talous_vanha_koodisto]    Script Date: 21.12.2022 10:57:47 ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [dw].[p_lataa_f_yo_talous_vanha_koodisto] as

truncate table dw.f_yo_talous_vanha_koodisto

insert into dw.f_yo_talous_vanha_koodisto
           (d_yo_id
		   ,d_opintoala95_id
		   ,d_tili_id
		   ,d_aineistotyyppi_id
...
select d1.id as d_yo_id
 ...
 from (
	select tilik
	  ...
	  ,'-1' as ohjauksenala
	  ,arvo
      ,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_1_tuloslaskelma' as src from
		(select
			   yliop_koodi
			  ,tilik
		  ...
		  unpivot (arvo for tili in
      	   ...
      	   WHERE Aineistotyyppi_osa_1 = case when (select count(*) from sa.sa_suorat_yo_talous_1_tuloslaskelma  where tilik < 2018 and tilik = unpvt.tilik and aineistotyyppi_osa_1 = 'L' and YLIOP_koodi = unpvt.YLIOP_koodi) > 0 then 'L' else 'E' end
      	   AND TILIK < 2019
	...
	select tilik
		  ...
		  ,'-1' as ohjauksenala
		  ,arvo
		  ,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_2_tase' as src
	from
		(select tilik
			  ,yliop_koodi
			  ,aineistotyyppi_osa_1
        ...
        WHERE aineistotyyppi_osa_1 = case when (select count(*) from sa.sa_suorat_yo_talous_2_tase where tilik < 2018 and tilik = unpvt.tilik and aineistotyyppi_osa_1 = 'L' and YLIOP_koodi = unpvt.YLIOP_koodi) > 0 then 'L' else 'E' end
        AND TILIK < 2019
	...
	select tilik
		  ...
		  ,'-1' as ohjauksenala
		  ,arvo
		  ,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_3_tuloslaskelman_toiminnot' as src from
		(select yliop_koodi
			  ,tilik
			  ,toiminto
			  ,ttavustu
	...
	select tilik
		  ...
		  ,arvo
		  ,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_4_tutkimusmenot' as src
	from
		(select
			   tilik
			  ,yliop_koodi
		  ...
		  unpivot (arvo for tili in
			  ...
			  ,kvjarj
			  ,ulkomuur)) as unpvt
			  -- otetaan fakta-taluun mukaan myös uuden talouskoodiston luvut
	...
	union all
		...
		,coalesce(ohjauksenala_koodi, '-1') as ohjauksenala
		,arvo
		,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_5_kustannustiedot' as src from
			(select tilik
				  ,yliop_koodi
				  ,opm95opa_koodi
				  ,ohjauksenala_koodi
	...
	,'-1' as ohjauksenala
	,arvo
	,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_5_kustannustiedot_harjoittelukoulut' as src from
		(select tilik
				,yliop_koodi
				,esiope
				,perusope
	...
	select tilik
		  ...
		  ,'-1' as ohjauksenala
		  ,arvo
		  ,'ETL: p_lataa_f_yo_talous_vanha_koodisto - SA_SUORAT_YO_TALOUS_6_liiketoiminnan_tuloslaskelma' as src
	from
		(select tilik
			,yliop_koodi
			,aineistotyyppi
      ...
      --where aineistotyyppi_osa_1 = case when (select count(*) from sa.sa_suorat_talous_6_liiketoiminnan_tuloslaskelma where tilik = unpvt.tilik and aineistotyyppi_osa_1 = 'L' and amk_koodi = unpvt.amk_koodi) > 0 then 'L' else 'E' end
      --rajoittaa uusien talouskoodistojen luvut fakta-taulusta
      WHERE aineistotyyppi = case when (select count(*) from sa.sa_suorat_amk_talous_6_liiketoiminnan_tuloslaskelma where tilik < 2018 and tilik = unpvt.tilik and aineistotyyppi = 'L' and yliop_koodi = unpvt.yliop_koodi) > 0 then 'L' else 'E' end
      AND TILIK < 2019
) f
