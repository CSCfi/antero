...
/****** Object:  View [dw].[v_amos_tutkinnot_ja_tutkinnonosat_spl]    Script Date: 29.11.2023 12:49:12 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_amos_tutkinnot_ja_tutkinnonosat_spl] AS

SELECT 
	[Luvanmukainen suorituskieli] = 
		case 
			when d17.kieli_koodi = '-2' and f.tilastovuosi > 2020 then 'Luvaton, ei hyväksytty suorituskieli' 
			else 'Ei luvaton, hyväksytty suorituskieli' 
		end
	,[Ikäryhmä II] = 
		case 
			when d13.ika_avain between 1 and 19 then '19 tai alle' 
			when d13.ika_avain = 20 then '20' 
			when d13.ika_avain > 20 then '21 tai yli' 
		end
	...
	,[Koulutuksen järjestäjä] = d1a.nimi_amos_spl
	...
	,[Koodit Koulutuksen järjestäjä] = d1a.ytunnus_amos_spl
	...
	,[hyvaksytaan_rahoitukseen] = case when d23.hyvaksytaan=1 /*and d25.koodi=1*/ then 1 else 0 end
...
LEFT JOIN dw.d_amos_spl_jarjestaja_linkki d1a on d1a.id = f.d_amos_spl_jarjestaja_linkki_id
...
) sa1 on sa1.ytunnus = d1a.ytunnus_amos_spl and sa1.vuosi = f.tilastovuosi
...
WHERE 1=1
and d1a.sisaltyy_amos_spl = 1 
and f.tilastovuosi = right(db_name(),4) - 1
