...
/****** Object:  StoredProcedure [dw].[p_lataa_f_horizon]    Script Date: 14.2.2025 15.02.20 ******/
SET ANSI_NULLS ON
GO
...
TRUNCATE TABLE ANTERO.dw.d_horizon_ohjelma
...
LEFT JOIN ANTERO.dw.d_horizon_pilari d7 on d7.ohjelma_koodi = CASE
																WHEN LEFT(d1.programme, CHARINDEX('.',d1.programme,10)) <> '' THEN LEFT(d1.programme, CHARINDEX('.',d1.programme,10))
																WHEN LEFT(d1.programme, CHARINDEX('.',d1.programme,9)) <> '' THEN LEFT(d1.programme, CHARINDEX('.',d1.programme,9))
																ELSE d1.programme
															END
