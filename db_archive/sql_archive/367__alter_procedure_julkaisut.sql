...
LEFT JOIN [dw].d_koulutusala_2002 s3 ON CONVERT(INT, CASE WHEN s1.[I Koulutusala] = 'NN' THEN -1 ELSE s3.koodi END) = s3.koodi
	AND s1.Vuosi < 2016
LEFT JOIN [dw].d_ohjauksenala s4 ON s1.[I Koulutusala] = s4.ohjauksenala_koodi
	AND s1.Vuosi >= 2016
LEFT JOIN [dw].d_julkaisutyyppi s5 ON s1.Julkaisutyyppi = s5.julkaisutyyppi_koodi 
