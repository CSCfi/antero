...
/****** Object:  View [dw].[v_virta_otp_opiskeluoikeudet]    Script Date: 21.11.2024 8:18:44 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	d8.kytkin_fi as 'Uusi opiskeluoikeus edellisenä lukuvuotena',
...
LEFT JOIN ANTERO.dw.d_kytkin d3 on d3.id = f.d_kytkin_usea_oo_samalta_alalta_id
LEFT JOIN ANTERO.dw.d_kytkin d4 on d4.id = f.d_kytkin_usea_oo_samasta_ammattikorkeakoulusta_id
LEFT JOIN ANTERO.dw.d_kytkin d5 on d5.id = f.d_kytkin_usea_oo_samasta_yliopistosta_id
LEFT JOIN ANTERO.dw.d_kytkin d6 on d6.id = f.d_kytkin_yliopisto_ja_ammattikorkeakoulu_id
LEFT JOIN ANTERO.dw.d_ika d7 on d7.id = f.d_ika_id
LEFT JOIN ANTERO.dw.d_kytkin d8 on d8.id = f.d_kytkin_uusi_oo_edellisena_lukuvuotena_id
