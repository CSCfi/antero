...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_ainekelpoisuudet_jaadytys]    Script Date: 4.1.2023 15:09:38 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE  [dw].[p_lataa_f_virta_otp_ainekelpoisuudet_jaadytys] AS
...
BEGIN
	...
	' FROM (' +
		...
		' FROM ANTERO.sa.sa_virta_otp_ope_patevyydet_2021 pat' +
	...
	' FROM (' +
		...
		' LEFT JOIN ANTERO.dw.d_virta_patevyydet d on d.patevyys_koodi = pat.patevyyskoodi' +
		' INNER JOIN ANTERO.sa.sa_virta_otp_ope_patevyydet_2021 pat2 on pat.henkilo = pat2.henkilo and pat2.patevyyskoodi in (' + '''' + 'ik' + '''' + ', ' + '''' + 'il' + '''' + ', ' + '''' + 'im' + '''' + ',' + '''' + 'ja' + '''' + ',' + '''' + 'jb' + '''' + ',' + '''' + 'jc' + '''' + ', ' + '''' + 'ir' + '''' + ', ' + '''' + 'jd' + '''' + ', ' + '''' + 'oa' + '''' + ')' +
		' INNER JOIN ANTERO.sa.sa_virta_otp_ope_tutkinnot_2021 tut on pat.henkilo = tut.henkilo' +
		...
		' LEFT JOIN ANTERO.sa.sa_virta_otp_ope_henkilotiedot_2021 ht on ht.henkilo = pat.henkilo' +
		...
		' SELECT DISTINCT' +
			...
			' kl.koulutusluokitus_koodi,' +
			' case' +
				' when pat.paivamaara_aine >= tut.suorituspaivamaara then pat.paivamaara_aine' +
	...
	' SELECT' +
		...
		' d3c.id as d_organisaatioluokitus_kelpoisuus3_id,' +
	...
	' FROM (' +
		' SELECT' +
			...
			' p3.aine as aine3,' +
			' p.laajuus,' +
			' coalesce(p.laajuus, ' + '''' + 'Tieto puuttuu' + '''' + ') as laajuus2,' +
			' coalesce(p.laajuus, ' + '''' + 'Tieto puuttuu' + '''' + ') as laajuus3,' +
			' p.aine_sektori' +
	...
	' WHERE (f.kelpoisuuden_vuosi = ' + @alkuVuosi + ') or (' + @alkuVuosi + ' = 2021 and f.kelpoisuuden_vuosi <= 2021)'
