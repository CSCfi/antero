...
/****** Object:  StoredProcedure [dw].[p_lataa_f_indikaattorit_kk_henkilostotiedot_amk]    Script Date: 17.4.2024 12:19:13 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_indikaattorit_kk_henkilostotiedot_amk] as
...
USING (
	...
	FROM (
		...
		LEFT JOIN ANTERO.dw.d_organisaatioluokitus d4 on d4.organisaatio_koodi = d1.amk_nykyinen_tunnus
	...
	OUTER APPLY (
		...
		LEFT JOIN ANTERO.dw.d_amk d1b on d1b.id = fb.d_amk_id 
...
USING (
	...
	FROM (
		...
		GROUP BY f.vuosi, d3.organisaatio_nykyinen_koodi
	) f
	LEFT JOIN (
		SELECT 
