USE [VipunenTK_SA]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_TK_Koski_lukio_ainevalinnat_suorittaneet]    Script Date: 4.11.2021 0:15:06 ******/
SET ANSI_NULLS ON
GO
...
@jaadytyspvmDDMMYYYY nvarchar(8) = '' --ex.g. '31102021'
,@viiveVuosina int = 0 --[vvvv-tilv]
,@linkedServerAlias nvarchar(50) = 'DWIVIPUARCH' --'' for testing
AS
...
-- Muutokset skriptiin voi ajaa versionhallinnan läpi ja ajon/vuosiajot Jenkinsistä, tarvittaessa käsin ekson kautta.
...
DECLARE @pvmPaate nvarchar(9) = case when @jaadytyspvmDDMMYYYY = '' then '' else '_' + @jaadytyspvmDDMMYYYY end
DECLARE @tilv nvarchar(4) = case when @jaadytyspvmDDMMYYYY = '' then year(getdate()) - @viiveVuosina else right(@jaadytyspvmDDMMYYYY,4) - @viiveVuosina end
...
DECLARE @taulu nvarchar(100) = @taulupohja + '_' + @tilv + (case when @jaadytyspvmDDMMYYYY = '' or @linkedServerPrefix = '' then '_TEST' else '' end)
DECLARE @procpohja nvarchar(100) = 'dbo.p_lataa_TK_Koski_lukio_ainevalinnat_suorittaneet'
DECLARE @proc nvarchar(100) = @procpohja + '_' + @tilv + (case when @jaadytyspvmDDMMYYYY = '' or @linkedServerPrefix = '' then '_TEST' else '' end)
...
LEFT JOIN ' + @linkedServerPrefix + 'Koski_SA_31102021.sa.[sa_koski_henkilo] h2 on h2.[oppija_oid] = oo.[oppija_oid]
