USE [VipunenTK_DW]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_f_tutkinnon_suorittaneiden_osuus_aloittaneista_erityisopiskelijat]    Script Date: 12.6.2018 12:15:46 ******/
SET ANSI_NULLS ON
GO
...
SET @periodStart = 0
SET @tilv = (SELECT MAX(tilv) FROM [VipunenTK_SA].[dbo].[v_sa_3_8_tutkinnon_suorittaneiden_osuus_aloittaneista_erityisopiskelijat])
SET @periodEnd = (SELECT MAX(suorv) FROM [VipunenTK_SA].[dbo].[v_sa_3_8_tutkinnon_suorittaneiden_osuus_aloittaneista_erityisopiskelijat]) - (SELECT MIN(alvv) FROM [VipunenTK_SA].[dbo].[v_sa_3_8_tutkinnon_suorittaneiden_osuus_aloittaneista_erityisopiskelijat])
...
(
	...
	FROM
		...
		FROM [VipunenTK_SA].[dbo].[v_sa_3_8_tutkinnon_suorittaneiden_osuus_aloittaneista_erityisopiskelijat]
		WHERE tilv = @tilv) f
	WHERE suorv = (SELECT MIN(suorv) FROM [VipunenTK_SA].[dbo].[v_sa_3_8_tutkinnon_suorittaneiden_osuus_aloittaneista_erityisopiskelijat] fb WHERE f.alvv = fb.alvv AND f.jarj = fb.jarj AND (/*f.opmala = fb.opmala OR*/ f.iscfibroad2013 = fb.iscfibroad2013) AND f.kmaak = fb.kmaak AND tilv = @tilv)
	UNION ALL
	SELECT [tilv]
      ,[lahde]
      ,[alvv]
