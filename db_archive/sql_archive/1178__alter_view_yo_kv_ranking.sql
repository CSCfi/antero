...
ALTER view [dw].[v_yo_kv_ranking] as
...
,null --tutkinnot
/** 
Kolme muuttujaa jotka poimivat ensin tutkijanuraportaan sa-taulusta jos sa-taulussa oleva ammattinumero vastaa faktan ammattinumeroa.
Sitten tarkastetaan eri tutkijanuratasot suhteessa saman rivin tutkijanuratasoon ja lisätään henkilötyövuosi jos tutkijanura matchaa
**/
,case (select am.Tutkijanurataso from sa.sa_ammattinimikkeet_the_qs as am where am.Ammattinumero=d4.koodi AND am.Kansallinen='AS') 
	when '1' then 
		case when d5.koodi=1 then f.henkilotyovuosi end
	when '2' then
		case when d5.koodi=2 then f.henkilotyovuosi end
	when '3' then
		case when d5.koodi=3 then f.henkilotyovuosi end
	when '4' then
		case when d5.koodi=4 then f.henkilotyovuosi end
 	when '2 ja 3' then
		case when d5.koodi in (2,3) then f.henkilotyovuosi end
   end as 'THE AS'
,case (select am.Tutkijanurataso from sa.sa_ammattinimikkeet_the_qs as am where am.Ammattinumero=d4.koodi AND am.Kansallinen='RS') 
	when '2' then 
		case when d5.koodi=2 then f.henkilotyovuosi end
	when '3' then
		case when d5.koodi=3 then f.henkilotyovuosi end
	...
	when '2 ja 3' then
		case when d5.koodi in (2,3) then f.henkilotyovuosi end
 	when '3 ja 4' then
		case when d5.koodi in (3,4) then f.henkilotyovuosi end
	end as 'THE RS'
,case (select am.Tutkijanurataso from sa.sa_ammattinimikkeet_the_qs as am where am.Ammattinumero=d4.koodi AND am.Kansallinen='FS')
	when '1' then 
		case when d5.koodi=1 then f.henkilotyovuosi end
	when '2' then 
		case when d5.koodi=2 then f.henkilotyovuosi end
	...
	when '4' then
		case when d5.koodi=4 then f.henkilotyovuosi end
	when '1 ja 2' then
		case when d5.koodi in (1,2) then f.henkilotyovuosi end
	when '2 ja 3' then
		case when d5.koodi in (2,3) then f.henkilotyovuosi end
 	when '3 ja 4' then
		case when d5.koodi in (3,4) then f.henkilotyovuosi end
