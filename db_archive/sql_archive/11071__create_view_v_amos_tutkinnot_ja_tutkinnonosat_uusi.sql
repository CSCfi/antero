...
SELECT 
	--lisätyt (pl. apusarakkeet)
	[Toteuma/Estimaatti] = case when generoitu = 1 then 'Estimaatti' else 'Toteuma' end
	,[Rahoituskausi] = f.[rahoituskausi]
	,Suorituskausi = f.suorituskausi
	,[Suoritepäätös (viimeisin)] = case when d27.sisaltyy_amos_spl = 1 then 'Kyllä' else 'Ei' end
	...
	,[Maksuttomuus] = case when d28.kytkin_fi = 'Kyllä' then 'Maksuton' else 'Ei maksuton' end
	...
	,d5.sisaltyy_okm_suoritusrahoitus
...
--WHERE EOMONTH(d6.kalenteri_avain) < GETDATE()
