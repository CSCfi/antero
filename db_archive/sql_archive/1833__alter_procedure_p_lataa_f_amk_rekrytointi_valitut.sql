...
LEFT JOIN [dw].[d_yo] s14 ON [s14].[yo_tunnus] = [s1].[Korkeakoulu]
...
-- ADD additional data  from VIRTA_OTP
MERGE dw.f_amk_rekrytointi_valitut AS target
USING (
		SELECT
			s2_1.[tilastovuosi] AS [tilastovuosi]
			,s2_2.[id] AS [amk_id]
			,s2_1.[henkilonumero] AS [henkilonumero]
			,COALESCE(s2_3.[id], '-1') AS [tohtoritutkintokorkeakoulu_id]
			,COALESCE(s2_4.[id],'-1') AS [tohtoritutkintomaa]
			,COALESCE([tohtoritutkintosuoritusvuosi],'-1') AS tohtoritutkintosuoritusvuosi
		FROM [ANTERO].[sa].[sa_virta_otp_rekrytohtorit] s2_1
		LEFT JOIN [dw].[d_amk] s2_2 ON [s2_2].[amk_tunnus] = [s2_1].[rekrykorkeakoulu]
		LEFT JOIN [dw].[d_yo] s2_3 ON [s2_3].[yo_tunnus] = [s2_1].[tohtoritutkintokorkeakoulu]
		LEFT JOIN [dw].[d_maatjavaltiot2] s2_4 ON [s2_4].[maatjavaltiot2_koodi] = [s2_1].[tohtoritutkintomaa]
		WHERE [tohtoritutkintomaa] = '246'
		AND  [tohtoritutkintosuoritusvuosi] > '1999'
		AND  [tohtoritutkintosuoritusvuosi] <= [tilastovuosi]
		) AS src2
ON target.[vuosi] = src2.[tilastovuosi]
	AND target.[d_amk_id] = src2.[amk_id]
	AND target.[henkilonumero] = src2.[henkilonumero]
WHEN MATCHED
	THEN UPDATE
		SET  target.[d_tohtorintutkinnon_suoritusmaa_id] = src2.[tohtoritutkintomaa]
			,target.[d_tohtorintutkinnon_suoritusyliopisto_id] = src2.[tohtoritutkintokorkeakoulu_id]
			,target.[tohtorintutkinnon_suoritusvuosi] =src2.[tohtoritutkintosuoritusvuosi];
