...
/****** Object:  Table [sa].[sa_virta_otp_tktutkintoennakko_2020]    Script Date: 17.1.2024 14:22:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[sa].[sa_virta_otp_tktutkintoennakko_2020]') AND type in (N'U') AND (SELECT COUNT(*) FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2020) > 0)
DROP TABLE [sa].[sa_virta_otp_tktutkintoennakko_2020]
GO
...
IF (SELECT COUNT(*) FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2020) > 0

BEGIN

CREATE TABLE [sa].[sa_virta_otp_tktutkintoennakko_2020](
...
INSERT INTO [sa].[sa_virta_otp_tktutkintoennakko_2020]
SELECT 
	[aikielir1x],
	[aineisto],
	[allk],
	...
	[source],
	[username]
FROM [sa].[sa_virta_otp_tktutkintoennakko] WHERE tilv = 2020
