...
SELECT   	
			...
			, [Korkeakoulu] =  coalesce(d5.organisaatio_fi,'Tuntematon')
			, [Korkeakoulukoodi] = coalesce(d5.organisaatio_koodi,'999999')
			...
			, [Korkeakoulu jarj] = coalesce(d5.jarjestys_organisaatio_koodi,'99999')	
			, [lkm] = '1' 
	FROM 	ANTERO.sa.sa_virta_otp_erikoistumiskoulutukset f 
			  ...
			  LEFT JOIN ANTERO.dw.d_organisaatio d5 on d5.organisaatio_koodi = f.oppilaitoskoodi