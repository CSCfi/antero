...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_opettajakelpoisuudet]    Script Date: 16.8.2023 9:29:23 ******/
SET ANSI_NULLS ON
GO
...
-- TEMP-tauluun kombinaatiot kelpoisuuskoodeista sekä kelpoisuuskoulujen korkeakoulu ja pvm
	...
	FROM (
		...
		SELECT DISTINCT
			...
			0 as paatelty_tutkinnosta,
			NULL as tutkinto_josta_paatelty
		FROM #temp_ope_pat1 p
		LEFT JOIN ANTERO.sa.sa_virta_otp_ope_patevyydet p1 on p.henkilo = p1.henkilo and YEAR(p.pvm1) = YEAR(p1.paivamaara) and p1.patevyyskoodi in ('ms','gf')
		...
		-- Kelpoisuudet, jotka on päätelty tutkinnoista
		SELECT DISTINCT
			t.henkilo,
			CASE
		...
		FROM ANTERO.sa.sa_virta_otp_ope_tutkinnot t
		LEFT JOIN ANTERO.sa.sa_virta_otp_ope_patevyydet p1 on p1.henkilo = t.henkilo and p1.patevyyskoodi = 'ms'
		LEFT JOIN ANTERO.sa.sa_virta_otp_ope_patevyydet p2 on p2.henkilo = t.henkilo and p2.patevyyskoodi = 'gf'
		LEFT JOIN ANTERO.sa.sa_virta_otp_ope_patevyydet p3 on p3.henkilo = t.henkilo and p3.patevyyskoodi in ('ip','in','ob','io')
		LEFT JOIN ANTERO.sa.sa_virta_otp_ope_patevyydet p4 on p4.henkilo = t.henkilo and p4.patevyyskoodi in ('ik','il','im','oa','ix','ja','jb','jc')
		WHERE 
...
-- Erityiskäsittely ammatillisille opettajankoulutuksille (JA, JB, JC, IX): Vuosi määräytyy pätevyyskoodin suoritushetken mukaan, ei tutkintovaatimusta. 
-- Näytetään kuitenkin tutkinnon tiedot, jos se on suoritettu samana vuonna tai ennen pätevyyskoodia
SELECT DISTINCT 
	p.henkilo,
	p.kelpoisuus,
	p.organisaatiokoodi,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then null else ot.koulutuskoodi end as koulutuskoodi,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then null when p.paatelty_tutkinnosta = 1 then ot.suorituspaivamaara else tutk.paivamaara end as tutk_pvm,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then null else ot.organisaatiokoodi end as tutk_organisaatiokoodi,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then 1 when p.yksi_org = 1 and ot.organisaatiokoodi = p.organisaatiokoodi then 1 else 0 end as yksi_org,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') then p.pvm when p.pvm < tutk.paivamaara then tutk.paivamaara else p.pvm end as pvm,
	case when (p.pvm1 > p.pvm2 or p.pvm2 is null) and d1a.patevyys_fi like '%erilliset%' THEN 2 when p.pvm2 >= p.pvm1 and d1b.patevyys_fi like '%erilliset%' then 2 else 1 end as suoritustapa,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then null else kork_tutk.koulutuskoodi end as koulutuskoodi_korkein_tutkinto,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then null else kork_tutk.organisaatio_koodi end as organisaatiokoodi_korkein_tutkinto,
	case when p.kelpoisuus in ('JA', 'JB', 'JC', 'IX') and YEAR(p.pvm) < YEAR(tutk.paivamaara) then null else kork_tutk.vuosi end as vuosi_korkein_tutkinto,
...
LEFT JOIN [sa].[sa_virta_otp_ope_tutkinnot] ot on ot.henkilo = p.henkilo and case when paatelty_tutkinnosta = 1 then p.tutkinto_josta_paatelty else tutk.paivamaara end = case when paatelty_tutkinnosta = 1 then ot.koulutuskoodi else ot.suorituspaivamaara end
...
-- Korkeakoulututkinto ei ole vaatimuksena ammatillisen opettajankoulutuksen kelpoisuuksille
...
FROM (
	...
	FROM (
		...
		OUTER APPLY (
			SELECT CASE WHEN p.paatelty_tutkinnosta = 1 THEN -1 ELSE MAX(f.yli_asetuksen) END as yli_asetuksen
			FROM (
				SELECT
					CASE 
