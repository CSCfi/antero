...
LEFT JOIN [dw].[d_amk_toimipisteen_toimipaikka] s15 ON [s15].[koodi_toimipaikka] = [s1].[toimipaikka] AND [s15].[koodi_amk] = [s1].[korkeakoulu]
LEFT JOIN [dw].[d_organisaation_alayksikot] s16 ON [s16].[alayksikko_koodi] = [s1].[alayksikkö] AND [s16].[korkeakoulu_koodi]=[s1].[Korkeakoulu] AND [s16].[vuosi]=[s1].[tilastovuosi]
...
LEFT JOIN [dw].[d_arvokysymys] s21 ON [s21].[kysymys_fi] = [s1].[Kysymys]
										AND (SELECT CASE
														WHEN [s1].[tilastovuosi] = 2017 THEN 243
														WHEN [s1].[tilastovuosi] = 2018 THEN 274
													ELSE -1
													END AS kysymysryhmä_id) = [s21].[kysymysryhmaid]
