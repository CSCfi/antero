...
/****** Object:  StoredProcedure [dbo].[p_lataa_f_tutkinnon_suorittaneiden_aiempi_koulutus_4_7]    Script Date: 21.12.2023 15:29:33 ******/
SET ANSI_NULLS ON
GO
...
--P‰ivitt‰‰ VipunenTK-kantaan t‰htimallin
--Heti sen j‰lkeen t‰ytt‰‰ tabular-mallisen taulun
...
SELECT --top 0
	  ...
	  ,ammatillisen_tutkinto_aiempi_suoritettu_ryhma3_id = isnull(d_amm_r3_ts.id, -2)
		...
		-- koulutusasteiden yhteenlasketut (netotut) henkilˆiden lukum‰‰r‰t
	  ...
	  -- Lis‰tty uudet mittarit 15.12.2023 / CSC Mika
	  ,koultasoa1_nostava_tutkinto = Case when f.[koulutustason1_nousu] = 'K' then henkiloiden_lkm else 0 end
	  ,koultasoa2_nostava_tutkinto = Case when f.[koulutustason2_nousu] = 'K' then henkiloiden_lkm else 0 end
      ,[koulutustason2_nousu]
	  ,[koulutustason2_nousu_id] = isnull(d_ke2.id, -1)
	  ,[koulutustason1_nousu]
	  ,[koulutustason1_nousu_id] =  isnull(d_ke1.id, -1)
...
LEFT JOIN VipunenTK.dbo.d_kylla_ei d_ke2
	ON d_ke2.kylla_ei_koodi = f.[koulutustason2_nousu]
LEFT JOIN VipunenTK.dbo.d_kylla_ei d_ke1
	ON d_ke1.kylla_ei_koodi = f.[koulutustason1_nousu]
