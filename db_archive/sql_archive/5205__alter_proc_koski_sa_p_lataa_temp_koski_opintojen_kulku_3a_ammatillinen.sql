...
/****** Object:  StoredProcedure [sa].[p_lataa_temp_koski_opintojen_kulku_3a_ammatillinen]    Script Date: 7.10.2021 9:16:57 ******/
SET ANSI_NULLS ON
GO
...
SELECT
 ...
 ,min_priorisoitu_tilanne_amm_koodi --vaihdosten kytkemiseksi priorisointiin ylemmässä kyselyssä
  = min(priorisoitu_tilanne_amm_koodi) over(partition by kohortti_vp
														,[oppija_oid]
														,opiskeluoikeus_oid
														,koulutus_kytkin
														,tutkinto_koodi
...
SELECT 
 ...
 ,priorisoitu_tilanne_amm_koodi = 
		...
		--	WHEN uusi.suorituksen_tyyppi in ('valma', 'telma') AND uusi.tila IN ('lasna', 'loma') THEN 5
			...
			WHEN uusi.suorituksen_tyyppi in ('muuammatillinenkoulutus','tutkinnonosaapienemmistäkokonaisuuksistakoostuvasuoritus' 
				/*JUHAN TESTI*/
				,'ammatillinentutkintoosittainen') AND uusi.tila IN ('lasna', 'loma', 'valiaikaisestikeskeytynyt', 'valmistunut'
			) THEN 7 
			WHEN uusi.koulutusmuoto != 'ammatillinenkoulutus' OR uusi.opiskeluoikeus_oid IS NULL OR uusi.tila is NULL 
				/*JUHAN TESTI*/ 
				OR uusi.tila NOT IN ('lasna', 'loma', 'valiaikaisestikeskeytynyt', 'valmistunut'
			) THEN 8 
			ELSE 99 
		END
