...
CREATE OR ALTER VIEW [dw].[v_amos_tyollistyminen_ja_jatkoopiskelu] AS

--lopulta poiminnat (limittäin) rahoituskausittain, käydään tuolloin läpi mm. aikamuuttujat

SELECT 
	[Varainhoitovuosi] = [tilv]+1 + ca2.tiedon_kayttokerta
	,[Rahoituskausi] = concat('01.07.', [tilv]-2 + ca2.tiedon_kayttokerta, ' - ', '30.06.', [tilv] + ca2.tiedon_kayttokerta)
	,[Suorituskausi] = [Rahoituskausi] --poiminnassa näin
	,[Tarkastelukausi] = concat('01.07.', [tilv], ' - ', '30.06.', [tilv]+1) --aineistossa tilv noin
	...
	,[Koulutuksen tavoite] = case [toteuma_tutk] when '1' then 'Koko tutkinto' when '2' then 'Tutkinnon osa/osia' end
	,[Koulutusala, taso 1] = v.kala_t1ni
	,[Koulutusaste, taso 2] = v.kaste_t2ni
	,[Tutkintotyyppi] = 
		case kaste_t2
	...
	,[Hankintakoulutus] = case when jarj_tutk is not null then 'Kyllä' else 'Ei' end
	,[Pohjakoulutuksen koulutusaste, taso 2] = coalesce(nullif(v.kaste_t2ni_e, 'P'), 'Tieto puuttuu')
	
	,[Suoritepäätös (viimeisin)] = case d1.sisaltyy_amos_spl when 1 then 'Kyllä' when 0 then 'Ei' end
	...
	,[Työllisenä (kk), koko tarkastelukausi] = 
		case when [ptoim1r2_tilv_07] in ('11','13') then 1 else 0 end
		+case when [ptoim1r2_tilv_08] in ('11','13') then 1 else 0 end
		+case when [ptoim1r2_tilv_09] in ('11','13') then 1 else 0 end
		+case when [ptoim1r2_tilv_10] in ('11','13') then 1 else 0 end
	...
	,[Työvoimassa (kk), koko tarkastelukausi] = 
	...
	,[jarj Koulutusaste, taso 2] = v.kaste_t2
...
CROSS APPLY (
	...
	select 5 union
	select 6 union 
	select 99 --tarvittaessa muiden kuin palkansaajana tai yrittäjänä työllistyneiden erottelemiseksi
) ca
CROSS APPLY (
	select 1 min_kk_tyollisena union
	select 2 union
...
CROSS APPLY (
	select 1 tiedon_kayttokerta union
	select 2
) ca2
...
AND [tilv]+1 + ca2.tiedon_kayttokerta > 2023
