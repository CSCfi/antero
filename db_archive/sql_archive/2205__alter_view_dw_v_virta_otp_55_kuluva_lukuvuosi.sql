-- Näkymä 55 op kuluva lukuvuosi, järjestyssarakkeet lisätty
 
IF NOT EXISTS ( SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'dw.v_virta_otp_55_kuluva_lukuvuosi') )
	EXEC dbo.sp_executesql @statement = N'CREATE VIEW dw.v_virta_otp_55_kuluva_lukuvuosi AS select 1 AS a'
GO
