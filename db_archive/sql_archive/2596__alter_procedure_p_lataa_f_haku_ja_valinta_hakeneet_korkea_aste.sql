...
/****** Object:  StoredProcedure [dw].[p_lataa_f_haku_ja_valinta_hakeneet_korkea_aste]    Script Date: 30.10.2019 8:56:10 ******/
SET ANSI_NULLS ON
GO
...
left join (
	...
	CASE
		WHEN MONTH(d8.loppuPvm) IS NULL AND d16.koulutuksen_alkamisvuosi >= YEAR(d32.valmistumispaivamaara) THEN d32.valmistumispaivamaara
		WHEN MONTH(d8.loppuPvm) < 8 THEN cast((cast(YEAR(d8.loppuPvm) as varchar(4)) + '-08-01') as date) 
