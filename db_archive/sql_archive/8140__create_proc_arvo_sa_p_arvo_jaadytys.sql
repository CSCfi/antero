...
/****** Object:  StoredProcedure [dw].[p_lataa_d_sukupuoli]    Script Date: 3.10.2023 14:08:33 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dbo].[p_arvo_jaadytys] AS
...
USING (
	...
	LEFT JOIN Arvo_SA.sa.sa_arvo_vastaajat va on va.vastaajaid = v.vastaajaid
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
	...
	src.[vastaajatunnus_loppupvm], src.[suorituskieli], src.[tutkintokoodi], src.[toimipaikka], src.[kuntakoodi],
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
	...
	src.kyselykertaid, src.kuntakoodi, src.tutkintokoodi, src.koulutusmuoto, src.toimipaikka, src.haun_numero, src.hankintakoulutuksen_toteuttaja,
...
USING (
	SELECT DISTINCT
		...
		'ETL: p_arvo_jaadytys' as source
	FROM [Arvo_SA].[sa].[sa_arvo_vastaukset] v
	LEFT JOIN Arvo_SA.sa.sa_virta_taustatiedot va on va.vastaajatunnusid = v.vastaajatunnusid
	LEFT JOIN [Arvo_SA].[sa].[sa_arvo_kyselykerrat] kk on kk.kyselykertaid = v.kyselykertaid
	INNER JOIN [Arvo_SA].[sa].[sa_arvo_tiedonkeruut] t on kk.tiedonkeruu_tunniste = t.tunniste and DATEDIFF(month, getdate(), t.kasittelyajan_paattymispvm) < 2
) AS src
ON target.vastaajatunnusid = src.vastaajatunnusid
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
   src.vastaajaid, src.vastaajatunnusid, src.taustatiedot, src.valmistumisajankohta, src.valintavuosi, src.valmistumisvuosi,
	...
	src.asteikko, src.arvosana, src.aidinkieli, src.source
  );
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
   src.vastausid, src.monivalintavaihtoehto_fi, src.monivalintavaihtoehto_sv, src.monivalintavaihtoehto_en, src.vastaajaid,
	src.kysymysid, src.kyselykertaid, src.koulutustoimija, src.numerovalinta, src.kyselyid, src.vastaajatunnusid, src.source
  );
