...
/****** Object:  StoredProcedure [dw].[p_lataa_api_tavoiteajassa_tutkinnon_suorittaneet_esilataus]    Script Date: 16.4.2024 18:33:06 ******/
SET ANSI_NULLS ON
GO
...
TRUNCATE TABLE [AnteroAPI].[dw].[api_tavoiteajassa_tutkinnon_suorittaneet]
 ...
 BEGIN
	...
	SET @ColumnType = 
		(
			SELECT y.name FROM ANTERO.sys.tables t 
			JOIN ANTERO.sys.columns c ON t.object_id = c.object_id 
			JOIN ANTERO.sys.types y ON y.user_type_id = c.user_type_id
			WHERE t.name = replace(@taulu, 'ANTERO.sa.', '') and c.name = 'tutkinnonsuorituspvm'
		)
