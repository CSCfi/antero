...
/****** Object:  View [dw].[v_valos_koulutuksenkustannukset]    Script Date: 19.1.2023 22:42:07 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	  ...
	  ,d1.kunta_koodi AS "Municipality of the education provider Code"
	  ...
	  ,d1.kunta_koodi AS "Utbildningsanordnarens kommun Kod"
...
--mm. Perusopetuksen aamu- ja iltapäivätoiminta ja vammaisopetus, ei toimintoerittelyä (sis. myös reknojen 5 ja 25 koulmto 120 erittelemättömät alvit)
UNION ALL
...
FROM (
	SELECT rekno,vuosi,ytunnus,kala,koulumto,OMISTAJA,KUNTA,VIRKUNTA
		,case when rekno in (5,25) and koulumto=120 then '0' else suorite end as suorite
		,case when rekno in (5,25) and koulumto=120 then '0' else k100l01 end as k100l01
		,case when rekno in (5,25) and koulumto=120 then '0' else k100l02 end as k100l02
		,k100l03
	FROM sa.sa_valos 
	WHERE rekno=55 
	OR koulumto in (133,134,135, 140,141,142) 
	OR (
		--vain alvit (kunnille/kuntayhtymille, 2021 poistumiseen asti) erittelemättömiä
		rekno in (5,25) 
		and koulumto=120 
		and coalesce(
			cast([k010l03] as decimal)
			+cast([k020l03] as decimal)
			+cast([k030l03] as decimal)
			+cast([k040l03] as decimal)
			+cast([k060l03] as decimal)
			+cast([k070l03] as decimal)
			+cast([k080l03] as decimal)
			+cast([k090l03] as decimal)
			,0
		) < cast([k100l03] as decimal)
	)
) p
UNPIVOT (
	summa FOR menolajitoiminto IN (k100l01,k100l02,k100l03)
) AS unpvt
