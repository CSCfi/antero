...
/****** Object:  View [dw].[v_koski_lukio_opiskelijat_laskentapaiva_cultures]    Script Date: 30.7.2025 10.44.05 ******/
SET ANSI_NULLS ON
GO
...
-- LASKENTAMUUTTUJAT 
	...
	,f.tilastopaiva_20_1 AS apusarake_tilastopaiva_20_1
...
--FI
	...
	-- HENKILÖMUUTTUJAT
		...
		,oppivelvollinen_1_8_2021_fi							= d18.kytkin_fi
...
--SV
	...
	-- KOULUTUSMUUTTUJAT
		,oppimaara_sv											= CASE 
																	WHEN oppimaara = 'Aikuisten oppimäärä' THEN 'Lärokursen för vuxna'
																	...
																	WHEN oppimaara = 'Ulkomainen tutkinto' THEN 'Utländsk examen' 
																  END
		,tavoitetutkinto_sv										= CASE 
																	WHEN tavoitetutkinto = 'Ylioppilastutkinto' THEN 'Studentexamen' 
	...
	-- HENKILÖMUUTTUJAT
		...
		,oppivelvollinen_1_8_2021_sv							= d18.kytkin_sv
...
-- JÄRJESTYSMUUTTUJAT
  ...
  FROM [ANTERO].[dw].[f_koski_lukio_opiskelijat_laskentapaiva] f
...
--  LEFT JOIN ANTERO.dw.d_alueluokitus AS d2 ON f.oppilaitos_kotipaikka = d2.kunta_koodi
  ...
  ) AS d14 ON f.aloitusvuosi = d14.vuosi AND f.aloituskuukausi = d14.kuukausi
  ...
  LEFT JOIN ANTERO.dw.d_kytkin AS d18 ON f.oppivelvollinen = d18.kytkin_koodi
