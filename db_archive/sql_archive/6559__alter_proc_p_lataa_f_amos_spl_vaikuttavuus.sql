...
LEFT JOIN dw.d_amos_spl_jarjestaja_linkki spl2 ON k13.jarj_opisk = spl2.ytunnus_avain
--	AND k13.tilv + 4 >= YEAR(spl.voimaan_alkaen)
LEFT JOIN dw.d_amos_spl_jarjestaja_linkki spl ON k13.jarj_tutk = spl.ytunnus_avain
--	AND k13.tilv + 4 >= YEAR(spl.voimaan_alkaen)
LEFT JOIN sa.z_tmp_TK_K1_K13_sopv_jarjkerroin_fuusiot jk ON jk.jarj_tutk = (CASE WHEN k13.tilv + 4 < YEAR(spl.voimaan_alkaen) THEN k13.jarj_tutk ELSE spl.ytunnus_amos_spl END) AND jk.tilv = k13.tilv
LEFT JOIN sa.sa_amos_siirtotiedosto_okm8 okm8 ON okm8.koul_jarj = spl.ytunnus_amos_spl AND year(okm8.voimaan_alkaen) = (case when k13.tilv+4 > @maxVuosi then k13.tilv+3 else k13.tilv+4 end)
LEFT JOIN dw.f_amos_spl_vaikuttavuus_maakunnat maak ON okm8.paa_asiallinen_toiminta_maakunta_koodi = maak.tilvasmaak AND maak.tilv = k13.tilv
