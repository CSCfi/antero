...
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_move]    Script Date: 21.10.2024 15:03:30 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_arvo_move] as
...
SELECT DISTINCT
	...
	vastaajaid,
	kysymys_fi,
	coalesce(numerovalinta, cast(replace(replace(vaihtoehto, 'kylla', '1'), 'ei', '0') as int)) as numerovalinta,
	v.kysymysid,
	v.kysymysversio
...
FROM (
	SELECT
		...
		CASE 
			...
			WHEN m7.numerovalinta >= p7.alaraja3 THEN 3 
			WHEN m7.numerovalinta BETWEEN p7.alaraja2 and p7.ylaraja2 THEN 2
			WHEN m7.numerovalinta <= p7.ylaraja1 THEN 1
			ELSE NULL
		END as vauhditon_5_loikka_pisteet,
