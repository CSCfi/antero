...
CREATE OR ALTER VIEW [dw].[v_amos_opiskelijavuodet_uusi_rahoitusperusteet] AS 
...
SELECT
	...
	,painotetut_opv_lahtotaso_hyv_ka = 0.5 * sum(cast(case when d31.koodi = 'opiskelijavuodet_painotettu_lahtotaso' then opv_ei_maksuton_kerroin * painotettu end as decimal (30,20)))
	,painotetut_opv_lahtotaso_hyv_ka_osuus = 
		cast(
			cast(sum(case when d31.koodi = 'opiskelijavuodet_painotettu_lahtotaso' then cast(opv_ei_maksuton_kerroin as decimal (15,14)) * cast(painotettu as decimal (20,10)) end) as decimal (18,10))
			/ cast(sum(sum(case when d31.koodi = 'opiskelijavuodet_painotettu_lahtotaso' then cast(opv_ei_maksuton_kerroin as decimal (15,14)) * cast(painotettu as decimal (20,10)) end)) over (partition by f.rahoituskausi, f.generoitu) as decimal (18,10))
			as decimal (20,19)
		)
	
	,painotetut_opv_kr_hyv_ka = 0.5 * sum(cast(case when d31.koodi != 'opiskelijavuodet_painotettu_lahtotaso' then opv_ei_maksuton_kerroin * painotettu end as decimal (30,20)))
...
JOIN (
	select rahoituskausi = max(concat('01.07.', Varainhoitovuosi-3, ' - 30.06.',Varainhoitovuosi-1)) 
	from [dw].[f_amos_koonti_rahoitusperusteet] 
) ca ON ca.rahoituskausi = f.rahoituskausi
