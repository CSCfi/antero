...
/****** Object:  View [dw].[v_lapset_ja_vakaan_osallistuneet_2_26]    Script Date: 3.12.2024 12:17:05 ******/
SET ANSI_NULLS ON
GO
...
-- Laskentamuuttujat
	...
	,[Ikä]								= d2.ika_fi
	,case when v.ika < 1 then 'alle 1-vuotiaat'
		when v.ika >= 1 and v.ika <= 2 then '1-2-vuotiaat'
		when v.ika > 2 and v.ika <= 5 then '3-5-vuotiaat'
		when v.ika = 6 then '6-vuotiaat' 
		else 'Tieto puuttuu'
		end as [Ikäryhmä]
	...
	,[Perhe]							= d8.perhe_fi
	,case when v.pekoko = 'P' then ' ' 
		else v.pekoko
	 end as [Perheen koko]		
	,case when v.pekoko = 'P' then ' ' 
		when v.pekoko > 6 then '7 ja yli'
		else v.pekoko
	 end as [Perheen koko (suppea)]
