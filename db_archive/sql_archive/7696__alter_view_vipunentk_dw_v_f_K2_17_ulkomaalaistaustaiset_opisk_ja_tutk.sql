...
/****** Object:  View [dbo].[v_f_K2_17_ulkomaalaistaustaiset_opisk_ja_tutk]    Script Date: 3.5.2023 11:31:28 ******/
SET ANSI_NULLS ON
GO
...
SELECT  
      ...
      ,opetushallinnon_koulutus_avain = 
		CASE 
			WHEN [ophal] IN ('1', '3') THEN '1'
			WHEN ophal = '2' THEN '2'
			ELSE '-1'
		END
...
/*** johdetut dimensiot ***/
      ...
      ,koulutussektori_avain = 
        CASE
			...
			WHEN klajir1 IN (31, 32, 33, 35, 61, 62, 63, 64, 65, 66, 71, 72, 74, 75, 77, 78, 80, 81) THEN 3
	  ...
	  ,maahantulo_ja_toisen_asteen_koul_aloittaminen_avain =   
		CASE
			WHEN klajir1 IN (21, 22, 31, 32, 33, 35, 61, 62, 63, 64, 65, 66, 71, 72, 74, 75, 77, 78, 80, 81) AND muuttov < alvv AND muuttov IS NOT NULL THEN 1 --'Maahantulo ennen opiskelun aloittamisvuotta'
			WHEN klajir1 IN (21, 22, 31, 32, 33, 35, 61, 62, 63, 64, 65, 66, 71, 72, 74, 75, 77, 78, 80, 81) AND muuttov = alvv AND muuttov IS NOT NULL THEN 2 --'Maahantulo opiskelun aloittamisvuonna'
	  ...
	  ,maahantulo_ja_korkeakoulutuksen_aloittaminen_avain =  
		...
		(
			case
				when klajir1 in (31, 32, 33, 61, 62, 63, 71, 72, 77, 78) then '1'
				when klajir1 in (35, 64, 65, 66, 74, 75, 80, 81) then '2'
			else '-1'
			end
		)
		as nvarchar(2))
...
/*** johdetut mittarit ***/
	  ...
	  ,hallinnonala2 = 
			case
				when tilv > 2020 then (case ophal when 1 then '1' when 2 then '3' when 3 then '2' else '-1' end)
				when ophal=1 then '1'
				--/*ei toimi, opmopa poistunut*/ and al.maakunta_koodi<>'21' and opmopa not in ('902','903','904') then '2'
				when ophal=2 then '3'
				else '-1'
			end
