...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_vakasuhteet_vakapaatokset]    Script Date: 19.8.2024 12:40:09 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	FROM varda_sa.sa.sa_varda_lapsi s
--	INNER JOIN varda_sa.sa.sa_varda_lapsi s2 on s2.henkilo_id = s.henkilo_id
	...
	WHERE 
	(coalesce(s4.paattymis_pvm, '9999-12-31') > coalesce(s5.paattymis_pvm, '9999-12-31') and s4.alkamis_pvm <= cast(concat(@alkuVuosi,'-12-31') as date) and coalesce(s5.paattymis_pvm, '9999-12-31') >= cast(concat(@alkuVuosi,'-12-31') as date))
	OR
	(coalesce(s4.paattymis_pvm, '9999-12-31') <= coalesce(s5.paattymis_pvm, '9999-12-31') and s4.alkamis_pvm <= cast(concat(@alkuVuosi,'-12-31') as date) and coalesce(s4.paattymis_pvm, '9999-12-31') >= cast(concat(@alkuVuosi,'-12-31') as date))
	
	AND s.action <> 'DELETED'
	AND s3.action <> 'DELETED'
	AND s4.action <> 'DELETED'
	AND s5.action <> 'DELETED'
	AND s6.action <> 'DELETED'
	and s.sukupuoli_koodi is not null
