...
/****** Object:  StoredProcedure [sa].[paivita_sa_koski_kotikuntahistoria]    Script Date: 30.1.2025 8.15.40 ******/
SET ANSI_NULLS ON
GO
...
OUTER APPLY (
	SELECT 
		MIN(syntymaaika) as syntymaaika
	FROM [Koski_SA].[sa].[sa_koski_henkilo] h
	WHERE h.master_oid = kkh.master_oid
) h
...
-- Poistetaan rivi, jos muuttopäivä = poismuuttopäivä, ja henkilöltä löytyy toinen merkintä samalta päivältä
...
OUTER APPLY (
	SELECT 
		COUNT(*) as rivit 
	FROM [Koski_SA].[sa].[sa_koski_kotikuntahistoria] kkh2
	WHERE coalesce(kkh2.muutto_pvm,'9998-01-01') <> coalesce(kkh2.poismuutto_pvm,'9999-01-01') and
	(kkh2.muutto_pvm = kkh.muutto_pvm or kkh2.poismuutto_pvm = kkh.poismuutto_pvm) and kkh.master_oid = kkh2.master_oid
) rivit
WHERE coalesce(kkh.muutto_pvm,'9998-01-01') = coalesce(kkh.poismuutto_pvm,'9999-01-01') and rivit.rivit > 0
