...
/****** Object:  View [dw].[v_koski_lukio_opiskelijat_uudet_cultures]    Script Date: 4.2.2025 15.59.11 ******/
SET ANSI_NULLS ON
GO
...
-- JÄRJESTYSMUUTTUJAT
  ...
  LEFT JOIN ANTERO.dw.f_koski_perusaste_keskiarvot AS f2 ON f.oppija_oid = f2.oppija_oid and f.vuosi = f2.vuosi
  ...
  LEFT JOIN (
	...
	) d14 on d14.henkilo_oid = f.oppija_oid and d14.organisaatio_nykyinen_koodi = d8.organisaatio_nykyinen_koodi and d14.koulutuksen_alkamisvuosi = f.vuosi
	LEFT JOIN (
	SELECT DISTINCT
		 f.henkilo_oid
		,f.koulutuksen_alkamisvuosi
	...
	WHERE d.hakuryhma_koodi != 2 and d3.koulutusastetaso2_koodi in (31, -1)
	and (f.vastaanoton_tila IN ('VASTAANOTTANUT_SITOVASTI','EHDOLLISESTI_VASTAANOTTANUT') or f.ilmoittautumisen_tila IN ('LASNA','LASNA_KOKO_LUKUVUOSI','LASNA_SYKSY'))
	) d15 on d15.henkilo_oid = f.oppija_oid and d15.organisaatio_nykyinen_koodi = d8.organisaatio_nykyinen_koodi and d15.koulutuksen_alkamisvuosi + 1 = f.vuosi
	LEFT JOIN (
	SELECT DISTINCT
		 f.henkilo_oid
		,f.koulutuksen_alkamisvuosi
	...
	WHERE d.hakuryhma_koodi != 2 and d3.koulutusastetaso2_koodi in (31, -1)
	and (f.vastaanoton_tila IN ('VASTAANOTTANUT_SITOVASTI','EHDOLLISESTI_VASTAANOTTANUT') or f.ilmoittautumisen_tila IN ('LASNA','LASNA_KOKO_LUKUVUOSI','LASNA_SYKSY'))
	) d16 on d16.henkilo_oid = f.oppija_oid and d16.organisaatio_nykyinen_koodi = d8.organisaatio_nykyinen_koodi and d16.koulutuksen_alkamisvuosi + 2 = f.vuosi
