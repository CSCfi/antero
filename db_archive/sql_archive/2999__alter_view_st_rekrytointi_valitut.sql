...
/****** Object:  View [dw].[v_st_rekrytointi_valitut]    Script Date: 20.4.2020 19:10:33 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dw].[v_st_rekrytointi_valitut] AS
 ...
 SELECT
	...
	,[Kansalaisuus (maanosa)] = d17.maanosa4_fi
	,[Äidinkieli] = d18.kieliryhma1_fi
	,[Ikä] = d19.ika_fi
	,[Ikäryhmä] = d19.ikaryhma4_fi
	...
	,[Sektor] = case when sektori='yo' then 'Universitet' when sektori='amk' then 'Yrkeshögskola' end
	,[Kön] = d16.sukupuoli_sv
	/*
	,[KansalaisuusSV] = d17.maatjavaltiot2_sv
	,[ÄidinkieliSV] = d18.kieliryhma1_SV
	,[IkäSV] = d19.ika_sv
	,[IkäryhmäSV] = d19.ikaryhma4_sv
	...
	*/
	...
	,[Sector] = case when sektori='yo' then 'University' when sektori='amk' then 'University of applied sciences' end
	,[Gender] = d16.sukupuoli_en
	/*
	,[KansalaisuusEN] = d17.maatjavaltiot2_en
	,[ÄidinkieliEN] = d18.kieliryhma1_en
	,[IkäEN] = d19.ika_en
	,[IkäryhmäEN] = d19.ikaryhma4_en
	...
	*/
	...
	,[jarj Kansalaisuus maanosa] = d17.jarjestys_maanosa4_koodi
	,[jarj Ikäryhmä] = d19.jarjestys_ikaryhma4
	...
	,[Hakunumero] = f.hakunumero
	,[Hakuavain] = CONCAT(f.hakunumero, d1.organisaatio_nykyinen_koodi, CASE WHEN q1.vuosi is null THEN f.vuosi-1 ELSE f.vuosi END) --, sukupuoli_koodi) --, (case when d17.maatjavaltiot2_koodi = '246' then 1 else 0 end), d17.maanosa0_koodi)
	,[henkilonumero]
	...
	,case when d1.organisaatio_nykyinen_koodi = d30.organisaatio_nykyinen_koodi and d1.organisaatio_koodi != '-1' and tohtorintutkinnon_suoritusvuosi <= f.vuosi then 1 else 0 end as tohtori_sama_yo
	,case when d1.organisaatio_nykyinen_koodi != d30.organisaatio_nykyinen_koodi and d1.organisaatio_koodi != '-1' and d30.organisaatio_koodi!='-1' and tohtorintutkinnon_suoritusvuosi <= f.vuosi then 1 else 0 end as tohtori_eri_yo
...
--Katsotaan onko haku ollut voimassa samana vuonna kun henkilö on valittu
LEFT JOIN (

	SELECT DISTINCT
		vuosi
		,d2.organisaatio_nykyinen_koodi as tunn
		,hakunumero
	FROM dw.f_amk_rekrytointihaut f
	JOIN dw.d_amk d on d.id = f.d_korkeakoulu_id
	JOIN dw.d_organisaatioluokitus d2 on d2.organisaatio_koodi = d.amk_tunnus
	...
	SELECT DISTINCT
		...
		,d2.organisaatio_nykyinen_koodi as tunn
		,hakunumero
	FROM dw.f_yo_rekrytointihaut f
	JOIN dw.d_yo d on d.id = f.d_yliopisto_id
	JOIN dw.d_organisaatioluokitus d2 on d2.organisaatio_koodi = d.yo_tunnus

) q1 ON q1.vuosi = f.vuosi AND q1.tunn = d1.organisaatio_nykyinen_koodi AND q1.hakunumero = f.Hakunumero
...
LEFT JOIN (
	...
	join dw.d_organisaatioluokitus d2 on d2.organisaatio_koodi = d.amk_tunnus
	group by vuosi,d2.organisaatio_nykyinen_koodi
...
) q2 ON q2.tunn = d1.organisaatio_nykyinen_koodi and q2.vuosi = f.vuosi
