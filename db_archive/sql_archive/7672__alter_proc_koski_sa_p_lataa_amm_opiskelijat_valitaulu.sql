...
/****** Object:  StoredProcedure [dbo].[p_lataa_amm_opiskelijat_valitaulu]    Script Date: 18.4.2023 13:21:54 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	,aiempi_oikeus_sama_jarj AS (
		...
		FROM (
			...
			LEFT JOIN (select distinct opiskeluoikeus_oid,alku,loppu,kytkin=0 from sa.sa_koski_opiskeluoikeus_aikajakso where opintojen_rahoitus in ('1','10','11','12')) ooa_m on ooa_m.opiskeluoikeus_oid = ll.opiskeluoikeus_oid and month(ooa_m.alku)=month(oo3.alkamispaiva) and year(ooa_m.alku)=year(oo3.alkamispaiva)
