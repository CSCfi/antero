...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_esi_ja_perus_oppilaat_ja_paattaneet]    Script Date: 26.4.2022 21:14:12 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_koski_esi_ja_perus_oppilaat_ja_paattaneet_jaadytys] AS

DECLARE @maxTilv int = (select max(vuosi) FROM Koski_SA.dbo.v_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu_jaadytys)
DECLARE @maxKk int = (select max(kuukausi) FROM Koski_SA.dbo.v_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu_jaadytys WHERE vuosi = @maxTilv)
DECLARE @kuluvaLvAlku varchar(8) = case when @maxKk > 7 then datefromparts(@maxTilv, 8, 1) else datefromparts(@maxTilv-1, 8, 1) end

/*
--kuukausittain
DROP TABLE IF EXISTS dw.f_koski_esi_ja_perus_oppilaat_ja_paattaneet_kuukausittain_jaadytys
...
INTO dw.f_koski_esi_ja_perus_oppilaat_ja_paattaneet_kuukausittain_jaadytys
	
FROM (
	SELECT 
		* 
	FROM Koski_SA.dbo.v_esi_ja_perus_oppilaat_ja_paattaneet_valitaulu_jaadytys
) k
