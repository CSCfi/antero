...
/****** Object:  StoredProcedure [dw].[p_lataa_f_varda_henkilosto_maaraaikainen]    Script Date: 19.8.2024 12:35:50 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		SET @pvm = DATEFROMPARTS(@alkuVuosi, @kuukausi, 1)
		...
		SELECT
			...
			coalesce(d0.id,'-1') as d_kalenteri_id,
			coalesce(d1.id,'-1') as d_ikaryhma_id,
...
--		LEFT JOIN [Varda_SA].[sa].[sa_varda_henkilo2022] s3 on s2.henkilo_id = s3.id
	...
	/*LEFT*/ JOIN [Varda_SA].[sa].[sa_varda_organisaatio] s8 on s2.vakajarjestaja_id = s8.id
		...
		LEFT JOIN dw.d_kalenteri d0 on d0.kalenteri_avain = @pvm
		LEFT JOIN dw.d_ika d1 on d1.ika_avain = DATEDIFF(YEAR, s2.syntyma_pvm, CAST(CONCAT(@alkuVuosi, '-01-01') as date))
		...
		WHERE @pvm between DATEFROMPARTS(YEAR(s.alkamis_pvm), MONTH(s.alkamis_pvm), 1) and coalesce(s.paattymis_pvm, '9999-01-01')
