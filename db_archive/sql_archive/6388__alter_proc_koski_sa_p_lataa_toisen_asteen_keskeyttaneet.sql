USE [Koski_SA]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_toisen_asteen_keskeyttaneet]    Script Date: 1.9.2022 13:11:51 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	WHERE (
		...
		OR (oo.koulutusmuoto = 'tuva' AND oo.tuva_jarjestamislupa IN ('lukio', 'ammatillinen'))
...
FROM (
	...
	FROM (
		...
		SELECT DISTINCT
			...
			,jatkaa_tutkintotyyppi = case when q1.opiskeluoikeus_oid is null then 0 when erot.tutkintotyyppi_koodi = q1.tutkintotyyppi_koodi /*kaikki valmentava on samaa*/ then 1 else 0 end
