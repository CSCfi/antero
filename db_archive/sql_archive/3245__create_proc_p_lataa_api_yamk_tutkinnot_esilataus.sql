...
/****** Object:  StoredProcedure [dw].[p_lataa_api_yamk_tutkinnot_esilataus]    Script Date: 3.9.2020 16:57:40 ******/
DROP PROCEDURE IF EXISTS [dw].[p_lataa_api_yamk_tutkinnot_esilataus]
GO
...
CREATE PROCEDURE [dw].[p_lataa_api_yamk_tutkinnot_esilataus] AS
...
SELECT [Korkeakoulu] 				= COALESCE(d1.organisaatio_fi,'Tuntematon')
	, [Tutkinnon suorittamisvuosi] 	= f.[tutkinnonsuoritusvuosi]		
	, [Suorituskuukausi]
	...
	(SELECT oppilaitosnro
		...
		(SELECT oppilaitosnro
			...
			--, koulutuskunta
			, rahoituslahde
			FROM ANTERO.sa.[sa_virta_otp_tutkinnot]
		) f0
