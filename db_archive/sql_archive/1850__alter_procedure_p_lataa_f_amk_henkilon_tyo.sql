...
SELECT COALESCE(s2.id,-1)
	...
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM([Sopimuksen alkupäivämäärä])), 104), -1) AS [sopimus_alkupvm]
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM([Sopimuksen loppupäivämäärä])), 104), - 1) AS [sopimus_loppupvm]
	,getdate()
...
LEFT JOIN [sa].[sa_suorat_amk1_henkilokunta_tk] s21 ON s21.Henkilönumero = s1.Henkilönumero AND s21.Vuosi = s1.Vuosi AND s21.Ammattikorkeakoulu = s1.Ammattikorkeakoulu
