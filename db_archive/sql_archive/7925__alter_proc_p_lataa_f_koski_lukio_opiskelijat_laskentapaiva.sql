...
/****** Object:  StoredProcedure [dw].[p_lataa_f_koski_lukio_opiskelijat_laskentapaiva]    Script Date: 31.7.2023 16:49:06 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT 
		...
		,rnk = rank() over (
			...
			order by alkamispaiva, suorituksen_tyyppi
		)
		,yhdistelmatutkinto
	...
	FROM (
		...
		SELECT  
			...
			,CASE 
				WHEN (kuukausi = 1 AND paivays > ooa.alku AND ooa.loppu IS NULL AND ps.vahvistus_paiva IS NULL) THEN 1
				WHEN (kuukausi = 1 AND paivays BETWEEN ooa.alku AND ooa.loppu) THEN 1
				ELSE 0
			END AS tilastopaiva_20_1
			,CASE 
				WHEN (kuukausi = 9 AND paivays > ooa.alku AND ooa.loppu IS NULL AND ps.vahvistus_paiva IS NULL) THEN 1
				WHEN (kuukausi = 9 AND paivays BETWEEN ooa.alku AND ooa.loppu) THEN 1
				ELSE 0
		...
		WHERE (
			ps.suorituksen_tyyppi IN ('ibtutkinto', 'preiboppimaara', 'lukionoppiaineenoppimaara', 'lukionoppimaara', 'diatutkintovaihe', 'diavalmistavavaihe', 'luva', 'internationalschooldiplomavuosiluokka', 'ebtutkinto', 'europeanschoolofhelsinkivuosiluokkasecondaryupper')
