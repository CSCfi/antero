USE [VipunenTK_DW]
GO
/****** Object:  StoredProcedure [dbo].[p_lataa_OPE]    Script Date: 22.12.2017 9:31:29 ******/
SET ANSI_NULLS ON
GO
...
SELECT tilastointivuosi = f.tilv
      ...
      ,kuntaryhmitys_oppilaitos = f.p_okunryh
      ,ika_id = d24.id
	  ,kelp_id = d25.id
...
SELECT tilastointivuosi = f.tilv
      ...
      ,koulutuksen_jarjestajan_kuntaryhmitys = f.p_ykunryh
...
SELECT tilastointivuosi = f.tilv
      ...
      ,kuntaryhmitys_oppilaitos = f.l_okunryh
      ,ika_id = d24.id
	  ,kelp_id = d25.id
	  ,koko_osa_aikainen_id = coalesce(d27.id,'-1')
	  ,osa_aikaisuuden_syy_id = coalesce(d28.id,'-1')
...
SELECT tilastointivuosi = f.tilv
      ...
      ,koulutuksen_jarjestajan_kuntaryhmitys = f.l_ykunryh
...
SELECT tilastointivuosi = [tilv]
      ...
      ,kelp_ei_muod = coalesce(d23.id,'-1')
  ...
  LEFT JOIN VipunenTK..d_opettajat_koulutusaste_amm d2 on d2.koulutusaste_koodi=f.a_aste
  LEFT JOIN VipunenTK..d_koulutusluokitus d3 on d3.koulutusluokitus_avain='OPMALA'+f.a_ala

  LEFT JOIN VipunenTK..d_opettajat_aine_amm d4 on d4.aine_koodi=f.a_aine
...
SELECT tilastointivuosi = [tilv]
  ...
  LEFT JOIN VipunenTK..d_opettajat_koulutusaste_amm d3 on d3.koulutusaste_koodi=f.a_aste
  LEFT JOIN VipunenTK..d_koulutusluokitus d4 on d4.koulutusluokitus_avain='OPMALA'+f.a_ala108
...
SELECT tilastointivuosi = [tilv]
  ...
  LEFT JOIN VipunenTK..d_opettajat_koulutusaste_amm d2 on d2.koulutusaste_koodi=f.a_aste
  LEFT JOIN VipunenTK..d_koulutusluokitus d3 on d3.koulutusluokitus_avain='OPMALA'+f.a_ala
  LEFT JOIN VipunenTK..d_koulutusluokitus d3b on d3b.koulutusluokitus_avain='OPMALA'+f.a_ala2
  LEFT JOIN VipunenTK..d_koulutusluokitus d3c on d3c.koulutusluokitus_avain='OPMALA'+f.a_ala3
  LEFT JOIN VipunenTK..d_opettajat_aine_amm d4 on d4.aine_koodi=f.a_aine
  LEFT JOIN VipunenTK..d_opettajat_aine_amm d4b on d4b.aine_koodi=f.a_aine2
  LEFT JOIN VipunenTK..d_opettajat_aine_amm d4c on d4c.aine_koodi=f.a_aine3
  --LEFT JOIN VipunenTK..d_koulutusluokitus d5 on d5.koulutusluokitus_avain=f.a_koulk
  --LEFT JOIN VipunenTK..d_koulutuksen_jarjestajan_koko d6 on d6.koko_koodi=f.a_eioppilkm
  --LEFT JOIN VipunenTK..d_koulutuksen_jarjestajan_koko d7 on d7.koko_koodi=f.a_oppilkm
  --LEFT JOIN VipunenTK..d_kieli2 d8 on d8.kieli_koodi=f.a_ykieli
...
SELECT tilastointivuosi = f.tilv
      ...
      ,sukupuoli_id = d30.id
      ,oppilaitoksen_maakunta_id = d10.id
      ,oppilaitoksen_kuntaryhmitys = f.v_okunryh
      ,ika_id = d31.id
	  ,koko_osa_aikainen_id = coalesce(d27.id,'-1')
	  ,osa_aikaisuuden_syy_id = coalesce(d28.id,'-1')
  ...
  LEFT JOIN VipunenTK..d_opettajat_aine_vst d4 on d4.aine_koodi=f.v_aine1
