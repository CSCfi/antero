USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_d_yo_toimipisteen_toimipaikka]    Script Date: 7.4.2020 9:39:16 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT
			...
			nimi,
			nimi,
			'ETL: p_lataa_d_yo_toimipisteen_toimipaikka_test'
...
ELSE
	BEGIN
		...
		SET
			...
			loadtime = GETDATE(),
			username = SUSER_NAME(),
			source = 'etl: p_lataa_d_yo_toimipisteen_toimipaikka_test'
...
USING (
	SELECT vuosi,
		...
		,'etl: p_lataa_d_yo_toimipisteen_toimipaikka_test' AS source
	FROM sa.sa_suorat_yo4_toimipisteet
	WHERE [Toimipisteen toimipaikan koodi] IS NOT NULL
	GROUP BY Yliopisto, vuosi, [Toimipisteen koodi]
