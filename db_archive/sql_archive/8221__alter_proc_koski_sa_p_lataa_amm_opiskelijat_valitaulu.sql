...
/****** Object:  StoredProcedure [dbo].[p_lataa_amm_opiskelijat_valitaulu]    Script Date: 18.10.2023 11:02:07 ******/
SET ANSI_NULLS ON
GO
...
DROP TABLE IF EXISTS Koski_SA.sa.temp_amm_koski_opiskeluoikeus
DROP TABLE IF EXISTS Koski_SA.sa.temp_amm_eka_lasna_jakso
DROP TABLE IF EXISTS Koski_SA.sa.temp_amm_naytto_opisk
...
SELECT 
	...
	oo.alkamispaiva
INTO Koski_SA.sa.temp_amm_koski_opiskeluoikeus
...
INTO Koski_SA.sa.temp_amm_eka_lasna_jakso
...
INTO Koski_SA.sa.temp_amm_naytto_opisk
...
BEGIN
	...
	DROP TABLE IF EXISTS Koski_SA.sa.temp_amm_opiskelijat_sopimusjaksot
	...
	SELECT distinct 
		 opiskeluoikeus_oid
		,sopimusmuoto
	INTO Koski_SA.sa.temp_amm_opiskelijat_sopimusjaksot
	...
	BEGIN
		...
		DROP TABLE IF EXISTS Koski_SA.sa.temp_amm_aiempi_oikeus_sama_jarj
		...
		INTO Koski_SA.sa.temp_amm_aiempi_oikeus_sama_jarj
		FROM (
			...
			LEFT JOIN Koski_SA.sa.temp_amm_naytto_opisk n_o on n_o.opiskeluoikeus_oid = oo3.opiskeluoikeus_oid and n_o.naytto = 0
	...
	FROM (
		...
		OUTER APPLY (
			SELECT 
				MIN(pst.tutkintonimike_koodiarvo) as tutkintonimike_koodiarvo
			FROM sa.sa_koski_paatason_suoritus_tutkintonimike pst
			WHERE pst.paatason_suoritus_id = ps.paatason_suoritus_id
		) pst
		LEFT JOIN (
			SELECT
		...
		LEFT JOIN Koski_SA.sa.temp_amm_opiskelijat_sopimusjaksot os on os.sopimusmuoto like '%oppisopimu%' and os.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
		LEFT JOIN Koski_SA.sa.temp_amm_opiskelijat_sopimusjaksot ks on ks.sopimusmuoto = 'Koulutussopimus' and ks.opiskeluoikeus_oid = oo.opiskeluoikeus_oid 
	...
	--muut opiskeluoikeudet samalla järjestäjällä
	LEFT JOIN Koski_SA.sa.temp_amm_aiempi_oikeus_sama_jarj oo2 on oo2.opiskeluoikeus_oid = q.opiskeluoikeus_oid and oo2.r_per = 1 and oo2.aiempi_perus = 1

	--muut opiskeluoikeudet samalla järjestäjällä (rahoitus)
	LEFT JOIN Koski_SA.sa.temp_amm_aiempi_oikeus_sama_jarj oo3 on oo3.opiskeluoikeus_oid = q.opiskeluoikeus_oid and oo3.r_rah = 1 and oo3.aiempi_rahoitus = 1

	--suoraan näyttöön
	LEFT JOIN Koski_SA.sa.temp_amm_naytto_opisk n_o on n_o.opiskeluoikeus_oid = q.opiskeluoikeus_oid

	--eka läsnäjakso
	LEFT JOIN Koski_SA.sa.temp_amm_eka_lasna_jakso ooa2 on ooa2.opiskeluoikeus_oid = q.opiskeluoikeus_oid
