USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_arvo_amis_tep]    Script Date: 29.3.2022 12:04:54 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	LEFT JOIN (
		--esim. vain taustakysymyksiin vastanneet kyselyn kohteiksi vaikkei itse kysymyksiä; poistettu alta vastaavasti aiemmat rajaus ja delete. JS 29.3.2022
		select * 
		from Arvo_SA.sa.sa_arvo_vastaukset
		where kysymysid not in (
			select kysymysid 
			from Arvo_SA.sa.sa_arvo_kysymykset
			where taustakysymyksen_tyyppi is not null or coalesce(valtakunnallinen, 1) != 1 or coalesce(vastaustyyppi,'') = 'vapaateksti'
		)
	) f on f2.vastaajatunnusid = f.vastaajatunnusid 
