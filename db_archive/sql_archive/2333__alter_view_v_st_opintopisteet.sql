...
/****** Object:  View [dw].[v_st_opintopisteet]    Script Date: 30.7.2019 10:59:19 ******/
SET ANSI_NULLS ON
GO
...
,[University of applied sciences] = d3.amk_nimi_en
,[University] = null
...
--where (d5.paivays like '%-03-01' and f.vuosi>2015) or (f.vuosi<=2015)
...
,[Sektorn] = 'Yrkehögskoleutbildning'

,[Yrkeshögskola] = null
,[Universitet] = d3.yo_nimi_sv
...
left join dw.d_ohjauksenala d2 on d2.id=f.d_ohjauksenala_id
left join dw.d_yo d3 on d3.id=f.d_yo_id
left join dw.d_kalenteri d5 on d5.id=f.d_tilannepvm_id
--where (d5.paivays like '%-03-01' and f.vuosi>2015) or (f.vuosi<=2015)
