...
/****** Object:  StoredProcedure [dw].[p_lataa_f_oppivelvollisten_opintojen_kulku]    Script Date: 6.9.2023 11:34:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dw].[p_lataa_f_koski_oppivelvollisten_opintojen_kulku] AS
	...
	BEGIN
		...
		SELECT DISTINCT 
			...
			FROM (
				...
				INNER JOIN [Koski_SA].[sa].[sa_koski_opiskeluoikeus] oo on oo.oppija_oid = ov.oppija_oid
				INNER JOIN [Koski_SA].[sa].[sa_koski_opiskeluoikeus_aikajakso] ooa on 
		...
		FROM (
			SELECT DISTINCT
				...
				CASE
					...
					WHEN v_lps_th.opiskeluoikeus_oid is not null and s1.suorituksen_tyyppi not in ('ammatillinentutkinto', 'ammatillinentutkintoosittainen') THEN 1
				...
				CASE
					...
					WHEN v_lps_th.opiskeluoikeus_oid is not null and s1.suorituksen_tyyppi not in ('ammatillinentutkinto', 'ammatillinentutkintoosittainen') THEN v_lps_th.suorituksen_tyyppi
					-- Suorittanut tutkinnon toisella sektorilla
			...
			LEFT JOIN [Koski_SA].[sa].[sa_lukio_paatason_suoritus] v_lps_th on v_lps_th.opiskeluoikeus_oid = v_oo_th.opiskeluoikeus_oid and v_lps_th.tavoitetutkinto <> 'Ei tutkintotavoitetta'
		...
		)s ) s WHERE s.tilanne = s.tilanne_prio and ranki_st = 1
