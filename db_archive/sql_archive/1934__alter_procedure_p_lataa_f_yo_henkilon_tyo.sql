USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_f_yo_henkilon_tyo]    Script Date: 7.11.2018 9:56:24 ******/
SET ANSI_NULLS ON
GO
...
SELECT COALESCE(s2.id,-1)
	...
	,[s1].VUOSI
	,HENKILOTYOVUOSI_LKM
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM(s22.[Sopimuksen alkupäivämäärä])), 104), -1) AS [sopimus_alkupvm]
	,COALESCE(TRY_CONVERT(Datetime, LTRIM(RTRIM(s22.[Sopimuksen loppupäivämäärä])), 104), - 1) AS [sopimus_loppupvm]	,getdate()
...
LEFT JOIN [dw].[d_yo_tutkijanuravaihe] s8 ON COALESCE(s1.TUTKIJANURAVAIHE_KOODI, -1) = s8.koodi -- tästä alkaen kusee, tarvitaan tuntematon
...
LEFT JOIN [sa].sa_suorat_yo1_henkilokunta_TK s22 ON s22.Vuosi = s1.vuosi AND s22.Yliopisto = s1.YLIOPISTO_TUNNUS  AND s22.Henkilönumero= s1.YO_HENKILONRO
