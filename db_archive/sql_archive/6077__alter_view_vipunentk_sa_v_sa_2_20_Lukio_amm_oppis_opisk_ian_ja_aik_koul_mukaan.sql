...
/****** Object:  View [dbo].[v_sa_2_20_Lukio_amm_oppis_opisk_ian_ja_aik_koul_mukaan]    Script Date: 15.3.2022 19:23:32 ******/
SET ANSI_NULLS ON
GO
...
UNION ALL
  ...
  SELECT [tilv] = (select max(cast(tilv as int)) from [TK_H9098_CSC].[dbo].[TK_K2_20_sopv_21]) + 1
      ,[lahde]
      ,[oltyp]
      ,[koulk]
      ,[koulk_opintoala]			=	(select top 1 opintoala2002_koodi		from [VipunenTK].[dbo].[d_koulutusluokitus] where koulutusluokitus_avain = [koulk])
      ...
      ,[rivinumero] = ROW_NUMBER() OVER(ORDER BY tilv)
  FROM [TK_H9098_CSC].[dbo].[TK_K2_20_sopv_22] --Tarkista myös tilv!
