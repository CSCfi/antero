...
/****** Object:  View [dw].[v_st_henkilosto]    Script Date: 20.11.2018 14:13:44 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,[Muun henkilöstön henkilöstöryhmä] = d14.selite_fi
	,[Työsuhteen nimitystapa] = d11.selite_fi
	,[Työsopimuksen pituus] = 
		CASE
			WHEN YEAR(q.sopimus_alkupvm) = 1899 THEN 'Tieto puuttuu'
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 3 THEN 'alle 3 kk'
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 7 THEN '3–6 kk'
	...
	,[Sektori]
	,[Koulutusala 02] = COALESCE(d4a.selite_fi,'Tieto Puuttuu')
	,[Koulutusala 95] = COALESCE(d4b.selite_fi,'Tieto Puuttuu')
	...
	,[Uppgiftsindelning] = COALESCE(d3a.selite_sv, d3b.selite_sv)
	...
	,[Anställningsavtalets längd] =
		CASE
			...
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 3 THEN 'mindre än 3 månader'
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 7 THEN '3–6 månader'
	...
	,[University of applied sciences] = d7.organisaatio_sv
	,[University] = d7.organisaatio_sv
	...
	,[Task categorisation] = COALESCE(d3a.selite_en, d3b.selite_en)
	...
	,[Length of employment contract] = 
		CASE
			...
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 3 THEN 'less than 3 months'
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 7 THEN '3–6 months'
	...
	,[Yrkeshögskola] = d7.organisaatio_en
	,[Universitet] = d7.organisaatio_en
	...
	--koodit
	,[Koodit Tehtäväjaottelu] = COALESCE(d3a.koodi, d3b.koodi)
	...
	,[Kansalaisuus järj] =
	  ...
	  END
	,[jarj Tehtäväjaottelu] = CASE COALESCE(d3a.selite_fi, d3b.selite_fi) WHEN 'Muu henkilökunta' THEN 4 ELSE COALESCE(d3a.jarjestys_koodi, d3b.jarjestys_koodi) END
	...
	,[jarj Työsopimuksen pituus] =
		CASE
			...
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 3 THEN 1
			WHEN DATEDIFF(MONTH,q.sopimus_alkupvm,q.sopimus_loppupvm) < 7 THEN 2
			...
			ELSE 9
		END
	,[jarj Koulutusala 02] = COALESCE(d4a.jarjestys_koodi,99999)
	,[jarj Koulutusala 95] = COALESCE(d4b.jarjestys_koodi,99999)
...
FROM (
	...
	SELECT 
		...
		,NULL AS d_tehtavanjaottelu_yo_id
		,NULL AS d_tutkijanuravaihe_id
		,NULL AS d_henkilostoryhma_id
		,NULL AS d_harjoittelukoulujen_henkilostoryhma_id
		...
		,NULL AS d_koulutusala95_id
		,f.d_ohjauksenala_id
		,f.d_tieteenala_id

		,d.amk_tunnus AS korkeakoulu_koodi

		,henkilotyovuosi

	FROM [dw].[f_amk_henkilon_tyo] f
	LEFT JOIN dw.d_amk d ON d.id=f.d_amk_id
	...
	SELECT 
		...
		,f.d_ika_id
		,f.d_kansalaisuus_id
		,NULL AS d_tehtavanjaottelu_amk_id
		...
		,d_tutkinnon_taso_id
		,NULL AS d_muun_henkiloston_henkilostoryhma_id
		,d_nimitystapa_id
		,sopimus_alkupvm
		,sopimus_loppupvm

		,'Yliopistokoulutus' AS sektori
		,NULL AS d_koulutusala02_id
		,f.d_koulutusala_id AS d_koulutusala95_id
		,f.d_ohjauksenala_id
		,f.d_tieteenala_id

		,d.yo_tunnus AS korkeakoulu_koodi
...
LEFT JOIN dw.d_koulutusala_2002 d4a on d4a.id=q.d_koulutusala95_id
...
LEFT JOIN dw.d_organisaatioluokitus d7 on d7.organisaatio_koodi=q.korkeakoulu_koodi
