...
/****** Object:  StoredProcedure [dbo].[p_lataa_f_tutkinnon_suorittaneiden_myohempi_koulutus_4_8]    Script Date: 17.4.2023 16:17:53 ******/
SET ANSI_NULLS ON
GO
...
LEFT JOIN VipunenTK.dbo.d_amm_sopimusjaksot d_amm_sj
	ON d_amm_sj.koodi = f.amm_sopimusjaksot
...
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_ulkom
	ON d_ulkom.tutkinto_suoritettu_ulkomailla_avain = f.ulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_luulkom
	ON d_luulkom.tutkinto_suoritettu_ulkomailla_avain = f.luulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_ammulkom
	ON d_ammulkom.tutkinto_suoritettu_ulkomailla_avain = f.ammulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_opistulkom
	ON d_opistulkom.tutkinto_suoritettu_ulkomailla_avain = f.opistulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_ammkaulkom
	ON d_ammkaulkom.tutkinto_suoritettu_ulkomailla_avain = f.ammkaulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_amkulkom
	ON d_amkulkom.tutkinto_suoritettu_ulkomailla_avain = f.amkulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_akkulkom
	ON d_akkulkom.tutkinto_suoritettu_ulkomailla_avain = f.akkulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_ylamkulkom
	ON d_ylamkulkom.tutkinto_suoritettu_ulkomailla_avain = f.ylamkulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_ykkulkom
	ON d_ykkulkom.tutkinto_suoritettu_ulkomailla_avain = f.ykkulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_laakerulkom
	ON d_laakerulkom.tutkinto_suoritettu_ulkomailla_avain = f.laakerulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_lisulkom
	ON d_lisulkom.tutkinto_suoritettu_ulkomailla_avain = f.lisulkom
LEFT JOIN VipunenTK.dbo.d_tutkinto_suoritettu_ulkomailla d_tohtulkom
	ON d_tohtulkom.tutkinto_suoritettu_ulkomailla_avain = f.tohtulkom
...
DROP INDEX IF EXISTS [NC_f_tutkinnon_suorittaneiden_myohempi_koulutus_4_8_rivinumero] ON [VipunenTK].[dbo].[f_tutkinnon_suorittaneiden_myohempi_koulutus_4_8]
...
--indeksin luonti
CREATE NONCLUSTERED INDEX [NC_f_tutkinnon_suorittaneiden_myohempi_koulutus_4_8_rivinumero] ON [VipunenTK].[dbo].[f_tutkinnon_suorittaneiden_myohempi_koulutus_4_8]
(
	[rivinumero] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]

--Insert 50 000 rivinumeroa kerrallaan
declare @maxrow int
declare @fromrow int
declare @torow int
