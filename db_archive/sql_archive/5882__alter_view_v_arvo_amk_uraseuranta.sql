...
/****** Object:  View [dw].[v_arvo_amk_uraseuranta]    Script Date: 11.2.2022 15:45:30 ******/
SET ANSI_NULLS ON
GO
...
,CASE 
	...
	WHEN CAST(CONCAT(f.tilastovuosi,'-11-30') AS date) <= CAST(GETDATE() AS date) THEN 1
	ELSE 0
...
LEFT JOIN dw.f_arvo_luodut_vastaajatunnukset f2 ON f2.kyselykertaid = d5.kyselykertaid and 
												   ...
												   f2.tutkintokoodi = d7.koulutusluokitus_koodi and 
												   YEAR(f2.kuukausi) = f.tilastovuosi and 
												   case when f.tilastovuosi = 2021 then 10 else d3.kuukausi end = MONTH(f2.kuukausi)
