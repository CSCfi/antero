USE [ANTERO]
GO
/****** Object:  StoredProcedure [sa].[p_lataa_ehoks_osaamisen_hankkimisjaksot_netto]    Script Date: 11.1.2023 11:44:22 ******/
SET ANSI_NULLS ON
GO
...
--Tyhjennys jos täyslatauksesta yli kuukausi
IF EXISTS (SELECT TOP 1 1 FROM sa.sa_ehoks_osaamisen_hankkimisjaksot_netto HAVING datediff(dd,min(coalesce(loadtime,'1900-01-01')),getdate())>30) AND DATEPART(WEEKDAY, GETDATE()) in (1,7)
	TRUNCATE TABLE sa.sa_ehoks_osaamisen_hankkimisjaksot_netto
;
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO sa.sa_ehoks_osaamisen_hankkimisjaksot_netto (vuosi,ehoks_id,opiskeluoikeus_oid,neljannes,neljannes_paivat,osaamisen_hankkimistapa_koodi,tyopaikan_ytunnus,paivat,loadtime) 
		SELECT vuosi,ehoks_id,opiskeluoikeus_oid,neljannes,neljannes_paivat,osaamisen_hankkimistapa_koodi,tyopaikan_ytunnus,paivat,loadtime FROM #temp_ehoks_netto
		--
		DROP TABLE #jaksot
