...
/****** Object:  StoredProcedure [dw].[p_lataa_data_model]    Script Date: 29.7.2022 7:58:38 ******/
SET ANSI_NULLS ON
GO
...
SELECT DISTINCT
	  ...
	  ,p.schema_proc
	  ,p.[procedure]
	  ,case when p2.manual_load = 1 then null else p2.referenced_database_name end as [procedure_table_database]
	  ,case when p2.manual_load = 1 then null else p2.referenced_schema_name end as [procedure_table_schema]
	  ,case when p2.manual_load = 1 then null else p2.referenced_entity_name end as [procedure_table]
	  ,loadtime = getdate()
	  ,[source] = 'ETL: p_lataa_data_model'
	  ,username = USER_NAME()
FROM #tempView v
INNER JOIN #tempProc p on v.view_table_database = p.database_proc and v.view_table_schema = p.schema_proc and concat('p_lataa_', v.view_table) = p.[procedure]
INNER JOIN #tempProc p2 on p2.[procedure] = p.[procedure] and p2.database_proc = p.database_proc and p2.schema_proc = p.schema_proc and (p2.[referenced_entity_name] <> v.view_table or p2.manual_load = 1)
...
SELECT DISTINCT
	...
	CASE WHEN UPPER(t.view_table_database) in('VIPUNENTK','VIPUNENTK_DW') and (upper(left(t.view_table,1)) = 'D' or p.[procedure] in ('p_lataa_OPE','ETL_tyhjenna_VipunenTK_faktat'))  THEN NULL ELSE p2.referenced_database_name END as procedure_table_database,
	CASE WHEN UPPER(t.view_table_database) in('VIPUNENTK','VIPUNENTK_DW') and (upper(left(t.view_table,1)) = 'D' or p.[procedure] in ('p_lataa_OPE','ETL_tyhjenna_VipunenTK_faktat')) THEN NULL ELSE p2.referenced_schema_name END as procedure_table_schema,
	CASE WHEN UPPER(t.view_table_database) in('VIPUNENTK','VIPUNENTK_DW') and (upper(left(t.view_table,1)) = 'D' or p.[procedure] in ('p_lataa_OPE','ETL_tyhjenna_VipunenTK_faktat')) THEN NULL ELSE p2.referenced_entity_name END as procedure_table,
	loadtime = getdate(),
	[source] = 'ETL: p_lataa_data_model',
	username = USER_NAME()
FROM #tempTable t
...
WHERE (p.[procedure] not like 'ETL%' or p.[procedure] like '%dimensiot' or p.[procedure] = 'ETL_tyhjenna_VipunenTK_faktat') and (t.view_table <> 'f_haku_ja_valinta_yoarvosanat' or p.[procedure] = 'p_lataa_f_haku_ja_valinta_yo_arvosanat') and t.view_table <> 'f_koski_esi_ja_perus_oppilaat_ja_paattaneet_lukuvuosittain'
