...
/****** Object:  StoredProcedure [dw].[p_lataa_f_virta_otp_ainekelpoisuudet]    Script Date: 20.9.2022 12:07:32 ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	SELECT DISTINCT
		...
		case when d.patevyys_koodi ='ms' then 'Musiikkipedagogiikka' when d.patevyys_koodi ='gf' then 'Teatteri- ja tanssipedagogiikka' else left(d.patevyys_fi, len(d.patevyys_fi) - charindex(',', reverse(d.patevyys_fi))) end as aine,
		coalesce(nullif(ltrim(rtrim(replace(reverse(substring(reverse(d.patevyys_fi),0,charindex(',',reverse(d.patevyys_fi)))),'op.',''))),''), 'Tieto puuttuu') as laajuus,
		tut.suorituspaivamaara as paivamaara_tutkinto,
		kl.koulutusluokitus_koodi,
		case 
