...
/****** Object:  View [dw].[v_koski_lukio_ainevalinnat_suorittaneet]    Script Date: 4.10.2023 19:34:14 ******/
SET ANSI_NULLS ON
GO
...
SELECT --TOP 1000
  ...
  --kurssit
	,[Kurssin OPS] = case kurssi_paikallinen_OPS_0_1 when 1 then 'Paikallinen' when 0 then 'Valtakunnallinen' else 'Tieto puuttuu' end
    ,[Kurssin tyyppi] = coalesce(d7.nimi, 'Tieto puuttuu')
    ,[Kurssin suorituskieli] = coalesce(d8.suorituskieli_yleissivistava_fi, 'Tieto puuttuu')
  ...
  --jarj
	...
	,[jarj_aidinkieli] = coalesce(d11.jarjestys_kieliryhma2, 9999)
