USE [ANTERO]
GO
/****** Object:  StoredProcedure [dw].[p_lataa_d_hakuaika]    Script Date: 12.3.2020 11:18:28 ******/
SET ANSI_NULLS ON
GO
...
INSERT INTO @hakuaikaId1 SELECT distinct HakuaikaId FROM [ANTERO].[sa].[sa_odw_hakeneet] sa JOIN ANTERO.dw.d_haku d on d.haku_oid = sa.HakuOid where d.haun_nimi_fi like '%ensimmäinen yhteishaku'
INSERT INTO @hakuaikaId2 SELECT distinct HakuaikaId FROM [ANTERO].[sa].[sa_odw_hakeneet] sa JOIN ANTERO.dw.d_haku d on d.haku_oid = sa.HakuOid where d.haun_nimi_fi like '%toinen yhteishaku'
...
USING (
	...
	SELECT hakuaikaId
		...
		,nimi_fi = 
			case
				when nimi_fi is null and hakuaikaId in (SELECT * FROM @hakuaikaId1) then 'Hakuaika 1'
				when nimi_fi is null and hakuaikaId in (SELECT * FROM @hakuaikaId2) then 'Hakuaika 2'
				else coalesce(nimi_fi,nimi_sv,nimi_en) 
			end
		,nimi_sv =
			case
				when nimi_sv is null and hakuaikaId in (SELECT * FROM @hakuaikaId1) then 'Ansökningstid 1'
				when nimi_sv is null and hakuaikaId in (SELECT * FROM @hakuaikaId2) then 'Ansökningstid 2'
				else coalesce(nimi_sv,nimi_fi,nimi_en) 
			end
		,nimi_en = 
			case
				when nimi_sv is null and hakuaikaId in (SELECT * FROM @hakuaikaId1) then 'Application period 1'
				when nimi_sv is null and hakuaikaId in (SELECT * FROM @hakuaikaId2) then 'Application period 2'
